...
| `inject_ready`, `inject_ready_bypass_governor`, `inject_cancel`, `schedule_internal` | Atomic wake-state check plus queue injection. | A + scheduler queues | A only | Wake-state dedup belongs to the task shard or a lock-free wake-state map. Scheduler queue locks are independent and must not be held while acquiring B/C. |
