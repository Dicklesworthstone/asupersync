...
pub static RFC6330_SYSTEMATIC_INDEX_TABLE: LazyLock<Vec<SystematicIndexEntry>> =
    LazyLock::new(|| {
        SYSTEMATIC_INDEX_ROWS
            .iter()
            .map(
                |&(k_prime, systematic_index, s, h, w)| SystematicIndexEntry {
                    k_prime: u16::try_from(k_prime)
                        .expect("RFC 6330 systematic table K' values fit into u16"),
                    systematic_index,
                    s,
                    h,
                    w,
...
pub struct TupleTestVector {
    ...
    pub expected_d1: u32,  // Expected d1 value
    pub expected_a1: u32,  // Expected a1 value
    pub expected_b1: u32,  // Expected b1 value
}

/// Golden tuple generation test vectors captured from the runtime's RFC-backed
...
pub fn get_systematic_index(k: u16) -> Option<u16> {
    RFC6330_SYSTEMATIC_INDEX_TABLE
        .iter()
        .find(|entry| entry.k_prime == k)
        .map(|entry| entry.systematic_index)
}
...
mod tests {
    ...
    fn test_systematic_index_lookup() {
        assert_eq!(get_systematic_index(10), Some(254));
        assert_eq!(get_systematic_index(48), Some(589));
    ...
    fn test_table_sizes() {
        ...
        assert!(RFC6330_SYSTEMATIC_INDEX_TABLE.len() > 400);
    }

    #[test]
    fn test_tuple_fixtures_match_runtime_goldens() {
        assert_eq!(RFC6330_TUPLE_TEST_VECTORS.len(), 18);
        assert_eq!(
            RFC6330_TUPLE_TEST_VECTORS[0],
            TupleTestVector {
                k: 10,
                symbol_index: 0,
                expected_d: 2,
