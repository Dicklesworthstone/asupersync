...
//! scope!(cx, "handler", budget: Budget::with_deadline_at_secs(5), {
//!     // body
//! })
//!
//! // Options can be combined
//! scope!(cx, "handler", state: &mut state, budget: Budget::with_deadline_at_secs(5), {
//!     let _child = spawn!(async { 42 });
//! })
//! ```
...
mod tests {
    ...
    fn test_parse_budget_scope() {
        let input: proc_macro2::TokenStream =
            quote! { cx, budget: Budget::with_deadline_at_secs(5), { let x = 1; } };
        let parsed: ScopeInput = syn::parse2(input).unwrap();
        assert!(parsed.name.is_none());
