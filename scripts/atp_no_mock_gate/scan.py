#!/usr/bin/env python3
"""ATP-specific implementation-completeness gate.
...
incomplete marker exits nonzero.
"""
...
SCHEMA_VERSION = f"atp-no-{TERM_MOCK}-gate-report-v1"
...
TOKEN_PATTERNS = {
    ...
    "deferred_macro": rf"\b{TERM_DEFERRED}\s*!\s*\(",
    "unready_macro": rf"\b{TERM_UNIMPLEMENTED}\s*!\s*\(",
    TERM_MOCK: rf"\b{TERM_MOCK}\b",
    TERM_FAKE: rf"\b{TERM_FAKE}\b",
    TERM_STUB: rf"\b{TERM_STUB}\b",
    TERM_PLACEHOLDER: rf"\b{TERM_PLACEHOLDER}\b",
    "deferred": rf"\b{TERM_DEFERRED}\b",
    "unready": rf"\b{TERM_UNIMPLEMENTED}\b",
}
...
def load_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema_version") != f"atp-no-{TERM_MOCK}-policy-v1":
...
def scan_hits(repo_root: Path, roots: list[str]) -> list[dict[str, Any]]:
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError("ripgrep ATP completeness scan failed")
...
def render_text(report: dict[str, Any]) -> str:
    ...
    lines = [
        f"ATP completeness gate: {summary['status']}",
    ...
    for violation in report["violations"][:20]:
        ...
        lines.append(
            ...
            f"ATP completeness violation tokens={tokens}; add scoped policy entry or remove incomplete marker"
        )
    return "\n".join(lines) + "\n"
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix=f"asupersync-atp-no-{TERM_MOCK}-gate-") as tmp_raw:
        root = Path(tmp_raw)
        (root / "src/atp").mkdir(parents=True)
        (root / "tests/atp/no_mock").mkdir(parents=True)
        (root / "src/atp/transport.rs").write_text(
            f"pub fn ship() {{ {TERM_DEFERRED}!(\"{TERM_PLACEHOLDER} {TERM_MOCK} transport\"); }}\n",
            encoding="utf-8",
        )
        (root / "tests/atp/no_mock/fixture.rs").write_text(
            f"struct {TERM_MOCK.title()}Peer; fn fixture() -> &'static str {{ \"{TERM_FAKE} fixture\" }}\n",
            encoding="utf-8",
        )
        policy = {
            "schema_version": f"atp-no-{TERM_MOCK}-policy-v1",
            "scan": {
    ...
    if report["summary"]["status"] != "fail":
        print("self-test failed: production incomplete marker was not rejected", file=sys.stderr)
        return 1
    ...
    if not any(row["path"] == "tests/atp/no_mock/fixture.rs" for row in report["covered"]):
        ...
        return 1
    print("self-test passed: allowed fixture covered and production incomplete marker rejected")
    return 0
