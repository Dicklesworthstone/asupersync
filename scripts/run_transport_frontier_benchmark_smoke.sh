...
ARTIFACT="artifacts/transport_frontier_benchmark_v1.json"
RCH_BIN="${RCH_BIN:-rch}"
MODE=""
SCENARIO=""
...
OUTDIR="target/transport-frontier-benchmark-smoke/$RUN_ID/$SCENARIO"
mkdir -p "$OUTDIR"

cat > "$OUTDIR/bundle_manifest.json" <<BUNDLE
{
  "schema": "transport-frontier-benchmark-smoke-bundle-v1",
...
{
  "schema": "transport-frontier-benchmark-smoke-run-report-v1",
  "scenario_id": "$SCENARIO",
  "run_id": "$RUN_ID",
  "exit_code": $EXITCODE,
  "timestamp": "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
