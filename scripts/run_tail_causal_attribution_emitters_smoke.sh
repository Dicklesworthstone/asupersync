...
COMMAND="timeout ${RCH_WRAPPER_TIMEOUT} rch exec -- env CARGO_INCREMENTAL=0 CARGO_PROFILE_TEST_DEBUG=0 RUSTFLAGS='-C debuginfo=0' CARGO_TARGET_DIR=\${TMPDIR:-/tmp}/rch_target_tail_causal_attribution_emitters ASUPERSYNC_TAIL_CAUSAL_ATTRIBUTION_REPORT_PATH=${SMOKE_REPORT_PATH} cargo test -p asupersync --test runtime_tail_latency_taxonomy_contract compact_tail_causal_attribution_smoke_emits_report --features test-internals -- --nocapture"
