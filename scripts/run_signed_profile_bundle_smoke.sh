...
COMMAND="rch exec -- env CARGO_INCREMENTAL=0 CARGO_TARGET_DIR=\${TMPDIR:-/tmp}/rch_target_signed_profile_bundle ASUPERSYNC_SIGNED_PROFILE_BUNDLE_CONTRACT_PATH=${ARTIFACT} ASUPERSYNC_SIGNED_PROFILE_BUNDLE_SCENARIO=${SCENARIO} ASUPERSYNC_SIGNED_PROFILE_BUNDLE_REPORT_PATH=${SCENARIO_REPORT_PATH} cargo test -p asupersync --test signed_profile_bundle_contract signed_profile_bundle_smoke_contract_emits_report --features test-internals -- --nocapture"
