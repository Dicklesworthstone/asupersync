...
COMMAND="${RCH_INVOCATION} exec -- env CARGO_INCREMENTAL=0 CARGO_TARGET_DIR=\${TMPDIR:-/tmp}/rch_target_adaptive_batch_sizing ASUPERSYNC_ADAPTIVE_BATCH_SIZING_CONTRACT_PATH=${ARTIFACT} ASUPERSYNC_ADAPTIVE_BATCH_SIZING_SCENARIO=${SCENARIO} ASUPERSYNC_ADAPTIVE_BATCH_SIZING_REPORT_PATH=${SCENARIO_REPORT_PATH} cargo test -p asupersync --test adaptive_batch_sizing_contract adaptive_batch_sizing_smoke_contract_emits_report --features test-internals -- --nocapture"
...
if [ "$MODE" = "dry-run" ]; then
    ...
    if [ "$COMMAND_EXIT_CODE" -ne 86 ] && grep -Eq '^\[RCH\] local \(|falling back to local' "$RUN_LOG_PATH" 2>/dev/null; then
        COMMAND_EXIT_CODE=86
        printf 'FATAL: rch local fallback detected; refusing local cargo execution\n' >>"$RUN_LOG_PATH"
    fi
