...
require_tools() {
    if ! command -v jq >/dev/null 2>&1; then
        echo "FATAL: jq is required for wait-cause remediation smoke runner" >&2
        exit 1
    fi
    if [ ! -f "$ARTIFACT" ]; then
...
COMMAND="timeout ${RCH_WRAPPER_TIMEOUT} rch exec -- env CARGO_INCREMENTAL=0 CARGO_PROFILE_TEST_DEBUG=0 RUSTFLAGS='-C debuginfo=0' CARGO_TARGET_DIR=\${TMPDIR:-/tmp}/rch_target_wait_cause_remediation ASUPERSYNC_WAIT_CAUSE_REMEDIATION_REPORT_PATH=${SMOKE_REPORT_PATH} cargo test -p asupersync --test runtime_wait_cause_remediation_contract wait_cause_remediation_smoke_emits_report --features test-internals -- --nocapture"
