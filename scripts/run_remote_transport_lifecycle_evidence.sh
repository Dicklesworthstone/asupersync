...
if [[ "${CONTRACT_ONLY}" -eq 1 ]]; then
    ...
    set +e
    timeout "${TIMEOUT_SEC}" "${TEST_COMMAND[@]}" > "${RUN_LOG_PATH}" 2>&1
    TEST_STATUS=$?
    set -e
    cat "${RUN_LOG_PATH}"
fi

python3 - "$REPO_ROOT" "$ARTIFACT_PATH" "$RUN_LOG_PATH" "$RUN_REPORT_PATH" "$RUN_ID" "$TEST_STATUS" "$CONTRACT_ONLY" "$TIMEOUT_SEC" <<'PY'
import json
import re
import sys
...
if not contract_only and missing_scenarios:
...
validation_passed = first_failure == "" and (contract_only or len(missing_scenarios) == 0)
