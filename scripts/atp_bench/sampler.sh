#!/usr/bin/env bash
# Background process sampler (br-asupersync-iiz6jk).
# Samples RSS/%CPU of processes matching a pattern + system loadavg every
# 0.5s, writing JSONL until killed. Used on both sender and receiver.
#
# Usage: sampler.sh <pgrep-pattern> <out.jsonl>
set -u
...
while true; do
    ...
    # Sum RSS (KB) and %CPU across all matching processes.
