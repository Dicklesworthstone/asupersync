...
The maintained public target is [`tests/supervision_regression.rs`](../tests/supervision_regression.rs), driven by [`scripts/test_obligation_cleanup_e2e.sh`](../scripts/test_obligation_cleanup_e2e.sh) in `--managed-supervision` mode. Its complete remote run on 2026-09-06 UTC passed both actual native SIGTERM subprocesses and rejected the planted zero-selection and early-exit controls. The same run passed the deterministic signal-source tests on three Lab seeds and all six native registered-finalizer/replacement cases described in [the channel recipe](channel_supervision_e2e.md). The full lane passed 40 native prerequisite tests, 506 focused units, and six public tests against one selected source and overlay fingerprint.
