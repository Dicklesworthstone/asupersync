...
★**FRONT A encrypted E802 — handshake FIXED but exposes a deeper decode wall (A/B run 20260701T181338Z, atp_e802):** SapphireHill hand-implemented the E802 fix (transport_quic reject_hello_reason bounded-accept + adopt hello.max_block_size into decode geometry; 6 handshake unit tests pass). Result: the E802 handshake REJECT is GONE (transfer now starts) + 50M encrypted no-regress (1.4s), BUT 500M+5G encrypted now TIMEOUT/error at ~300s instead of the old fast reject. ROOT CAUSE: large files force ≥2MB blocks (the 256-block-per-object cap is a wire constraint — SBN is u8), and QUIC's SINGLE-THREADED decode of large-K blocks (2MB/1141 ≈ 1795 symbols/block) is too slow → timeout. This is exactly why GreenMarsh capped the QUIC default at 512KB (e2f9b720b: "4MiB default aborted all encrypted transfers"). The handshake fix is NECESSARY but INSUFFICIENT and NOT landed alone (fast-reject → 300s hang = worse UX). The real Front A win = PARALLELIZE QUIC large-K block decode (the rq path has this via F3; transport_quic does not). Handshake fix kept staged as the prerequisite; decode-parallelization scoped for GreenMarsh/codex. Honest: encrypted-large is a decode-perf problem, not a handshake bug.
