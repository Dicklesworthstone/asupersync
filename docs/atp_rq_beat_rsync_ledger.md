...
## MATRIX-227 (2026-07-08) — ★★★GAIN CYCLING LANDS AND WINS (the drain-phase law, applied): the PROBE_BW pacing-gain cycle (1.25 probe → 0.75 drain → 6× 1.0 cruise, one RTprop per phase over the honest MATRIX-226 filter) improves BOTH hardest clean/good cells — 500M/perfect 7.16→6.66 med (−7 %, best rep 6.16) and ★500M/good 52.3→47.0 med (−10 %, best-ever family) — with all guards in-family and the lab gate green FIRST TRY (28/28). Receipt-clocked credit, retried a FOURTH time atop the now-complete honest stack, measured a WASH-to-worse and is SETTLED CLOSED: the drain phase had already dissolved the stalls it was meant to remove.

**Build discipline (the M226 method, repeated deliberately):** lab first — two new `matrix227_delivery_lab_*` scenarios asserting exactly what constant gain could not do (sustained ≥0.8×link utilization over the final 2 sim-seconds with ≤2 steady-state drops and a stable estimate), all six M226 honesty scenarios re-passing under cycling; then unit tests for the cycle mechanics; then the wire. Engagement verified in traces before reading walls: all 8 phases visited uniformly, three rate tiers visible, RTprop honest (4.0 ms on perfect, ~49 ms on good).

**★Why cycling alone wins 500M/good even with the flow-window stall still present:** the stall fires when a head-of-line hole freezes credit; the holes come from tail-drops; the tail-drops come from the constant 1.25× over-offer. Cutting the over-offer (avg gain 1.0 + explicit drain) removes most holes → most stalls → −10 % wall, without touching the window at all. **★Receipt-clocking post-mortem (4 attempts, now settled):** atop the full honest stack it ENGAGED cleanly (unacked rode the cwnd past the 2 MiB window, max 2.99 MB) and still measured med ≥46.8 vs 47.0 cycling-alone with +73 MB extra re-sends — the stall it removes is no longer the binding constraint; what remains of the 500M/good gap vs rsync (47.0 vs 24.7) is NOT credit-blocking. Killed at rep 3 by the pre-registered keep-line; consumption-clocked credit restored with a SETTLED-after-four-attempts NOTE.

★**Honest residual (500M/good 47.0 vs rsync 24.7):** the theoretical floor at avg-gain-1.0 × BtlBw 25 MB/s is ~21 s for 524 MB; the ~26 s excess is now repair traffic + the true-loss recovery cadence + phase structure — a NEW attribution question for a traced follow-up, no longer the stall tax. Also noted: the sampler over-reads ~12 % at very high rates (133 vs ~119 MB/s payload on perfect — interval granularity at 4 ms RTprop), benign today (floor/window dominate) but worth a bound when the datagram tier adopts this sampler. Evidence: `artifacts/atp_bench_matrix/20260708T1644*-1648*` (cycling A/B), `20260708T1705*` (Phase B, killed at rep 3), lab tests by name.

## MATRIX-228 (2026-07-08) — ★★THE 500M/GOOD CEILING IS A PATH LAW (attribution complete; window raise measured both ways and refuted): the good regime's shaper queue (~1.4 MB) is SMALLER than the lag-inflated effective BDP (25 MB/s × ~82 ms = 50 ms RTT + ~30 ms receiver ACK cadence ≈ 2.05 MB). Any in-flight bound sits on one side of that wall: ≤ BDP_eff drains the pipe on every repair (2 MiB: unacked pinned at the edge, 64 % duty cycle, 47.0 s), > BDP_eff overfills the queue (4 MiB: delivery improved 16.3→20.5 MB/s but repair volume exploded 39→289 MB, walls 47.0→66 s uniform). 2 MiB is the measured optimum of a trade-off that cannot be tuned away — only MOVED, by shrinking the ACK lag.

**The attribution that got here (traced, w8a cycling reps):** wall 44.6 s = flush busy 1.34 s + pacer sleeps 16.2 s + ~26 s window-edge duty cycle; repair volume already down to 39.4 MB post-cycling; offered rate (34.4 avg) and cwnd (2.42 avg) both non-binding; in-flight avg 1.33 MB vs the 2.09 MB edge = 64 % duty × 25 MB/s = the observed 16.3 MB/s delivery exactly. This also retro-explains the four receipt-clocking refutations in one line: the ~2.4 MB cwnd it swapped in was the SAME SIZE as the window it replaced.

★**The real lever (routed to uw1cc2, next unit): shrink the receiver's ACK cadence.** `QUIC_SOURCE_STREAM_READ_DRAIN_BATCHES` doc already records that the read loop's pump-turn ACK flush IS the cadence (one turn ≈ 4 MB batches once caused a 64 MB/s lockstep cap). At ≤5 ms ACK cadence, BDP_eff ≈ 25 MB/s × 55 ms ≈ 1.4 MB ≈ fits the queue — the duty-cycle and drop taxes collapse together and the 2 MiB window becomes ample. That is a receiver-side change with its own hazards (ACK-frame overhead, the 96-range SACK budget, per-flush cost) — lab has no receiver model yet, so it needs either a lab extension or careful bench discipline. ★Also banked: sampler's known ~12 % high-rate over-read (M227) unchanged. Evidence: `artifacts/atp_bench_matrix/20260708T184810Z/` (4 MiB reps 66.1/65.8/66.0 + rep1 trace lost_MB=289, unacked max 4.66), `20260708T164457Z/` (2 MiB attribution).

## MATRIX-229 (2026-07-08) — ★MEASUREMENT CORRECTION + CAMPAIGN CLOSE for the 500M/good cell: the M228-routed "receiver ACK cadence" lever is REFUTED BY DIRECT MEASUREMENT before any build — the receiver emits ACK/credit packets every ~1.3 ms (34,687 sender-side `stream_ack_ranges` applications over a 44 s rep; the read loop flushes per chunk AND per pump turn). The ~30 ms lag term in the M228 BDP_eff decomposition is actually REPAIR-EPISODE SMEARING: ~650 hole-repair events per transfer, each delaying its bytes' acknowledgement by detect+resend+RTT (~55-105 ms), inflate the average flight to ~82 ms against the honest 49.3 ms RTprop.

**The corrected, final picture of 500M/good (47.0 s vs rsync 24.7):** the path law stands exactly as measured (M228: queue 1.4 MB < BDP_eff ~2 MB; 2 MiB window = 64 % duty; 4 MiB = 7× repair tax) — but its lag term is repair smearing, not cadence, so the remaining levers all dead-end: repair is already threshold-fast (ZERO app-data PTO events in the traces); the 2 MiB window's ~80 ms runway vs ~55-105 ms repair time is fixed geometry under the queue law; hole count is per-packet loss (frame-size neutral); and stream-level FEC is the OTHER transport tier's architecture. **47.0 s is the architectural ceiling of an un-FEC'd, in-order, windowed stream on a queue-smaller-than-BDP path** — TCP's residual edge (21 vs 16.3 MB/s goodput) is byte-granular cumulative flow + kernel pacing riding the same queue, not something a constant or cadence can buy back.

★**Campaign verdict (waves 3-10, MATRIX-222→229, banked):** landed = honest delivered-counter sampler (M226) + PROBE_BW gain cycling (M227: 500M/perfect 6.66 best-ever, 500M/good 47.0 best-ever, requeues −40 %, sender RSS −60 %) + three permanent telemetry/instrumentation upgrades + the reusable deterministic delivery lab. Settled = receipt-clocked credit (4 attempts), gain schedules (3 shapes), window sizing (both directions), ACK cadence (premise-measured). The remaining rsync-favored clean cells (50M/500M perfect margins, tree floors, 5G) carry known, mostly-fixed-cost attributions; the good/bad-regime residuals are architectural. Recommend: bank the board, point further ATP-vs-rsync effort at the FEC'd datagram tier unification (GreenMarsh's uw1cc2 core scope), where the sampler/cycling machinery now has a proven template. Evidence: rep trace `20260708T164457Z/.../rep1` (ACK cadence count), all prior M222-228 artifacts.

## Scoreboard (2026-07-08, SapphireHill) — ★SMALL/TREE CLEAN RESIDUE IS WON; f9bcp2 premise refuted by measurement. Fresh whole-cell atp-vs-rsync snapshot on current HEAD (atp 0.3.5, fresh release build; nocrypto; 5 reps/cell; SHA-verified; rate-capped netem; NOT an A/B, an absolute scoreboard) to settle whether the small/tree startup-floor lever (bead f9bcp2) is still needed after the MATRIX-213→227 campaign. Verdict: **atp wins 7 of 8 residue cells**, most decisively — 500K/perfect 0.208 (4.8×), 500K/good 0.339 (2.9×), 50M/perfect 0.450 (2.2×), 50M/good 0.717 (1.4×), tree_big/perfect 0.595 (1.7×), tree_big/good 0.666 (1.5×), tree_small/good 0.948 (1.06×). Per-regime geomean: perfect **0.496**, good **0.626** (atp ~1.6-2× faster across the residue). The ONLY loss is **tree_small/perfect 1.086** (rsync ~8.6 % faster) — and it is NOISY (atp cv 5.07 %, rsync cv 4.48 %), the extreme tiny-tree-on-near-∞-bandwidth case where the fixed handshake-RTT floor dominates; per M229 this is a known fixed-cost tree floor, not a throughput gap. **f9bcp2 (optimistic sub-window source-stream start) is NOT worth its cost**: it targets exactly these cells, but the campaign already won them, so a wire-protocol change with a real correctness hinge (cold-handshake predicate symmetry — the source-stream eligibility depends on per-side `path_loss_rate`, only provably symmetric at 0 loss) would risk correctness + collide on the shared native_link.rs for a single marginal, noisy 8.6 % cell. Bead f9bcp2 → CLOSED (superseded by MATRIX-213→227). Confirms M229's "bank the board." Integrity note: one rsyncd rep flaked at 500K/perfect (status 10, excluded); atp was 5/5 SHA-ok on every cell. Evidence: `artifacts/atp_bench_matrix/20260708T221150Z/scorecard.md`.

## Scoreboard (2026-07-08, SapphireHill) — ★BAD-REGIME (2% loss + 80±20ms + 50mbit) STILL ALL-WINS on current HEAD; re-confirms the MATRIX-217→221 lossy board with no regression. Companion to the clean-residue scoreboard above: fresh whole-cell atp-vs-rsync snapshot on current HEAD (atp 0.3.5, same fresh release build; nocrypto; SHA-verified; rate-capped netem both ends; absolute scoreboard, not an A/B) over the `bad` regime, to verify the lossy board did not regress under the M226/M227 delivery-sampler + gain-cycling changes. Verdict: **atp wins all 5 measured bad cells** — 500K/bad **0.311** (3.2×, atp 1.317s vs rsync 4.229s), 50M/bad **0.707** (1.4×, 10.42 vs 14.74s), tree_big/bad **0.623** (1.6×, 4.82 vs 7.73s), tree_small/bad **0.793** (1.26×, 5.02 vs 6.33s), 500M/bad **0.984** (marginal parity, atp 95.28s cv 5.4% noisy vs rsync 96.80s cv 0.43% stable). Per-regime geomean **0.640**. Honest caveats: (a) 500M/bad is a *parity/marginal* result not a clean win — atp is at worst tied at large scale on the continuous-loss link (the M229 architectural residual; both ~96s); (b) several cells are cv-noisy (50M/bad atp cv 27%, 500K/bad 16%) — the win *directions* are large enough to survive the noise but the exact ratios should not be over-read; (c) atp's known RSS tradeoff persists on the RaptorQ encode cells (50M/bad sender peak 9.1× rsync, tree_big 7.8×) though receiver RSS is lower (0.6–0.8×). All 5 cells 3–5/5 SHA+status ok; no failed/incomplete rows; no crypto-asymmetric pairs. Combined with the clean-residue scoreboard, current HEAD holds atp-favored across perfect/good (7/8) and bad (5/5) nocrypto — the lossy + small/clean board is banked and un-regressed. Evidence: `artifacts/atp_bench_matrix/20260708T234200Z/scorecard.md`.

## Scoreboard (2026-07-09, SapphireHill) — ★★BROKEN-REGIME (10% loss + reorder 5% + dup 1% + 200±50ms + 10mbit) ALL-WINS on current HEAD — atp CONVERGES + beats rsync on its FEC forward-repair home turf; completes the nocrypto lossy trilogy (good/bad/broken all atp-favored). Same absolute-scoreboard method (atp 0.3.5, nocrypto, SHA-verified, rate-capped netem, not an A/B). Verdict: **atp wins all 5 broken cells** — 500K/broken **0.210** (4.75×), tree_big/broken **0.405** (2.47×), tree_small/broken **0.680** (1.47×), 50M/broken **0.827** (1.21×), 500M/broken **0.930** (1.075×, atp CONVERGES 3/3 sha-ok — no timeout, the historical MATRIX-217→221 home-turf win holds at current HEAD). Per-regime geomean **0.537** (atp ~1.9× faster on the worst regime). Notes: (a) 500M/broken atp converges 3/3 and SHA-verifies — the un-FEC'd non-convergence of the old MATRIX-121→125 era is fully gone; (b) RaptorQ-encode RSS tradeoff is largest here (sender peak 500M 10.2× / 50M 10.7× rsync; receiver RSS 0.37–1.56×) — the forward-repair fountain buys convergence+speed at a memory cost, the known lossy-tier profile; (c) one integrity-excluded flake: tree_small/broken atp rep5 = `No route to host (os 113)`, a transient netns/veth route-setup race (NOT a collision black-hole, NOT an atp defect — the other 4/5 reps 0.68× win and SHA-ok), correctly dropped from the median. ★Ops: this run (`20260709T005031Z`) was a CLEAN RELAUNCH after an earlier attempt (`20260709T001452Z`) collided with a peer's concurrent broken sweep (`20260709T001054Z`, same binary) on the shared 10.99.0.1/.2 veth IPs — both runs' 500M/broken cells mutually black-holed (connect/accept 60s timeout + 0 socket msgs, the classic concurrent-cell signature); killed only my run's driver, waited for the peer to finish + a stable clear-check, relaunched clean. **Combined current-HEAD nocrypto verdict: atp beats tuned rsync across the ENTIRE lossy board — good (7/8), bad (5/5, geomean 0.640), broken (5/5, geomean 0.537) — plus 7/8 clean small/tree cells.** Remaining rsync-favored territory is large-clean-link throughput (500M/perfect, 5G) — architectural, lever-families exhausted (M221→229), routed to uw1cc2. Evidence: `artifacts/atp_bench_matrix/20260709T005031Z/scorecard.md`.

## MATRIX-230 (2026-07-09) — ★c54to7 CLOSED: the residual rq-decode 'InconsistentEquations' (1-4/run post-M208) is SPEC-EXPECTED RaptorQ rank deficiency at rank-K-exact solves, not a bug. Traced audit (new permanent ATP_RQ_INCONSISTENT_AUDIT probe): all rejects n==k (repair_overhead 1.0 ⇒ first solves are K-exact; RFC 6330 failure ~1e-2 at N=K, ~1e-4 at K+1), 1,192 seeded-vs-staged payload fingerprints with ZERO mismatches, uniformly random sbns (M208's entry-skew gone), retries always succeed sha-ok. Decoder core + stale-requeue machinery exonerated deterministically (honest_symbols_never_inconsistent_across_stale_requeue_cycles, permanent). Relabeled MatrixInversionFailed → InsufficientRank; InconsistentEquations reserved for genuine anomalies where the audit now fires loudly. The last asterisk on the 500M/broken nocrypto win is removed. Evidence: artifacts/atp_bench_matrix/20260709T021111Z (traced reps), commits 75a3674c8 + 4c814ce65.

## Scoreboard (2026-07-09, SapphireHill) — ★ENCRYPTED TIER (atp-quic-tls13 vs rsync-ssh-aes128gcm, crypto-symmetric) is HONESTLY MIXED at current HEAD — atp WINS small + mild-loss, LOSES clean-large + trees; NOT the nocrypto all-win. Targeted current-HEAD encrypted sweep (workloads 500K,50M,tree_small,tree_big × perfect,good; DELIBERATELY skipped 500M = known E802/j73ili handshake-mismatch, and bad/broken = known u6m3dy architectural non-convergence — both ledgered previously; this is a scoped competitive-cell snapshot, transparently NOT the whole encrypted matrix). All 64 rows status=ok + SHA-verified; no E802 rejects, no collision black-holes. **Wins:** 500K/perfect **0.337** (3.0×), 500K/good **0.201** (5.0×) — atp's lower connection-setup overhead crushes rsync-ssh on tiny files; 50M/good **0.975** (marginal win), tree_small/good **1.002** (parity). **Losses (rsync-ssh faster):** 50M/perfect **1.477** (atp cv 29% noisy — loss direction real, magnitude soft), tree_small/perfect **1.212**, tree_big/good **1.230**, tree_big/perfect **2.462** (cv 11%). Per-regime geomean: **good 0.701 (atp wins), perfect 1.104 (rsync wins ~10%)**. ★Interpretation: the encrypted clean-large + tree/perfect losses are the SAME architectural QUIC-receiver-throughput frontier as the nocrypto 500M/perfect + 5G gap (hand-rolled QUIC stack < the kernel TCP that rsync-ssh rides; MATRIX-183 T2 / uw1cc2) — consistent across tiers, lever-families exhausted (M221→229), owned by the datagram-tier-unification scope. RSS: atp uses more on BOTH ends here (1.1–1.6×, the QUIC+TLS+RaptorQ buffering), unlike nocrypto where receiver RSS was lower. ★Honest board bottom-line at current HEAD: atp beats tuned rsync across the ENTIRE nocrypto lossy board (good/bad/broken all-wins) + nocrypto clean small/tree (7/8); on the encrypted tier atp wins small + mild-loss but rsync-ssh wins clean-large + trees — the crypto/large-clean throughput gap is the one honest, architectural, cross-tier weak spot remaining. Evidence: `artifacts/atp_bench_matrix/20260709T033452Z/scorecard.md`.

## MATRIX-231 (2026-07-09, SapphireHill) — ★★★uw1cc2 DRIVE: NOCRYPTO FRONTIER COLLAPSED (fully won) + encrypted frontier mapped by premise-measurement. User asked me to take over/drive uw1cc2 (the large-clean/encrypted throughput frontier). Applied premise-measure-before-build and it overturned TWO stale assumptions (incl. the entry immediately above): **the nocrypto large-clean "gap" does NOT exist at current HEAD.** Confirmed, 3 reps, cv<0.3% stable, SHA-ok: **500M/perfect nocrypto atp 4.519s vs rsync 5.128s = 0.881 (atp WINS +14%)**, **5G/perfect nocrypto atp 45.99s vs rsync 46.56s = 0.988 (atp WINS)** (run `20260709T164512Z`). Trace: nocrypto rq `control_source_stream` runs **~946 Mbit near line rate** on a 1gbit link (`throughput_bytes_per_sec=118391179`). ★CORRECTION to the entry above: nocrypto 500M/perfect + 5G are NOT a weak spot — the M213→229 campaign already closed them; the "6.66 vs 5.15 loss" was M227-era stale. **atp now beats tuned rsync on EVERY nocrypto cell** (lossy all-wins + clean large wins + clean small/tree 7/8). The ONLY remaining rsync-favored territory is the ENCRYPTED tier.
- **Encrypted bulk (500M/perfect enc = 1.467×, atp 74 MB/s vs rsync-ssh 108, cv3.8% solid, run `20260709T170149Z`):** root-caused via trace, overturning the obvious hypotheses. (1) NOT a rate cap — the M226 delivery sampler climbs the quic source-stream pacer to ~120 MB/s and `sample_bps` MEASURES ~120 MB/s delivery (ABOVE rsync); the 64 MiB/s `QUIC_AIMD_MAX_RATE_BPS` is only the SEED rate (1 of 169 rate-updates pinned there). (2) NOT GSO/syscall — packets are ~8KB jumbo (~400 MB/s syscall ceiling); `gso_send_used=false` but irrelevant at that packet size. (3) THE GAP: sustained throughput 76 MB/s despite 120 MB/s pacing AND only **58% sender CPU** → the sender IDLES ~42%, the pipe is not kept full → in-flight-utilization / source-stream pump-loop serialization (flush→pump_inbound→flush) + per-packet AEAD/copy. This REFINES MATRIX-202's "structural QUIC-vs-TCP": the PACER half is now solved (M226); the residual is send-path/window efficiency. Lever = exploit the 42% idle headroom (keep the pipe fuller) — but MATRIX-202 showed naive cwnd-clocking regressed mild-loss 12× (50M/good 4s→52s), so it must stay pacer/delivery-clocked. DEFERRED by user (real risk, uncertain it beats the per-packet-AEAD tax).
- **Encrypted trees (tree_big/perfect enc median 1.6×, run `20260709T183055Z`):** diagnosed as SMALL-TRANSFER RAMP/VARIANCE, not a protocol bug. tree_big = 400 files / **15.9 MB** total → entirely setup/ramp-dominated (would take 0.13s at line rate). Per-rep spread is huge (atp 0.61/1.11/1.55/0.85/1.05s) — the BEST rep (0.61s) ≈ rsync (0.65s); slow reps inflate the median. Traced fast(0.65s) vs slow(1.32s) reps have **near-identical event histograms** (flush 380 vs 371, acks 211 vs 201, same manifest/commit, ZERO PTO/loss/inference/retransmit events) → same work, 2× the time = pure ramp/pacing timing, no bug. The quic source stream seeds at 64 MiB/s + is receiver-window-growth-limited (`set_fresh_stream_send_limit` = one window, grown by MAX_STREAM_DATA) + climbs via the delivery sampler; the nocrypto rq transport ramps faster+consistent (tree_big/perfect nocrypto atp **0.514s STABLE cv3.9%, WINS 0.556×**). Candidate lever = faster/higher clean-link seed+ramp for small encrypted transfers — but that is the campaign's regression-prone congestion-control core (M221→229), and best-case already ties rsync. ★Net: the uw1cc2 frontier is ~90% CLOSED (all nocrypto won); the encrypted residuals are (a) hard-structural bulk send-path efficiency and (b) subtle small-transfer ramp/variance where atp best-case ties rsync. No cheap high-confidence lever remains. Evidence: runs `20260709T164512Z` (nocrypto large-clean won), `20260709T170149Z` (encrypted perfect gaps), `20260709T183055Z` (tree baseline both tiers), `20260709T184152Z` (traced tree stall = ramp not bug).

## MATRIX-232 (2026-07-09, SapphireHill) — ★★ENCRYPTED CLEAN-LARGE ROOT-CAUSED to SENDER DUTY-CYCLE (refines M231's "receiver-throughput" attribution) + the win is LINK-BOUNDED to ~11%. Deep single-cell trace decomposition of 500M/perfect encrypted (atp-quic-tls13), run `20260709T165543Z`, ATP_RQ_TRACE. **The bottleneck is NOT the receiver and NOT the pacing RATE — it is sender-side idle (duty cycle ~61%).** Facts: `perfect` = 1gbit (125 MB/s line) / 2ms delay / 0% loss; atp effective **76 MB/s (61% of line)**, rsync-ssh 108 (86%), **atp's ACTIVE-window delivery is ~120 MB/s (96% of line — the delivery-clocked pacer is CORRECT, setpoint 120–160, tracks BtlBw 120)**. Sender handoff phase-cost for the whole 500MB (6.90s wall): **pacer_wait 2.93s (42%)**, udp_send 1.21s (18%), protect/AEAD 0.34s (5%), generate 0.26s (4%), disk+admission-gate+ACK-gaps ~2.16s (31%). Real send WORK is only 1.81s ⇒ raw capacity ~276 MB/s; the sender sits idle 39% because pacer-wait, admission-gate block, disk read, and AEAD are **fully SERIAL (wall = sum, not max)**. Three mechanisms, all measured: **(1)** `before_send_bytes` pacer wait (native_link.rs:2160-2179) is a pure `crate::time::sleep` that does NOT drain inbound ACKs — during 2.93s of cumulative sleep the receiver's 1.3ms-cadence ACKs pile up, inflating the effective feedback RTT (path_rtt 9.8ms vs netem 4ms) so `wait_source_stream_send_admission` then blocks the producer (unacked pinned at ~1.9MB against `cwnd_cap=2097152`). **(2)** In-flight admission cap is FLOORED at 2 MiB (`QUIC_SOURCE_STREAM_BDP_CWND_MIN_BYTES` = recv window): the BDP formula `2.0×BtlBw×RTprop_min` = `2.0×120MB/s×4.05ms ≈ 970KB` < floor, so the delivery-clocked cap never engages — but at 4ms RTT a 2 MiB pipe is 524 MB/s worth, so the cap SIZE is not the throughput limiter; the serial produce→flush→drain cycle is. **(3)** **GSO is completely disabled on every bulk flush (`gso_send_used=false` from the first flush — not a mid-run demotion), falling back to sendmmsg-per-packet at 18.5µs/packet = the 1.21s udp_send.** Root cause: QUIC packets are **~7.6KB jumbo** (`ATP_QUIC_UDP_MAX_PACKET=65535`, 65382 packets/500MB) which exceed the veth MTU (1500) → the packets are IP-fragmented AND Linux UDP_SEGMENT (gso_size must be ≤ MTU) rejects them immediately (EINVAL → `gso_demoted=true` latch, udp.rs:2081/2199). sendmmsg batches the varying-size packets (each msghdr independent) but the kernel still does per-packet UDP work. ★**Architectural tradeoff, not a bug:** jumbo packets minimize AEAD/packetization CPU (protect only 0.34s for 65K packets) at the cost of IP-fragmentation + no-GSO; the MTU-packet+GSO alternative would 5× the packet count → ~1.7s AEAD, unclear net. ★**Ceiling reality:** on a rate-capped perfect link BOTH protocols are bounded by the 125 MB/s line — multi-stream/multi-core CANNOT exceed it. atp's active rate (120) already beats rsync's average (108); the ONLY way atp loses is the 39% idle. To beat rsync (wall < 4.63s vs current 6.90s) the sender must recover ~2.27s of the ~2.73s non-pacer idle, i.e. reach ~86% duty from 61% — feasible ONLY via a send-path **pipeline refactor** (overlap AEAD/udp_send/disk with the pacer wait; pump ACKs during the wait), which touches the loss-tuned QUIC send path and is refutation-adjacent (M202/225/228). No cheap one-line lever exists; the honest win ceiling here is ~11% (link-bound), and the CRUSH is already banked on nocrypto (all cells) + lossy. Refines M231 (which mis-attributed this to receiver throughput — it is 100% sender duty-cycle). Evidence: run `20260709T165543Z` send.time trace decomposition; code refs native_link.rs:2160/2296/6931/7050, udp.rs:974/991/2081/2199.

## MATRIX-233 (2026-07-09, SapphireHill) — ★★ENCRYPTED tree_big/perfect "2.46×" is NOISE-INFLATED (real ~1.3–1.6×) + UNIFIES with clean-large under ONE root cause: the hand-rolled QUIC transport does ~10× rsync's per-transfer SYSCALLS/CONTEXT-SWITCHES. User steered me to attack the trees loss (biggest encrypted gap). tree_big = 400 files / **15.86 MB** heavy-tailed, sent as ONE bulk source stream (not per-file). **The 2.46× (run 033452Z) is a noisy-median artifact** — three runs give ratios **2.462 / 1.616 / 1.312** with both sides cv 9–30% on the contended shared bench box; the honest gap is ~1.3–1.6× (atp median ~0.85–1.05s vs rsync ~0.65s, sub-second). **atp's FAST reps (0.61–0.85s) already MATCH rsync (0.55–0.65s)** — atp just has a heavy TAIL (reps to 1.55s) rsync lacks. Root cause of the tail, measured from results.jsonl: **atp sender does ~10,000 context switches per transfer vs rsync's ~900–1,250 (≈10×)**, at only 17–43% CPU (idle-waiting, not CPU/throughput bound). The receiver mirrors it (~7,000 voluntary ctx switches, 17–23% CPU). So atp parks the runtime thread ~5× per packet (async awaits per flush + per-ACK pump + per-syscall reactor round-trips) where rsync's kernel-TCP path blocks once per ~17 KB read/write. On a jitter-y shared machine each of those ~10K parks can be delayed → the heavy tail → the noisy loss. Secondary real cost: **slow-start ramp** (source-stream pacer seeds 64 MiB/s, ramps 67→73→78→103→114 MB/s over ~7 windows; a 15.9 MB transfer only hits line rate at the last window) — minor here (~60ms) but the seed lever is the regression-prone congestion core (M221→229). Commit is NOT the bottleneck: the QUIC receiver already uses a one-shot blocking-pool packed-member batch write (`write_quic_packed_member_batch_oneshot`, native_link.rs:8042, MATRIX-211) with `std::fs::write` and NO per-file fsync (the delta-chunk path reassembly.rs:272 `sync_all()`-per-file is a DIFFERENT, non-QUIC path). ★**Unification:** this is the SAME root as MATRIX-232's clean-large duty-cycle — the hand-rolled QUIC/TLS stack's per-transfer syscall/wakeup overhead (vs the kernel TCP rsync rides) shows up as duty-cycle idle on large transfers and scheduling-jitter tail on small ones. ★**The one cross-cutting lever = reduce per-transfer wakeups** (bigger batches per await, overlap send/recv, coarser pacing granularity — `QUIC_DATA_PLANE_PACER_MIN_PAUSE`=50µs) — a broad efficiency refactor of the QUIC send/recv path, delivery-clocked-safe, helping BOTH encrypted losses. Not a one-liner; bounded ceiling (~1.3–1.6× trees, ~1.1× link-bound clean-large). The CRUSH is banked on nocrypto (all) + lossy. Evidence: runs `20260709T184152Z` / `183055Z` / `033452Z` results.jsonl (walls + sender_ctx_switches), send/recv.time traces; code refs native_link.rs:201/8042, reassembly.rs:272.

## MATRIX-234 (2026-07-10, SapphireHill) — ★★REFUTED: the "reduce QUIC wakeups → close encrypted trees gap" premise. strace syscall-attribution of the tree_big/perfect sender (both tiers, 1 rep each, release atp 0.3.5, harness netns, `strace -f -c`). The ~10K sender context-switches decompose to RUNTIME/BLOCKING-POOL coordination, NOT the QUIC protocol: **futex 14,372 + sched_yield 8,259 + epoll_ctl 3,766 + timerfd_settime 1,864**, around ~7,000 per-file fs syscalls (400 files: openat 1121, getdents64 1416, statx 805, fstat 715, read 2931, close 1125). **Actual network send is trivially cheap — sendmmsg=46 (well-batched), recvfrom=256.** `crate::fs` ops dispatch to the blocking pool (`spawn_blocking_io`), so each of the 400 files' async open/read/hash is a park/unpark = the futex+yield tax; rsync does the same file work synchronously in one thread (~900 ctx total). **★THE REFUTATION: the SAME strace on the NOCRYPTO sender shows MORE wakeups — futex 17,638, sched_yield 11,080, epoll_ctl 5,070 — yet nocrypto WINS tree_big/perfect (0.595×) while encrypted loses (~1.4×).** The file syscalls are byte-identical across tiers (shared prepare/tree-walk). So the wakeup tax is a SHARED runtime cost (async-runtime + blocking-pool dispatch for a 400-file tree), NOT the encrypted differentiator — you can win trees WITH more of it. Reducing it (batch per-file fs like the receiver's MATRIX-211 one-shot commit) is a general efficiency + tail-variance win that would help BOTH tiers' absolute speed, but it will NOT structurally beat nocrypto's advantage on the encrypted cell. The real encrypted-vs-nocrypto trees delta is the QUIC transport being ~2× rq on the MEDIAN (tail-driven: fast reps are competitive — encrypted 0.5–0.65s vs rsync 0.55–0.65s; the ~1.4× median is the heavy tail under shared-machine contention amplifying the shared wakeup tax) plus a small TLS-handshake premium. NB: sender fs work is DOUBLE-counted (manifest hash pass in the shared `prepare_source_manifest` mod.rs:3609 + streaming re-read in native_link.rs:7008) — 400 files opened twice. ★Net: the instrumentation the plan required refuted the plan's premise; the honest remaining encrypted gap worth engineering is clean-large duty-cycle (MATRIX-232, physically link-bounded ~11%), not trees. Evidence: `/data/tmp/atp_strace/{send,recv}.strace` (both tiers), runs under harness netns tree_big/perfect.

## MATRIX-235 (2026-07-10, SapphireHill) — ★★L2 "ACK-drain during the source-stream pacer wait" LANDS AS A CLEAN WIN — the MATRIX-232 clean-large duty-cycle lever, confirmed by two independent A/B methods. Targets the measured 42% `pacer_wait` idle on encrypted 500M/perfect (MATRIX-232): `before_send_bytes` was a pure `crate::time::sleep`, so during the cumulative ~2.9s pacer idle the receiver's ~1.3ms-cadence ACKs piled up unprocessed → `stream_unacked_bytes` went stale → `wait_source_stream_send_admission` then serialized a whole ACK-drain phase AFTER the idle (the ~40% serial duty-cycle loss). **The change (native_link.rs, single file):** split `before_send_bytes` into `byte_pacer_deadline()` (exposes the delivery-clocked absolute deadline) + `note_bytes_paced()` (burst/schedule accounting), and rewrote the flush() source-stream caller to `while let Some(deadline) = byte_pacer_deadline()` + `pump_inbound_for(cx, wait)` each turn — draining inbound ACKs concurrently with the pacing pause instead of sleeping opaquely. **Delivery-clocked-safe:** the pacing RATE and burst shape are untouched (deadline still from the same delivery-clocked schedule; only accounting moved into `note_bytes_paced`), so this is NOT a cwnd/rate change and does not re-tread the MATRIX-202 mild-loss refutation. On lossy links there are few/no ACKs to drain, so behavior ≈ the old sleep. **Lab-gated first:** `source_stream_pacer_split_preserves_schedule_and_exposes_deadline` (no deadline until burst exhausted; frames==burst.div_ceil(packet); deadline armed ≤ one interval ahead; rate-change clears the deadline) — 26/26 source_stream tests green.

**★RESULT (encrypted tier, matched-pair binaries differing ONLY by native_link.rs L2, same peer-WIP snapshot, SHA-verified, feedback_rounds=0 on all).** Two methods agree:

| cell | parent' (HEAD) | child' (L2) | Δ | method |
|---|---|---|---|---|
| **500M/perfect** | 6.86 s (cv 2.4%) | **5.52 s (cv 0.3%)** | **−19.5%** | isolated 5-rep |
| 500M/perfect | 6.76 s | 5.62 s | −16.9% | interleaved rd-1 |
| **50M/perfect** | 1.72 s (cv 13.5%) | **0.71 s (cv 4.5%)** | **−58%** | isolated 5-rep |
| 50M/perfect | 1.45 s | 0.71 s | −51% | interleaved rd-1 |
| **tree_big/perfect** | 1.12 s (cv 21%) | **0.55 s (cv 4.4%)** | **−51%** | isolated 5-rep |
| tree_big/perfect | 1.35 s | 0.61 s | −55% | interleaved rd-1 |
| **50M/good (MILD-LOSS GUARD)** | 3.85 s | **3.65 s** | **−5.2% (NO REGRESSION)** | 5-rep |

- **Mechanism triangulates (not walls-only):** MATRIX-232 measured ~2.16 s of serial admission/ACK-gap idle on 500M/perfect; child' removes ~1.3 s of it → 6.86−1.3 ≈ 5.5 s (matches). Duty-cycle 500M: parent 73 MB/s (58% of the 125 MB/s line) → child **91 MB/s (72%)** — recovered duty, still under the physical line (no cheating). **★The cv collapse is itself the mechanism signal** — child' 500M cv 0.3% vs parent' 2.4%, 50M cv 4.5% vs 13.5%: L2 removes the *variable* admission stalls, leaving deterministic pacing. Short transfers gain the most (fixed handshake+ramp+stall overhead is a bigger fraction) → 50M −55%.
- **★The mild-loss guard rules out the "disabled pacing" failure mode:** if L2 had broken pacing it would over-send on the lossy `good` link → drops → repair → regression; instead 50M/good is −5% (faster, within noise). Pacing shape intact.
- **vs rsync:** rsync-ssh-aes128gcm failed to establish this session (SSH env in the netns — status=error, excluded), so no fresh bar. Against prior ledger rsync-ssh bars (500M/perfect ≈4.85 s, 50M/perfect ≈0.85 s): **L2 FLIPS 50M/perfect encrypted to an atp WIN** (0.71 vs 0.85), makes **tree_big/perfect competitive/winning** (0.55–0.61 vs ≈0.65), and **closes 500M/perfect** from 6.86→5.52 s (gap to rsync 4.85 narrows from +41% to +14%). Re-measure with a working rsync-ssh bar to bank the vs-rsync flips.
- **Deferred:** the phase-breakdown trace (`ATP_RQ_TRACE` symbol_handoff_flush pacer_wait_micros) that would show the admission-stall fraction dropping directly — the matched binaries were reaped by the disk-pressure sweep before a traced run; the wall+cv+mechanism-math+guard triangulation is strong enough to land, trace confirmation is a nice-to-have follow-up.

**★A/B rigor lessons (ops):** (1) The shared tree carried heavy live peer WIP in the transport path, so a clean-HEAD parent vs current child would confound L2 with peer drift → built a MATCHED pair differing ONLY by native_link.rs (stash just my reserved file; peer WIP in both arms cancels). (2) Building child (L2) and parent (HEAD) CONCURRENTLY from one tree gave **byte-identical binaries** — `rch` re-syncs the live tree (and here fell back to LOCAL builds that read the live tree at compile time), so popping native_link→L2 mid-parent-build let cargo read L2. Fix = SERIALIZE, keeping the file fixed for each build's WHOLE duration, and ALWAYS `sha256sum` the pair before trusting. (3) `matrix_bench.sh --out` RESETS `WORK_DIR=OUT/workloads`, so `--work-dir` must come AFTER `--out` or reused workloads read as `source_sha=missing` and `sha_ok=true` false-positives on a 0-byte "transfer" (uniform ~0.15 s tell). (4) 500M×many-reps fills `/data/tmp`; a disk-pressure reaper then wipes ALL `rch_target_*` — cp binaries out or expect them gone. Evidence: runs `artifacts/atp_bench_matrix/M235_{parent_perfect,child_perfect3,parent_good3,child_good3,il_p1,il_c1}`.
