...
The public `StreamingMultipartError` retains a machine-readable `kind()` and
the exact HTTP `status()`. In particular, malformed input is 400, the multipart
parser's own absolute or idle-read timeout is 408, body-limit exhaustion is
413, and a non-multipart media type is 415. Enclosing handler or body-context
cancellation, including `CancelKind::Deadline`, retains cancellation's 499 or
503 mapping and its exact `CancelKind` through `cancel_kind()`.
...
- Branch on `StreamingMultipartError::kind()` or `status()`; do not parse its
  display message.
...
display text beginning `[ASUP-E504]` before the live body is taken.
