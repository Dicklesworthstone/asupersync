...
| `reference_topology` | `dependency-capability-baseline-versioned-reference-topology` | Preserve the four historical back-references and two independently reviewed current references; the eight-node content-addressed graph is acyclic. The scanner retains three resolved target receipts, including one prior-observation identity, and two unresolved target identities. Separate immutable provenance is still required for the two unresolved targets. |
