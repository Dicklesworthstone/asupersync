...
| `reference_cycle` | `dependency-capability-baseline-reference-cycle` | Break one full-file edge in each of the three reciprocal cycles with immutable provenance or a canonical semantic projection before repinning. |
...
- The reference-cycle row does not make cyclic pins current, authorize blind hash refresh, or replace immutable provenance.
