...
## Execute protected work, not just a lease guard

`OwnedMembershipController::run_scoped` joins the guard to real task/subtree
ownership. Its report types are in `distributed::membership::owned::work`.
Pass the admitting task's `&Cx`, node and exact incarnation, a finite lease
duration, `ChildRegionSpec`, and a `Send + 'static` factory receiving a child Cx:

```rust,ignore
let report = controller.run_scoped(
    &cx, &node, incarnation, std::time::Duration::from_secs(30),
    asupersync::cx::ChildRegionSpec::inherit(),
    move |child_cx| async move {
        // Perform the actual work; spawn descendants through child_cx.
        perform_work(child_cx).await
    },
).await?;
if report.is_success() {
    let application_result = report.task?;
    // Also check application_result when it represents an application error.
} else {
    // Inspect trigger, task, close, and lease independently. In particular,
    // a returned cleanup value after revocation is NOT authorized work success.
}
```

The lease is admitted in the calling task and stays there until this operation
finishes. A distinct child region inherits the parent capability and scheduler
budgets and can only tighten them; the body runs as an admitted task in that child.
The factory is rechecked at its first task poll, covering delayed task admission.
Do not route descendants through a separately captured parent context and then
claim they belong to this subtree. The runner does not grant new authority.

The lease duration begins before region admission and is fixed for this call.
The runner waits on body completion, owned cancellation, lease invalidation and
its own timer. No external expiry driver is required for this call, and no
background renewal or retry is created. A due lease wins a tie with an observed
ready body. Cancellation remains cooperative: synchronous factory/poll work is
not preempted between two instructions by a later membership event.

After body completion OR a stop condition, the child is closed through the
existing runtime close protocol: cancel remaining descendants, drain them and
run finalizers. A body value cannot skip this step. The resulting report retains
body cancellation/panic/value, stop cause, aggregate child outcome, separate
cleanup outcome, and lease settlement. A close error is not a quiescence receipt.
`is_success()` requires a completed body, successful child/cleanup outcomes and
an accepted checked lease commit. It does not interpret arbitrary T as an
application-level result. A body or descendant panic is not hidden by revocation.
Only `Released` certifies that checked commit won; other lease statuses describe
local retirement, not whether abort beat an independent runtime terminal decision.

Validity is checked again after drain and during lease release. Revocation or
expiry while a blocking descendant drains therefore prevents a late clean commit,
even if the body already returned a value. Parent cancellation is observed but
not generated by membership retirement. Outer ordinary-spawn cancellation/result
attribution is unchanged: an application returning a typed cleanup result from a
cancelled holder still uses its normal cancellation-acknowledgement protocol.

Dropping or unwinding the runner requests child close and aborts its lease, but
returns no synchronous drain receipt. The runtime still owns the subtree; keep
polling the runner to receive its actual close result. A stuck synchronous poll
or blocking worker can delay drain. Closing a local TCP owner does not roll back
bytes or guarantee arbitrary remote-process quiescence or compensation.

The `membership_scoped_native` tests register an actual protected computation
alongside the existing membership control handler. After a real TCP read parks
and the native runtime projects the checked Lease, a separately authenticated
control request first fails MAC validation, then revokes the work. The work RPC
must return an explicit retirement response only after child drain and socket
owner destruction; the independent TCP peer must observe connection close.
Other native tests hold a blocking descendant through cancellation and revoke
during drain, and retain a descendant panic instead of accepting a returned body.
These are authored tests, not native execution or performance receipts.

```sh
RCH_REQUIRE_REMOTE=1 rch exec -- cargo test -p asupersync --lib distributed::membership::owned::work::
RCH_REQUIRE_REMOTE=1 rch exec -- cargo test -p asupersync --features tls,test-internals --test membership_scoped_native -- --nocapture
```
