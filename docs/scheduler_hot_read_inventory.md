...
| Rich task state | `Worker::execute_task` at `src/runtime/scheduler/worker.rs:375,591,754`; `ThreeLaneWorker::{execute_task,complete_polled_record}` at `src/runtime/scheduler/three_lane.rs:7552,7885`; `TaskSnapshot::from_record` at `src/runtime/state.rs:8867` | poll completion/unwind or cold snapshot | Non-scalar lifecycle and outcome semantics under the record owner lock. |
