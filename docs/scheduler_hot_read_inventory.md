...
| Cancellation publication state | `CxInner`'s private `Arc<CxCancellationState>` | Wakers retain the stable envelope `Arc` and only a `Weak<CxInner>` for rich state. Its private flag says cancellation may need routing; it does not own the reason. | Writers call `CxInner::set_cancel_requested` under the write lock, which updates the lock-backed bit and publishes Release. `Cx::checkpoint` and ordinary wakers query Acquire. A waker that observes true reacquires the `CxInner` read lock before reading the reason. |
...
| Public cancellation query | `Cx::is_cancel_requested` at `src/cx/cx.rs:2139` | caller-selected | Acquire-queries the stable cancellation envelope; it does not read the rich cancellation reason. |
