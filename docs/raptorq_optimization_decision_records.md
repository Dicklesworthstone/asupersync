...
- Added an explicit E5 `command_surface_split` record to keep Track-E reproducibility semantics machine-checkable:
  - manifest-level comparator/rollback `command_bundle` stays anchored to `rch exec -- cargo bench --bench raptorq_benchmark --features simd-intrinsics,criterion-benches -- gf256_primitives`
  - probe-specific `repro_command` stays anchored to `rch exec -- cargo bench --bench raptorq_benchmark --features simd-intrinsics,criterion-benches -- gf256_dual_policy`
