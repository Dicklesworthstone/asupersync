...
| `path_rtprop_micros`, `path_bottleneck_bytes_per_s` | the wall-clock path figures: minimum send→ACK flight time from the source-stream delivery sampler (falls back to the handshake RTT sample before the first ACK) and the highest max-filtered delivery rate the rate controller held while the admission gate ran |
...
| `stream_window_requests`, `peak_stream_send_window_bytes` | STREAM_DATA_BLOCKED frames the sender sent to ask for a larger source-stream window (only while credit-bound and loss-clean), and the largest send window the admission gate observed (remaining credit + unacked bytes) — against the 2 MiB HelloAck window this shows whether the receiver grew the window (A4) |
