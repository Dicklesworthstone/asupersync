...
Evidence state is `BASELINE_PLANNED`. Fourteen scenario IDs across the four
binaries, owned by `asupersync-5z2scg.7.1` (baseline), `.7.6`/`.7.7`/`.7.8`
(unit) and `.7.11` (installed E2E).

The current capability registry also requires `offline-tuner-logging-parity`,
owned by `asupersync-d24mms.3` and routed to the real binary tests in
`tests/offline_tuner_env_logger_parity.rs`. That additional logging scenario is
included in the fourteen-ID union; it does not authorize a parser replacement
or establish the help, argv, and platform evidence required below.
