...
| --- | --- | --- |
| Default features | `nightly-2026-08-31` from `rust-toolchain.toml`; default features include `nightly-outcome-try` | Pinned contributor/release snapshot plus a tracked direct-minimal consumer lock and fresh remote compile; not a numeric stable MSRV or full transitive-minimal proof |
...
| Dependency versions | The semver requirements in the consumer's resolved Asupersync manifest | Direct-minimal standalone-consumer coverage; full transitive-minimal coverage remains blocked precisely below |

`nightly-2026-08-31` is a point-in-time toolchain pin, not an MSRV-style
range. A change to `[toolchain].channel` must update this row, the README, and
the checked downstream-consumer artifact in the same review.

The synthesized fixture at
`tests/fixtures/downstream-consumer-proof/Cargo.toml` exercises selected public
API and feature profiles through a path dependency. It now commits the
independent `tests/fixtures/downstream-consumer-proof/Cargo.lock`, generated by:

```bash
cargo +nightly-2026-08-31 -Z direct-minimal-versions generate-lockfile \
  --manifest-path tests/fixtures/downstream-consumer-proof/Cargo.toml
```

The citeable compile lane is:

```bash
RCH_REQUIRE_REMOTE=1 rch exec -- env \
  CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_downstream_consumer_direct_minimal_default \
  CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 \
  RUSTFLAGS='-D warnings -C debuginfo=0' \
  cargo +nightly-2026-08-31 check -j 2 -Z direct-minimal-versions --locked \
    --manifest-path tests/fixtures/downstream-consumer-proof/Cargo.toml \
    --bin default_consumer
```

That lane proves only that the checked standalone fixture lock, generated under
Cargo's direct-minimal policy, compiles the default consumer with the pinned
nightly. It does not minimize every transitive dependency and does not establish
a numeric stable MSRV.

The stronger `-Z minimal-versions` probe is explicitly blocked. On 2026-08-23,
remote job `29988810699833376` selected `curve25519-dalek 4.0.0` through
`asupersync -> nkeys 0.4.5 -> ed25519-dalek 2.0.0`; that crate requests the
removed nightly `stdsimd` feature and fails with `E0635`. Adding an otherwise
unused direct production dependency merely to constrain that transitive range
would conceal the ecosystem/toolchain boundary, so the project keeps full
transitive-minimal support unclaimed. The authoritative hashes, green
direct-minimal receipt, blocker, update procedure, and no-claim boundaries live
in `artifacts/downstream_consumer_proof_v1.json` and are checked by
`tests/downstream_consumer_proof_contract.rs`.

Establishing a numeric MSRV or full transitive-minimum claim still requires a
toolchain-compatible green receipt across the exact feature and target profiles;
any owned lower bound that fails must be raised before it is published. The
...
the repository's current `nightly-2026-08-31` rustup override from
`rust-toolchain.toml`; the August lock receipt used `nightly-2026-07-05`.
Its direct `libfuzzer-sys 0.4.13` edge depends on
