...
| --- | --- | --- |
| Default features | `nightly-2026-07-05` from `rust-toolchain.toml`; default features include `nightly-outcome-try` | Pinned contributor/release snapshot plus a tracked direct-minimal consumer lock and fresh remote compile; not a numeric stable MSRV or full transitive-minimal proof |
...
```bash
cargo +nightly-2026-07-05 -Z direct-minimal-versions generate-lockfile \
...
RCH_REQUIRE_REMOTE=1 rch exec -- env \
  ...
  cargo +nightly-2026-07-05 check -j 2 -Z direct-minimal-versions --locked \
    --manifest-path tests/fixtures/downstream-consumer-proof/Cargo.toml \
