...
| `T37C-02` | FS cancel-safety protocol across async operations | Every public mutator in the scoped `File`, `OpenOptions`, path, directory, optional io_uring, and VFS surfaces has an operation-specific classification; direct blocking mutations are soft-cancelled and may commit after future drop; deterministic cancellation proves a direct write can commit late; `WritePermit` is not claimed as path rollback. | Emit `FS-G2` with the owning filesystem module and the missing or overclaimed cancellation boundary. | `src/fs/file.rs`, `src/fs/open_options.rs`, `src/fs/path_ops.rs`, `src/fs/dir.rs`, `src/fs/uring.rs`, `src/fs/vfs.rs`, `src/fs/mod.rs` | Source modules, `docs/tokio_cancel_safe_fs_process_signal.json`, and both cancellation/parity contract tests | `rch exec -- env CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_tokio_fs_process_signal_docs cargo test --test tokio_fs_process_signal_parity_matrix t37c_02 -- --nocapture` |
| `T37C-03` | Atomic write and error fidelity | `stage_write_atomic` never mutates the target before synchronous `StagedAtomicWrite::commit`; `write_atomic` stages then commits with no async cancellation point around rename; deterministic cancellation preserves the target and cleans the temporary file; `try_exists` and IO error fidelity remain intact. | Emit `FS-G1` with owner `src/fs/path_ops.rs` and the missing staging, commit, cleanup, API, or fidelity invariant. | `src/fs/path_ops.rs`, `src/fs/mod.rs` | Source modules, `docs/tokio_cancel_safe_fs_process_signal.json`, and both cancellation/parity contract tests | `rch exec -- env CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_tokio_fs_process_signal_docs cargo test --test tokio_fs_process_signal_parity_matrix t37c_03 -- --nocapture` |
...
| `T310-E2E-02` | filesystem | Atomic write cancelled at the deterministic post-stage gate | Target remains unchanged and the discarded staged value cleans its temporary file; an uncancelled call commits the full replacement | `correlation_id`, `op_type=write_atomic`, `outcome={committed,staging_discarded}`, `timeline_ms` | `src/fs/path_ops.rs` |
