...
**clean-overlay proof orchestration contract:** `artifacts/clean_overlay_proof_orchestration_v1.json`, checked by `tests/clean_overlay_proof_orchestration_contract.rs`, is the scoped PROOF-ORCH A4 operator packet for the shared-`main` clean-overlay proof lane that overlays only an agent's owned, reserved paths on `HEAD` so peer dirt is never synced into the build. Its focused manifest lane is `clean-overlay-proof-orchestration-contract`; cite it only for the operator runbook — prerequisites, command examples, reservation expectations, RCH heartbeat-fresh/progress-stale cancellation guidance, peer-dirty blocker receipts, non-destructive cleanup/rollback, Agent Mail and `br` comment handoff templates, README/AGENTS markers, proof manifest/status rows, and no-claim boundaries. The runbook is `docs/clean_overlay_proof_orchestration_runbook.md`. It does not prove release readiness, broad workspace health, runtime correctness outside the cited A1-A3 clean-overlay planner/blocker/overlay-command surfaces, performance improvement, live RCH fleet availability, permission to delete files, or local Cargo fallback approval.

**proof-traffic final signoff:** `artifacts/proof_traffic_final_signoff_v1.json`, checked by `tests/proof_traffic_final_signoff_contract.rs`, is the scoped A6 operator packet for the proof-traffic controller under `asupersync-proof-traffic-control-kuyx64`. Its focused manifest lane is `proof-traffic-final-signoff`; cite it only for A1-A5 evidence aggregation, the capability-drift gate, admission receipt taxonomy, clean-overlay handshake, proof parking lot, blocked-loop e2e packet, proof manifest/status rows, README/AGENTS markers, no-local-fallback/no-peer-cancel policies, dependency-cycle receipt/checklist, and no-claim boundaries. The runbook is `docs/proof_traffic_control.md`. It does not prove release readiness, broad workspace health, runtime correctness, performance improvement, live RCH fleet availability, local Cargo fallback approval, permission to delete files, or permission to cancel peer builds.
