...
**dependency CI/provenance final signoff:** `artifacts/dependency_ci_provenance_final_signoff_v1.json`, checked by `tests/dependency_ci_provenance_final_signoff_contract.rs`, is the scoped P6 aggregate for `asupersync-mnotoo.3.7`. Its focused manifest lane is `dependency-ci-provenance-final-signoff`; cite it only for the P1-P5 tracker/artifact joins, reviewed full-SHA action inventory, owner-selected static-only JavaScript receipt, recorded fresh root/excluded-fuzz scanner receipt, fuzz native/Tokio quarantine, direct-minimal pass plus full-transitive blocker, generated AGENTS drift receipt, deterministic replay commands, safe negative mutations, closeout checklist, proof manifest/status rows, README/AGENTS markers, and no-claim boundaries. The verdict is `PASS_SCOPED_KEEP_DEFER`. It grants no JavaScript package-manager, dependency/feature cutover, automatic AGENTS rewrite, file-deletion, local-Cargo-fallback, release-readiness, broad workspace-health, runtime-correctness, performance, undisclosed-vulnerability, legal, publisher-identity, fuzz-behavior, or parent-epic-closure claim. The runbook is `docs/dependency_ci_provenance_final_signoff.md`; rerun `scripts/ci/audit_dependencies.sh run` before citing current scanner health.
