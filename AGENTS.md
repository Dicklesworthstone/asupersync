...
**fourth-wave governor proof map:** `docs/fourth_wave_swarm_governor_runbook.md`, checked by `tests/fourth_wave_swarm_governor_runbook_contract.rs`, is the operator runbook for the fourth-wave governor lanes. Its focused manifest lane is `fourth-wave-governor-signoff-runbook`; cite it only for the runbook, README/AGENTS markers, manifest rows, proof-status rows, no-local-fallback language, decision taxonomy, rollback guidance, and no-claim boundaries. The proof-status snapshot distinguishes `fourth-wave-governor-schema-contract`, `fourth-wave-governor-policy-engine`, `fourth-wave-swarm-replay-corpus`, `fourth-wave-runtime-bridge-contract`, the fourth-wave benchmark no-claim contract, and the fourth-wave final aggregated signoff. The benchmark row records no fresh benchmark result and does not prove performance improvement, no regression, production-on-by-default control, broad workspace health, or RCH fleet availability.
