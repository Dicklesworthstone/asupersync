...
impl RuntimeInterface for DemoRuntime {
    ...
    fn oneshot_channel<T: Send + 'static>(
        ...
        let receiver: Self::OneshotReceiver<T> = Box::pin(async move { Err(OneshotRecvError) });
...
impl<T> DummyBroadcastReceiver<T> {
    fn new() -> Self {
        Self {
            _phantom: std::marker::PhantomData,
        }
...
fn atp_security_conformance_harness_demo() {
    ...
    for result in &summary.results {
        let status = if result.result.passed {
            "✅ PASS"
        } else {
            "❌ FAIL"
        };
