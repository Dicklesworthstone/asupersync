...
fn test_encoder_stats_display_golden() {
    ...
    insta::assert_snapshot!("encoder_stats_initial_scrubbed", encoder.stats().to_string());
    
    let _ = encoder.emit_systematic();
    insta::assert_snapshot!("encoder_stats_after_systematic_scrubbed", encoder.stats().to_string());
    
    let _ = encoder.emit_repair(4);
    insta::assert_snapshot!("encoder_stats_after_repair_scrubbed", encoder.stats().to_string());
}