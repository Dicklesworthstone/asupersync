...
const DOCS_PATH: &str = "docs/semantic_lint_cleanup_budget_rule.md";
...
fn contract_links_inventory_runner_docs_and_fixtures() {
    ...
    assert_eq!(
        ...
        Some("semantic-lint-cleanup-budget-rule-contract-v1")
    );
...
fn contract_matches_l1_inventory_decision() {
    ...
    let row = rows
        ...
        .expect("cleanup budget rule must be in inventory");
    ...
    let inventory_paths = array(row, "target_paths")
        ...
        .map(|value| value.as_str().expect("target path string"))
        .collect::<BTreeSet<_>>();
    assert!(target_paths.is_superset(&inventory_paths));
}

#[test]
fn runner_reports_unbounded_cleanup_fixture() {
    let (_, result) =
        run_runner(&["tests/fixtures/semantic_lint/cleanup_budget/positive_unbounded.rs"]);
    ...
    for finding in findings {
        assert_eq!(string(finding, "kind"), "unbounded_cleanup_budget");
        assert_eq!(string(finding, "severity"), "warning");
        assert!(
            string(finding, "diagnostic").contains("Budget")
                || string(finding, "diagnostic").contains("cleanup")
                || string(finding, "diagnostic").contains("duration"),
            "diagnostic must explain the budget boundary"
        );
    }
}

#[test]
fn runner_accepts_budget_derived_cleanup_paths() {
    let (_, result) =
        run_runner(&["tests/fixtures/semantic_lint/cleanup_budget/negative_bounded.rs"]);
...
fn allow_markers_require_reason_and_owner_bead() {
    let (_, allowed) = run_runner(&["tests/fixtures/semantic_lint/cleanup_budget/valid_allow.rs"]);
    ...
    let (_, invalid) =
        run_runner(&["tests/fixtures/semantic_lint/cleanup_budget/invalid_allow.rs"]);
    ...
    assert!(
        findings
            .iter()
            .any(|finding| string(finding, "kind") == "unbounded_cleanup_budget"),
        "invalid marker must not suppress the cleanup-budget finding"
    );
}
...
fn runner_output_is_stable_and_docs_name_no_claim_boundary() {
    let paths = [
        "tests/fixtures/semantic_lint/cleanup_budget/positive_unbounded.rs",
        "tests/fixtures/semantic_lint/cleanup_budget/valid_allow.rs",
        "tests/fixtures/semantic_lint/cleanup_budget/invalid_allow.rs",
    ];
    ...
    assert!(docs.contains("not a proof that every cleanup path is bounded"));
