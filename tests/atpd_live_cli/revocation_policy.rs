...
fn policy_bytes(generation: u64, revoked: &[&str]) -> Vec<u8> {
    serde_json::to_vec(&json!({"schema_version": 1, "generation": generation,
        "revoked_certificates": revoked})).unwrap()
}
...
fn reload(process: &Process) {
    kill(Pid::from_raw(i32::try_from(process.child.id()).unwrap()), Signal::SIGHUP).unwrap();
}
...
fn spawn_revoking(fixture: &Fixture, config: Value, path: &Path, ledger: Option<&Path>) -> Process {
    ...
    command.arg(if ledger.is_some() { "serve-durable" } else { "serve-resumable" })
        .arg("--config").arg(config).arg("--revocations").arg(path)
        .args(["--max-sessions", "4", "--max-sessions-per-client", "2",
            "--max-session-keys", "16", "--attempts-per-session", "8",
            "--idle-retention-secs", "60", "--proof-recovery-secs", "60"]);
    if let Some(ledger) = ledger {
        command.arg("--session-ledger").arg(ledger).arg("--recover-committed");
    }
    launch(fixture, command)
}

fn start_revoking(
    fixture: &Fixture, config: Value, path: &Path, ledger: Option<&Path>,
    generation: u64, denied: usize,
) -> (Process, SocketAddr) {
...
fn paths(directory: &Path) -> Vec<PathBuf> {
    let mut paths: Vec<_> = std::fs::read_dir(directory).unwrap()
...
fn staged_bytes(directory: &Path) -> Vec<Vec<u8>> {
    paths(directory).into_iter().filter(|path| {
        path.file_name().unwrap().to_string_lossy().starts_with(".atp-live-")
    }).map(|path| std::fs::read(path).unwrap()).collect()
}
...
fn sighup_revokes_one_acknowledged_client_while_another_finishes_on_the_same_listener() {
    for workers in [1, 2] {
        ...
        assert!(!paths(&fixture.inbox).iter().any(|path| path.extension().is_some_and(|ext| ext == "bin")));
        ...
        assert_eq!(healthy["transfer"]["receipt"]["sha256"], hex::encode(Sha256::digest(b"xyz")));
        let inode = std::fs::metadata(&published).unwrap();
        assert_eq!(paths(&other_inbox).iter().filter(|path| {
...
fn failed_requested_policy_cancels_and_reports_owned_work_before_unsuccessful_exit() {
    for variant in 0..5 {
        ...
        match variant {
            ...
            1 => replace_policy(&fixture, &policy, &policy_bytes(2, &[&unrelated, &fixture.allowed])),
        ...
        assert!(cancelled["transfer"]["completed_receipt"].is_null());
        assert_eq!(cancelled["publication"]["state"], "staged");
        assert!(peer.receive().is_err()); drop(peer);
        assert!(!process.exit().success());
        assert!(std::net::TcpStream::connect_timeout(&address, Duration::from_millis(250)).is_err());
        assert_eq!(fixture.entries(), retained);
        assert_eq!(staged_bytes(&fixture.inbox), vec![b"abc".to_vec()]);
    }
}
...
fn persisted_revocation_blocks_historical_proof_after_receiver_restart_without_changing_history() {
    ...
    let (mut process, address) = start_revoking(&fixture, config.clone(), &policy, Some(&ledger), 1, 0);
    let mut peer = Peer::connect(&fixture, address, "allowed");
    ...
    let published = fixture.inbox.join(complete["publication"]["filename"].as_str().unwrap());
    assert_eq!(std::fs::read(&published).unwrap(), b"abc");
    let inode = std::fs::metadata(&published).unwrap();
    assert_eq!(records(&ledger).len(), 2);
    let history = std::fs::read(&ledger).unwrap(); let retained = fixture.entries();
    replace_policy(&fixture, &policy, &policy_bytes(2, &[&fixture.allowed]));
    reload(&process);
    let applied = process.event("revocation_policy_applied");
    ...
    drop(peer); stop(&mut process);

    let (mut restarted, address) = start_revoking(&fixture, config, &policy, Some(&ledger), 2, 1);
    for nonce in [91, 92] { assert_tls_refused(&fixture, &mut restarted, address, nonce); }
...
fn invalid_policy_never_reaches_readiness_or_creates_an_inbox_sink() {
    for variant in 0..6 {
        ...
        let retained = fixture.entries();
        let mut process = spawn_revoking(&fixture, fixture.receiver_config(1, 64, 65536), &policy, None);
        assert!(!process.exit().success());
        assert!(process.events.try_iter().all(|event| event.unwrap()["event"] != "ready"));
        assert_eq!(fixture.entries(), retained);
        if variant == 0 { assert!(!policy.exists()); }
    }
}
