...
fn verify_resumed_file(fixture: &Fixture, sent: &Value, received: &Value, bytes: &[u8]) {
    ...
    assert_eq!(
        received["completed_receipt"]["sha256"],
    ...
    assert_eq!(
        entries.len(),
        3,
        "exactly one lock, one staging alias, and one destination"
    );
    assert_eq!(
        entries
            .iter()
...
fn fault_ready(directory: &Path, fault: Fault) -> bool {
    std::fs::read_dir(directory).unwrap().any(|entry| {
        ...
        match fault {
            Fault::Prefix(length) => {
                name.starts_with(".atp-live-") && entry.metadata().unwrap().len() >= length
            }
                ...
                let upstream = thread::spawn(move || {
                    ...
                    while !up_stop.load(Ordering::SeqCst) && !up_end.load(Ordering::SeqCst) {
                        match client_read.read(&mut bytes) {
                            Ok(0) => break,
                            Ok(n) => {
                                if server_write.write_all(&bytes[..n]).is_err() {
                                    break;
                                }
                            }
                            Err(error) if retry_read(&error) => {}
                ...
                while !stop.load(Ordering::SeqCst) && !ended.load(Ordering::SeqCst) {
                    match server_read.read(&mut bytes) {
                        ...
                        Ok(n) => {
                            ...
                            if client_write.write_all(&bytes[..n]).is_err() {
                                break;
                            }
                        }
                        Err(error) if retry_read(&error) => {}
...
fn executable_reconnect_reconciles_a_witnessed_lost_epoch_ack_without_new_inbox_charge() {
    for workers in [1, 2] {
        let bytes: Vec<u8> = (0..196_609_usize)
            .map(|n| n.wrapping_mul(37).to_le_bytes()[0])
            .collect();
        let fixture = Fixture::new();
        let (mut receiver, address) = fixture.resume_receiver(workers, bytes.len() as u64);
        ...
        assert!(
            relay.faulted.load(Ordering::SeqCst),
            "actual post-prefix response must be lost"
        );
        ...
        assert_eq!(
            receive_attempts[0]["transfer"]["flushed_prefix_bytes"],
            65536
        );
...
fn executable_reconnect_recovers_lost_final_proof_without_republishing() {
    ...
    assert!(
        receive_attempts
            .iter()
            .any(|record| record["transfer"]["receipt_reused"] == true
...
fn executable_sender_does_not_retry_certificate_refusal_and_receiver_never_publishes() {
    ...
    let mut sender =
        fixture.spawn_resumable("send-resumable", config, Some(b"not authorized"), 4, 25, 1);
    let (sent, attempts) = until(&mut sender, "send_result");
    assert_eq!(attempts.len(), 1);
    ...
    assert!(!sender.exit().success());
    kill(
        Pid::from_raw(i32::try_from(receiver.child.id()).unwrap()),
        Signal::SIGTERM,
    )
    .unwrap();
    ...
    assert_eq!(
        fixture.entries().len(),
        2,
        "startup created only one staging file and its lock"
    );
    assert!(
        !fixture
            .entries()
...
fn executable_shutdown_between_retries_preserves_partial_bytes_without_fabricated_eof() {
    ...
    kill(
        Pid::from_raw(i32::try_from(sender.child.id()).unwrap()),
        Signal::SIGTERM,
    )
    .unwrap();
...
fn executable_resume_attempt_budget_bounds_a_real_silent_peer() {
    ...
    let peer = thread::spawn(move || {
        ...
        for _ in 0..2 {
            ...
            assert!(
                ...
                "actual TLS ClientHello required"
            );
    ...
    let mut sender =
        fixture.spawn_resumable("send-resumable", config, Some(b"not delivered"), 2, 25, 1);
