...
fn certificate(name: &str) -> CertificateDer<'static> {
    ...
    let pem = if name == "ca" {
        ...
        fixture["identities"][name]["certificate"].as_str()
    }
    .unwrap();
...
fn run<T: Send + 'static>(workers: usize, future: impl Future<Output = T> + Send + 'static) -> T {
    ...
    let future: Pin<Box<dyn Future<Output = T> + Send>> = Box::pin(async move {
        ...
        asupersync::time::timeout(cx.now(), WAIT, future)
            .await
            .expect("shared journal deadline")
    });
...
impl PeerJob {
    fn start(f: impl FnOnce() + Send + 'static) -> Self {
...
impl Peer {
    fn connect(address: SocketAddr, name: &str) -> Self {
        ...
        .with_client_auth_cert(vec![certificate(name)], private_key(name))
        .unwrap();
...
impl AsyncWrite for Sink {
    fn poll_write(
        ...
        let maximum = this
            .stop_at
            .map_or(bytes.len(), |end| end.saturating_sub(this.written) as usize)
            .min(bytes.len());
        if maximum == 0 {
...
impl Catalog {
    ...
    fn factory(
        &self,
    ) -> impl Fn(Cx, ResumeSessionKey) -> Ready<io::Result<JournaledSession<Sink>>>
    + Clone
    + Send
    + Sync
    + 'static {
...
fn restored(root: &Path, name: &str, probe: Arc<Probe>) -> JournaledSession<Sink> {
    ...
    let retained = Reader {
        file: AsyncFile::from_std(std::fs::File::open(data).unwrap()),
        probe,
    };
...
fn two_clients_restore_partial_sessions_on_one_listener_without_rewriting_surviving_bytes() {
    for workers in [1, 2] {
        ...
        catalog.put(
            key("allowed"),
            fresh(&root, "allowed", None, 32, Arc::new(Probe::default())),
        );
        catalog.put(
            key("unlisted"),
            fresh(&root, "unlisted", Some(11), 32, Arc::clone(&blocked)),
        );
        let address = run(workers, async move {
            ...
            config.max_attempts_per_session = 4;
            ...
            loop {
                let completion = or(
                    async {
                        Some(
                            service
                                .next_journaled(&cx, &scope, catalog.factory())
                                .await
                                .unwrap()
                                .unwrap(),
                        )
            ...
            assert_eq!(
                service
                    .revoke_client(client("unlisted"), reason.clone())
                    .unwrap()
        ...
        for (name, bytes) in [
            ("allowed", b"abcdefgh".as_slice()),
            ("unlisted", b"qrstuvwxyzA".as_slice()),
        ] {
            let (wal, data) = paths(&root, name);
            ...
            original.insert(
                name,
                ((inode.dev(), inode.ino()), std::fs::read(wal).unwrap()),
            );
        }
        for committed_only in [false, true] {
            ...
            catalog.put(
                key("allowed"),
                restored(&root, "allowed", Arc::clone(&a_probe)),
            );
            catalog.put(
                key("unlisted"),
                restored(&root, "unlisted", Arc::clone(&b_probe)),
            );
            run(workers, async move {
                ...
                let mut service = authority
                    .bind_resumable_service::<Sink>(&cx, address, limits())
                    .await
                    .unwrap();
                assert_eq!(authority.active_streams(), 2);
                let jobs: Vec<_> = [
                    ("allowed", b"abcdefghijklmnop".as_slice()),
                    ("unlisted", b"qrstuvwxyzABCDEF".as_slice()),
                ]
                .into_iter()
                .map(|(name, bytes)| {
                    PeerJob::start(move || {
                ...
                assert_eq!(catalog.calls.load(Ordering::Relaxed), 2);
                assert_eq!(
                    a_probe.bytes.load(Ordering::Relaxed),
                    if committed_only { 0 } else { 8 }
                );
                assert_eq!(
                    b_probe.bytes.load(Ordering::Relaxed),
                    if committed_only { 0 } else { 5 }
                );
                assert_eq!(
                    a_probe.commits.load(Ordering::Relaxed),
                    usize::from(!committed_only)
                );
                assert_eq!(
                    b_probe.commits.load(Ordering::Relaxed),
                    usize::from(!committed_only)
                );
                assert!(service.drain_next().await.is_none());
                assert!(service.is_drained());
...
fn changing_the_next_api_cannot_remove_a_retained_sessions_journal_barriers() {
    ...
    catalog.put(
        key("allowed"),
        fresh(&root, "allowed", None, 1, Arc::clone(&probe)),
    );
    run(1, async move {
        let cx = Cx::current().unwrap();
        let scope = cx.scope();
        ...
        let mut service = authority
            ...
            .unwrap();
        let address = service.local_addr();
        let first = PeerJob::start(move || {
            Peer::connect(address, "allowed").hello().unwrap();
        });
        let result = service
            .next_journaled(&cx, &scope, catalog.factory())
            .await
            .unwrap()
            .unwrap();
        assert!(transfer(result.outcome).outcome.is_err());
        first.join();
        let second = PeerJob::start(move || {
            assert!(Peer::connect(address, "allowed").hello().is_err());
        });
    ...
    for workers in [1, 2] {
        ...
        run(workers, async move {
            ...
            let revocation = service
                .revoke_client(client("allowed"), reason.clone())
                .unwrap();
            assert!(revocation.newly_revoked);
            assert_eq!(revocation.signalled_connections, 1);
            assert_eq!(revocation.retained_sessions, 1);
            let premature = or(
                async { Some(service.next_journaled(&cx, &scope, catalog.factory()).await) },
                async {
                    for _ in 0..16 {
...
fn invalid_restoration_is_tombstoned_without_reads_for_wrong_identity_or_policy() {
    for variant in ["client", "data", "attempts"] {
        ...
        let probe = Arc::new(Probe::default());
        let catalog = Catalog::default();
        let peer_name = if variant == "client" {
            "unlisted"
        } else {
            "allowed"
        };
        catalog.put(
            key(peer_name),
            restored(&root, "allowed", Arc::clone(&probe)),
        );
        run(2, async move {
            let cx = Cx::current().unwrap();
            let scope = cx.scope();
            ...
            let mut config = limits();
            if variant == "attempts" {
                config.max_attempts_per_session = 3;
            }
            let mut service = authority
                .bind_resumable_service::<Sink>(&cx, "127.0.0.1:0".parse().unwrap(), config)
                .await
            ...
            if variant == "data" {
                assert!(probe.reads.load(Ordering::Relaxed) > 0);
            } else {
    ...
    for workers in [1, 2] {
        ...
        for round in 0..3 {
            ...
            let requested = if round == 0 {
                ...
                std::fs::read_to_string(root.join("endpoint"))
                    .unwrap()
                    .parse()
                    .unwrap()
            };
            let address = run(workers, async move {
                ...
                let mut service = authority
                    .bind_resumable_service::<_>(&cx, requested, config)
                    .await
                    .unwrap();
                let address = service.local_addr();
                assert_eq!(authority.active_streams(), 2);
                ...
                .map(|(name, bytes)| {
                    PeerJob::start(move || {
                        ...
                        let expected = if round == 0 || bytes.is_empty() {
                            0
                        } else if round == 1 {
                            8
                        } else {
                        ...
                        assert_eq!(
                            &state[108..140],
                            Sha256::digest(&bytes[..expected as usize]).as_slice()
                        );
                ...
                for _ in 0..2 {
                    let completion = service
                        ...
                        .unwrap()
                        .unwrap();
                    let name = if completion.session.unwrap() == key("allowed") {
                        "allowed"
                    } else {
                        "unlisted"
                    };
            ...
            if round == 0 {
                let mut endpoint = OpenOptions::new()
                    .write(true)
                    .create_new(true)
                    .open(root.join("endpoint"))
                    .unwrap();
                write!(endpoint, "{address}").unwrap();
            }
            for name in ["allowed", "unlisted"] {
                let (wal, data) = paths(&root, name);
                let expected = if name == "unlisted" {
                    b"".as_slice()
                } else if round == 0 {
                    b"abcdefgh".as_slice()
                } else {
                ...
                if round == 0 {
                    original_history.insert(name, history);
                } else {
                    assert!(history.starts_with(&original_history[name]));
                }
            }
        }
...
fn built_in_shared_restore_does_not_trust_file_handoff_as_content_validation() {
    ...
    run(1, async move {
        ...
        let mut service = authority
            .bind_resumable_service::<_>(&cx, "127.0.0.1:0".parse().unwrap(), limits())
            .await
            .unwrap();
        let address = service.local_addr();
        let peer = PeerJob::start(move || {
        ...
        run(2, async move {
            ...
            match report.outcome {
                Err(ResumeError::ReceiverJournal(error)) => {
                    ...
                    assert_eq!(
                        error.source.as_ref().map(io::Error::kind),
                        fail.then_some(io::ErrorKind::StorageFull)
                    );
                    assert!(matches!(error.interruption.as_deref(),
                        Some(LiveStreamError::Cancelled(Some(actual))) if actual == &reason));
                }
            ...
            assert_eq!(
                report.completed.as_ref().unwrap().source_sha256,
                Sha256::digest(b"abcdefgh").as_slice()
            );
