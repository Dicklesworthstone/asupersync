...
const CONTRACT_PATH: &str = "artifacts/semantic_lint_ignored_outcome_rule_contract_v1.json";
const INVENTORY_PATH: &str = "artifacts/semantic_lint_rule_inventory_v1.json";
const DOCS_PATH: &str = "docs/semantic_lint_ignored_outcome_rule.md";
const RUNNER_PATH: &str = "scripts/semantic_lint.py";
const RULE_ID: &str = "ignored-outcome-severity";
...
fn contract_links_inventory_runner_docs_and_fixtures() {
    ...
    assert_eq!(
        ...
        Some("semantic-lint-ignored-outcome-rule-contract-v1")
    );
    ...
    for key in [
        ...
        "runner",
        "docs",
        "positive_cancelled_fixture",
        "positive_ignored_fixture",
        "negative_fixture",
        "valid_allow_fixture",
    ...
    assert_eq!(string(implementation, "selected_engine"), "rustc-hir");
...
fn contract_matches_l1_inventory_decision() {
    ...
    let row = rows
        ...
        .expect("ignored Outcome rule must be in inventory");

    assert_eq!(string(row, "owner_bead"), BEAD_ID);
    assert_eq!(string(row, "selected_engine"), "rustc-hir");
...
#[test]
fn runner_reports_cancelled_to_ok_and_ignored_outcome_fixtures() {
    let (_, result) = run_runner(&[
        "tests/fixtures/semantic_lint/ignored_outcome/positive_cancelled_to_ok.rs",
        "tests/fixtures/semantic_lint/ignored_outcome/positive_ignored_outcome.rs",
    ]);
    ...
    assert_eq!(result["engine"].as_str(), Some("rustc-hir"));
    assert_eq!(result["verdict"].as_str(), Some("fail"));
    assert_eq!(result["summary"]["findings"].as_u64(), Some(2));

    let kinds = array(&result, "findings")
        .iter()
        .map(|finding| string(finding, "kind"))
        .collect::<BTreeSet<_>>();
    assert_eq!(
        kinds,
        BTreeSet::from(["cancelled_collapsed_to_ok", "ignored_outcome_value"])
    );
    for finding in array(&result, "findings") {
        assert!(
            string(finding, "diagnostic").contains("Outcome"),
            "diagnostic must name the Outcome severity risk"
        );
    }
}

#[test]
fn runner_accepts_severity_preserving_conversion() {
    let (_, result) =
        run_runner(&["tests/fixtures/semantic_lint/ignored_outcome/negative_preserve_severity.rs"]);
...
fn allow_markers_require_reason_and_owner_bead() {
    let (_, allowed) = run_runner(&["tests/fixtures/semantic_lint/ignored_outcome/valid_allow.rs"]);
    ...
    let (_, invalid) =
        run_runner(&["tests/fixtures/semantic_lint/ignored_outcome/invalid_allow.rs"]);
    ...
    assert!(
        findings
            .iter()
            .any(|finding| string(finding, "kind") == "ignored_outcome_value"),
        "invalid marker must not suppress the Outcome finding"
    );
}
...
fn runner_output_is_stable_and_docs_name_no_claim_boundary() {
    let paths = [
        "tests/fixtures/semantic_lint/ignored_outcome/positive_cancelled_to_ok.rs",
        "tests/fixtures/semantic_lint/ignored_outcome/positive_ignored_outcome.rs",
        "tests/fixtures/semantic_lint/ignored_outcome/valid_allow.rs",
        "tests/fixtures/semantic_lint/ignored_outcome/invalid_allow.rs",
    ];
