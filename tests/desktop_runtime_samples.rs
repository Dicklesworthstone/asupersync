...
use asupersync::desktop_profile::{
    DesktopRuntimeProfile, DesktopRuntimeStartError, ForeignCallCompletion, run_foreign_call,
};
use asupersync::desktop_samples::{
    HostClockSample, OwnedRuntimeSample, SampleClockError, validate_priority_max_meet,
};
...
fn owned_sample_refuses_unbounded_profile_before_any_thread_spawns() {
    ...
    assert!(matches!(error, DesktopRuntimeStartError::InvalidProfile(_)));
}
...
fn priority_max_meet_yields_max_priority_and_tightest_bounds() {
    ...
    assert_eq!(
        met.deadline.map(Time::as_nanos),
        child.deadline.map(Time::as_nanos)
    );
...
fn meet_is_commutative_and_infinite_is_the_identity() {
    ...
    assert_eq!(
        ab.priority, 200,
        "priority-max selects the more urgent side"
    );
...
fn assert_budgets_equal(actual: &Budget, expected: &Budget, context: &str) {
    ...
    assert_eq!(
        actual.poll_quota, expected.poll_quota,
        "{context}: poll quota"
    );
    assert_eq!(
        actual.cost_quota, expected.cost_quota,
        "{context}: cost quota"
    );
