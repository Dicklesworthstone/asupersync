...
fn cert(name: &str) -> CertificateDer<'static> {
    let f = fixtures();
    let text = if name == "ca" { &f["ca"] } else { &f["identities"][name]["certificate"] };
...
fn roots() -> RootCertStore {
    let mut roots = RootCertStore::empty(); roots.add(cert("ca")).unwrap(); roots
}
fn sdk() -> AtpSdk {
...
fn sender() -> LiveStreamSender {
    sdk().live_stream_sender(config(Duration::from_secs(5)), ServerName::try_from("localhost").unwrap(),
        roots(), identity("allowed")).unwrap()
}
fn receiver(timeout: Duration) -> LiveStreamReceiver {
    let id = NativeClientCertificateId::from_certificate(&cert("allowed"));
    sdk().live_stream_receiver(config(timeout), identity("server"),
        NativeClientAuthorization::new(roots(), [id]).unwrap()).unwrap()
}
fn run<T: Send + 'static>(workers: usize, f: impl Future<Output = T> + Send + 'static) -> T {
    ...
    while !runtime.is_quiescent() {
        assert!(start.elapsed() < Duration::from_secs(5), "children must drain");
...
async fn witnessed(cx: &Cx, test: impl Fn() -> bool) {
    asupersync::time::timeout(cx.now(), Duration::from_secs(5), async {
        while !test() { yield_now().await; }
    }).await.expect("required causal witness was not reached");
}
...
impl Gate {
    ...
    fn wait(&self, cx: &Context<'_>, receipt: &LiveStreamReceipt) -> bool {
        ...
        let retired = self.waiter.lock().unwrap().replace(candidate); drop(retired);
        {
            let mut bound = self.receipt.lock().unwrap();
            if let Some(bound) = bound.as_ref() { assert_eq!(bound, receipt); }
            else { *bound = Some(receipt.clone()); }
        }
...
impl AsyncWrite for Transaction {
    ...
    fn poll_flush(self: Pin<&mut Self>, _: &mut Context<'_>) -> Poll<io::Result<()>> {
        self.gate.flushes.fetch_add(1, Ordering::SeqCst); Poll::Ready(Ok(()))
    }
...
impl LiveStreamCommitSink for Transaction {
    fn poll_commit(mut self: Pin<&mut Self>, cx: &mut Context<'_>, receipt: &LiveStreamReceipt) -> Poll<io::Result<()>> {
        ...
        if !self.gate.wait(cx, receipt) { return Poll::Pending; }
        self.done = true;
        self.gate.completed.fetch_add(1, Ordering::SeqCst);
        Poll::Ready(if self.fail { Err(io::Error::from(io::ErrorKind::PermissionDenied)) } else { Ok(()) })
    }
}
fn transaction(gate: &Arc<Gate>, fail: bool) -> Transaction {
    Transaction { gate: Arc::clone(gate), bytes: Vec::new(), fail, done: false }
}
...
fn final_proof_waits_for_application_commit_on_both_native_backends() {
    for workers in [1, 2] {
        run(workers, async {
            ...
            let receive = receiver(Duration::from_secs(5)); let send = sender();
            let listener = receive.bind(&cx, "127.0.0.1:0".parse().unwrap()).await.unwrap();
            let address = listener.local_addr().unwrap(); let gate = Arc::new(Gate::default());
            let mut receiving = listener.spawn_receive_committing(&cx, &scope, transaction(&gate, false)).unwrap();
            let mut sending = send.spawn_send_reader(&cx, &scope, address, b"verified-data".as_slice()).unwrap();
            witnessed(&cx, || gate.entered.load(Ordering::SeqCst)).await;
            assert!(gate.flushes.load(Ordering::SeqCst) >= 2);
            assert!(!sending.is_finished(), "flush alone must not establish sender completion");
            assert!(!receiving.is_finished()); assert_eq!(receive.active_streams(), 1);
            gate.release();
...
fn commit_failure_prevents_proof_and_preserves_the_actual_sink_error() {
    run(1, async {
        let cx = Cx::current().unwrap(); let send = sender();
        let receive = receiver(Duration::from_secs(5));
        let listener = receive.bind(&cx, "127.0.0.1:0".parse().unwrap()).await.unwrap();
        let address = listener.local_addr().unwrap(); let gate = Arc::new(Gate::default()); gate.release();
        let mut sink = transaction(&gate, true);
        let (sent, received) = zip(send.send_reader(&cx, address, b"data".as_slice()), listener.receive_committing(&cx, &mut sink)).await;
        assert!(sent.outcome.is_err());
        match received.outcome.unwrap_err() {
            LiveStreamError::Commit(error) => match *error {
                LiveStreamCommitError::Unconfirmed { receipt, interruption, source } => {
                    assert_eq!(receipt.prefix.bytes, 4); assert!(interruption.is_none());
        ...
        assert_eq!(sink.bytes, b"data"); assert_eq!(gate.completed.load(Ordering::SeqCst), 1);
    });
}
...
fn source_failure_and_legacy_flush_only_calls_never_commit_the_sink() {
    ...
    impl AsyncRead for FailedSource {
        fn poll_read(mut self: Pin<&mut Self>, _: &mut Context<'_>, out: &mut ReadBuf<'_>) -> Poll<io::Result<()>> {
            if self.0 { return Poll::Ready(Err(io::Error::from(io::ErrorKind::InvalidData))); }
            self.0 = true; out.put_slice(b"part"); Poll::Ready(Ok(()))
        }
    }
    run(1, async {
        let cx = Cx::current().unwrap(); let send = sender();
        ...
        for legacy in [false, true] {
            ...
            if legacy {
                let (sent, received) = zip(send.send_reader(&cx, address, b"part".as_slice()), listener.receive_into(&cx, &mut sink)).await;
                assert_eq!(sent.outcome.unwrap(), received.outcome.unwrap());
            } else {
                let (sent, received) = zip(send.send_reader(&cx, address, FailedSource(false)), listener.receive_committing(&cx, &mut sink)).await;
                assert!(sent.outcome.is_err()); assert!(received.outcome.is_err());
            }
            assert_eq!(sink.bytes, b"part"); assert!(!gate.entered.load(Ordering::SeqCst));
        }
    });
}
...
fn cancellation_drains_a_started_commit_and_keeps_both_terminal_facts() {
    for fail in [false, true] {
        run(2, async move {
            ...
            let listener = receive.bind(&cx, "127.0.0.1:0".parse().unwrap()).await.unwrap();
            let address = listener.local_addr().unwrap(); let gate = Arc::new(Gate::default());
            let mut receiving = listener.spawn_receive_committing(&cx, &scope, transaction(&gate, fail)).unwrap();
            let mut sending = send.spawn_send_reader(&cx, &scope, address, b"data".as_slice()).unwrap();
            witnessed(&cx, || gate.entered.load(Ordering::SeqCst)).await;
            let polls = gate.polls.load(Ordering::SeqCst);
            let reason = CancelReason::user("cancel during commit"); receiving.abort_with_reason(reason.clone());
            ...
            assert!(!receiving.is_finished()); assert_eq!(receive.active_streams(), 1);
            gate.release();
            let received = receiving.join(&cx).await.expect("acknowledged cancellation retains the commit outcome");
            ...
            let cause = match *error {
                LiveStreamCommitError::CommittedWithoutProof { receipt, source } if !fail => {
                    assert_eq!(receipt.prefix.bytes, 4); source
                }
                LiveStreamCommitError::Unconfirmed { receipt, interruption, source } if fail => {
                    assert_eq!(receipt.prefix.bytes, 4); assert_eq!(source.kind(), io::ErrorKind::PermissionDenied);
...
fn commit_timeout_does_not_release_credit_before_the_sink_finishes() {
    run(1, async {
        let cx = Cx::current().unwrap(); let scope = cx.scope();
        let receive = receiver(Duration::from_secs(1)); let send = sender();
        let listener = receive.bind(&cx, "127.0.0.1:0".parse().unwrap()).await.unwrap();
        let address = listener.local_addr().unwrap(); let gate = Arc::new(Gate::default());
        let mut receiving = listener.spawn_receive_committing(&cx, &scope, transaction(&gate, false)).unwrap();
        let mut sending = send.spawn_send_reader(&cx, &scope, address, b"data".as_slice()).unwrap();
...
fn service_drain_keeps_committing_workers_and_their_receipts() {
    run(2, async {
        ...
        let gate = Arc::new(Gate::default()); let factory_gate = Arc::clone(&gate);
        let factory = move |_child: Cx, _peer| {
            let gate = Arc::clone(&factory_gate); async move { Ok(transaction(&gate, false)) }
        };
        let mut sending = send.spawn_send_reader(&cx, &scope, service.local_addr(), b"data".as_slice()).unwrap();
        or(async {
        ...
        service.stop_accepting();
        {
            ...
            assert!(drain.as_mut().poll(&mut Context::from_waker(Waker::noop())).is_pending());
        }
        assert_eq!(receive.active_streams(), 1); assert!(!service.is_drained());
        gate.release();
        let received = service.drain_next().await.unwrap().result.unwrap();
        assert_eq!(received.transfer.outcome.unwrap(), sending.join(&cx).await.unwrap().outcome.unwrap());
...
mod files {
    ...
    fn directory() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("atp-live-commit-{}-{stamp}-{}",
            std::process::id(), SEQ.fetch_add(1, Ordering::Relaxed)));
        std::fs::DirBuilder::new().mode(0o700).create(&dir).unwrap(); dir
    }
    ...
    fn service_publishes_verified_files_without_clobbering_existing_destinations() {
        for workers in [1, 2] {
            run(workers, async {
                let cx = Cx::current().unwrap(); let scope = cx.scope(); let dir = directory();
                let receive = receiver(Duration::from_secs(5)); let send = sender();
                let mut service = receive.bind_service(&cx, "127.0.0.1:0".parse().unwrap(), 1).await.unwrap();
                let publications = Arc::new(Mutex::new(Vec::new()));
                for attempt in 0..2 {
                    let target = dir.clone(); let observed = Arc::clone(&publications);
                    ...
                    let (sent, received) = zip(send.send_reader(&cx, address, b"verified".as_slice()),
                        service.next_committing(&cx, &scope, factory)).await;
                ...
                assert_eq!(publications[1].status().error_kind, Some(io::ErrorKind::AlreadyExists));
                ...
                assert_eq!((original.dev(), original.ino()), (final_file.dev(), final_file.ino()));
    ...
    fn file_flush_does_not_publish_and_commit_rehashes_actual_staging_bytes() {
        run(1, async {
            let cx = Cx::current().unwrap(); let dir = directory();
            let mut sink = LiveFileSink::create(&cx, dir, "object.bin".into(), 4).await.unwrap();
            let publication = sink.publication();
            sink.write_all(b"good").await.unwrap(); sink.flush().await.unwrap();
            ...
            let receipt = LiveStreamReceipt {
                prefix: asupersync::net::atp::sdk::native_auth::live::LiveStreamPrefix {
                    stream_nonce: [1; 32], epochs: 1, bytes: 4, chain: [2; 32],
                },
                source_sha256: Sha256::digest(b"good").into(),
            };
            let error = poll_fn(|ctx| Pin::new(&mut sink).poll_commit(ctx, &receipt)).await.unwrap_err();
            assert_eq!(error.kind(), io::ErrorKind::InvalidData);
            assert!(!publication.destination_path().exists());
            assert_eq!(publication.status().error_kind, Some(io::ErrorKind::InvalidData));
    ...
    fn file_sink_rejects_unsafe_names_and_limits_without_touching_other_files() {
        run(1, async {
            let cx = Cx::current().unwrap(); let dir = directory();
            for name in ["", ".", "..", "../escape", "/absolute", "a/b", "a\\b", ".atp-live-reserved"] {
                assert!(LiveFileSink::create(&cx, dir.clone(), name.into(), 4).await.is_err());
            ...
            assert!(!publication.destination_path().exists());
            drop(sink); assert_eq!(std::fs::read(publication.staging_path()).unwrap(), b"data");
        });
    }
}
    ...
    run(1, async {
        ...
        let peer = async {
            ...
            let mut tls = rustls::ClientConfig::builder_with_provider(Arc::new(rustls::crypto::ring::default_provider()))
                ...
                .with_root_certificates(roots()).with_client_auth_cert(vec![cert("allowed")], key).unwrap();
            ...
            hello.extend_from_slice(&4u32.to_be_bytes()); hello.extend_from_slice(&1000u64.to_be_bytes());
            let frame = Frame::new(ProtocolVersion::V0, FrameType::Handshake, hello).unwrap();
            stream.write_all(&frame.to_wire_bytes().unwrap()).await.unwrap(); stream.flush().await.unwrap();
            let mut codec = AtpFrameCodec::new(); let mut buffer = BytesMut::new();
            let ack = loop {
                if let Some(frame) = codec.decode(&mut buffer).unwrap() { break frame; }
                let mut bytes = [0; 256]; let count = stream.read(&mut bytes).await.unwrap();
                assert!(count > 0); buffer.extend_from_slice(&bytes[..count]);
            };
            assert_eq!(ack.frame_type(), FrameType::HandshakeAck);
            let wrong = Frame::new(ProtocolVersion::V0, FrameType::ObjectComplete, vec![0; 80]).unwrap();
            stream.write_all(&wrong.to_wire_bytes().unwrap()).await.unwrap(); stream.flush().await.unwrap();
        };
        let (_, received) = zip(peer, listener.receive_committing(&cx, &mut sink)).await;
        assert!(matches!(received.outcome, Err(LiveStreamError::Protocol("wrong final stream commitment"))));
        assert!(!gate.entered.load(Ordering::SeqCst)); assert!(sink.bytes.is_empty());
    });
}
