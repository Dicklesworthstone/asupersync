...
mod raptorq_decoder_metamorphic_tests {
    ...
    use asupersync::types::{Budget, ObjectId, RegionId, TaskId, Time};
    ...
    fn make_lab_runtime(seed: u64) -> LabRuntime {
        LabRuntime::builder()
            .deterministic()
            .with_virtual_clock(VirtualClock::starting_at(Time::from_secs(seed + 1000)))
            .build()
    }
    ...
    fn mr1_symbol_addition_monotonicity() {
        ...
        proptest!(|(
            ...
            let runtime = make_lab_runtime(seed);
                ...
                match minimal_result {
                    Ok(minimal_decoded) => {
                        ...
                        match extended_result {
                            Ok(extended_decoded) => {
                                let extended_data = reconstruct_original_data(&extended_decoded.source, data.len());
                                prop_assert_eq!(
        ...
        proptest!(|(
            ...
            runtime.block_on(async {
                ...
                if symbols.len() >= k + 3 {
                    ...
                    if let (Ok(sys_decoded), Ok(mixed_decoded)) = (&systematic_result, &mixed_result) {
            ...
            runtime.block_on(async {
                ...
                match (original_result, permuted_result) {
                    ...
                    (Err(_), Ok(_)) => {
                        prop_assert!(
                            false,
                            "MR3 VIOLATION: permutation improved decode success"
                        );
                    }
                    (Err(_), Err(_)) => {
    ...
    fn mr5_repair_symbol_convergence() {
        ...
        proptest!(|(
            ...
            runtime.block_on(async {
                ...
                for (step, symbol_count, result) in &convergence_results {
                    match result {
                        Ok(decoded) => {
                            let decoded_data = reconstruct_original_data(&decoded.source, data.len());
                            prop_assert_eq!(
                        ...
                        Err(_) => {
                            ...
                            if success_found {
                                prop_assert!(
                                    ...
                                    step, first_success_step.unwrap()
                                );
                            }
                        }
                    }
