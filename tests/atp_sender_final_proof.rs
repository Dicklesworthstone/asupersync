...
use asupersync::net::atp::sdk::native_auth::live::commit::resume::finalization::{
    FinalProofCheckpoint, FinalProofStore,
};
...
fn run<T: Send + 'static>(workers: usize, future: impl Future<Output = T> + Send + 'static) -> T {
    ...
    let future: Pin<Box<dyn Future<Output = T> + Send>> = Box::pin(async move {
        ...
        asupersync::time::timeout(cx.now(), WAIT, future)
            .await
            .expect("final-Proof test deadline")
    });
...
impl AsyncRead for Source {
    fn poll_read(
        ...
        out.put_slice(&self.remaining[..count]);
...
impl Wire {
    fn new(tcp: TcpStream, config: Arc<rustls::ServerConfig>) -> Self {
        ...
        Self {
            tls: rustls::StreamOwned::new(rustls::ServerConnection::new(config).unwrap(), tcp),
    ...
    fn start(path: PathBuf, reply: Reply) -> Self {
        ...
        let worker = thread::spawn(move || {
            ...
            let mut config = rustls::ServerConfig::builder_with_provider(provider)
                ...
                .with_single_cert(vec![cert("server")], key("server"))
                .unwrap();
            ...
            let Some(mut first) = accept() else {
                return 0;
            };
            let hello = first.receive().unwrap();
            assert_eq!(hello.frame_type(), FrameType::Handshake);
            let hello = hello.payload().to_vec();
            assert_eq!(hello.len(), 60);
            assert_eq!(
                first.tls.conn.peer_certificates().unwrap()[0],
                cert("allowed")
            );
            let mut h = Sha256::new();
            h.update(b"asupersync.atp.live.resume.hello.v1");
            h.update(&hello);
            let mut prefix = vec![0; 16];
            prefix.extend_from_slice(&h.finalize());
            let mut state = hello.clone();
            state.extend_from_slice(&prefix);
            state.extend_from_slice(&Sha256::digest(b""));
            state.push(0);
            ...
            let final_value = loop {
                ...
                if frame.frame_type() == FrameType::ObjectComplete {
                    assert!(
                        !matches!(reply, Reply::StoreFailure),
                        "store failure leaked finalization"
                    );
                ...
                chain.update(&prefix[16..]);
                chain.update(epoch);
                data.extend_from_slice(&epoch[80..]);
                epochs += 1;
                prefix = epochs.to_be_bytes().to_vec();
                prefix.extend_from_slice(&(data.len() as u64).to_be_bytes());
            ...
            let Some(mut recovered) = accept() else {
                return data.len();
            };
            let request = recovered.receive().unwrap();
            assert_eq!(request.frame_type(), FrameType::Handshake);
            assert_eq!(request.payload(), hello);
            let mut state = hello;
            state.extend_from_slice(&final_value);
            state.push(1);
            match reply {
            ...
            if matches!(reply, Reply::Incomplete | Reply::ChangedPrefix) {
                assert!(
                    recovered.receive().is_err(),
                    "refused state must not receive a finalization request"
                );
            } else {
                let request = recovered.receive().unwrap();
                assert_eq!(
                    ...
                    "recovery retransmitted source bytes"
                );
...
fn capture(path: &Path, address: SocketAddr, workers: usize, data: &'static [u8]) {
    ...
    run(workers, async move {
        ...
        assert_eq!(
            store.checkpoint().unwrap().intent().prefix.bytes,
            data.len() as u64
        );
...
fn recover(path: &Path, address: SocketAddr, workers: usize, success: bool) {
    ...
    run(workers, async move {
        ...
        wrong.set_port(address.port().wrapping_add(1));
        assert!(
            authority
                .restore_final_proof(&cx, wrong, checkpoint.clone(), 2)
                .is_err()
        );
        assert_eq!(authority.active_streams(), 0);
        let mut restored = authority
            .restore_final_proof(&cx, address, checkpoint.clone(), 1)
            .unwrap();
        ...
        if success {
            ...
            let cached = restored.send(&cx).await;
            ...
            assert!(report.completed.is_none());
            assert!(matches!(
                restored.send(&cx).await.outcome,
...
fn source_free_recovery_requires_committed_exact_state_and_exact_proof() {
    for workers in [1, 2] {
        for reply in [
            ...
            let path = directory().join("final.checkpoint");
            let mut peer = Peer::start(path.clone(), reply);
...
fn persistence_failure_withholds_object_complete() {
    let path = directory().join("never-created");
    let mut peer = Peer::start(path.clone(), Reply::StoreFailure);
...
fn final_proof_subprocess_worker() {
    let Ok(phase) = std::env::var("ASUPERSYNC_FINAL_PROOF_TEST_PHASE") else {
        return;
    };
    let path = PathBuf::from(std::env::var_os("ASUPERSYNC_FINAL_PROOF_TEST_PATH").unwrap());
    let address = std::env::var("ASUPERSYNC_FINAL_PROOF_TEST_ADDRESS")
        .unwrap()
        .parse()
        .unwrap();
    if phase == "capture" {
        capture(&path, address, 1, DATA);
    } else {
        assert_eq!(phase, "recover");
        recover(&path, address, 2, true);
    }
...
fn a_new_sender_process_recovers_proof_without_the_original_source_owner() {
    let path = directory().join("sender.checkpoint");
    let mut peer = Peer::start(path.clone(), Reply::Exact);
    for phase in ["capture", "recover"] {
        let log = std::fs::OpenOptions::new()
            .write(true)
            .create_new(true)
            .open(path.with_extension(format!("{phase}-log")))
            .unwrap();
        let mut child = ChildGuard(
            Command::new(std::env::current_exe().unwrap())
                ...
                .env(
                    "ASUPERSYNC_FINAL_PROOF_TEST_ADDRESS",
        ...
        child.wait();
        assert_eq!(
            std::fs::read(path.with_extension(format!("{phase}-witness"))).unwrap(),
            b"verified\n"
        );
    }
    assert_eq!(peer.finish(), DATA.len());
...
fn empty_source_recovery_also_requires_prior_remote_commit() {
    for reply in [Reply::Exact, Reply::Incomplete] {
        let path = directory().join("empty.checkpoint");
        let mut peer = Peer::start(path.clone(), reply);
...
struct SlowStore {
    finish: Option<Pin<Box<dyn Future<Output = ()> + Send>>>,
    polls: usize,
}
impl FinalProofStore for SlowStore {
    fn poll_store(
        ...
        if this.finish.is_none() {
            this.finish = Some(Box::pin(asupersync::time::sleep(
...
fn started_store_is_drained_after_timeout_without_sending_finalization() {
    let path = directory().join("memory-only-test-store");
    let mut peer = Peer::start(path, Reply::StoreFailure);
    let address = peer.address;
    run(2, async move {
        let cx = Cx::current().unwrap();
        let authority = sender();
        let mut transfer = authority.resumable_reader(&cx, address, DATA, 1).unwrap();
        let mut store = SlowStore {
            finish: None,
            polls: 0,
        };
        let report = transfer.send_checkpointed(&cx, &mut store).await;
        match report.outcome {
            Err(ResumeError::Checkpoint(error)) => {
                ...
                assert!(matches!(
                    error.interruption.as_deref(),
                    Some(
                        asupersync::net::atp::sdk::native_auth::live::LiveStreamError::Timeout(
