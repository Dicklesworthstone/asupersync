...
fn metric_sample_unit_round_trips_within_scale() {
    ...
    for units in [-9_000_000_000_i64, -42, -1, 0, 1, 7, 1_000, 9_000_000_000] {
        ...
        assert_eq!(sample.as_units(), units, "whole-unit round trip for {units}");
        assert_eq!(
...
fn build_detector_preserves_series_kind_and_starts_fresh() {
    ...
    for row in rows {
        ...
        assert_eq!(snapshot.sample_count, 0, "a built detector has consumed 0 samples");
...
fn build_detector_is_pure_and_independent() {
    ...
    assert_eq!(first, second, "two builds from one template are byte-identical");

    let mut live = row.build_detector();
    let fired = live.update(MetricSample::from_units(100));
    assert!(fired.is_some(), "control: the chosen sample crosses the threshold");
    ...
    assert_eq!(after, second, "post-advance build still equals the original build");
}
...
fn assembly_is_equivalent_across_config_builder_and_register() {
    ...
    assert_eq!(via_config, via_chain, "config.build_monitor == chained builder");
    assert_eq!(via_chain, via_register, "chained builder == imperative register");
...
fn disabled_monitor_is_inert_and_advances_no_state() {
    ...
    for value in [10, 10, 10, 10, 10, 18, 18, 18, 18, 18] {
        ...
        assert!(got.is_none(), "a disabled monitor never reports a detection");
    }
    assert_eq!(
...
fn conservative_scheduler_defaults_build_disabled_and_inert() {
    ...
    assert!(!monitor.is_enabled(), "conservative profile is disabled until opt-in");
    assert_eq!(monitor.len(), 4, "the four design-note series are installed");
    ...
    for series in [
        ...
        for value in [0, 0, 0, 1_000, 1_000, 1_000] {
            assert!(monitor.observe(series, MetricSample::from_units(value)).is_none());
        }
    }
    assert!(
    ...
    let enabled =
        ChangePointMonitorConfig::conservative_scheduler_defaults().enable().build_monitor();
