...
use asupersync::io::replay::{
    IoCaptureError, IoCaptureLimits, IoReplayCompletionError, IoReplayError,
    IoReplayMismatch, IoTape, IoTapeDecodeLimits, RecordingIo,
};
...
use asupersync::util::entropy_replay::{EntropyCaptureLimits, EntropyForkMatching, RecordingEntropy};
...
fn limits() -> IoCaptureLimits { IoCaptureLimits::new(256, 4096, 4096, 32) }
fn decode_limits() -> IoTapeDecodeLimits { IoTapeDecodeLimits::new(65536, limits(), 65536) }

fn run_native<T: Send + 'static>(
    workers: usize, entropy: Option<Arc<dyn EntropySource>>,
    future: impl Future<Output = T> + Send + 'static,
) -> T {
    let mut builder = if workers == 1 { RuntimeBuilder::current_thread() }
        else { RuntimeBuilder::multi_thread().worker_threads(workers).with_sharded_state(true) };
    if let Some(entropy) = entropy { builder = builder.with_entropy_source(entropy); }
    ...
    while !runtime.is_quiescent() {
        assert!(start.elapsed() < Duration::from_secs(5), "native work failed to drain");
...
#[derive(Clone, Copy)]
enum Reply { Valid, BadChecksum, Truncated }
...
impl Peer {
    fn new(reply: Reply, gated: bool) -> Self {
        ...
        let worker = std::thread::spawn(move || {
            ...
            let (mut stream, _) = loop {
                match listener.accept() {
                    ...
                    Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                        assert!(start.elapsed() < Duration::from_secs(15), "client never connected");
            ...
            if gated { wait.recv_timeout(Duration::from_secs(10)).unwrap(); }
            ...
            match reply {
                ...
                Reply::Valid | Reply::BadChecksum => {
                    for chunk in BODY.chunks(3) { stream.write_all(chunk).unwrap(); }
                    let checksum = BODY.iter().fold(0u8, |sum, byte| sum.wrapping_add(*byte));
                    stream.write_all(&[checksum ^ u8::from(matches!(reply, Reply::BadChecksum))]).unwrap();
                }
            }
            stream.shutdown(std::net::Shutdown::Write).unwrap();
            command
        });
        Self { address, release, worker: Some(worker) }
    }

    fn finish(mut self) -> [u8; 8] {
        self.worker.take().unwrap().join().expect("real peer must finish successfully")
    }
}
...
async fn read_reply<S: AsyncRead + AsyncWrite + Unpin>(stream: &mut S) -> io::Result<Vec<u8>> {
    ...
    if length > 128 { return Err(io::Error::from(io::ErrorKind::InvalidData)); }
    let mut bytes = vec![0; length];
    stream.read_exact(&mut bytes).await?;
    let mut checksum = [0; 1];
    stream.read_exact(&mut checksum).await?;
    let mut eof = [0; 1];
    if stream.read(&mut eof).await? != 0 { return Err(io::Error::from(io::ErrorKind::InvalidData)); }
...
async fn transaction<S: AsyncRead + AsyncWrite + Unpin>(stream: &mut S, command: &[u8; 8]) -> ResultShape {
    async {
...
fn capture(workers: usize, reply: Reply, cap: IoCaptureLimits) -> (ResultShape, Result<IoTape, IoCaptureError>) {
    ...
    assert_eq!(peer.finish(), *COMMAND, "the consumer must contact the real peer");
    result
}
...
fn native_tcp_conversation_replays_after_peer_and_source_runtime_are_gone() {
    for workers in [1, 2] {
        ...
        assert_eq!(replay_in_lab(IoTape::from_canonical_bytes(encoded.as_ref(), decode_limits()).unwrap()), original);
        let actual = run_native(workers, None, async move {
            let mut replay = tape.replay();
            let result = transaction(&mut replay, COMMAND).await;
            replay.verify_complete().unwrap(); result
        });
...
fn real_protocol_failure_and_truncated_reply_are_reproduced_offline() {
    for (reply, kind) in [(Reply::BadChecksum, io::ErrorKind::InvalidData), (Reply::Truncated, io::ErrorKind::UnexpectedEof)] {
...
fn changed_request_is_sticky_even_when_the_consumer_ignores_the_error() {
    ...
    run_native(1, None, async move {
        ...
        let failure = *error.get_ref().unwrap().downcast_ref::<IoReplayError>().unwrap();
        ...
        assert_eq!(later.get_ref().unwrap().downcast_ref::<IoReplayError>(), Some(&failure));
        assert_eq!(buffer, [0xa5; 4]);
        assert_eq!(replay.verify_complete(), Err(IoReplayCompletionError::Diverged(failure)));
    });
}
...
fn dropping_a_witnessed_pending_read_does_not_capture_fabricated_eof() {
    ...
    let tape = run_native(1, None, async move {
        ...
        let (socket, tape) = captured.into_parts(); drop(socket); tape.unwrap()
    });
...
fn native_context_entropy_and_io_replay_together_without_original_providers() {
    for workers in [1, 2] {
        let entropy = Arc::new(RecordingEntropy::new(Arc::new(OsEntropy),
            EntropyCaptureLimits::new(256, 4096, 64)).unwrap());
        let peer = Peer::new(Reply::Valid, false);
        let address = peer.address;
        let (tape, expected_request) = run_native(workers, Some(entropy.clone()), async move {
            let cx = Cx::current().unwrap();
            let mut request = [0; 8]; cx.random_bytes(&mut request);
            let socket = TcpStream::connect(address).await.unwrap();
            let mut capture = RecordingIo::new(socket, limits());
            assert_eq!(transaction(&mut capture, &request).await, Ok(BODY.to_vec()));
            let (socket, tape) = capture.into_parts(); drop(socket); (tape.unwrap(), request)
        });
        assert_eq!(peer.finish(), expected_request);
        let saved = entropy.finish().unwrap(); drop(entropy);
        ...
        run_native(workers, Some(Arc::new(replay_entropy.clone())), async move {
            let cx = Cx::current().unwrap();
            let mut request = [0; 8]; cx.random_bytes(&mut request);
...
fn persisted_io_capture_replays_in_an_independent_process() {
    ...
    let directory = std::env::temp_dir().join(format!("asupersync-io-replay-{}-{stamp}", std::process::id()));
    std::fs::create_dir(&directory).unwrap();
    std::fs::write(directory.join("public-io.tape"), encoded.as_ref()).unwrap();
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .args(["--exact", "persisted_io_capture_replays_in_an_independent_process", "--nocapture"])
        .env(CHILD, &directory).spawn().unwrap();
    let start = Instant::now();
    let status = loop {
        if let Some(status) = child.try_wait().unwrap() { break status; }
        if start.elapsed() > Duration::from_secs(15) {
            child.kill().unwrap(); let _ = child.wait(); panic!("offline child failed to terminate");
        }
        std::thread::sleep(Duration::from_millis(5));
    };
    assert!(status.success());
    assert_eq!(std::fs::read(directory.join("replayed")).unwrap(), b"exact bad-checksum result",
