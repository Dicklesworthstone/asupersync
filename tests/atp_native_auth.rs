...
use asupersync::net::atp::transport_quic::{QuicConfig, QuicReceiveOptions, ReceiveReport, SendReport};
...
fn fixtures() -> &'static Fixtures {
    ...
    FIXTURES.get_or_init(|| serde_json::from_str(include_str!(
        "fixtures/atp_native_auth_identities.json"
...
fn certificate(pem: &str) -> CertificateDer<'static> {
    CertificateDer::pem_reader_iter(&mut std::io::Cursor::new(pem.as_bytes()))
        .next().expect("certificate present").expect("valid fixture PEM")
}
...
fn key(name: &str) -> PrivateKeyDer<'static> {
    PrivateKeyDer::pem_reader_iter(&mut std::io::Cursor::new(fixtures().identities[name].key.as_bytes()))
        .next().expect("key present").expect("valid fixture key")
}
...
fn sender(name: &str) -> NativeTransferClient {
    sdk().native_sender_with_identity(
        config(), ServerName::try_from("localhost").unwrap(), roots(), identity(name),
    ).unwrap()
}

fn receiver(policy: NativeClientAuthorization) -> NativeTransferClient {
    sdk().authorized_native_receiver(config(), identity("server"), policy).unwrap()
}
...
fn anonymous_sender() -> NativeTransferClient {
    ...
    config.client_tls = Some(QuicClientTls {
        ...
        config: client_config(vec![certificate(&fixtures().ca)], vec![ATP_QUIC_ALPN.to_vec()]).unwrap(),
    });
...
fn fixture(label: &str) -> PathBuf {
    ...
    let root = std::env::temp_dir().join(format!(
        "asupersync-native-auth-{label}-{}-{stamp}-{sequence}", std::process::id(),
    ));
...
fn run<T: Send + 'static>(workers: usize, future: impl Future<Output = T> + Send + 'static) -> T {
    ...
    while !runtime.is_quiescent() {
        assert!(Instant::now() < deadline, "native authentication workers did not drain");
        runtime.block_on(async { asupersync::runtime::yield_now().await; });
    }
...
async fn transfer(
    ...
    let bound = receiver.bind_receiver(
        cx, "127.0.0.1:0".parse().unwrap(), destination, QuicReceiveOptions::default(),
    ).await.unwrap();
    ...
    (sent.expect("sender canonical join"), received.expect("receiver canonical join"))
}
...
fn assert_delivered(results: TransferResults, expected: &[u8]) {
    ...
    assert_eq!(std::fs::read(&received.committed_paths[0]).unwrap(), expected);
}

fn assert_tls_refused(results: TransferResults, destination: &Path) {
    let (sent, received) = results;
    assert!(sent.is_err(), "unauthorized sender must not obtain a receipt");
    let error = received.expect_err("unauthorized receiver must fail the TLS handshake");
    assert!(format!("{error:?}").contains("read_hs_fatal_alert"),
        "must reach actual TLS rejection, not fail for an absent peer/configuration: {error:?}");
    assert!(!destination.exists(), "unauthorized handshake must not enter filesystem publication");
}
...
fn native_mtls_allowed_client_delivers_on_both_native_backends() {
    for workers in [1, 2] {
        ...
        run(workers, async move {
            ...
            assert_delivered(transfer(&cx, &send, &receive, &source, &root.join("received")).await, &bytes);
        });
...
fn native_mtls_anonymous_and_valid_unlisted_clients_cannot_publish() {
    run(2, async move {
        ...
        let receive = receiver(authorization(&["allowed"]));
        for (index, send) in [anonymous_sender(), sender("unlisted")].into_iter().enumerate() {
            let destination = root.join(format!("denied-{index}"));
            assert_tls_refused(transfer(&cx, &send, &receive, &source, &destination).await, &destination);
        }
...
fn native_mtls_pins_do_not_bypass_issuer_expiry_or_client_purpose() {
    run(1, async move {
        ...
        for name in ["foreign", "expired", "server"] {
            ...
            assert_tls_refused(transfer(&cx, &sender(name), &receive, &source, &destination).await, &destination);
        }
    });
}
...
fn native_mtls_allowlist_rotation_is_shared_and_cannot_resume_old_authority() {
    run(2, async move {
        ...
        let send = sender("allowed");
        assert_delivered(transfer(&cx, &send, &receive, &source, &root.join("first")).await, b"rotation");
        ...
        assert_tls_refused(transfer(&cx, &send, &receive, &source, &denied).await, &denied);
        controller.replace_allowed([selector("allowed")]).unwrap();
        assert!(matches!(controller.replace_allowed(std::iter::repeat_n(
            selector("unlisted"), MAX_NATIVE_AUTHORIZED_CLIENTS + 1,
        )), Err(NativeAuthenticationError::TooManyClients)));
        assert_eq!(controller.allowed_count(), 1, "failed update must preserve old authority");
        assert_delivered(transfer(&cx, &send, &receive, &source, &root.join("restored")).await, b"rotation");
    });
}
...
fn native_mtls_client_identity_does_not_disable_server_name_verification() {
    run(1, async move {
        ...
        let send = sdk().native_sender_with_identity(config(), ServerName::try_from("wrong.invalid").unwrap(),
        ...
        assert!(format!("{error:?}").contains("read_hs_fatal_alert"), "{error:?}");
...
fn native_mtls_writer_uses_authorized_transport_and_retains_peer_receipt() {
    run(2, async move {
        ...
        let send = sender("allowed");
        let receive = receiver(authorization(&["allowed"]));
        let bound = receive.bind_receiver(&cx, "127.0.0.1:0".parse().unwrap(), root.join("received"),
            QuicReceiveOptions::default()).await.unwrap();
        let mut writer = send.open_writer(&cx, &scope, bound.local_addr(),
            NativeUploadOptions::new(&root, "object.bin")).unwrap();
        writer.write_all(b"authenticated producer input").await.unwrap();
        writer.flush().await.unwrap();
        assert!(writer.terminal().is_none(), "spooling is not authenticated delivery");
        ...
        assert_eq!(sent.transfer_id, received.transfer_id);
        assert!(sent.receipt.committed && sent.receipt.sha_ok && sent.receipt.merkle_ok);
        assert_eq!(std::fs::read(&received.committed_paths[0]).unwrap(), b"authenticated producer input");
...
fn native_mtls_checkpoint_retry_reauthenticates_instead_of_trusting_local_journal() {
    use asupersync::net::atp::sdk::native::upload::recovery::{NativeCheckpointRetry, NativeCheckpointState};
    run(2, async move {
        let cx = Cx::current().unwrap();
        let root = fixture("checkpoint");
        let policy = authorization(&["allowed"]);
        let receive = receiver(policy.clone());
        let bound = receive.bind_receiver(&cx, "127.0.0.1:0".parse().unwrap(), root.join("denied"),
            QuicReceiveOptions::default()).await.unwrap();
        let remote = bound.local_addr();
        let prepared = sender("allowed").prepare_buffer(&cx, remote, NativeUploadOptions::new(&root, "data.bin"),
            b"retained and authenticated").await.unwrap();
        ...
        let (attempt, received) = zip(unlisted.send_checkpoint(&cx, &directory, remote,
            NativeCheckpointRetry::Never), bound.receive(&cx)).await;
        ...
        let bound = receive.bind_receiver(&cx, remote, root.join("received"), QuicReceiveOptions::default()).await.unwrap();
        let allowed = sender("allowed");
        let (attempt, received) = zip(allowed.send_checkpoint(&cx, &directory, remote,
            NativeCheckpointRetry::AcknowledgeUncertain { attempt: 1 }), bound.receive(&cx)).await;
        let attempt = attempt.unwrap();
        let received = received.unwrap();
        ...
        assert_eq!(std::fs::read(&received.committed_paths[0]).unwrap(), b"retained and authenticated");
    });
}

#[test]
fn native_mtls_configuration_is_role_restricted_and_rejects_bad_identity() {
    assert!(matches!(sdk().native_sender_with_identity(config(), ServerName::try_from("localhost").unwrap(),
        RootCertStore::empty(), identity("allowed")), Err(NativeAuthenticationError::EmptyRoots)));
    let mismatched = NativeTlsIdentity::new(vec![leaf("allowed")], key("unlisted")).unwrap();
    assert!(matches!(sdk().native_sender_with_identity(config(), ServerName::try_from("localhost").unwrap(),
        roots(), mismatched), Err(NativeAuthenticationError::Tls(_))));
    run(1, async move {
        let cx = Cx::current().unwrap();
        let root = fixture("roles");
        let send = sender("allowed");
        assert!(matches!(send.bind_receiver(&cx, "127.0.0.1:0".parse().unwrap(), &root,
            QuicReceiveOptions::default()).await, Err(NativeTransferError::MissingServerTls)));
        let receive = receiver(authorization(&["allowed"]));
        assert!(matches!(receive.send_path(&cx, "127.0.0.1:9".parse().unwrap(), &root).await,
