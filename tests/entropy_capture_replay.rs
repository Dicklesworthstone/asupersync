...
use asupersync::util::entropy_replay::{
    EntropyCaptureError, EntropyCaptureLimits, EntropyReplayCompletionError,
    EntropyRequest, EntropyTape, EntropyTapeDecodeLimits, RecordingEntropy,
};
...
fn run_contexts(entropy: Arc<dyn EntropySource>, seed: u64) -> Values {
    ...
    for label in 0..8 {
        ...
        let (task, _handle) = lab.state.create_task(region, Budget::INFINITE, async move {
    ...
    assert_eq!(values.len(), 8, "a task panic cannot masquerade as a successful replay");
...
fn task_contexts_replay_actual_os_entropy_without_a_seed_or_provider_fallback() {
    ...
    assert!(tape.streams() >= 9, "the runtime must actually fork task-local providers");
...
fn caught_context_divergence_cannot_turn_runtime_quiescence_into_replay_success() {
    fn scenario(entropy: Arc<dyn EntropySource>, changed: bool) -> bool {
        ...
        let (task, _handle) = lab.state.create_task(region, Budget::INFINITE, async move {
            ...
            let outcome = catch_unwind(AssertUnwindSafe(|| {
                if changed { cx.random_bytes(&mut bytes[..12]); }
                else { cx.random_bytes(&mut bytes); }
            }));
            if changed { assert_eq!(bytes, [0xa5; 13]); }
    ...
    let error = replay.failure().expect("the caught panic must leave a sticky refusal");
...
fn capture_finish_refuses_a_witnessed_inflight_provider_and_can_be_retried() {
    ...
    impl EntropySource for Parked {
        fn fill_bytes(&self, _: &mut [u8]) { panic!("unexpected byte call"); }
        fn next_u64(&self) -> u64 {
            self.entered.send(()).unwrap();
            self.release.lock().unwrap().recv_timeout(Duration::from_secs(10)).unwrap();
            0x1234_5678_9abc_def0
        }
        fn fork(&self, _: TaskId) -> Arc<dyn EntropySource> { panic!("unexpected fork"); }
        fn source_id(&self) -> &'static str { "parked-test" }
    }
    let (entered_tx, entered) = mpsc::sync_channel(1);
    let (release, release_rx) = mpsc::sync_channel(1);
    let capture = Arc::new(RecordingEntropy::new(Arc::new(Parked {
    ...
    witness.unwrap();
    assert!(matches!(unfinished, Err(EntropyCaptureError::InFlight { calls: 1 })));
    let replay = capture.finish().unwrap().replay();
...
fn provider_unwind_is_preserved_and_never_published_as_a_complete_capture() {
    ...
    impl EntropySource for Panics {
        fn fill_bytes(&self, _: &mut [u8]) { panic!("provider sentinel"); }
        fn next_u64(&self) -> u64 { panic!("provider sentinel"); }
        fn fork(&self, _: TaskId) -> Arc<dyn EntropySource> { panic!("provider sentinel"); }
        fn source_id(&self) -> &'static str { "panicking-test" }
    }
    for request in 0..3 {
        ...
        let result = catch_unwind(AssertUnwindSafe(|| match request {
            ...
            1 => { capture.next_u64(); }
            _ => { capture.fork(TaskId::from_arena(ArenaIndex::new(1, 2))); }
        }));
        ...
        assert_eq!(capture.failure(), Some(EntropyCaptureError::InterruptedCall));
        assert!(matches!(capture.finish(), Err(EntropyCaptureError::InterruptedCall)));
    }
}
...
fn provider_can_reenter_capture_inspection_without_a_bookkeeping_lock_deadlock() {
    ...
    impl EntropySource for Reentrant {
        ...
        fn next_u64(&self) -> u64 {
            ...
            assert!(matches!(capture.finish(), Err(EntropyCaptureError::InFlight { calls: 1 })));
            42
        }
        fn fork(&self, _: TaskId) -> Arc<dyn EntropySource> { panic!("unexpected fork"); }
        fn source_id(&self) -> &'static str { "reentrant-test" }
    }
    ...
    *source.0.lock().unwrap() = Arc::downgrade(&capture);
    ...
    let worker_capture = Arc::clone(&capture);
    let worker = std::thread::spawn(move || { done.send(worker_capture.next_u64()).unwrap(); });
    let value = completed.recv_timeout(Duration::from_secs(5))
...
fn concurrent_child_sources_replay_in_reverse_order_with_exact_values() {
    ...
    let expected: Vec<_> = workers.into_iter().map(|thread| thread.join().unwrap()).collect();
    let replay = capture.finish().unwrap().replay();
    let children: Vec<_> = (0..8).map(|index| {
        replay.try_fork(TaskId::from_arena(ArenaIndex::new(index, 9))).unwrap()
    }).collect();
    for index in (0..8).rev() {
        let actual: Vec<_> = (0..32).map(|_| children[index].try_next_u64().unwrap()).collect();
...
fn entropy_capture_survives_producer_process_exit_and_replays_context_calls() {
    ...
    const TEST_NAME: &str = "entropy_capture_survives_producer_process_exit_and_replays_context_calls";
    ...
    if let Some(directory) = std::env::var_os(CHILD_DIRECTORY) {
        ...
        let capture = Arc::new(RecordingEntropy::new(Arc::new(DetEntropy::new(44)), limits()).unwrap());
        let expected = run_contexts(capture.clone(), 57);
        let tape = capture.finish().unwrap();
        let bytes = tape.to_canonical_bytes(decode_limits().max_encoded_bytes).unwrap();
        write_new(&directory.join("tape.bin"), bytes.as_ref());
        write_new(&directory.join("values.json"), &serde_json::to_vec(&expected).unwrap());
        return;
    }
    ...
    let directory = std::env::temp_dir().join(format!(
        "asupersync-entropy-replay-{}-{stamp}", std::process::id(),
    ));
    ...
    let status = loop {
        ...
        assert!(began.elapsed() < Duration::from_secs(15), "producer process did not terminate");
        std::thread::sleep(Duration::from_millis(2));
    };
    assert!(status.success(), "the producer must actually execute successfully");
    let encoded = std::fs::read(directory.join("tape.bin")).unwrap();
    let expected: Values = serde_json::from_slice(&std::fs::read(directory.join("values.json")).unwrap()).unwrap();
    assert_eq!(expected.len(), 8, "a zero-test subprocess cannot pass the witness");
    let tape = EntropyTape::from_canonical_bytes(&encoded, decode_limits()).unwrap();
    let replay = tape.replay();
    let actual = run_contexts(Arc::new(replay.clone()), 999);
    assert_eq!(actual, expected);
    replay.verify_complete().unwrap();
    assert!(directory.join("tape.bin").exists(), "fixtures are retained, never auto-deleted");
}
