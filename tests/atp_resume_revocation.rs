...
use asupersync::net::atp::sdk::native_auth::live::commit::resume::service::{
    MAX_REVOKED_RESUME_CLIENTS, ResumeRevokeError, ResumeRetireError, ResumeServiceConfig,
...
fn certificate(name: &str) -> CertificateDer<'static> {
    let data = fixture();
    let pem = if name == "ca" { data["ca"].as_str() }
        else { data["identities"][name]["certificate"].as_str() }.unwrap();
...
fn receiver() -> LiveStreamReceiver {
    let authorization = NativeClientAuthorization::new(roots(), [id("allowed"), id("unlisted")]).unwrap();
    sdk().live_stream_receiver(profile(), identity("server"), authorization).unwrap()
}
...
fn limits() -> ResumeServiceConfig {
    ResumeServiceConfig {
        max_connections: 4, max_sessions: 4, max_sessions_per_client: 2,
...
fn run(workers: usize, future: impl Future<Output = ()> + Send + 'static) {
    ...
    let future: Pin<Box<dyn Future<Output = ()> + Send>> = Box::pin(async move {
        ...
        asupersync::time::timeout(cx.now(), Duration::from_secs(30), future).await.unwrap();
    });
    runtime.block_on(runtime.handle().spawn(future));
    let started = Instant::now();
    while !runtime.is_quiescent() {
        assert!(started.elapsed() < Duration::from_secs(5), "revocation left owned tasks live");
...
async fn witness(cx: &Cx, condition: impl Fn() -> bool) {
    ...
    }).await.expect("required parked state was never reached");
}
fn start<P: Policy>(
    cx: &Cx, scope: &Scope<'_, P>, address: SocketAddr, name: &str, bytes: &'static [u8],
    ...
    cx.spawn_in(scope, move |child| {
        let future: Pin<Box<dyn Future<Output = ResumeReport> + Send>> = Box::pin(async move {
            let mut outgoing = authority.resumable_reader(&child, address, bytes, 1).unwrap();
...
fn transfer(outcome: ResumeServiceOutcome) -> ResumeReport {
    match outcome { ResumeServiceOutcome::Transfer(report) => report, other => panic!("expected transfer: {other:?}") }
}
...
impl AsyncWrite for Sink {
    fn poll_write(self: Pin<&mut Self>, cx: &mut Context<'_>, bytes: &[u8]) -> Poll<io::Result<usize>> {
        ...
        self.probe.bytes.lock().unwrap().extend_from_slice(&bytes[..count]);
        Poll::Ready(Ok(count))
    }
...
impl Store {
    fn factory(&self) -> impl Fn(Cx, ResumeSessionKey) -> Ready<io::Result<Sink>> + Clone + Send + Sync + 'static {
        ...
        move |_, key| {
            ...
            assert!(store.probes.lock().unwrap().insert(key, Arc::clone(&probe)).is_none());
    ...
    fn client(&self, client: NativeClientCertificateId) -> Option<(ResumeSessionKey, Arc<Probe>)> {
        self.probes.lock().unwrap().iter().find(|(key, _)| key.client == client)
...
impl Wire {
    async fn connect(address: SocketAddr, name: &str) -> Self {
        let mut config = rustls::ClientConfig::builder_with_provider(Arc::new(rustls::crypto::ring::default_provider()))
            ...
            .with_client_auth_cert(vec![certificate(name)], key(name)).unwrap();
    ...
    async fn hello(&mut self, nonce: [u8; 32]) -> io::Result<Frame> {
        let encoded = Frame::new(ProtocolVersion::V0, FrameType::Handshake, hello(nonce)).unwrap()
...
fn revocation_stops_only_the_target_parked_writer_and_preserves_its_join() {
    for workers in [1, 2] {
        run(workers, async {
            let cx = Cx::current().unwrap(); let scope = cx.scope(); let receiver = receiver();
            let mut service = receiver.bind_resumable_service::<Sink>(&cx, "127.0.0.1:0".parse().unwrap(), limits()).await.unwrap();
            let store = Store { pause_write: true, ..Store::default() };
            let _release = ReleaseAll(store.clone());
            let target = id("allowed"); let other = id("unlisted");
            ...
            }, witness(&cx, || [target, other].iter().all(|id| {
                store.client(*id).is_some_and(|(_, probe)| probe.write_parked.load(Ordering::SeqCst))
            }))).await;
            ...
            assert_eq!((changed.signalled_connections, changed.retained_sessions), (1, 1));
            ...
            match report.outcome {
                Err(ResumeError::Transfer(LiveStreamError::Cancelled(Some(actual)))) => assert_eq!(actual, reason),
            ...
            assert!(!probe.dropped.load(Ordering::SeqCst));
            assert_eq!(service.session_status(&other_key), Some(ResumeSessionStatus::Active));
            ...
            assert!(probe.dropped.load(Ordering::SeqCst));
            assert!(poll_fn(|ctx| a.poll_join(ctx)).await.unwrap().outcome.is_err());
            other_probe.release();
            let completion = service.next(&cx, &scope, store.factory()).await.unwrap().unwrap();
            assert_eq!(completion.session, Some(other_key));
            assert!(transfer(completion.outcome).outcome.is_ok());
            assert!(poll_fn(|ctx| b.poll_join(ctx)).await.unwrap().outcome.is_ok());
...
fn handshake_started_before_revocation_cannot_create_a_sink_or_consume_a_key() {
    run(2, async {
        ...
        let mut wire = or(async {
            let early = service.next(&cx, &scope, store.factory()).await;
            panic!("no hello exists to complete: {early:?}");
        }, Wire::connect(address, "allowed")).await;
        let changed = service.revoke_client(id("allowed"), CancelReason::user("certificate revoked")).unwrap();
        assert_eq!(changed.signalled_connections, 0);
        let (refused, completion) = zip(wire.hello([7; 32]), service.next(&cx, &scope, store.factory())).await;
        assert!(refused.is_err());
        assert!(matches!(completion.unwrap().unwrap().outcome, ResumeServiceOutcome::Rejected(ResumeServiceRejection::Revoked)));
        assert_eq!((store.calls.load(Ordering::SeqCst), service.resident_sessions(), service.retained_keys()), (0, 0, 0));
        drop(wire);
        // New nonces and new connections cannot bypass the same certificate denial.
        let address = service.local_addr();
        let (refused, completion) = zip(async {
            Wire::connect(address, "allowed").await.hello([8; 32]).await
...
fn revoking_a_started_commit_drains_and_retains_committed_without_proof() {
    for workers in [1, 2] {
        run(workers, async {
            let cx = Cx::current().unwrap(); let scope = cx.scope(); let receiver = receiver();
            let mut service = receiver.bind_resumable_service::<Sink>(&cx, "127.0.0.1:0".parse().unwrap(), limits()).await.unwrap();
            let store = Store { pause_commit: true, ..Store::default() };
            let _release = ReleaseAll(store.clone());
            let mut sending = start(&cx, &scope, service.local_addr(), "allowed", b"abcdefgh");
            or(async {
                let early = service.next(&cx, &scope, store.factory()).await;
                panic!("commit completed before release: {early:?}");
            }, witness(&cx, || store.client(id("allowed")).is_some_and(|(_, probe)| probe.commit_parked.load(Ordering::SeqCst)))).await;
            let (key, probe) = store.client(id("allowed")).unwrap();
            let reason = CancelReason::user("revoke during publication");
            assert_eq!(service.revoke_client(key.client, reason.clone()).unwrap().signalled_connections, 1);
            ...
            assert_eq!(receiver.active_streams(), 4);
            probe.release();
            let report = transfer(service.next(&cx, &scope, store.factory()).await.unwrap().unwrap().outcome);
            let committed = report.completed.as_ref().expect("completed commit must survive revocation");
            match &report.outcome {
                Err(ResumeError::Transfer(LiveStreamError::Commit(error))) => match error.as_ref() {
                    LiveStreamCommitError::CommittedWithoutProof { receipt, source } => {
                        ...
                        assert!(matches!(source.as_ref(), LiveStreamError::Cancelled(Some(actual)) if actual == &reason));
                    }
            ...
            assert!(probe.dropped.load(Ordering::SeqCst));
            assert!(poll_fn(|ctx| sending.poll_join(ctx)).await.unwrap().outcome.is_err());
            assert!(service.drain_next().await.is_none());
        });
    }
}
...
fn revocation_preserves_completed_receipts_but_refuses_their_reconnects() {
    run(1, async {
        ...
        let complete = service.next(&cx, &scope, store.factory()).await.unwrap().unwrap();
        let key = complete.session.unwrap();
        let report = transfer(complete.outcome);
        assert!(report.outcome.is_ok());
        assert!(poll_fn(|ctx| sending.poll_join(ctx)).await.unwrap().outcome.is_ok());
        let changed = service.revoke_client(key.client, CancelReason::user("deny receipt access")).unwrap();
        assert_eq!((changed.signalled_connections, changed.retained_sessions), (0, 1));
        assert_eq!(service.session_snapshot(&key).unwrap().completed, report.completed);
        let again = service.revoke_client(key.client, CancelReason::user("must not change prior attribution")).unwrap();
        assert!(!again.newly_revoked);
        assert_eq!(service.revoked_clients(), 1);
        let address = service.local_addr();
        let (refused, completion) = zip(async {
            Wire::connect(address, "allowed").await.hello(key.nonce).await
        }, service.next(&cx, &scope, store.factory())).await;
        assert!(refused.is_err());
        assert!(matches!(completion.unwrap().unwrap().outcome, ResumeServiceOutcome::Rejected(ResumeServiceRejection::Revoked)));
        assert_eq!(store.calls.load(Ordering::SeqCst), 1);
        assert_eq!(service.retire(&key).unwrap().unwrap().completed, report.completed);
        assert!(service.is_client_revoked(&key.client));
        assert!(service.drain_next().await.is_none());
        assert!(service.is_client_revoked(&key.client));
        assert!(!service.revoke_client(key.client, CancelReason::user("duplicate after drain")).unwrap().newly_revoked);
        assert_eq!(service.revoke_client(id("unlisted"), CancelReason::user("already drained")).unwrap_err(), ResumeRevokeError::Drained);
    });
}
...
fn revoked_client_never_invokes_a_historical_receipt_restoration_factory() {
    run(2, async {
        ...
        assert!(service.revoke_client(client, CancelReason::user("old receipts do not renew authority")).unwrap().newly_revoked);
        let calls = Arc::new(AtomicUsize::new(0)); let count = Arc::clone(&calls);
        let address = service.local_addr();
        let (refused, completion) = zip(async {
            Wire::connect(address, "allowed").await.hello([9; 32]).await
        }, service.next_restoring(&cx, &scope, move |_, key| {
            count.fetch_add(1, Ordering::SeqCst);
            let mut hash = Sha256::new(); hash.update(b"asupersync.atp.live.resume.hello.v1"); hash.update(hello(key.nonce));
            ready(Ok(ResumeSessionInit::<Sink>::Committed(LiveStreamReceipt {
                prefix: LiveStreamPrefix { stream_nonce: key.nonce, epochs: 0, bytes: 0, chain: hash.finalize().into() },
...
fn revocation_cancels_a_parked_factory_without_recreating_its_session() {
    run(2, async {
        let cx = Cx::current().unwrap(); let scope = cx.scope(); let receiver = receiver();
        let mut service = receiver.bind_resumable_service::<Sink>(&cx, "127.0.0.1:0".parse().unwrap(), limits()).await.unwrap();
        let entered = Arc::new(AtomicBool::new(false)); let marker = Arc::clone(&entered);
        ...
        }, witness(&cx, || entered.load(Ordering::SeqCst))).await;
        let reason = CancelReason::user("revoke during factory");
        assert_eq!(service.revoke_client(id("allowed"), reason.clone()).unwrap().signalled_connections, 1);
        ...
        assert_eq!((service.resident_sessions(), service.retained_keys()), (0, 1));
        assert!(poll_fn(|ctx| sending.poll_join(ctx)).await.unwrap().outcome.is_err());
        assert!(service.drain_next().await.is_none());
    });
}
...
fn revocation_capacity_is_independent_of_session_capacity_and_never_evicts_denials() {
    run(1, async {
        let cx = Cx::current().unwrap(); let receiver = receiver();
        let mut service = receiver.bind_resumable_service::<Sink>(&cx, "127.0.0.1:0".parse().unwrap(), limits()).await.unwrap();
        for number in 0..MAX_REVOKED_RESUME_CLIENTS as u32 {
            let mut digest = [0; 32]; digest[..4].copy_from_slice(&number.to_be_bytes());
            assert!(service.revoke_client(NativeClientCertificateId::from_sha256(digest), CancelReason::user("bounded denial")).unwrap().newly_revoked);
        }
        assert_eq!(service.revoked_clients(), MAX_REVOKED_RESUME_CLIENTS);
        assert_eq!((service.retained_keys(), service.resident_sessions(), service.in_flight()), (0, 0, 0));
        assert_eq!(service.revoke_client(id("allowed"), CancelReason::user("must not pretend success")).unwrap_err(), ResumeRevokeError::Capacity);
        assert!(!service.is_client_revoked(&id("allowed")));
        assert!(service.is_client_revoked(&NativeClientCertificateId::from_sha256([0; 32])));
        assert!(!service.revoke_client(NativeClientCertificateId::from_sha256([0; 32]), CancelReason::user("idempotent at capacity")).unwrap().newly_revoked);
...
fn revocation_does_not_relabel_an_uncollected_success_as_cancellation() {
    run(2, async {
        ...
        let store = Store { pause_commit: true, ..Store::default() };
        let _release = ReleaseAll(store.clone());
        let mut sending = start(&cx, &scope, service.local_addr(), "allowed", b"already committed");
        or(async {
            let early = service.next(&cx, &scope, store.factory()).await;
        ...
        assert_eq!(probe.commits.load(Ordering::SeqCst), 1);
        let changed = service.revoke_client(key.client, CancelReason::user("late revocation")).unwrap();
        ...
        assert_eq!(service.retire(&key).unwrap().unwrap().completed, Some(receipt));
        assert!(probe.dropped.load(Ordering::SeqCst));
        assert!(service.drain_next().await.is_none());
    });
}
