...
fn config(name: &'static str, server: bool) -> QuicConfig {
    let mut config = QuicConfig {
        chunk_size: 4096, max_transfer_bytes: 1024 * 1024,
        idle_timeout: Duration::from_secs(20), handshake_timeout: Duration::from_secs(20),
    ...
    if server {
        ...
        config.server_tls = Some(QuicServerTls {
            config: server_config(vec![certificate(LEAF)], key, vec![ATP_QUIC_ALPN.to_vec()]).unwrap(),
        });
    } else {
...
fn client(label: &str, config: QuicConfig, capacity: u32) -> NativeTransferClient {
    AtpSdk::new_in_process(SessionConfig {
        local_peer: PeerId::from_label(label), max_concurrent_transfers: capacity,
        ..SessionConfig::default()
    }).native_transfers(config).unwrap()
}

fn sender() -> NativeTransferClient { client("checkpoint-sender", config("localhost", false), 1) }
fn data() -> Vec<u8> { (0u8..=255).cycle().take(128 * 1024 + 17).collect() }

fn fixture() -> PathBuf {
    static NEXT: AtomicU64 = AtomicU64::new(0);
    let root = std::env::temp_dir().join(format!("atp-checkpoint-test-{}-{}-{}",
...
fn run<T: Send + 'static>(future: impl Future<Output = T> + Send + 'static) -> T {
    ...
    while !runtime.is_quiescent() {
        assert!(start.elapsed() < Duration::from_secs(5), "owned children did not drain");
...
fn finish_child(child: &mut OwnedChild) {
    ...
    loop {
        ...
        assert!(start.elapsed() < Duration::from_secs(30), "subprocess did not finish");
        std::thread::sleep(Duration::from_millis(1));
    }
}
fn child_context(mode: &str) -> Option<(PathBuf, SocketAddr)> {
    if std::env::var("ASUP_CHECKPOINT_CHILD").ok().as_deref() != Some(mode) { return None; }
    Some((PathBuf::from(std::env::var_os("ASUP_CHECKPOINT_ROOT").unwrap()),
        std::env::var("ASUP_CHECKPOINT_REMOTE").unwrap().parse().unwrap()))
}
...
fn no_packet(socket: &UdpSocket) {
    ...
    assert_eq!(socket.recv_from(&mut [0; 2048]).unwrap_err().kind(), io::ErrorKind::WouldBlock);
}
...
fn checkpoint_survives_process_exit_and_reuses_the_durable_peer_receipt() {
    if let Some((root, remote)) = child_context("prepare") {
        run(async move {
            ...
            let prepared = sender().prepare_buffer(&cx, remote,
                NativeUploadOptions::new(&root, "data.bin"), &data()).await.unwrap();
            assert!(prepared.cleanup_error.is_none());
            std::fs::write(root.join("checkpoint-path"), prepared.checkpoint.directory.to_str().unwrap()).unwrap();
        });
        return;
    }
    let root = fixture();
    ...
    let (checkpoint_dir, remote, original_id) = run(async move {
        ...
        let receiver = receiver_client.bind_receiver(&cx, "127.0.0.1:0".parse().unwrap(),
            root.join("received"), QuicReceiveOptions::default()).await.unwrap();
        let remote = receiver.local_addr();
        let mut child = start_child("checkpoint_survives_process_exit_and_reuses_the_durable_peer_receipt",
            &root, remote, "prepare");
        ...
        let info = sender.inspect_checkpoint(&cx, &directory, remote).await.unwrap();
        ...
        let scope = cx.scope();
        let mut send = sender.spawn_send_checkpoint(&cx, &scope, &directory, remote,
            NativeCheckpointRetry::Never).unwrap();
        ...
        assert_eq!(attempt.checkpoint.state, NativeCheckpointState::Acknowledged);
    ...
    run(async move {
        let cx = Cx::current().unwrap();
        let attempt = sender().send_checkpoint(&cx, &checkpoint_dir, remote,
            NativeCheckpointRetry::Never).await.unwrap();
        assert!(attempt.cached_receipt);
    ...
    no_packet(&socket);
    assert_eq!(std::fs::read(retained_root.join("keep")).unwrap(), b"unrelated");
}
...
fn checkpoint_crashed_sender_requires_exact_acknowledgement_before_retry() {
    if let Some((root, remote)) = child_context("crash-send") {
        run(async move {
            ...
            let prepared = sender.prepare_buffer(&cx, remote,
                NativeUploadOptions::new(&root, "data.bin"), &data()).await.unwrap();
            std::fs::write(root.join("checkpoint-path"), prepared.checkpoint.directory.to_str().unwrap()).unwrap();
            let _ = sender.send_checkpoint(&cx, &prepared.checkpoint.directory, remote,
                NativeCheckpointRetry::Never).await;
    ...
    let mut child = start_child("checkpoint_crashed_sender_requires_exact_acknowledgement_before_retry",
        &root, remote, "crash-send");
    assert!(socket.recv_from(&mut [0; 65535]).unwrap().0 > 0, "native send must actually start");
    child.0.kill().unwrap();
    ...
    let (socket, directory, root) = run(async move {
        ...
        let sender = sender();
        let info = sender.inspect_checkpoint(&cx, &directory, remote).await.unwrap();
        assert_eq!((info.state, info.attempt), (NativeCheckpointState::Sending, 1));
        assert!(matches!(sender.send_checkpoint(&cx, &directory, remote, NativeCheckpointRetry::Never).await,
            Err(NativeCheckpointError::Uncertain { attempt: 1 })));
        assert!(matches!(sender.send_checkpoint(&cx, &directory, remote,
            NativeCheckpointRetry::AcknowledgeUncertain { attempt: 0 }).await,
    ...
    run(async move {
        ...
        let receiver = receiver_client.bind_receiver(&cx, remote, root.join("received"),
            QuicReceiveOptions::default()).await.unwrap();
        let sender = sender();
        let (sent, received) = zip(sender.send_checkpoint(&cx, &directory, remote,
            NativeCheckpointRetry::AcknowledgeUncertain { attempt: 1 }), receiver.receive(&cx)).await;
...
fn checkpoint_os_lock_fences_another_process_and_releases_on_exit_without_drop() {
    if let Some((directory, _)) = child_context("lock") {
        let file = OpenOptions::new().read(true).write(true).open(directory.join("lock")).unwrap();
    ...
    run(async move {
        ...
        let directory = sender.prepare_buffer(&cx, remote, NativeUploadOptions::new(root, "data.bin"),
            b"retained").await.unwrap().checkpoint.directory;
        let mut child = start_child("checkpoint_os_lock_fences_another_process_and_releases_on_exit_without_drop",
            &directory, remote, "lock");
        let start = Instant::now();
        while !directory.join("lock-held").exists() {
            assert!(start.elapsed() < Duration::from_secs(5));
            std::thread::sleep(Duration::from_millis(1));
        }
        assert!(matches!(sender.inspect_checkpoint(&cx, &directory, remote).await, Err(NativeCheckpointError::Busy)));
        assert_eq!(sender.active_transfers(), 0);
        drop(child.0.stdin.take());
        finish_child(&mut child);
        assert_eq!(sender.inspect_checkpoint(&cx, &directory, remote).await.unwrap().state,
...
fn checkpoint_rehash_rejects_same_size_mutation_truncation_and_growth_before_networking() {
    run(async move {
        ...
        let socket = UdpSocket::bind("127.0.0.1:0").unwrap();
        let remote = socket.local_addr().unwrap();
        for replacement in [b"xxxxxxx".as_slice(), b"short".as_slice(), b"larger-than-seven".as_slice()] {
            let directory = sender.prepare_buffer(&cx, remote,
                NativeUploadOptions::new(fixture(), "data.bin"), b"correct").await.unwrap().checkpoint.directory;
            ...
            assert_eq!(sender.inspect_checkpoint(&cx, &directory, remote).await.unwrap().attempt, 0);
            assert_eq!(sender.active_transfers(), 0);
            no_packet(&socket);
        }
    });
...
fn checkpoint_binding_and_stricter_limits_are_checked_before_any_new_effect() {
    run(async move {
        ...
        let directory = sender.prepare_buffer(&cx, remote,
            NativeUploadOptions::new(fixture(), "data.bin"), b"correct").await.unwrap().checkpoint.directory;
        assert!(matches!(sender.send_checkpoint(&cx, &directory, other.local_addr().unwrap(),
            NativeCheckpointRetry::Never).await, Err(NativeCheckpointError::BindingMismatch)));
        for (label, name) in [("different-client", "localhost"), ("checkpoint-sender", "wrong.example")] {
            let changed = client(label, config(name, false), 1);
            assert!(matches!(changed.send_checkpoint(&cx, &directory, remote, NativeCheckpointRetry::Never).await,
        ...
        assert!(matches!(smaller.send_checkpoint(&cx, &directory, remote, NativeCheckpointRetry::Never).await,
            Err(NativeCheckpointError::Upload(NativeUploadError::TooLarge { limit: 6 }))));
        assert_eq!(sender.inspect_checkpoint(&cx, &directory, remote).await.unwrap().attempt, 0);
        no_packet(&socket);
        no_packet(&other);
    });
...
fn checkpoint_queued_cancellation_preserves_source_without_recording_an_attempt() {
    run(async move {
        ...
        let directory = sender.prepare_buffer(&cx, remote,
            NativeUploadOptions::new(fixture(), "data.bin"), b"correct").await.unwrap().checkpoint.directory;
        let mut task = sender.spawn_send_checkpoint(&cx, &scope, &directory, remote,
            NativeCheckpointRetry::Never).unwrap();
        assert_eq!(sender.active_transfers(), 1);
        assert!(matches!(sender.clone().spawn_send_checkpoint(&cx, &scope, &directory, remote,
            NativeCheckpointRetry::Never),
            Err(NativeCheckpointError::Native(NativeTransferError::CapacityExceeded { limit: 1 }))));
        let reason = CancelReason::user("cancel queued checkpoint");
        task.abort_with_reason(reason.clone());
        assert!(matches!(task.join(&cx).await, Err(JoinError::Cancelled(actual)) if actual == reason));
        assert_eq!(sender.active_transfers(), 0);
        assert_eq!(sender.inspect_checkpoint(&cx, &directory, remote).await.unwrap().attempt, 0);
        assert_eq!(std::fs::read(directory.join("payload/data.bin")).unwrap(), b"correct");
        no_packet(&socket);
    });
}
...
fn checkpoint_empty_input_is_valid_and_oversize_input_creates_no_checkpoint() {
    ...
    run(async move {
        ...
        assert!(matches!(sender.prepare_buffer(&cx, remote, options.clone(), b"x").await,
            Err(NativeCheckpointError::Upload(NativeUploadError::TooLarge { limit: 0 }))));
        assert_eq!(std::fs::read_dir(&root).unwrap().count(), 1);
        let prepared = sender.prepare_buffer(&cx, remote, options, b"").await.unwrap();
        ...
        assert_eq!(File::open(prepared.checkpoint.directory.join("payload/empty.bin")).unwrap().metadata().unwrap().len(), 0);
        assert!(prepared.cleanup_error.is_none());
    });
