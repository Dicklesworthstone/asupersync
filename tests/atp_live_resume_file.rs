...
fn certificate(name: &str) -> CertificateDer<'static> {
    let data: serde_json::Value = serde_json::from_str(include_str!("fixtures/atp_native_auth_identities.json")).unwrap();
    let pem = if name == "ca" { data["ca"].as_str() } else { data["identities"][name]["certificate"].as_str() }.unwrap();
...
fn identity(name: &str) -> NativeTlsIdentity {
    ...
    let key = PrivateKeyDer::pem_reader_iter(&mut io::BufReader::new(pem.as_bytes())).next().unwrap().unwrap();
...
fn profile() -> LiveStreamConfig {
    ...
    config.epoch_bytes = 4096; config.max_bytes = 16384;
...
struct GatedFile { file: LiveFileSink, gate: Arc<Gate> }
impl AsyncWrite for GatedFile {
    fn poll_write(self: Pin<&mut Self>, cx: &mut Context<'_>, bytes: &[u8]) -> Poll<io::Result<usize>> {
        ...
        if already >= 4109 && !this.gate.released.load(Ordering::SeqCst) {
            ...
            let retired = this.gate.waiter.lock().unwrap().replace(incoming); drop(retired);
            this.gate.parked.store(true, Ordering::SeqCst);
            if !this.gate.released.load(Ordering::SeqCst) { return Poll::Pending; }
        }
        let window = if already < 4109 { bytes.len().min(4109 - already) } else { bytes.len() };
        let result = Pin::new(&mut this.file).poll_write(cx, &bytes[..window]);
        if let Poll::Ready(Ok(count)) = &result { this.gate.written.fetch_add(*count, Ordering::SeqCst); }
        result
    }
...
fn native_file_resume_keeps_flushed_prefix_and_partial_epoch_without_duplicate_publication() {
    for workers in [1, 2] {
        let runtime = if workers == 1 { RuntimeBuilder::current_thread() }
            ...
            .blocking_threads(1, 4).build().unwrap();
        let task: Pin<Box<dyn Future<Output = ()> + Send>> = Box::pin(async {
            let cx = Cx::current().unwrap();
            let directory = tempfile::tempdir().unwrap().keep();
            std::fs::set_permissions(&directory, std::fs::Permissions::from_mode(0o700)).unwrap();
            let file = LiveFileSink::create(&cx, directory, "resumed.bin".to_owned(), 16384).await.unwrap();
            let publication = file.publication(); let gate = Arc::new(Gate::default());
            let sdk = AtpSdk::new_in_process(SessionConfig { max_concurrent_transfers: 1, ..SessionConfig::default() });
            ...
            let tx = sdk.live_stream_sender(profile(), ServerName::try_from("localhost").unwrap(), roots(), identity("allowed")).unwrap();
            let rx = sdk.live_stream_receiver(profile(), identity("server"), policy).unwrap();
            let output = GatedFile { file, gate: Arc::clone(&gate) };
            let mut incoming = rx.bind_resumable_committing(&cx, "127.0.0.1:0".parse().unwrap(), client, output, 3).await.unwrap();
            let data: Vec<u8> = (0_usize..8209).map(|index| ((index * 37) ^ (index / 251)).to_le_bytes()[0]).collect();
            let mut outgoing = tx.resumable_reader(&cx, incoming.local_addr().unwrap(), data.as_slice(), 3).unwrap();
            asupersync::time::timeout(cx.now(), Duration::from_secs(15), or(async {
                let premature = zip(outgoing.send(&cx), incoming.receive(&cx)).await;
                panic!("real file must be parked mid-epoch: {premature:?}");
            }, async {
                while !gate.parked.load(Ordering::SeqCst) { yield_now().await; }
            })).await.unwrap();
            ...
            assert_eq!(std::fs::read(publication.staging_path()).unwrap(), data[..4109]);
        ...
        while !runtime.is_quiescent() {
            assert!(started.elapsed() < Duration::from_secs(5)); runtime.block_on(yield_now());
        }
