...
impl Fixture {
    fn new() -> Self {
        ...
        for name in ["server", "allowed", "unlisted"] {
            std::fs::write(
                ...
                data["identities"][name]["certificate"].as_str().unwrap(),
            )
            .unwrap();
        ...
        let certificate = CertificateDer::pem_reader_iter(&mut BufReader::new(
            data["identities"]["allowed"]["certificate"]
                .as_str()
                .unwrap()
                .as_bytes(),
        ))
    ...
    fn ledger(&self) -> PathBuf {
        ...
        assert_eq!(
            process.until("session_ledger_initialized").0["maximum_keys"],
            1
        );
    ...
    fn serve(&self, config: &Value, ledger: &Path) -> (Process, SocketAddr) {
        ...
        let mut process = self.launch(command);
        let ready = process.until("ready").0;
...
impl Process {
        ...
        for _ in 0..32 {
            let value = self
                ...
                .unwrap_or_else(|error| {
                    panic!("missing {wanted}: {error}; {}", self.log.display())
                });
    ...
    fn stop(&mut self) {
        self.signal();
        assert_eq!(self.until("stopped").0["drained"], true);
...
fn files(directory: &Path) -> Vec<(String, u64, u64)> {
    let mut values: Vec<_> = std::fs::read_dir(directory)
        .unwrap()
        .map(|entry| {
...
fn checkpoint_bytes(path: &Path, data: &[u8]) -> Vec<u8> {
    ...
    assert!((302..=554).contains(&bytes.len()));
    assert_eq!(&bytes[..8], b"ATPFNL01");
    ...
    assert_eq!(
        u64::from_be_bytes(bytes[136..144].try_into().unwrap()),
        data.len() as u64
    );
...
fn committed_record(ledger: &Path) -> Option<Vec<u8>> {
    ...
    if bytes.len() != 48 + 2 * 256 || bytes[48 + 256 + 8] != 2 {
        return None;
    }
    ...
    if bytes[16..48] != previous {
        return None;
    }
    for row in bytes[48..].chunks_exact(256) {
        let sum = hash(&previous, &row[..224]);
        if row[224..] != sum {
            return None;
        }
        previous = sum;
...
fn verify_delivery(
    ...
    assert_eq!(result["transfer"]["status"], "complete");
    assert_eq!(result["drained"], true);
    assert_eq!(result["source_free"], source_free);
    assert_eq!(result["checkpoint_persisted"], true);
    assert_eq!(result["final_proof_direction"], "received");
    assert_eq!(
        result["transfer"]["receipt"]["sha256"],
    ...
    fn new(remote: SocketAddr, ledger: PathBuf) -> Self {
        ...
        let worker = thread::spawn(move || {
            while !stop.load(Ordering::SeqCst) {
                ...
                let _ = sr.shutdown(Shutdown::Both);
...
fn checkpointed_command_delivers_and_recovers_nonempty_and_empty_files() {
    for workers in [1, 2] {
        for data in [
            b"actual checkpointed executable transfer".as_slice(),
            b"".as_slice(),
        ] {
            let fixture = Fixture::new();
            let ledger = fixture.ledger();
            let (mut server, address) =
                fixture.serve(&fixture.receiver(workers, data.len()), &ledger);
            let config = fixture.sender(address, workers, data.len());
            let checkpoint = fixture.unique("checkpoint");
            let input = fixture.input(data);
            let mut sender = fixture.run_sender(&config, &checkpoint, Some(&input), 1, 25);
            let result = sender.until("send_result").0;
            assert!(sender.exit().success());
            verify_delivery(&fixture, &ledger, &result, data, false);
            let saved = checkpoint_bytes(&checkpoint, data);
            let before = files(&fixture.inbox);
            let history = std::fs::read(&ledger).unwrap();
            server.until("resume_completion");
            let mut recovery = fixture.run_sender(&config, &checkpoint, None, 1, 25);
            let recovered = recovery.until("send_result").0;
            assert!(recovery.exit().success());
            verify_delivery(&fixture, &ledger, &recovered, data, true);
            assert_eq!(
                result["transfer"]["receipt"],
                recovered["transfer"]["receipt"]
            );
            assert_eq!(checkpoint_bytes(&checkpoint, data), saved);
            assert_eq!(files(&fixture.inbox), before);
...
fn both_executables_restart_after_lost_proof_without_reopening_source_or_republishing() {
    for workers in [1, 2] {
        ...
        let input = fixture.input(&data);
        let mut sender = fixture.run_sender(&config, &checkpoint, Some(&input), 1, 25);
        let failed = sender.until("send_result").0;
        assert!(!sender.exit().success());
        assert!(relay.cut.load(Ordering::SeqCst));
        assert_eq!(failed["checkpoint_persisted"], true);
        assert!(failed["transfer"]["receipt"].is_null());
        assert_eq!(failed["final_proof_direction"], "not_received");
        let saved = checkpoint_bytes(&checkpoint, &data);
        let before = files(&fixture.inbox);
        let history = std::fs::read(&ledger).unwrap();
        assert!(committed_record(&ledger).is_some());
        ...
        std::fs::rename(&input, fixture.unique("retained-input")).unwrap();
        std::fs::create_dir(&input).unwrap();
        receiver_config["bind"] = json!(address);
        let (mut restarted, same) = fixture.serve(&receiver_config, &ledger);
...
fn source_free_recovery_refuses_a_fresh_uncommitted_receiver_even_for_empty_input() {
    for data in [b"content".as_slice(), b"".as_slice()] {
        ...
        let config = fixture.sender(address, 1, data.len());
        let checkpoint = fixture.unique("checkpoint");
        let input = fixture.input(data);
        let mut sender = fixture.run_sender(&config, &checkpoint, Some(&input), 1, 25);
        assert_eq!(
            sender.until("send_result").0["transfer"]["status"],
            "complete"
        );
        assert!(sender.exit().success());
        receiver.stop();
        let new_ledger = fixture.ledger();
        let empty_inbox = fixture.unique("fresh-inbox");
        std::fs::create_dir(&empty_inbox).unwrap();
        std::fs::set_permissions(&empty_inbox, std::fs::Permissions::from_mode(0o700)).unwrap();
        let mut changed = fixture.receiver(1, data.len());
        changed["bind"] = json!(address);
        ...
        let (failed, attempts) = recovery.until("send_result");
        assert!(!recovery.exit().success());
        assert_eq!(failed["transfer"]["status"], "continuity_refused");
        assert!(failed["transfer"]["receipt"].is_null());
    ...
    for changed in [
        {
            ...
            c["server_name"] = json!("wrong.invalid");
            c
        },
        {
            let mut c = config.clone();
            c["max_transfer_bytes"] = json!(data.len() - 1);
            c
        },
        {
            let mut c = config.clone();
            c["epoch_bytes"] = json!(1);
            c
        },
    ] {
        ...
        assert!(
            process
                .events
                .try_iter()
                .all(|row| row["event"] != "checkpoint_attempt")
        );
    }
    assert_eq!(checkpoint_bytes(&checkpoint, data), before);
    let missing = fixture.unique("missing-checkpoint");
    let mut process = fixture.run_sender(&config, &missing, None, 1, 25);
    assert!(!process.exit().success());
...
fn recovery_uses_current_client_authorization_without_recreating_a_checkpoint() {
    ...
    let mut config = fixture.sender(address, 2, data.len());
    let checkpoint = fixture.unique("checkpoint");
    let input = fixture.input(data);
    let mut original = fixture.run_sender(&config, &checkpoint, Some(&input), 1, 25);
    original.until("send_result");
    assert!(original.exit().success());
    receiver.until("resume_completion");
    let before = files(&fixture.inbox);
    let history = std::fs::read(&ledger).unwrap();
    let saved = checkpoint_bytes(&checkpoint, data);
    config["identity"] = fixture.identity("unlisted");
    let mut denied = fixture.run_sender(&config, &checkpoint, None, 4, 25);
    let (failed, attempts) = denied.until("send_result");
    assert!(!denied.exit().success());
    assert!(failed["transfer"]["receipt"].is_null());
    assert_eq!(attempts.len(), 1);
    assert_eq!(attempts[0]["retry_eligible"], false);
    let refusal = receiver.until("resume_completion").0;
    assert_eq!(refusal["transfer"]["status"], "tls_failed");
    assert!(refusal["session"].is_null());
    assert_eq!(files(&fixture.inbox), before);
    assert_eq!(std::fs::read(&ledger).unwrap(), history);
    assert_eq!(checkpoint_bytes(&checkpoint, data), saved);
    receiver.stop();
...
fn signal_joins_a_real_pending_handshake_and_releases_checkpoint_ownership() {
    ...
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    listener.set_nonblocking(true).unwrap();
    let mut config = fixture.sender(listener.local_addr().unwrap(), 2, 10);
    config["operation_timeout_secs"] = json!(15);
    let checkpoint = fixture.unique("checkpoint");
    let input = fixture.input(b"pending");
    let mut sender = fixture.run_sender(&config, &checkpoint, Some(&input), 4, 60_000);
    let mut socket = accept(&listener);
    socket.set_read_timeout(Some(WAIT)).unwrap();
    assert!(
        socket.read(&mut [0; 4096]).unwrap() > 0,
        "real TLS bytes precede the signal"
    );
    ...
    let result = sender.until("send_result").0;
    assert!(!sender.exit().success());
    assert_eq!(result["transfer"]["status"], "cancelled");
    assert_eq!(result["transfer"]["attempts"], 1);
    assert_eq!(result["checkpoint_persisted"], false);
    assert_eq!(result["drained"], true);
