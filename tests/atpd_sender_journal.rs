    ...
    fn new() -> Self {
        ...
        for name in ["allowed", "server"] {
            std::fs::write(
                ...
                identities["identities"][name]["certificate"]
                    .as_str()
                    .unwrap(),
            )
            .unwrap();
            ...
            std::fs::write(
                &key,
                identities["identities"][name]["key"].as_str().unwrap(),
            )
            .unwrap();
            std::fs::set_permissions(key, std::fs::Permissions::from_mode(0o600)).unwrap();
        }
        let pem = identities["identities"]["allowed"]["certificate"]
            .as_str()
            .unwrap();
        let certificate = CertificateDer::pem_reader_iter(&mut BufReader::new(pem.as_bytes()))
            .next()
            .unwrap()
            .unwrap();
        Self {
            ...
            client: hex::encode(Sha256::digest(certificate.as_ref())),
        }
    }
...
impl Process {
    fn event(&mut self, expected: &str) -> Value {
        let row = self
            .events
            .recv_timeout(WAIT)
            .unwrap_or_else(|e| panic!("missing {expected}: {e}; {}", self.stderr.display()));
        assert_eq!(row["event"], expected, "unexpected process result: {row}");
        row
    }
    fn result(&mut self) -> Value {
        for _ in 0..8 {
            let row = self.events.recv_timeout(WAIT).unwrap();
            if row["event"] == "send_result" {
                return row;
            }
...
fn staging(directory: &Path) -> Option<PathBuf> {
    paths(directory).into_iter().find(|path| {
        path.file_name()
            .unwrap()
            .to_string_lossy()
                ...
                let upstream = thread::spawn(move || {
                    ...
                    while !up_end.load(Ordering::SeqCst) && !up_stop.load(Ordering::SeqCst) {
                        match from_client.read(&mut bytes) {
                            Ok(0) => break,
                            Ok(count) => {
                                if to_server.write_all(&bytes[..count]).is_err() {
                                    break;
                                }
                            }
                            Err(error) if retry_read(&error) => {}
                    ...
                    match from_server.read(&mut bytes) {
                        ...
                        Ok(count) => {
                            if !fault.load(Ordering::SeqCst)
                                && staging(&inbox).is_some_and(|path| {
                                    std::fs::metadata(path).unwrap().len() >= threshold
                                })
                            {
                                fault.store(true, Ordering::SeqCst);
                                break;
                            }
                            if to_client.write_all(&bytes[..count]).is_err() {
                                break;
                            }
                        }
                        Err(error) if retry_read(&error) => {}
...
fn snapshots(path: &Path) -> (Vec<u8>, Vec<Vec<u8>>) {
    ...
    assert_eq!(&bytes[..8], b"ATPSJL01");
    let hash = |previous: &[u8], body: &[u8]| {
        let mut h = Sha256::new();
        h.update(b"asupersync.atp.sender-journal.v1");
        h.update(previous);
        h.update(body);
        h.finalize().to_vec()
    };
    ...
    for (sequence, record) in bytes[48..].chunks_exact(720).enumerate() {
        ...
        assert!(n <= 667);
        assert_eq!(&record[10..16], &[0; 6]);
        assert!(record[16 + n..688].iter().all(|b| *b == 0));
        let next = hash(&previous, &record[..688]);
        assert_eq!(&record[688..], next);
        previous = next;
        let saved = &record[16..16 + n];
        assert_eq!(&saved[..8], b"ATPSND01");
        let body = saved.len() - 32;
        let mut h = Sha256::new();
        h.update(b"asupersync.atp.sender-checkpoint.v1");
        h.update(&saved[..body]);
        assert_eq!(&saved[body..], h.finalize().as_slice());
        snapshots.push(saved.to_vec());
    }
    (bytes, snapshots)
}
...
fn sender_process_dies_before_eof_and_resumes_into_the_same_receiver_inode() {
    for workers in [1, 2] {
        for threshold in [4096_u64, 8192] {
            ...
            let data: Vec<_> = (0_usize..12293)
                .map(|n| ((n * 29) ^ (n / 251)).to_le_bytes()[0])
                .collect();
            let input = fixture.source(&data);
            ...
            assert!(
                SenderJournalFile::open_existing(&journal).is_err(),
                "the original process still owns its journal"
            );
            let receive_failure = receiver.event("resume_completion");
            assert_eq!(
                receive_failure["transfer"]["flushed_prefix_bytes"],
                threshold
            );
            ...
            let (before, states) = snapshots(&journal);
            let saved = states.last().unwrap();
            ...
            assert_eq!(
                u64::from_be_bytes(saved[136..144].try_into().unwrap()),
                threshold - 4096
            );
            ...
            let complete = restarted.result();
            assert!(restarted.exit().success());
            assert_eq!(complete["transfer"]["status"], "complete");
            assert_eq!(complete["transfer"]["attempts"], 2);
            assert_eq!(complete["resumed"], true);
            assert_eq!(complete["drained"], true);
            ...
            assert_eq!(
                complete["transfer"]["receipt"]["sha256"],
                hex::encode(Sha256::digest(&data))
            );
            assert_eq!(
                paths(&fixture.inbox).len(),
                3,
                "reconnect must not charge/create a second sink"
            );
            let (after, states) = snapshots(&journal);
...
fn journal_capacity_failure_precedes_data_and_cannot_reset_on_restart() {
    let fixture = Fixture::new();
    let input = fixture.source(b"not sent");
    let journal = fixture.unique("journal");
    ...
    assert_eq!(
        std::fs::metadata(staging(&fixture.inbox).unwrap())
            .unwrap()
            .len(),
        0
    );
...
fn ordinary_and_empty_journaled_sends_publish_exactly_one_file() {
    for bytes in [b"ordinary journaled delivery".as_slice(), b"".as_slice()] {
        let fixture = Fixture::new();
        let input = fixture.source(bytes);
        let journal = fixture.unique("journal");
