...
fn oracle_detects_stuck_region_creation() {
    ...
    oracle.on_region_created(RegionId(1), None, None, Budget::INFINITE);
...
fn oracle_allows_normal_region_lifecycle() {
    ...
    oracle.on_region_created(RegionId(1), None, None, Budget::INFINITE);
    oracle.on_region_activated(RegionId(1));
    oracle.on_task_spawned(TaskId(100), RegionId(1), None);
    oracle.on_task_polled(TaskId(100));
    oracle.on_task_completed(TaskId(100), Outcome::Ok(()));
    oracle.on_region_closing(RegionId(1), 0);
    oracle.on_region_closed(RegionId(1));
