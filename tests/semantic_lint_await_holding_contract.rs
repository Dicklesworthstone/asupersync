...
const CONTRACT_PATH: &str = "artifacts/semantic_lint_await_holding_rule_contract_v1.json";
const INVENTORY_PATH: &str = "artifacts/semantic_lint_rule_inventory_v1.json";
const DOCS_PATH: &str = "docs/semantic_lint_await_holding_rule.md";
const RUNNER_PATH: &str = "scripts/semantic_lint.py";
const RULE_ID: &str = "await-while-holding-capability-resource";
...
fn contract_links_inventory_runner_docs_and_fixtures() {
    ...
    assert_eq!(
        ...
        Some("semantic-lint-await-holding-rule-contract-v1")
    );
...
fn contract_matches_l1_inventory_decision() {
    ...
    let row = rows
        ...
        .expect("await-holding rule must be in inventory");
...
#[test]
fn runner_reports_guard_and_permit_fixtures() {
    let (_, result) = run_runner(&[
        "tests/fixtures/semantic_lint/await_holding/positive_mutex_guard.rs",
        "tests/fixtures/semantic_lint/await_holding/positive_permit.rs",
    ]);
    ...
    assert_eq!(
        kinds,
        BTreeSet::from(["await_while_holding_guard", "await_while_holding_permit",])
    );
    for finding in array(&result, "findings") {
        assert!(
            string(finding, "diagnostic").contains("live across .await"),
            "diagnostic must name the await-holding lifetime risk"
        );
    }
}

#[test]
fn runner_accepts_explicit_drop_before_await() {
    let (_, result) =
        run_runner(&["tests/fixtures/semantic_lint/await_holding/negative_drop_before_await.rs"]);
...
fn allow_markers_require_reason_and_owner_bead() {
    let (_, allowed) = run_runner(&["tests/fixtures/semantic_lint/await_holding/valid_allow.rs"]);
    ...
    let (_, invalid) = run_runner(&["tests/fixtures/semantic_lint/await_holding/invalid_allow.rs"]);
    ...
    assert!(
        findings
            .iter()
            .any(|finding| string(finding, "kind") == "await_while_holding_permit"),
        "invalid marker must not suppress the await-holding finding"
    );
}
...
fn runner_output_is_stable_and_docs_name_no_claim_boundary() {
    let paths = [
        "tests/fixtures/semantic_lint/await_holding/positive_mutex_guard.rs",
        "tests/fixtures/semantic_lint/await_holding/positive_permit.rs",
        "tests/fixtures/semantic_lint/await_holding/valid_allow.rs",
        "tests/fixtures/semantic_lint/await_holding/invalid_allow.rs",
    ];
