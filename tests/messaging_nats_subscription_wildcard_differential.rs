...
const CASES: &[(&str, &str, bool, &str)] = &[
    ...
    ("foo", "foo.bar", false, "literal does not match longer subject"),
    ("foo.bar", "foo", false, "literal does not match shorter subject"),
    ("foo", "Foo", false, "subject matching is case-sensitive"),

    // ── single-token wildcard `*` ─────────────────────────────────
    ("*", "foo", true, "* matches exactly one token at root"),
    ("*", "foo.bar", false, "* matches exactly one token (not multi)"),
    ("*", "", false, "* requires a non-empty token"),
    ("foo.*", "foo.bar", true, "* matches any single tail token"),
    ("foo.*", "foo", false, "trailing * requires the corresponding token"),
    ("foo.*", "foo.bar.baz", false, "* does not span multiple tokens"),
    ("*.bar", "foo.bar", true, "leading * matches first token"),
    ("foo.*.baz", "foo.qux.baz", true, "interior * matches any single middle token"),
    ("foo.*.baz", "foo.qux.zzz.baz", false, "interior * does not span tokens"),
    ("*.*", "a.b", true, "two * tokens require exactly two-token subject"),
    ("*.*", "a", false, "two * tokens reject single-token subject"),
    ("*.*", "a.b.c", false, "two * tokens reject longer subject"),
    ("*.*.*", "a.b.c", true, "three * tokens match three-token subject"),

    // ── tail wildcard `>` ─────────────────────────────────────────
    (">", "foo", true, "> alone matches one token"),
    (">", "foo.bar", true, "> alone matches multi-token subject"),
    (">", "foo.bar.baz.qux", true, "> alone matches arbitrarily deep subject"),
    ("foo.>", "foo.bar", true, "> matches a single trailing token"),
    ("foo.>", "foo.bar.baz", true, "> matches multiple trailing tokens"),
    ("foo.>", "foo", false, "> requires at least one trailing token"),
    ("foo.>", "bar.baz", false, "> after literal does not change literal match"),

    // ── mixed `*` and `>` ─────────────────────────────────────────
    ("*.>", "a.b", true, "*.> requires at least two tokens"),
    ("*.>", "a.b.c", true, "*.> matches deeper subjects"),
    ("*.>", "a", false, "*.> rejects single-token subject"),
    ("foo.*.>", "foo.x.y", true, "interior * + trailing > combine cleanly"),
    ("foo.*.>", "foo.x.y.z", true, "interior * + trailing > matches deeper"),
    ("foo.*.>", "foo.x", false, "*.> tail still requires a trailing token"),
    ("foo.*.>", "foo", false, "*.> tail rejects pattern-shorter subject"),

    // ── invalid pattern shapes (must not match anything) ─────────
    ("foo.>.bar", "foo.x.bar", false, "> only valid as last token"),
    ...
    ("foo.bar.", "foo.bar.x", false, "trailing empty token is invalid"),
