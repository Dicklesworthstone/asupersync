...
const CONTRACT_PATH: &str = "artifacts/semantic_lint_drop_race_loser_rule_contract_v1.json";
const INVENTORY_PATH: &str = "artifacts/semantic_lint_rule_inventory_v1.json";
const DOCS_PATH: &str = "docs/semantic_lint_drop_race_loser_rule.md";
const RUNNER_PATH: &str = "scripts/semantic_lint.py";
const RULE_ID: &str = "drop-based-race-loser-handling";
...
fn contract_links_inventory_runner_docs_and_fixtures() {
    ...
    assert_eq!(
        ...
        Some("semantic-lint-drop-race-loser-rule-contract-v1")
    );
    ...
    for key in [
        ...
        "docs",
        "positive_fixture",
        "negative_abort_join_fixture",
        "negative_drop_guard_fixture",
        "valid_allow_fixture",
...
fn contract_matches_l1_inventory_decision() {
    ...
    let row = rows
        ...
        .expect("drop race loser rule must be in inventory");
...
#[test]
fn runner_reports_drop_loser_and_defuse_fixtures() {
    let (_, result) =
        run_runner(&["tests/fixtures/semantic_lint/drop_race_loser/positive_drop_loser.rs"]);
    ...
    assert_eq!(
        kinds,
        BTreeSet::from(["defuse_without_loser_drain_proof", "drop_based_race_loser",])
    );
    for finding in array(&result, "findings") {
        assert!(
            string(finding, "diagnostic").contains("loser-drain invariant"),
            "diagnostic must name the loser-drain invariant"
        );
    }
}

#[test]
fn runner_accepts_abort_join_and_documented_drop_guard() {
    let (_, result) = run_runner(&[
        "tests/fixtures/semantic_lint/drop_race_loser/negative_abort_join.rs",
        "tests/fixtures/semantic_lint/drop_race_loser/negative_documented_drop_guard.rs",
    ]);
...
fn allow_markers_require_reason_and_owner_bead() {
    let (_, allowed) = run_runner(&["tests/fixtures/semantic_lint/drop_race_loser/valid_allow.rs"]);
    ...
    let (_, invalid) =
        run_runner(&["tests/fixtures/semantic_lint/drop_race_loser/invalid_allow.rs"]);
    ...
    assert!(
        findings
            .iter()
            .any(|finding| string(finding, "kind") == "drop_based_race_loser"),
        "invalid marker must not suppress the drop-loser finding"
    );
}
...
fn runner_output_is_stable_and_docs_name_no_claim_boundary() {
    let paths = [
        "tests/fixtures/semantic_lint/drop_race_loser/positive_drop_loser.rs",
        "tests/fixtures/semantic_lint/drop_race_loser/valid_allow.rs",
        "tests/fixtures/semantic_lint/drop_race_loser/invalid_allow.rs",
    ];
