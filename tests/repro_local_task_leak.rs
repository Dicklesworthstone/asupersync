...
//!
//! The executable regression lives in `src/runtime/scheduler/three_lane_tests.rs`.
...
const THREE_LANE_SOURCE: &str = include_str!("../src/runtime/scheduler/three_lane_tests.rs");
...
fn moved_local_task_leak_regression_still_exists() {
    assert!(
        ...
        "moved local-task leak regression test is missing from three_lane_tests.rs"
    );
}
