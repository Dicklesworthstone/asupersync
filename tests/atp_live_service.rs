...
fn receiver(capacity: u32) -> (LiveStreamReceiver, NativeClientAuthorization) {
    ...
    let receiver = sdk(capacity)
        .live_stream_receiver(config(), identity("server"), authorization.clone())
        .unwrap();
...
fn run<T: Send + 'static>(workers: usize, future: impl Future<Output = T> + Send + 'static) -> T {
    ...
    while !runtime.is_quiescent() {
        assert!(
            ...
            "service children did not drain"
        );
...
async fn witness(cx: &Cx, predicate: impl Fn() -> bool) {
    ...
    .await
    .expect("required accepted/parked state was never reached");
}
...
impl Store {
    ...
    fn create(&self, cx: Cx, peer: LiveStreamPeer) -> Sink {
        assert_eq!(
            Cx::current().unwrap().task_id(),
            cx.task_id(),
            "factory must run in its actual child context"
        );
        let state = Arc::new(SinkState::default());
        self.entries
            .lock()
            .unwrap()
            .push((peer, Arc::clone(&state)));
        Sink {
            state,
    ...
    fn factory(
        &self,
    ) -> impl Fn(Cx, LiveStreamPeer) -> Ready<io::Result<Sink>> + Clone + Send + Sync + 'static
    {
        let store = self.clone();
        move |cx, peer| ready(Ok(store.create(cx, peer)))
    }
    fn entries(&self) -> Vec<(LiveStreamPeer, Arc<SinkState>)> {
        self.entries.lock().unwrap().clone()
    }
    fn parked(&self) -> usize {
        self.entries()
            .iter()
            .filter(|(_, state)| state.parked.load(Ordering::SeqCst))
            .count()
    }
    fn release(&self) {
...
fn assert_cancelled(completion: LiveStreamCompletion, reason: &CancelReason) {
    let report = completion
        .result
        .expect("witnessed parked worker must preserve its acknowledged domain result");
    assert!(report.peer.is_some());
    assert!(
        matches!(report.transfer.outcome, Err(LiveStreamError::Cancelled(Some(actual))) if &actual == reason)
    );
    let prefix = report.transfer.prefix.unwrap();
    assert_eq!((prefix.epochs, prefix.bytes), (0, 0));
    assert_eq!(
        report.transfer.sink_written_bytes, 5,
        "partial publication must survive cancellation"
    );
}
...
fn one_bound_service_handles_repeated_clients_without_rebinding() {
    for workers in [1, 2] {
        run(workers, async {
            ...
            for (id, payload) in [b"first".as_slice(), b"", b"third payload crosses epochs"]
                .into_iter()
                .enumerate()
            {
                let (completion, sent) = zip(
                    service.next(&cx, &scope, store.factory()),
                    send.send_reader(&cx, address, payload),
                )
                .await;
                ...
                assert_eq!(
                    report.peer.unwrap().certificate,
                ...
                assert_eq!(
                    ...
                    1,
                    "service retains its capacity partition while open"
                );
            }
...
fn concurrent_connections_and_uncollected_workers_obey_the_reserved_bound() {
    for workers in [1, 2] {
        run(workers, async {
            ...
            let mut service = receive
                .bind_service(&cx, "127.0.0.1:0".parse().unwrap(), 2)
                .await
                .unwrap();
            ...
            for payload in [b"one".as_slice(), b"two", b"three"] {
                senders.push(
                    send.spawn_send_reader(&cx, &scope, service.local_addr(), payload)
                        .unwrap(),
                );
            }
            or(
            ...
            assert_eq!(
                store.entries().len(),
                2,
                "third connection cannot allocate a sink behind full service capacity"
            );
            store.release();
            let mut ids = Vec::new();
            for _ in 0..3 {
            ...
            for sender in &mut senders {
                assert!(sender.join(&cx).await.unwrap().outcome.is_ok());
            }
            let mut actual: Vec<_> = store
                .entries()
                .iter()
                .map(|(_, state)| state.bytes.lock().unwrap().clone())
                .collect();
            actual.sort();
            let mut expected = vec![b"one".to_vec(), b"two".to_vec(), b"three".to_vec()];
            expected.sort();
...
fn unauthorized_clients_never_construct_sinks_and_rotation_keeps_the_port_open() {
    run(2, async {
        ...
        let (failed, sent) = zip(
            service.next(&cx, &scope, store.factory()),
            unlisted.send_reader(&cx, address, b"denied".as_slice()),
        )
        .await;
        let failed = failed.unwrap().unwrap().result.unwrap();
        assert!(failed.peer.is_none());
        assert!(matches!(
            failed.transfer.outcome,
            Err(LiveStreamError::Tls(_))
        ));
        assert!(sent.outcome.is_err());
        assert!(store.entries().is_empty());
        policy
            .replace_allowed([NativeClientCertificateId::from_certificate(&certificate(
                "unlisted",
            ))])
            .unwrap();
        let (completed, sent) = zip(
            service.next(&cx, &scope, store.factory()),
            unlisted.send_reader(&cx, address, b"allowed after rotation".as_slice()),
        )
        .await;
        let completed = completed.unwrap().unwrap();
        assert_eq!(completed.connection, 1);
        assert_eq!(
            completed.result.unwrap().transfer.outcome.unwrap(),
            sent.outcome.unwrap()
        );
...
fn factory_errors_and_panics_are_per_connection_not_service_failures() {
    run(2, async {
        ...
        let factory = {
            ...
            move |cx, peer| match calls.fetch_add(1, Ordering::SeqCst) {
        ...
        for attempt in 0..3 {
            let (completion, sent) = zip(
                ...
                send.send_reader(&cx, address, b"payload".as_slice()),
            )
            .await;
            let completion = completion.unwrap().unwrap();
            ...
            match attempt {
                0 => {
                    ...
                    assert!(
                        matches!(report.transfer.outcome, Err(LiveStreamError::Io(error)) if error.kind() == io::ErrorKind::PermissionDenied)
                    );
                    assert!(sent.outcome.is_err());
                }
                1 => {
                    assert!(matches!(completion.result, Err(JoinError::Panicked(_))));
                    assert!(sent.outcome.is_err());
                }
                _ => {
                    assert_eq!(
                        completion.result.unwrap().transfer.outcome.unwrap(),
                        sent.outcome.unwrap()
                    );
                }
            }
...
fn graceful_stop_and_resumed_drain_keep_the_full_peer_receipt() {
    run(2, async {
        ...
        let mut sending = send
            .spawn_send_reader(&cx, &scope, service.local_addr(), b"first".as_slice())
            .unwrap();
        or(
            async {
                let premature = service.next(&cx, &scope, store.factory()).await;
...
fn idle_service_wakes_on_a_cross_thread_stop_and_releases_the_actual_listener() {
    run(1, async {
        ...
        let rebound =
            std::net::TcpListener::bind(address).expect("the real listening socket must be closed");
        drop(rebound);
    });
}
...
fn service_capacity_and_bind_failures_roll_back_before_any_accept() {
    run(1, async {
        let cx = Cx::current().unwrap();
        let (receive, _) = receiver(3);
        let occupied = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
        let address = occupied.local_addr().unwrap();
        let one_shot = receive
            .bind(&cx, "127.0.0.1:0".parse().unwrap())
            .await
            .unwrap();
        assert!(matches!(
            receive.bind_service(&cx, address, 3).await,
        ...
        assert_eq!(receive.active_streams(), 0);
        assert!(
            matches!(receive.bind_service(&cx, address, 3).await, Err(LiveStreamError::Io(error)) if error.kind() == io::ErrorKind::AddrInUse)
        );
        assert_eq!(receive.active_streams(), 0);
        for count in [0, 4, usize::MAX] {
            assert!(matches!(
                receive.bind_service(&cx, address, count).await,
    ...
    run(2, async {
        ...
        let mut manager = cx
            .spawn_in(&scope, move |child| {
                let future: Pin<
                    Box<dyn Future<Output = (LiveStreamError, Vec<LiveStreamCompletion>)> + Send>,
                > = Box::pin(async move {
                    let scope = child.scope();
                    let error = service
                        .next(&child, &scope, manager_store.factory())
                        .await
                        .unwrap_err();
                    let mut completions = Vec::new();
                    while let Some(completion) = service.drain_next().await {
        ...
        let send = sender("allowed");
        let mut sending = send
            .spawn_send_reader(&cx, &scope, address, b"first".as_slice())
            .unwrap();
        witness(&cx, || store.parked() == 1).await;
        let reason = CancelReason::user("cancel owning manager");
        manager.abort_with_reason(reason.clone());
        let (error, mut completions) = manager
            .join(&cx)
            .await
            .expect("acknowledged manager cancellation must retain its drained report");
