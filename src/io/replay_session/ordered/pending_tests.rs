...
impl PendingPipe {
    fn new() -> Self {
        Self { polls: [0; 5], offset: 0, written: Vec::new(), reset: false, panic: false }
    }
    fn pending(&mut self, operation: usize, cx: &Context<'_>) -> bool {
        ...
        if pending { cx.waker().wake_by_ref(); }
        pending
    }
}
...
fn with_limits(pending: PendingIoCaptureLimits, max_effects: usize) -> OrderedRecordingSession<PendingPipe, VirtualClock> {
    OrderedRecordingSession::new_with_pending_io(
        PendingPipe::new(), Arc::new(DetEntropy::new(42)), Arc::new(VirtualClock::new()),
        limits(), max_effects, pending,
    ).unwrap()
}
...
fn value<T>(poll: Poll<io::Result<T>>) -> io::Result<T> {
    match poll { Poll::Ready(result) => result, Poll::Pending => panic!("expected ready") }
}
...
fn race_read<'a, I: AsyncRead + Unpin>(io: &'a mut I) -> ReplayConsumerFuture<'a, io::Result<bool>> {
    Box::pin(async move {
        let mut bytes = [0; 2];
        ...
        if !io_won { io.read_exact(&mut bytes).await?; }
        Ok(io_won)
    })
}
...
fn negative_control_completed_only_replay_changes_race_winner_but_poll_aware_preserves_it() {
    let mut old = OrderedRecordingSession::new(
        PendingPipe::new(), Arc::new(DetEntropy::new(42)), Arc::new(VirtualClock::new()), limits(), 512,
    ).unwrap();
    ...
    assert!(drive(old.into_parts().1.unwrap().replay().run(16, |p| race_read(p.io))).unwrap().unwrap());
    ...
    assert!(!drive(tape.replay().run(16, |p| race_read(p.io))).unwrap().unwrap());
}
...
fn pending_write_fingerprint_rejects_changed_bytes_and_poison_reaches_other_sources() {
    ...
    let error = *error.get_ref().unwrap().downcast_ref::<OrderReplayError>().unwrap();
...
fn pending_vectors_preserve_boundaries_even_with_identical_concatenated_bytes() {
    ...
    let mut recording = capture();
    assert!(Pin::new(recording.io()).poll_write_vectored(&mut cx(), &original).is_pending());
    let mut replay = recording.into_parts().1.unwrap().replay();
...
fn dropped_pending_attempts_keep_their_place_relative_to_entropy_and_clock() {
    ...
    assert!(Pin::new(recording.io()).poll_shutdown(&mut cx()).is_pending());
    let mut replay = recording.into_parts().1.unwrap().replay();
    let p = replay.inputs();
...
fn unconsumed_pending_tail_prevents_success_even_with_empty_component_tapes() {
    ...
    assert!(matches!(error, OrderedRunError::Replay(OrderedReplayError {
        order: Some(OrderCompletionError::Remaining { remaining: 1 }), components: None,
    })));
}
...
fn pending_limits_refuse_capture_but_never_change_live_results_or_ownership() {
    for (bounds, total, expected, vectored) in [
        (PendingIoCaptureLimits::new(0, 16, 8), 10, OrderCaptureError::PendingLimit("polls"), false),
        (PendingIoCaptureLimits::new(10, 0, 8), 10, OrderCaptureError::PendingLimit("write bytes"), false),
        (PendingIoCaptureLimits::new(10, 16, 0), 10, OrderCaptureError::PendingLimit("vectored slices"), true),
        (PendingIoCaptureLimits::new(10, 16, 8), 0, OrderCaptureError::Limit, false),
    ] {
        let mut recording = with_limits(bounds, total);
        if vectored {
            let bufs = [IoSlice::new(b"abc")];
            assert!(Pin::new(recording.io()).poll_write_vectored(&mut cx(), &bufs).is_pending());
            assert_eq!(value(Pin::new(recording.io()).poll_write_vectored(&mut cx(), &bufs)).unwrap(), 3);
        } else {
            assert!(Pin::new(recording.io()).poll_write(&mut cx(), b"abc").is_pending());
            assert_eq!(value(Pin::new(recording.io()).poll_write(&mut cx(), b"abc")).unwrap(), 3);
        }
...
fn pending_byte_budget_is_aggregate_not_reset_by_successful_polls() {
    ...
    assert!(Pin::new(recording.io()).poll_write(&mut cx(), b"abc").is_pending());
    assert_eq!(recording.into_parts().1.unwrap_err().order, Some(OrderCaptureError::PendingLimit("write bytes")));
}

#[test]
fn zero_pending_budget_still_allows_ready_io_and_empty_strict_windows() {
    let mut pipe = PendingPipe::new(); pipe.polls[3] = 1;
    let mut recording = OrderedRecordingSession::new_with_pending_io(
        pipe, Arc::new(DetEntropy::new(42)), Arc::new(VirtualClock::new()), limits(), 4,
        PendingIoCaptureLimits::new(0, 0, 0),
    ).unwrap();
    value(Pin::new(recording.io()).poll_flush(&mut cx())).unwrap();
    let tape = recording.into_parts().1.unwrap();
    assert!(tape.is_poll_aware()); assert_eq!(tape.pending_io_polls(), 0);
    drive(tape.replay().run(1, |p| Box::pin(async move { p.io.flush().await }))).unwrap().unwrap();
    let empty = with_limits(PendingIoCaptureLimits::new(0, 0, 0), 0).into_parts().1.unwrap();
    assert!(empty.is_poll_aware()); empty.replay().verify_complete().unwrap();
}

#[test]
fn original_read_error_after_pending_remains_an_application_error() {
    let mut recording = capture(); recording.io.inner = RecordingIo::new(
        PendingPipe { reset: true, ..PendingPipe::new() }, limits().io,
    );
    let mut bytes = [0; 2];
    assert_eq!(drive(recording.io().read_exact(&mut bytes)).unwrap_err().kind(), io::ErrorKind::ConnectionReset);
    let tape = recording.into_parts().1.unwrap();
    let result = drive(tape.replay().run(4, |p| Box::pin(async move {
        p.io.read_exact(&mut [0; 2]).await
    }))).unwrap();
    assert_eq!(result.unwrap_err().kind(), io::ErrorKind::ConnectionReset);
}

struct Probe { order: Option<Arc<ReplayOrder>>, calls: AtomicUsize, panic: bool }
impl Wake for Probe {
    fn wake(self: Arc<Self>) {
        if let Some(order) = &self.order { let _ = order.verify(); }
...
fn recorded_pending_wakes_once_outside_locks_and_contains_callback_panics() {
    let mut recording = capture();
    let live = Arc::new(Probe { order: None, calls: AtomicUsize::new(0), panic: false });
    let waker = Waker::from(Arc::clone(&live));
    assert!(Pin::new(recording.io()).poll_flush(&mut Context::from_waker(&waker)).is_pending());
    assert_eq!(live.calls.load(Ordering::Relaxed), 1); // Capture added no wake.
    value(Pin::new(recording.io()).poll_flush(&mut cx())).unwrap();
    let mut replay = recording.into_parts().1.unwrap().replay();
    let probe = Arc::new(Probe { order: Some(Arc::clone(&replay.order)), calls: AtomicUsize::new(0), panic: true });
    let waker = Waker::from(Arc::clone(&probe));
    assert!(Pin::new(replay.inputs().io).poll_flush(&mut Context::from_waker(&waker)).is_pending());
...
fn captured_source_unwind_never_publishes_a_poll_prefix() {
    let mut recording = capture(); recording.io.inner = RecordingIo::new(
        PendingPipe { panic: true, ..PendingPipe::new() }, limits().io,
    );
...
fn pending_fingerprints_never_expose_payload_or_digest_in_debug() {
    ...
    assert!(!debug.contains("private")); assert!(!debug.contains("digest"));
