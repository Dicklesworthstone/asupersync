...
impl AsyncRead for Choppy {
    fn poll_read(
        ...
        assert!(!this.panic_read, "provider sentinel");
...
fn session(io: Choppy) -> OrderedRecordingSession<Choppy, VirtualClock> {
    OrderedRecordingSession::new(
        ...
        SessionCaptureLimits {
            ...
            entropy: EntropyCaptureLimits::new(256, 4096, 8),
            clock_observations: 256,
        },
        1024,
    )
    .unwrap()
...
pub(super) async fn receive<I, E, C>(
    io: &mut I,
    entropy: &E,
    clock: &C,
) -> Result<(Vec<u8>, Vec<(Time, u64)>), io::ErrorKind>
where
    I: AsyncRead + Unpin,
    ...
    C: TimeSource + ?Sized,
{
    ...
    loop {
        ...
        .await?;
        if n == 0 {
            return Ok((out, observations));
        }
        out.extend_from_slice(&byte[..n]);
    }
}
pub(super) fn capture() -> PolledRecordedSession {
    let (original, _, tape) = drive(
        session(Choppy::new())
            .record_polls(bounds(), |p| Box::pin(receive(p.io, p.entropy, p.clock))),
    )
    .unwrap();
...
fn actual_pending_replay_progresses_without_the_original_source_wakeup() {
    ...
    let (original, source, tape) = drive(
        session(source).record_polls(bounds(), |p| Box::pin(receive(p.io, p.entropy, p.clock))),
    )
    .unwrap();
...
fn negative_control_completed_effect_order_alone_loses_the_pending_wakeup() {
    ...
    assert!(matches!(
        drive(
            tape.ordered
                .replay()
                .run(128, |p| { Box::pin(receive(p.io, p.entropy, p.clock)) })
        ),
        Err("no wakeup")
    ));
}
...
fn capture_and_replay_futures_are_send_for_native_owned_tasks() {
    ...
    let record = session(Choppy::new())
        .record_polls_send(bounds(), |p| Box::pin(receive(p.io, p.entropy, p.clock)));
    assert_send(&record);
    let (original, _, tape) = drive(record).unwrap();
    let replay = tape
        .unwrap()
        .run_send(128, |p| Box::pin(receive(p.io, p.entropy, p.clock)));
    assert_send(&replay);
...
fn recorded_errors_remain_application_outputs() {
    let mut io = Choppy::new();
    io.fail = true;
    let (original, _, tape) =
        drive(session(io).record_polls(bounds(), |p| Box::pin(receive(p.io, p.entropy, p.clock))))
            .unwrap();
...
fn pending_request_mismatch_poison_is_shared_with_entropy() {
    ...
    .unwrap();
    let replay = drive(tape.unwrap().run(128, |p| {
        Box::pin(async move {
    ...
    assert!(matches!(
        replay,
        Err(PolledRunError::Poll(PollReplayError {
            reason: PollMismatch::Request,
            ..
        }))
    ));
}
...
fn pending_read_capacity_is_checked_before_modifying_the_destination() {
    let ((), _, tape) = drive(session(Choppy::new()).record_polls(bounds(), |p| {
        Box::pin(async move {
            let mut bytes = [0];
            p.io.read(&mut bytes).await.unwrap();
        })
    }))
    .unwrap();
    let replay = drive(tape.unwrap().run(128, |p| {
...
async fn vector<I: AsyncWrite + Unpin>(io: &mut I, changed: bool) {
    let bufs = if changed {
        [IoSlice::new(b"ab"), IoSlice::new(b"c")]
    } else {
...
fn vectored_boundaries_are_checked_even_on_a_pending_poll() {
    let ((), _, tape) =
        drive(session(Choppy::new()).record_polls(bounds(), |p| Box::pin(vector(p.io, false))))
            .unwrap();
    assert!(matches!(
        drive(tape.unwrap().run(128, |p| Box::pin(vector(p.io, true)))).unwrap(),
        Err(PolledRunError::Poll(_))
    ));
}
...
fn pending_write_flush_and_shutdown_are_all_reproduced() {
    ...
    let (out, source, tape) =
        drive(session(Choppy::new()).record_polls(bounds(), |p| Box::pin(send(p.io)))).unwrap();
    out.unwrap();
    assert_eq!(source.written, b"abcde");
    let tape = tape.unwrap();
    assert_eq!(tape.io_polls(), 10);
    drive(tape.run(128, |p| Box::pin(send(p.io))))
        .unwrap()
        .unwrap()
        .unwrap();
}
...
fn capture_limits_do_not_change_live_outcome_or_discard_stream_owner() {
    for kind in 0..4 {
        ...
        match kind {
            0 => limit.max_polls = 0,
            1 => limit.max_io_polls = 0,
            2 => limit.max_write_bytes = 0,
            _ => limit.max_vectored_slices = 0,
        }
        let ((), source, tape) = drive(session(Choppy::new()).record_polls(limit, |p| {
            Box::pin(async move {
...
fn replay_poll_ceiling_refuses_before_invoking_factory() {
    ...
    assert!(matches!(
        result,
        Err(PolledRunError::Budget { limit: 0, .. })
    ));
...
fn changed_consumer_boundary_is_terminal_not_an_unbounded_self_wake_loop() {
    let ((), _, tape) =
        drive(session(Choppy::new()).record_polls(bounds(), |_| Box::pin(async {}))).unwrap();
    let result = drive(
        tape.unwrap()
            .run(1, |_| Box::pin(std::future::pending::<()>())),
    )
    .unwrap();
    assert!(matches!(
        result,
        Err(PolledRunError::Poll(PollReplayError {
            reason: PollMismatch::Boundary,
            ..
        }))
    ));
}
...
fn missing_destructor_effect_refuses_an_otherwise_complete_result() {
    let ((), _, tape) =
        drive(session(Choppy::new()).record_polls(bounds(), |p| Box::pin(FinalClock(p.clock))))
            .unwrap();
    assert!(matches!(
        drive(tape.unwrap().run(1, |_| Box::pin(async {}))).unwrap(),
        Err(PolledRunError::Poll(PollReplayError {
            reason: PollMismatch::Completion,
            ..
        }))
    ));
}
...
fn missing_factory_effect_is_refused_before_first_consumer_poll() {
    ...
    assert!(matches!(
        drive(tape.unwrap().run(1, |_| Box::pin(async {}))).unwrap(),
        Err(PolledRunError::Poll(PollReplayError {
            reason: PollMismatch::Construction,
            ..
        }))
    ));
}
...
fn swallowed_provider_panic_invalidates_capture() {
    let mut source = Choppy::new();
    source.panic_read = true;
    let ((), _, tape) = drive(session(source).record_polls(bounds(), |p| {
        Box::pin(async move {
            let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                let mut b = [0];
                let mut rb = ReadBuf::new(&mut b);
                let _ = Pin::new(&mut *p.io)
                    .poll_read(&mut Context::from_waker(Waker::noop()), &mut rb);
            }));
        })
    }))
    .unwrap();
...
fn already_used_ordered_session_cannot_publish_a_partial_poll_window() {
    let source = session(Choppy::new());
    source.clock().now();
    let (value, _, tape) = drive(source.record_polls(bounds(), |_| Box::pin(async { 7 }))).unwrap();
    assert_eq!(value, 7);
    assert_eq!(
        tape.unwrap_err().polls,
        Some(PollCaptureError::Inconsistent)
    );
}
...
fn extra_pending_io_after_tape_end_is_not_manufactured_eof() {
    let ((), _, tape) =
        drive(session(Choppy::new()).record_polls(bounds(), |_| Box::pin(async {}))).unwrap();
    let result = drive(tape.unwrap().run(1, |p| {
        Box::pin(async move {
            let mut b = [0];
            assert!(p.io.read(&mut b).await.is_err());
        })
    }))
    .unwrap();
    assert!(matches!(
...
pub(super) fn empty() -> PolledRecordedSession {
    drive(session(Choppy::new()).record_polls(bounds(), |_| Box::pin(async {})))
        .unwrap()
        .2
        .unwrap()
}
...
fn dropping_a_suspended_replay_destroys_its_consumer_once_without_a_result() {
    let recorded_drop = Arc::new(AtomicUsize::new(0));
    let seen = Arc::clone(&recorded_drop);
    ...
    let replay_drop = Arc::new(AtomicUsize::new(0));
    let seen = Arc::clone(&replay_drop);
    let mut driver = Box::pin(tape.unwrap().run(128, move |_| {
        Box::pin(YieldOnce {
            yielded: false,
            dropped: seen,
        })
