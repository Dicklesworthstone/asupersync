...
fn negative_control_independent_windows_accept_swap_but_ordered_replay_refuses_it() {
    let independent = RecordingSession::new(
        Pipe::new(),
        Arc::new(DetEntropy::new(42)),
        Arc::new(VirtualClock::new()),
        bounds(),
    ...
    drive(independent.into_parts().1.unwrap().replay().run(1, |p| {
        Box::pin(async move {
    ...
    recording.clock().now();
    let error = drive(recording.into_parts().1.unwrap().replay().run(1, |p| {
        Box::pin(async move {
    ...
    assert!(matches!(
        ...
        OrderedRunError::Replay(OrderedReplayError {
            order: Some(OrderCompletionError::Diverged(OrderReplayError {
                index: 0,
                reason: OrderReplayMismatch::Effect,
                ..
            })),
            ..
        })
...
fn nested_forks_keep_global_creation_order_and_exact_values() {
    ...
    let result = drive(recording.into_parts().1.unwrap().replay().run(10, |p| {
        Box::pin(async move {
            let child = p.entropy.try_fork(task(1, 4)).unwrap();
            let nested = child.try_fork(task(2, 5)).unwrap();
            (
                nested.try_next_u64().unwrap(),
                p.clock.try_now().unwrap(),
...
fn captured_connection_error_is_not_misclassified_as_replay_divergence() {
    ...
    let mut bytes = [0; 2];
    assert_eq!(
        value(Pin::new(recording.io()).poll_read(&mut cx(), &mut ReadBuf::new(&mut bytes)))
            .unwrap_err()
            .kind(),
    ...
    let mut replay = recording.into_parts().1.unwrap().replay();
    assert_eq!(
        value(Pin::new(replay.inputs().io).poll_read(&mut cx(), &mut ReadBuf::new(&mut bytes)))
            .unwrap_err()
            .kind(),
        io::ErrorKind::ConnectionReset
    );
...
struct ReentrantClock {
    entropy: Mutex<Option<Arc<OrderedRecordingEntropy>>>,
}
...
fn reentrant_provider_call_is_forwarded_but_capture_fails_without_deadlock() {
    let clock = Arc::new(ReentrantClock {
        entropy: Mutex::new(None),
    });
    let recording = OrderedRecordingSession::new(
        Pipe::new(),
        Arc::new(DetEntropy::new(42)),
        Arc::clone(&clock),
        bounds(),
    ...
    *clock.entropy.lock() = Some(recording.entropy());
    ...
    assert_eq!(
        ...
        Some(OrderCaptureError::Overlap)
    );
}
...
fn caught_source_panic_cannot_publish_a_partial_order() {
    let recording = OrderedRecordingSession::new(
        Pipe::new(),
        Arc::new(DetEntropy::new(42)),
        Arc::new(PanickingClock(AtomicBool::new(false))),
        bounds(),
        10,
    )
    .unwrap();
    assert!(
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| recording.clock().now())).is_err()
    );
...
fn divergence_wakes_both_directions_even_when_one_callback_panics_and_reenters() {
    ...
    let read = Waker::from(Arc::new(InspectWake {
        order: Arc::clone(&replay.order),
        calls: Arc::clone(&calls),
        panic: true,
    }));
    let write = Waker::from(Arc::new(InspectWake {
        order: Arc::clone(&replay.order),
        calls: Arc::clone(&calls),
        panic: false,
    }));
    ...
    assert!(
        Pin::new(&mut *p.io)
            .poll_write(&mut Context::from_waker(&write), b"x")
            .is_pending()
    );
...
impl Drop for DropClock<'_> {
    fn drop(&mut self) {
        let _ = self.0.try_now();
    }
}
#[test]
fn consumer_future_destructor_is_inside_the_verified_window() {
    ...
    drive(
        recording
            ...
            .unwrap()
            .replay()
            .run(1, |p| Box::pin(DropClock(p.clock))),
    )
    .unwrap();
    let error = drive(
        capture(10)
    ...
    assert!(matches!(
        error,
        OrderedRunError::Replay(OrderedReplayError {
            order: Some(OrderCompletionError::Diverged(_)),
            ..
        })
    ));
}
...
fn zero_poll_budget_refuses_before_factory_and_unused_effects_refuse_output() {
    ...
    let error = drive(
        recorded_exchange()
            .replay()
            .run(1, |_| Box::pin(async { 42 })),
    )
    .unwrap_err();
    assert!(matches!(
        error,
        OrderedRunError::Replay(OrderedReplayError {
            order: Some(OrderCompletionError::Remaining { .. }),
            ..
        })
    ));
}
...
fn interrupted_replay_admission_is_sticky_and_unconsumed() {
    let order = ReplayOrder::new(OrderTape {
        entries: vec![gate::Entry {
            effect: OrderedEffect::Clock,
            child: 0,
            pending: None,
        }],
        poll_aware: false,
    ...
    let error = match order.enter(OrderedEffect::Clock) {
        Err(error) => error,
        Ok(_) => panic!("poisoned admission"),
    };
