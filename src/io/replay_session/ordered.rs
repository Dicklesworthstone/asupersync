...
use super::{
    RecordedSession, RecordingSession, ReplayConsumerFuture, ReplaySession, SessionCaptureError,
    SessionCaptureLimits, SessionReplayError,
};
...
impl<T, S: ?Sized> fmt::Debug for OrderedRecordingSession<T, S> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OrderedRecordingSession")
...
impl<T: AsyncWrite, S: TimeSource + ?Sized> OrderedRecordingSession<T, S> {
    ...
    pub fn new_with_pending_io(
        ...
        Self::with_order(
            ...
            clock,
            limits,
            RecordOrder::with_pending(max_effects, Some(pending)),
        )
    }
    ...
    fn with_order(
        ...
        Ok(Self {
            io: OrderedRecordingIo {
                inner: io,
                order: Arc::clone(&order),
            },
            entropy: Arc::new(OrderedRecordingEntropy {
                inner: source,
                order: Arc::clone(&order),
                source: 0,
            }),
            clock: Arc::new(OrderedRecordingClock {
                inner: clock,
                order: Arc::clone(&order),
            }),
            recorder,
...
impl<T, S: TimeSource + ?Sized> OrderedRecordingSession<T, S> {
    ...
    pub fn io(&mut self) -> &mut OrderedRecordingIo<T> {
        &mut self.io
    }

    /// Share the ordered root entropy provider; forks remain ordered.
    #[must_use]
    pub fn entropy(&self) -> Arc<OrderedRecordingEntropy> {
        Arc::clone(&self.entropy)
    }

    /// Share the ordered serial clock provider.
    #[must_use]
    pub fn clock(&self) -> Arc<OrderedRecordingClock<S>> {
    ...
    pub fn into_parts(self) -> (T, Result<OrderedRecordedSession, OrderedCaptureError>) {
        ...
        let base = RecordingSession {
            ...
            clock: Arc::clone(&self.clock.inner),
        };
        ...
        let result = match (order, components) {
            ...
            (Ok(_), Ok(_)) => Err(OrderedCaptureError {
                order: Some(OrderCaptureError::InconsistentWindow),
                components: None,
            }),
            _ => Err(errors),
        };
...
impl fmt::Debug for OrderedRecordedSession {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OrderedRecordedSession")
            .field("effects", &self.order.entries.len())
            .finish_non_exhaustive()
    }
}
...
impl OrderedRecordedSession {
    ...
    pub fn effects(&self) -> usize {
        self.order.entries.len()
    }

    /// Whether every I/O poll must be reproduced, not just completed results.
    #[must_use]
    pub fn is_poll_aware(&self) -> bool {
        self.order.poll_aware
    }
...
impl fmt::Debug for OrderedReplaySession {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OrderedReplaySession")
            .finish_non_exhaustive()
    }
}
...
impl OrderedReplaySession {
    ...
    pub fn verify_complete(&self) -> Result<(), OrderedReplayError> {
        ...
        let components =
            (components.io.is_some() || components.entropy.is_some() || components.clock.is_some())
                .then_some(components);
        let order = self.order.verify().err();
        if order.is_none() && components.is_none() {
            Ok(())
        } else {
            Err(OrderedReplayError { order, components })
        }
    }
    ...
    where
        F: for<'a> FnOnce(OrderedReplayInputs<'a>) -> ReplayConsumerFuture<'a, T>,
    {
        if max_polls == 0 {
            return Err(OrderedRunError::PollLimit { limit: 0 });
        }
        let result = {
...
pub struct OrderedRecordingIo<T> {
    inner: RecordingIo<T>,
    order: Arc<RecordOrder>,
}
...
impl<T> OrderedRecordingIo<T> {
    fn poll_with<R>(
        ...
        if let Some(guard) = guard {
            guard.finish_io(input, result.is_ready());
        }
        result
    }
}
impl<T: AsyncRead + Unpin> AsyncRead for OrderedRecordingIo<T> {
    fn poll_read(
        ...
        self.get_mut()
            .poll_with(cx, PendingInput::Read(buf.remaining()), |io, cx| {
...
impl<T: AsyncWrite + Unpin> AsyncWrite for OrderedRecordingIo<T> {
    fn poll_write(
        ...
        self.get_mut()
            .poll_with(cx, PendingInput::Write(buf), |io, cx| {
    ...
    fn poll_write_vectored(
        ...
        self.get_mut()
            .poll_with(cx, PendingInput::Vectored(bufs), |io, cx| {
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        self.get_mut().poll_with(cx, PendingInput::Flush, |io, cx| {
            Pin::new(io).poll_flush(cx)
        })
    }
    fn poll_shutdown(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        self.get_mut()
            .poll_with(cx, PendingInput::Shutdown, |io, cx| {
...
pub struct OrderedRecordingEntropy {
...
impl fmt::Debug for OrderedRecordingEntropy {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OrderedRecordingEntropy")
...
impl EntropySource for OrderedRecordingEntropy {
    fn fill_bytes(&self, dest: &mut [u8]) {
        ...
        if let Some(guard) = guard {
            guard.finish(OrderedEffect::Entropy(self.source), true);
        }
    }
    ...
    fn fork(&self, task: TaskId) -> Arc<dyn EntropySource> {
        ...
        let source = guard.map_or(0, |guard| {
            guard.finish(OrderedEffect::Fork(self.source), true)
        });
        Arc::new(Self {
            inner,
...
pub struct OrderedRecordingClock<S: ?Sized> {
    inner: Arc<RecordingTimeSource<S>>,
    order: Arc<RecordOrder>,
}
impl<S: ?Sized> fmt::Debug for OrderedRecordingClock<S> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OrderedRecordingClock")
            .finish_non_exhaustive()
    }
}
...
impl OrderedReplayIo {
    fn poll_with<R>(
        ...
        if let Some(expected) = guard.pending_request() {
            ...
            let _ =
                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| cx.waker().wake_by_ref()));
            return Poll::Pending;
        }
        let result = poll(&mut self.inner, cx);
...
#[derive(Clone)]
pub struct OrderedReplayEntropy {
    inner: ReplayEntropy,
    order: Arc<ReplayOrder>,
    source: usize,
}
impl fmt::Debug for OrderedReplayEntropy {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OrderedReplayEntropy")
            .field("source", &self.source)
            .finish_non_exhaustive()
    }
}
impl OrderedReplayEntropy {
    ...
    pub fn try_fork(&self, task: TaskId) -> Result<Self, OrderReplayError> {
        ...
        guard.finish(result.is_ok())?;
        Ok(Self {
            inner: result.expect("component success checked"),
            order: Arc::clone(&self.order),
            source,
        })
    }
