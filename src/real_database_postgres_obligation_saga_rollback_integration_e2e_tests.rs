...
mod tests {
    ...
    use crate::{
        cx::{Cx, Scope},
        database::postgres::{
            ConnectionOptions, PostgresConfig, PostgresConnection, PostgresError, PostgresPool,
            PostgresTransaction, QueryResult, TransactionError, TransactionState,
        },
        error::{Error, Result},
        obligation::{
            ObligationId, ObligationRegistry, ObligationState,
            saga::{
                CompensationChain, CompensationResult, SagaCompensation, SagaContext,
                SagaDefinition, SagaError, SagaExecution, SagaOrchestrator, SagaState, SagaStep,
                StepResult,
            },
        },
    ...
    impl PostgresSagaTestFramework {
        ...
        async fn define_money_transfer_saga(&self) -> Result<SagaDefinition> {
            ...
            saga.add_step(
                SagaStep::new("validate_accounts")
            ...
            saga.add_step(
                SagaStep::new("create_transaction")
                    .with_action(Box::new(|ctx: &SagaContext| -> StepResult {
                        Ok(serde_json::json!({
            ...
            saga.add_step(
                SagaStep::new("credit_destination")
                    .with_action(Box::new(|ctx: &SagaContext| -> StepResult {
                        Ok(serde_json::json!({
            ...
            saga.add_step(
                SagaStep::new("finalize_transaction")
                    .with_action(Box::new(|ctx: &SagaContext| -> StepResult {
                        Ok(serde_json::json!({
                            "status": "COMPLETED",
        ...
        async fn execute_transaction_with_saga(
            ...
            let execution_result = self
                .execute_saga_with_db_operations(
                    cx,
                    &saga_def,
                    saga_context,
                    &mut pg_tx,
            ...
            match execution_result {
                Ok(execution) => {
                    ...
                    match pg_tx.commit().await {
                        Ok(()) => {
                            framework_ref
                                .record_event(SagaTransactionEvent {
                                    ...
                                    error_message: None,
                                    timestamp: Instant::now(),
                                    metadata: [("outcome".to_string(), "success".to_string())]
                                        .into(),
                                })
                                .await;
                            Ok(execution)
                        ...
                        Err(commit_error) => {
                            ...
                            framework_ref
                                .execute_compensation_chain(cx, &execution)
                                .await?;
        ...
        async fn execute_saga_with_db_operations(
            ...
            for (step_index, step) in saga_def.steps().iter().enumerate() {
                ...
                match step.name() {
                    "validate_accounts" => {
                        self.execute_validate_accounts_step(&mut context, pg_tx)
                            .await?;
                    }
                    "reserve_funds" => {
                    ...
                    "create_transaction" => {
                        self.execute_create_transaction_step(&mut context, pg_tx)
                            .await?;
                    }
                    "credit_destination" => {
                        self.execute_credit_destination_step(&mut context, pg_tx)
                            .await?;
                    }
                    "finalize_transaction" => {
                        ...
                        self.execute_finalize_transaction_step(&mut context, pg_tx)
                            .await?;
                    }
                    _ => {
        ...
        async fn execute_reserve_funds_step(
            ...
            if updated_rows == 0 {
                return Err(Error::new(
                    "Failed to reserve funds - insufficient balance or account not found",
                ));
            }
        ...
        async fn execute_compensation_chain(
            ...
            for step_name in execution.completed_steps().iter().rev() {
                match self
                    .execute_step_compensation(cx, step_name, execution)
                    .await
                {
                    Ok(()) => {
            ...
            // Update metrics
            {
                ...
                metrics.max_compensation_chain_length = metrics
                    .max_compensation_chain_length
                    .max(compensations_executed as usize);
        ...
        async fn execute_step_compensation(
            ...
            let compensation_result = match step_name {
                "finalize_transaction" => {
                    self.compensate_finalize_transaction(execution, &mut tx)
                        .await
                }
                "credit_destination" => {
                ...
                "reserve_funds" => self.compensate_reserve_funds(execution, &mut tx).await,
                "validate_accounts" => {
                    // No compensation needed for validation
                    Ok(())
                }
                _ => Err(Error::new(&format!(
                    "Unknown step for compensation: {}",
                    step_name
                ))),
            };
        ...
        async fn inject_failure(&self, failure_scenario: &FailureScenario) -> Result<Error> {
            let error_msg = match &failure_scenario.failure_type {
                ...
                FailureType::NetworkTimeout => {
                    "Network timeout during database operation".to_string()
                }
                FailureType::ConstraintViolation => "Database constraint violation".to_string(),
                FailureType::SerializationFailure => {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                let result = framework
                    .execute_transaction_with_saga(cx, business_tx, Some(failure_scenario))
                    .await;

                assert!(
                    result.is_err(),
                    "Transaction should fail due to injected commit failure"
                );

                // Verify compensation was triggered
                let compensation_started = framework
                    .get_event_count(SagaTransactionEventType::CompensationStarted)
                    .await;
                assert_eq!(compensation_started, 1, "Compensation should be triggered");

                let compensations_executed = framework
                    .get_event_count(SagaTransactionEventType::CompensationExecuted)
                    .await;
                ...
                assert!(
                    is_consistent,
                    "Database should remain consistent after compensation"
                );
    ...
    async fn test_successful_transaction_no_compensation() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                let result = framework
                    .execute_transaction_with_saga(cx, business_tx, None)
                    .await;
                ...
                let compensation_started = framework
                    ...
                    .await;
                assert_eq!(
                    compensation_started, 0,
                    "No compensation should be triggered for successful transaction"
                );

                // Verify saga completed
                let saga_completed = framework
                    .get_event_count(SagaTransactionEventType::SagaCompleted)
                    .await;
    ...
    async fn test_mid_transaction_failure_rollback() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                assert!(
                    result.is_err(),
                    "Transaction should fail due to injected step failure"
                );

                // Verify saga was aborted (not completed with compensation)
                let saga_aborted = framework
                    .get_event_count(SagaTransactionEventType::SagaAborted)
                    .await;
                ...
                assert!(
                    is_consistent,
                    "Database should remain consistent after rollback"
                );
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                assert!(
                    ...
                    "Should execute all step compensations"
                );
    ...
    async fn test_concurrent_transactions_with_failures() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                let tasks: Vec<_> = (0..5)
                    .map(|i| {
                        spawn(cx, Budget::unlimited(), async move {
                            ...
                            let result = framework_ref
                                .execute_transaction_with_saga(cx, business_tx, failure_scenario)
                                .await;
                ...
                let compensations_triggered = framework
                    .get_event_count(SagaTransactionEventType::CompensationStarted)
                    .await;
                assert!(
                    compensations_triggered > 0,
                    "Should have triggered compensations for failed transactions"
                );
                ...
                assert!(
                    is_consistent,
                    "Database should remain consistent after concurrent operations"
                );
