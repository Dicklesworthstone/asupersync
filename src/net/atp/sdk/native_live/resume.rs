...
use super::super::{
    EPOCH_HEADER_BYTES, HELLO_BYTES, Hello, LiveStreamConfig, LiveStreamError,
    LiveStreamPrefix, LiveStreamReceipt, LiveStreamReceiver, LiveStreamSender,
    Permit, Wire, advance, authorize, bounded, encode_epoch, encode_final,
    encode_prefix, expect,
};
...
fn validate_attempts(attempts: u32) -> Result<(), LiveStreamError> {
    if !(1..=1024).contains(&attempts) {
        return Err(LiveStreamError::Configuration("resume attempts must be 1..=1024"));
    }
    Ok(())
}
...
fn peer_certificate(tls: &TlsStream<TcpStream>) -> Result<[u8; 32], ResumeError> {
    ...
    let cert = tls.peer_leaf_certificate_der().ok_or(ResumeError::PeerIdentity)?;
...
impl LiveStreamSender {
    ...
    pub fn resumable_reader<R: AsyncRead + Unpin>(
        &self, cx: &Cx, remote: SocketAddr, source: R, max_attempts: u32,
        ...
        Ok(ResumableSender {
            source, connector: TlsConnector::new(tls), domain: self.domain.clone(),
            remote, config: self.config.clone(),
...
impl<R: AsyncRead + Unpin> ResumableSender<R> {
    ...
    #[must_use]
    pub fn acknowledged_prefix(&self) -> Option<&LiveStreamPrefix> { self.prefix.as_ref() }

    /// Final peer Proof retained before returning success, including after a dropped wait.
    #[must_use]
    pub fn completed_receipt(&self) -> Option<&LiveStreamReceipt> { self.completed.as_ref() }
    ...
    async fn send_inner(&mut self, cx: &Cx) -> Result<LiveStreamReceipt, ResumeError> {
        ...
        let tcp = bounded(cx, timeout, "resume connect", TcpStream::connect(self.remote)).await?;
        let tls = bounded(cx, timeout, "resume TLS", self.connector.connect(&self.domain, tcp)).await?;
        ...
        bounded(cx, timeout, "resume hello", wire.send(FrameType::Handshake, offer(&self.offered))).await?;
        ...
        loop {
            if self.pending.is_none() && self.final_receipt.is_none() {
                ...
                let count = bounded(cx, timeout, "resume source", poll_fn(|ctx| {
                    ...
                    match polled {
                        ...
                        Poll::Ready(Err(error)) => { self.failed = true; Poll::Ready(Err(error)) }
                ...
                if count == 0 {
                    self.final_receipt = Some(LiveStreamReceipt {
                        prefix: prefix.clone(), source_sha256: digest(&self.hash),
                    });
                } else {
                    let payload = encode_epoch(&prefix, &self.buffer[..count]);
                    let next = advance(&prefix, &payload, epoch_bytes, max_bytes)?;
                    self.pending = Some(PendingEpoch { payload, next, written: 0 });
                }
            }
            if let Some(pending) = &self.pending {
                // Wire owns only a copy; the session keeps the retransmission window.
                bounded(cx, timeout, "resume epoch write", wire.send(FrameType::ObjectData, pending.payload.clone())).await?;
                let ack = bounded(cx, timeout, "resume epoch acknowledgement", wire.receive()).await?;
            ...
            let receipt = self.final_receipt.as_ref().expect("source finished").clone();
            let payload = encode_final(&receipt);
            bounded(cx, timeout, "resume final write", wire.send(FrameType::ObjectComplete, payload.clone())).await?;
    ...
    fn reconcile(&mut self, bytes: &[u8]) -> Result<(), ResumeError> {
        ...
        if agreed.nonce != self.offered.nonce || agreed.epoch_bytes > self.offered.epoch_bytes
            || agreed.max_bytes > self.offered.max_bytes {
            return Err(ResumeError::Continuity("peer widened or rebound the session"));
        }
        ...
        let current_matches = prefix_bytes == encode_prefix(current) && hash_bytes == digest(&self.hash);
        ...
        if !current_matches && !pending_matches {
            return Err(ResumeError::Continuity("remote prefix is outside the retained window"));
        }
        if complete && self.final_receipt.as_ref().is_none_or(|receipt| {
            prefix_bytes != encode_prefix(&receipt.prefix) || hash_bytes != receipt.source_sha256
        }) {
            return Err(ResumeError::Continuity("peer completed before verified source EOF"));
        }
...
impl<W: LiveStreamCommitSink + Unpin> ResumableReceiver<W> {
    ...
    #[must_use]
    pub fn flushed_prefix(&self) -> Option<&LiveStreamPrefix> { self.prefix.as_ref() }
    ...
    #[must_use]
    pub const fn sink_written_bytes(&self) -> u64 { self.sink_written_bytes }
    ...
    async fn receive_wire(
        ...
        bounded(cx, timeout, "resume state write", wire.send(FrameType::HandshakeAck, state)).await?;
        loop {
            ...
            if frame.frame_type() == FrameType::ObjectComplete {
                ...
                if self.pending.is_some() || expect(&frame, FrameType::ObjectComplete)? != encode_final(&receipt) {
                    return Err(ResumeError::Continuity("wrong final commitment"));
                }
                self.final_receipt = Some(receipt.clone());
                self.finalize(cx).await?;
                let proof = bounded(cx, timeout, "resume Proof write", wire.send(FrameType::Proof, encode_final(&receipt))).await;
            ...
            if let Some(pending) = &self.pending {
                ...
                let next = advance(self.prefix.as_ref().expect("agreed prefix"), payload, agreed.epoch_bytes, agreed.max_bytes)?;
                self.pending = Some(PendingEpoch { payload: payload.to_vec(), next, written: 0 });
            }
            bounded(cx, timeout, "resume sink epoch", poll_fn(|ctx| self.poll_epoch(ctx))).await?;
            let prefix = encode_prefix(self.prefix.as_ref().expect("completed epoch"));
            bounded(cx, timeout, "resume epoch acknowledgement", wire.send(FrameType::Control, prefix)).await?;
        }
    }
    ...
    fn poll_epoch(&mut self, ctx: &mut Context<'_>) -> Poll<io::Result<()>> {
        ...
        if pending.written < pending.bytes().len() {
            self.failed = true;
            let result = Pin::new(&mut self.sink).poll_write(ctx, &pending.bytes()[pending.written..]);
            self.failed = false;
            match result {
                Poll::Pending => return Poll::Pending,
                Poll::Ready(Err(error)) => { self.failed = true; return Poll::Ready(Err(error)); }
                Poll::Ready(Ok(count)) => {
    ...
    fn poll_commit(&mut self, ctx: &mut Context<'_>) -> Poll<io::Result<()>> {
        if self.completed.is_some() { return Poll::Ready(Ok(())); }
        self.commit_started = true;
        self.failed = true;
        let receipt = self.final_receipt.as_ref().expect("validated final commitment");
    ...
    async fn finalize(&mut self, cx: &Cx) -> Result<(), ResumeError> {
        if self.completed.is_some() { return Ok(()); }
        let timeout = self.config.operation_timeout;
        if !self.commit_started {
            bounded(cx, timeout, "resume final flush", poll_fn(|ctx| {
                self.failed = true;
                let result = Pin::new(&mut self.sink).poll_flush(ctx);
        ...
        let observed = bounded(cx, timeout, "resume sink commit", poll_fn(|ctx| self.poll_commit(ctx))).await;
        ...
        match result {
            ...
            Err(source) => Err(LiveStreamError::Commit(Box::new(LiveStreamCommitError::Unconfirmed {
                receipt: Box::new(receipt.clone()), interruption: interruption.map(Box::new), source,
            })).into()),
        }
    }
}
