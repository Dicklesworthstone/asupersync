...
use super::{
    Budget, Credit, Hello, LiveStreamError, LiveStreamPrefix, LiveStreamReceipt, LiveStreamSender,
    OFFER_BYTES, PendingEpoch, RESUMABLE_LIVE_ALPN, ResumableSender, ResumeError, ResumeReport,
    advance, authorize, bounded, decode_offer, digest, encode_epoch, encode_prefix, initial, offer,
    validate_attempts,
};
use crate::cx::Cx;
...
fn invalid() -> io::Error {
    io::Error::new(
        ...
        "invalid sender checkpoint or source prefix",
    )
}
...
impl SenderCheckpoint {
    pub(super) fn capture<R>(sender: &ResumableSender<R>) -> Result<Self, ResumeError> {
        ...
        if let Some(pending) = &sender.pending {
            read_hash.update(pending.bytes());
        }
        let saved = Self {
            ...
            agreed: offer(agreed)
                ...
                .map_err(|_| ResumeError::LocalFailure)?,
            prefix,
            prefix_hash: digest(&sender.hash),
            read_hash: read_hash.finalize().into(),
            server: sender.peer.ok_or(ResumeError::PeerIdentity)?,
            remote: sender.remote,
            domain: sender.domain.clone(),
            used: sender.budget.used,
            maximum: sender.budget.maximum,
            eof: sender.final_receipt.is_some(),
            pending: sender.pending.as_ref().map(|p| PendingIntent {
                bytes: p.bytes().len() as u32,
                hash: Sha256::digest(p.bytes()).into(),
                chain: p.next.chain,
            }),
        };
    ...
    pub fn pending_bytes(&self) -> usize {
        self.pending
            .as_ref()
            .map_or(0, |pending| pending.bytes as usize)
    }
    ...
    fn validate(&self) -> io::Result<Hello> {
        ...
        if let Some(pending) = &self.pending {
            if pending.bytes == 0
                || pending.bytes as usize > agreed.epoch_bytes
                || p.bytes
                    .checked_add(u64::from(pending.bytes))
                    .is_none_or(|bytes| bytes > agreed.max_bytes)
                || p.epochs.checked_add(1).is_none()
            {
                return Err(invalid());
            }
        } else if self.prefix_hash != self.read_hash {
            return Err(invalid());
        }
        Ok(agreed)
    }
    ...
    pub fn to_canonical_bytes(&self) -> io::Result<Zeroizing<Vec<u8>>> {
        ...
        let mut bytes = Zeroizing::new(Vec::with_capacity(
            FIXED + self.domain.len() + PENDING_BYTES + 32,
        ));
    ...
    pub fn from_canonical_bytes(bytes: &[u8]) -> io::Result<Self> {
        if bytes.len() < FIXED + 33 || bytes.len() > MAX_SENDER_CHECKPOINT_BYTES {
            return Err(invalid());
        }
        let body = bytes.len() - 32;
        let name = usize::from(u16::from_be_bytes(
            bytes[308..310].try_into().expect("bounded name"),
        ));
        let pending =
            u32::from_be_bytes(bytes[310..314].try_into().expect("bounded pending")) as usize;
        if &bytes[..8] != MAGIC
            || name == 0
            || name > MAX_NAME
            || (pending != 0 && pending != PENDING_BYTES)
            || body != FIXED + name + pending
            || bytes[307] > 1
        ...
        let u64_at =
            |at: usize| u64::from_be_bytes(bytes[at..at + 8].try_into().expect("bounded u64"));
        ...
        let remote = match address[0] {
            ...
            6 => SocketAddr::V6(SocketAddrV6::new(
                Ipv6Addr::from(<[u8; 16]>::try_from(&address[1..17]).expect("bounded IPv6")),
                port,
        ...
        let saved = Self {
            ...
            eof: bytes[307] == 1,
            domain: std::str::from_utf8(&bytes[FIXED..FIXED + name])
                .map_err(|_| invalid())?
                .to_owned(),
    ...
    pub fn validate_successor(&self, old: &Self) -> io::Result<()> {
        self.validate()?;
        old.validate()?;
        if self.offered != old.offered
            || self.agreed != old.agreed
            || self.remote != old.remote
            || self.domain != old.domain
            || self.server != old.server
            || self.maximum != old.maximum
            || self.used < old.used
            || (old.eof && !self.eof)
        {
            return Err(invalid());
        ...
        if self.prefix == old.prefix {
            if self.prefix_hash != old.prefix_hash
                || (old.pending.is_some()
                    && (self.pending != old.pending || self.read_hash != old.read_hash))
            {
                return Err(invalid());
            }
        } else {
            ...
            if old.eof
                || self.prefix.epochs != old.prefix.epochs.checked_add(1).ok_or_else(invalid)?
                || self.prefix.bytes
                    != old
                        .prefix
                        .bytes
                        .checked_add(u64::from(pending.bytes))
                        .ok_or_else(invalid)?
                || self.prefix.chain != pending.chain
                || self.prefix_hash != old.read_hash
            {
                return Err(invalid());
            }
        }
...
/// Persistence for successive snapshots of one operation.
///
    ...
    let (result, interruption) = match observed {
        ...
        Err(error) if !operation.started => {
            return Err(Box::new(SenderCheckpointPersistError {
                stored: false,
                interruption: Some(Box::new(error)),
                source: None,
            })
            .into());
        }
    ...
    pub async fn restore_journaled_reader<R: AsyncRead + Unpin>(
        ...
        if remote != saved.remote || self.domain != saved.domain {
            return Err(ResumeError::Continuity(
                "journal endpoint or TLS name changed",
            ));
        }
        if agreed.max_bytes > self.config.max_bytes || agreed.epoch_bytes > self.config.epoch_bytes
        {
            return Err(
                LiveStreamError::Configuration("journal exceeds current transfer policy").into(),
            );
        }
        ...
        bounded(
            ...
            async {
                ...
                if digest(&hash) != saved.prefix_hash {
                    return Err(invalid());
                }
                ...
                if let Some(pending) = &saved.pending {
                    ...
                    .map_err(|_| invalid())?;
                    if next.chain != pending.chain {
                        return Err(invalid());
                    }
                    pending_payload = Some(PendingEpoch {
                        payload,
                        next,
                        written: 0,
                    });
                }
                if digest(&read_hash) != saved.read_hash {
                    return Err(invalid());
                }
                if saved.eof && read_checked(&mut source, &mut buffer[..1]).await? != 0 {
                    return Err(invalid());
                }
                Ok(())
            },
        ...
        let final_receipt = saved.eof.then(|| LiveStreamReceipt {
            ...
            source_sha256: saved.prefix_hash,
        });
        ...
        Ok(ResumableSender {
            ...
            offered,
            agreed: Some(agreed),
            peer: Some(saved.server),
            prefix: Some(saved.prefix),
            hash,
            pending,
            buffer: Vec::new(),
            final_receipt,
...
mod tests {
    ...
    fn sample() -> SenderCheckpoint {
        ...
        SenderCheckpoint {
            ...
            prefix,
            prefix_hash: Sha256::digest(b"abcdefgh").into(),
            read_hash: Sha256::digest(b"abcdefghijklmnop").into(),
            server: [3; 32],
            remote: "127.0.0.1:9443".parse().unwrap(),
            ...
            pending: Some(PendingIntent {
                bytes: 8,
                hash: Sha256::digest(b"ijklmnop").into(),
                chain: next.chain,
            }),
        }
    }
    ...
    fn bounded_metadata_roundtrip_keeps_pending_and_attempt_identity() {
        ...
        assert_eq!(loaded.maximum_attempts(), 4);
        assert!(!loaded.source_eof());
        assert_eq!(loaded.pending_bytes(), 8);
    ...
    fn decoder_rejects_torn_corrupt_and_noncanonical_records() {
        ...
        for end in 0..bytes.len() {
            assert!(SenderCheckpoint::from_canonical_bytes(&bytes[..end]).is_err());
        }
        ...
        for (offset, value) in [(277, 1), (307, 2), (313, 67)] {
            ...
            let hash = checksum(&changed[..body]);
    ...
    fn successors_cannot_rebind_skip_or_change_a_pending_epoch() {
        let old = sample();
        old.validate_successor(&old).unwrap();
        let mut attempt = old.clone();
        attempt.used += 1;
        attempt.validate_successor(&old).unwrap();
        let next = acknowledged(&old);
        next.validate_successor(&old).unwrap();
        for changed in [
            {
                let mut c = old.clone();
                c.server[0] ^= 1;
            ...
            {
                let mut c = old.clone();
                c.pending.as_mut().unwrap().hash[0] ^= 1;
                c
            },
            {
                let mut c = old.clone();
    ...
    fn eof_is_sticky_and_cannot_coexist_with_pending_data() {
        let old = sample();
        let mut eof = acknowledged(&old);
        eof.eof = true;
        eof.validate_successor(&old).unwrap();
        let roundtrip =
            SenderCheckpoint::from_canonical_bytes(&eof.to_canonical_bytes().unwrap()).unwrap();
        assert!(roundtrip.source_eof());
        let mut changed = eof.clone();
        changed.eof = false;
    ...
    fn empty_state_is_bound_to_the_negotiated_hello_and_empty_hash() {
        ...
        let hello = decode_offer(&saved.agreed).unwrap();
        saved.prefix = initial(&hello);
        saved.prefix_hash = digest(&Sha256::new());
        saved.read_hash = saved.prefix_hash;
        saved.pending = None;
        saved.eof = true;
        saved.validate().unwrap();
        let mut bad = saved.clone();
        bad.prefix.chain[0] ^= 1;
