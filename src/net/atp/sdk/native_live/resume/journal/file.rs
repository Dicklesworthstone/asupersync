...
fn invalid() -> io::Error {
    io::Error::new(io::ErrorKind::InvalidData, "invalid sender checkpoint journal")
}
...
impl Storage {
    fn open(path: &Path, create: Option<u32>) -> io::Result<Self> {
        ...
        Self::verify(&path, &directory, &file)?;
        Ok(Self {
            path, directory, maximum,
            state: Mutex::new(State { file, records, previous, latest, poisoned: false }),
        })
    }
    ...
    fn append(&self, checkpoint: SenderCheckpoint) -> io::Result<u32> {
        ...
        if state.poisoned {
            return Err(io::Error::other("sender journal persistence is unconfirmed"));
        }
...
impl SenderJournalFile {
    ...
    pub fn create_new(path: &Path, maximum_snapshots: u32) -> io::Result<Self> {
        Ok(Self::from_storage(Storage::open(path, Some(maximum_snapshots))?))
    }
    ...
    fn from_storage(storage: Storage) -> Self {
        ...
        drop(state);
        Self {
            storage: Arc::new(storage), latest, records,
            pending: None, pending_bytes: None, failure: None,
        }
    }
    ...
    #[must_use]
    pub const fn persisted_snapshots(&self) -> u32 { self.records }

    /// Lifetime snapshot limit, preserved across reopen.
    #[must_use]
    pub fn maximum_snapshots(&self) -> u32 { self.storage.maximum }
}

impl SenderCheckpointStore for SenderJournalFile {
    fn poll_store(
        self: Pin<&mut Self>, cx: &mut Context<'_>, checkpoint: &SenderCheckpoint,
    ) -> Poll<io::Result<()>> {
        ...
        if this.pending_bytes.as_ref().is_some_and(|pending| pending.as_slice() != bytes.as_slice()) {
        ...
        let result = ready!(this.pending.as_mut().expect("one journal append").as_mut().poll(cx));
        this.pending = None;
...
mod tests {
    ...
    fn sample() -> SenderCheckpoint {
        ...
        SenderCheckpoint {
            ...
            prefix, prefix_hash: digest(&Sha256::new()), read_hash: Sha256::digest(b"abcdefgh").into(),
            server: [3; 32], remote: "127.0.0.1:8443".parse().unwrap(), domain: "localhost".into(),
            used: 1, maximum: 4, eof: false,
            pending: Some(PendingIntent { bytes: 8, hash: Sha256::digest(b"abcdefgh").into(), chain: next.chain }),
        }
    }
    ...
    fn transitions_survive_reopen_without_resetting_bounds_or_attempts() {
        ...
        let mut exhausted = next.clone(); exhausted.used = 3;
        assert_eq!(store.storage.append(exhausted).unwrap_err().kind(), io::ErrorKind::StorageFull);
        ...
        assert_eq!(loaded.to_canonical_bytes().unwrap().as_slice(), next.to_canonical_bytes().unwrap().as_slice());
        ...
        for variant in 0..5 {
            ...
            match variant {
                ...
                4 => {
                    ...
                    let mut h = Sha256::new(); h.update(b"asupersync.atp.sender-checkpoint.v1");
                    h.update(&bytes[start + 16..end]);
                    bytes[end..end + 32].copy_from_slice(&h.finalize());
                    let hash = checksum(&bytes[HEADER + BODY..HEADER + RECORD], &bytes[start..start + BODY]);
                    bytes[start + BODY..start + RECORD].copy_from_slice(&hash);
                }
                _ => unreachable!(),
            }
            let rejected = path();
            let mut file = OpenOptions::new().write(true).create_new(true).mode(0o600).open(&rejected).unwrap();
    ...
    fn uncertain_append_poison_is_sticky_and_does_not_hide_tail_damage() {
        let path = path();
        let store = SenderJournalFile::create_new(&path, 3).unwrap();
        let first = sample(); store.storage.append(first.clone()).unwrap();
    ...
    fn missing_empty_aliased_and_nonprivate_journals_are_refused() {
        ...
        store.storage.append(sample()).unwrap(); drop(store);
