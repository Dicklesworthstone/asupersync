...
use super::super::super::super::{
    LiveStreamConfig, LiveStreamError, LiveStreamReceipt, Wire, authorize, bounded, encode_final,
    encode_prefix,
};
...
fn validate_receipt(
    ...
    if prefix.stream_nonce != nonce
        || prefix.bytes > maximum_bytes
        ...
        || (prefix.bytes == 0 && receipt.source_sha256 != empty_hash)
    {
        return Err(LiveStreamError::Configuration(
            "restored receipt conflicts with key or limits",
        ));
    }
    Ok(())
}
...
impl<W> ServiceReceiver<W> {
    pub(super) fn new(
        initialized: ResumeSessionInit<W>,
    ...
    pub(super) async fn attempt(
        ...
        match self {
            ...
            Self::Receipt(receiver) => {
                ...
                ResumeReport {
                    outcome,
                    prefix: Some(receiver.receipt.prefix.clone()),
                    attempts: receiver.budget.used,
                    receipt_reused: true,
...
impl ReceiptReceiver {
    fn response(&mut self, offered: &[u8]) -> Result<Vec<u8>, ResumeError> {
        ...
        if self.receipt.prefix.bytes == 0 && initial(&agreed) != self.receipt.prefix {
            return Err(ResumeError::Continuity(
                "restored empty-stream hello changed",
            ));
        }
    ...
    async fn receive_wire(
        ...
        if frame.frame_type() != FrameType::ObjectComplete || frame.payload() != payload {
            return Err(ResumeError::Continuity(
                "restored session requires its exact final commitment",
            ));
        }
        bounded(
            cx,
            timeout,
            "restored final Proof",
            wire.send(FrameType::Proof, payload),
        )
        .await
