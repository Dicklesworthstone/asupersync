...
use super::{
    ...
    OFFER_BYTES, PendingEpoch, RESUMABLE_LIVE_ALPN, ResumableReceiver, ResumeError, ResumeReport,
    advance, authorize, bounded, decode_offer, digest, encode_prefix, initial, offer,
    validate_attempts,
};
use crate::cx::Cx;
...
fn invalid() -> io::Error {
    io::Error::new(
        ...
        "invalid receiver checkpoint or retained sink",
    )
}
...
impl ReceiverCheckpoint {
    fn capture<W>(receiver: &ResumableReceiver<W>) -> Result<Self, ResumeError> {
        ...
        if let Some(pending) = &receiver.pending {
            pending_hash.update(pending.bytes());
        }
        let saved = Self {
            ...
            agreed: offer(agreed)
                ...
                .map_err(|_| ResumeError::LocalFailure)?,
            prefix,
            prefix_hash: digest(&receiver.hash),
            pending_hash: digest(&pending_hash),
            used: receiver.budget.used,
            maximum: receiver.budget.maximum,
            phase: if receiver.completed.is_some() {
                ReceiverCheckpointPhase::Committed
            } else if receiver.final_receipt.is_some() {
                ReceiverCheckpointPhase::Finalizing
            } else {
                ReceiverCheckpointPhase::Receiving
            },
            pending: Zeroizing::new(
                receiver
                    .pending
                    .as_ref()
                    .map_or_else(Vec::new, |p| p.payload.clone()),
            ),
        };
    ...
    fn receipt(&self) -> LiveStreamReceipt {
        LiveStreamReceipt {
            ...
            source_sha256: self.prefix_hash,
        }
    }
    ...
    fn validate(&self) -> io::Result<Hello> {
        ...
        if prefix.bytes == 0
            && (*prefix != initial(&agreed) || self.prefix_hash != digest(&Sha256::new()))
        {
            return Err(invalid());
        }
        if self.pending.is_empty() {
            if self.pending_hash != self.prefix_hash {
                return Err(invalid());
            }
        } else {
            if self.phase != ReceiverCheckpointPhase::Receiving {
                return Err(invalid());
            }
            advance(prefix, &self.pending, agreed.epoch_bytes, agreed.max_bytes)
                .map_err(|_| invalid())?;
        }
        Ok(agreed)
    }
    ...
    pub fn from_canonical_bytes(bytes: &[u8]) -> io::Result<Self> {
        if !(FIXED + 32..=MAX_RECEIVER_CHECKPOINT_BYTES).contains(&bytes.len()) {
            return Err(invalid());
        }
        let body = bytes.len() - 32;
        let length =
            u32::from_be_bytes(bytes[281..285].try_into().expect("bounded length")) as usize;
        if &bytes[..8] != MAGIC
            || length != body - FIXED
        ...
        let number =
            |at: usize| u64::from_be_bytes(bytes[at..at + 8].try_into().expect("bounded number"));
        let saved = Self {
            client: NativeClientCertificateId::from_sha256(
                bytes[8..40].try_into().expect("bounded client"),
            ),
            offered,
            agreed,
            prefix: LiveStreamPrefix {
    ...
    pub fn validate_successor(&self, old: &Self) -> io::Result<()> {
        ...
        old.validate()?;
        if self.client != old.client
            || self.offered != old.offered
        ...
        if self.prefix == old.prefix {
            ...
            if !old.pending.is_empty()
                && (self.pending.as_slice() != old.pending.as_slice()
                    || self.pending_hash != old.pending_hash)
            {
                return Err(invalid());
            }
            let legal = match old.phase {
                ReceiverCheckpointPhase::Receiving => {
                    self.phase != ReceiverCheckpointPhase::Committed
                }
                ReceiverCheckpointPhase::Finalizing => {
                    self.phase != ReceiverCheckpointPhase::Receiving
                }
                ReceiverCheckpointPhase::Committed => {
                    self.phase == ReceiverCheckpointPhase::Committed
                }
        ...
        } else if old.pending.is_empty()
            || !self.pending.is_empty()
            || old.phase != ReceiverCheckpointPhase::Receiving
            || self.phase != ReceiverCheckpointPhase::Receiving
            || self.prefix
                != advance(
                    &old.prefix,
    ...
    async fn revalidate<R: AsyncRead + Unpin>(
        &self,
        source: &mut R,
    ) -> io::Result<(Sha256, Option<PendingEpoch>, u64)> {
        let agreed = self.validate()?;
        ...
        if digest(&hash) != self.prefix_hash {
            return Err(invalid());
        }
        let mut written = 0;
        let pending = if self.pending.is_empty() {
            ...
            entire.update(bytes);
            if digest(&entire) != self.pending_hash {
                return Err(invalid());
            }
            while written < bytes.len() {
                ...
                if buffer[..count] != bytes[written..written + count] {
                    return Err(invalid());
                }
                written += count;
                crate::runtime::yield_now().await;
        ...
        if read_checked(source, &mut buffer[..1]).await? != 0 {
            return Err(invalid());
        }
...
impl LiveStreamReceiver {
    ...
    pub async fn bind_restored_receiver<W: LiveStreamCommitSink + Unpin, R: AsyncRead + Unpin>(
        ...
        if saved.client != expected_client {
        ...
        if agreed.epoch_bytes > self.config.epoch_bytes || agreed.max_bytes > self.config.max_bytes
        {
            return Err(LiveStreamError::Configuration(
                "receiver checkpoint exceeds current policy",
            )
            .into());
        }
        ...
        let (hash, pending, sink_written_bytes) = bounded(
            cx,
            self.config.operation_timeout,
            "receiver sink revalidation",
            saved.revalidate(&mut retained),
        )
        .await?;
        drop(retained);
        let final_receipt =
        ...
        Ok(ResumableReceiver {
            ...
            offered: Some(saved.offered.to_vec()),
            agreed: Some(agreed),
            ...
            sink_written_bytes,
            final_receipt,
            commit_started: completed.is_some(),
            completed,
            failed: false,
            budget: Budget {
                used: saved.used,
...
mod tests {
    ...
    fn sample() -> ReceiverCheckpoint {
        ...
        let prefix = advance(
            &initial(&hello),
            &encode_epoch(&initial(&hello), b"prefix01"),
            8,
            64,
        )
        .unwrap();
        ...
        ReceiverCheckpoint {
            ...
            prefix,
            prefix_hash: Sha256::digest(b"prefix01").into(),
    ...
    fn canonical_receiver_checkpoint_roundtrips_exact_pending_bytes_and_client() {
        ...
        assert_eq!(
            decoded.to_canonical_bytes().unwrap().as_slice(),
            bytes.as_slice()
        );
    ...
    fn receiver_decoder_refuses_torn_corrupt_oversized_and_noncanonical_data() {
        ...
        for end in 0..valid.len() {
            assert!(ReceiverCheckpoint::from_canonical_bytes(&valid[..end]).is_err());
        }
        let mut longer = valid.to_vec();
        longer.push(0);
        assert!(ReceiverCheckpoint::from_canonical_bytes(&longer).is_err());
        assert!(
            ReceiverCheckpoint::from_canonical_bytes(&vec![0; MAX_RECEIVER_CHECKPOINT_BYTES + 1])
                .is_err()
        );
        for offset in [0, 32, 208, FIXED + EPOCH_HEADER_BYTES, valid.len() - 1] {
            let mut altered = valid.to_vec();
            altered[offset] ^= 1;
        ...
        for (offset, value) in [(280, 3), (284, 1)] {
            ...
            let hash = checksum(&altered[..body]);
    ...
    fn receiver_transitions_never_skip_wal_or_erase_commit_uncertainty() {
        ...
        let stable = flushed(&pending);
        stable.validate_successor(&pending).unwrap();
        let mut committing = stable.clone();
        committing.phase = ReceiverCheckpointPhase::Finalizing;
        ...
        wrong.prefix.chain[0] ^= 1;
        assert!(wrong.validate_successor(&pending).is_err());
        let mut rebound = pending.clone();
        rebound.client = NativeClientCertificateId::from_sha256([4; 32]);
    ...
    fn unresolved_publication_is_not_restarted_even_when_all_file_bytes_match() {
        ...
        let (_, pending, written) =
            finish(saved.revalidate(&mut b"prefix01abcdefgh".as_slice())).unwrap();
        assert!(pending.is_none());
        assert_eq!(written, 16);
        assert_eq!(
            saved.committed_receipt().unwrap().source_sha256,
            Sha256::digest(b"prefix01abcdefgh").as_slice()
        );
    }
}
