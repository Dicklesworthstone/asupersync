...
const QUIC_CONTROL_SOURCE_DATA_HEADER: usize = 4 + 8;
/// Control-stream payload chunk for the encrypted clean source-stream lane.
///
/// Fill each ATP frame close to the largest safe no-extension `ObjectData` frame.
/// This bypasses per-symbol RaptorQ/AEAD work on clean QUIC transfers while
/// retaining the manifest hash/merkle verifier at commit.
const QUIC_CONTROL_SOURCE_CHUNK_BYTES: usize = QUIC_CONTROL_SOURCE_FRAME_MAX_BYTES
...
const QUIC_CONTROL_SOURCE_FLUSH_BYTES: usize = 8 * 1024 * 1024;
...
impl NativeQuicFrameTransport {
    ...
    fn send_control_source_data(
        ...
        let wire = quic_control_source_data_wire_frame(entry, offset, data)?;
        let len = wire.len();
...
struct QuicControlSourceData<'a> {
    entry: u32,
    offset: u64,
    data: &'a [u8],
}
...
fn apply_quic_control_source_data_frame(
    ...
    let decoder = decoders
        .iter_mut()
        .find(|decoder| decoder.index == chunk.entry)
        .ok_or_else(|| {
            QuicTransportError::Integrity(format!(
...
async fn send_native_control_source_entries(
    ...
    for entry in &prepared.entries {
        ...
        while offset < entry.size {
            ...
            control.send_control_source_data(cx, conn, entry.index, offset, &buf[..read])?;
...
async fn receive_native_control_source_stream(
    ...
    loop {
        ...
        match frame.frame_type() {
            FrameType::ObjectData => {
                let accepted = apply_quic_control_source_data_frame(&frame, decoders)?;
                bytes_received =
    ...
    F: FnMut(NativeSenderDrivePoint, &mut NativeQuicConnection) -> Result<(), QuicTransportError>,
{
    ...
    if ack.control_source_stream {
        ...
        let bytes_sent =
            send_native_control_source_entries(cx, conn, &mut control, prepared).await?;
...
mod tests {
    ...
    fn quic_control_source_data_frame_roundtrips_entry_offset_and_payload() {
        let frame =
            quic_control_source_data_frame(7, 123_456, b"payload").expect("source data frame");
        ...
        let direct =
            quic_control_source_data_wire_frame(7, 123_456, b"payload").expect("direct wire");
    ...
    fn quic_control_source_data_chunk_stays_within_frame_cap() {
        ...
        let direct =
            quic_control_source_data_wire_frame(7, 123_456, &payload).expect("chunk cap fits");
        let max_frame_size = usize::try_from(MAX_FRAME_SIZE).unwrap_or(usize::MAX);
        ...
        assert!(quic_control_source_data_wire_frame(7, 123_456, &too_large).is_err());
    }
