...
impl HandshakeTracer {
    ...
    pub fn trace_handshake_event(&mut self, event: &HandshakeEvent, region_id: Option<String>) {
        let timestamp_ms = self.start_time.elapsed()
            .unwrap_or(Duration::ZERO)
            .as_millis() as u64;

        let entry = match event {
            HandshakeEvent::Started { role, initial_version, region_id: event_region } => {
                TraceEntry::new(
                    timestamp_ms,
                    TraceLevel::Info,
                    "handshake",
            ...
            HandshakeEvent::VersionNegotiation { supported_versions, selected_version } => {
                TraceEntry::new(
                    timestamp_ms,
                    TraceLevel::Info,
                    "handshake",
            ...
            HandshakeEvent::Retry { original_dest_cid, retry_token, retry_source_cid } => {
                TraceEntry::new(
                    timestamp_ms,
                    TraceLevel::Info,
                    "handshake",
            ...
            HandshakeEvent::InitialPacket { packet_number, crypto_offset, crypto_length, source_cid, dest_cid } => {
                TraceEntry::new(
                    timestamp_ms,
                    TraceLevel::Debug,
                    "handshake",
            ...
            HandshakeEvent::HandshakePacket { packet_number, crypto_offset, crypto_length } => {
                TraceEntry::new(
                    timestamp_ms,
                    TraceLevel::Debug,
                    "handshake",
            ...
            HandshakeEvent::KeyPhaseTransition { space, phase } => {
                TraceEntry::new(
                    timestamp_ms,
                    TraceLevel::Info,
                    "handshake",
    ...
    pub fn trace_connection_id(&mut self, event: &str, cid: &[u8], sequence: Option<u64>) {
        ...
        if let Some(seq) = sequence {
            data.as_object_mut().unwrap().insert("sequence".to_string(), json!(seq));
        }
    ...
    pub fn to_qlog(&self) -> Value {
        let events: Vec<Value> = self.entries.iter().map(|entry| {
            let mut event = json!({
            ...
            if let Some(ref region_id) = entry.region_id {
                event.as_object_mut().unwrap().insert("region_id".to_string(), json!(region_id));
            }

            if let Some(space) = entry.packet_space {
                event.as_object_mut().unwrap().insert("packet_space".to_string(), json!(format!("{:?}", space)));
            }
    ...
    pub fn summary(&self) -> Value {
        ...
        let total_duration = self.entries.last()
...
impl TraceAnalyzer {
    ...
    pub fn count_packets_by_space(entries: &[TraceEntry]) -> std::collections::HashMap<PacketSpace, usize> {
    ...
    pub fn validate_trace(entries: &[TraceEntry]) -> Vec<String> {
        ...
        let initial_packets = entries.iter()
            .filter(|e| e.packet_space == Some(PacketSpace::Initial))
            .count();
        let handshake_packets = entries.iter()
...
mod tests {
    ...
    fn test_trace_filtering_by_level() {
        ...
        let verbose_entry = TraceEntry::new(2, TraceLevel::Verbose, "test", "verbose_event", json!({}));
    ...
    fn test_summary_generation() {
        ...
        tracer.trace(TraceEntry::new(0, TraceLevel::Info, "handshake", "started", json!({})));
        tracer.trace(TraceEntry::new(50, TraceLevel::Debug, "protection", "encrypt", json!({})));
        tracer.trace(TraceEntry::new(100, TraceLevel::Info, "handshake", "completed", json!({})));
