...
mod tests {
    ...
    use crate::observability::diagnostics::{
        DiagnosticCollector, DiagnosticLevel, DiagnosticReport, SystemHealth,
    };
    use crate::observability::metrics::{MetricCollector, MetricType, MetricValue};
    use crate::runtime::{Runtime, RuntimeBuilder};
    use crate::time::{Duration, Instant};
    use crate::trace::event::{EventContext, EventKind, EventSeverity, EventTracer, TraceEvent};
    ...
    impl DiagnosticEventSystem {
        ...
        async fn start(&mut self, cx: &Cx) -> Result<(), AsupersyncError> {
            ...
            let diagnostic_handle = cx
                .spawn(|cx| async move { self.run_diagnostic_collection(cx).await })
                .await?;

            // Start event tracing loop
            let event_handle = cx
                .spawn(|cx| async move { self.run_event_tracing(cx).await })
                .await?;

            // Start correlation engine
            let correlation_handle = cx
                .spawn(|cx| async move { self.run_correlation_engine(cx).await })
                .await?;

            // Start analysis engine
            let analysis_handle = cx
                .spawn(|cx| async move { self.run_analysis_engine(cx).await })
                .await?;

            // Start health monitoring
            let health_handle = cx
                .spawn(|cx| async move { self.run_health_monitoring(cx).await })
                .await?;
        ...
        async fn collect_diagnostic(
            ...
            let diagnostic = self
                .diagnostic_collector
                .collect_diagnostic(source, level, message)?;
        ...
        async fn trace_event(
            ...
            let event = self
                .event_tracer
                .trace_event(kind, severity, message, context)?;
        ...
        async fn run_diagnostic_collection(&mut self, cx: &Cx) -> Result<(), AsupersyncError> {
            while self.is_running.load(Ordering::SeqCst) {
                ...
                self.metric_collector.record_metric(
                    "system.health.memory_pressure",
                    MetricValue::Gauge(system_health.memory_pressure),
                )?;
                self.metric_collector.record_metric(
                    "system.health.cpu_load",
                    MetricValue::Gauge(system_health.cpu_load),
                )?;
                self.metric_collector.record_metric(
                    "system.health.active_tasks",
                    MetricValue::Counter(system_health.active_tasks as u64),
                )?;
        ...
        async fn run_correlation_engine(&mut self, cx: &Cx) -> Result<(), AsupersyncError> {
            while self.is_running.load(Ordering::SeqCst) {
                ...
                match &self.correlation_strategy {
                    ...
                    CorrelationStrategy::AdaptiveCorrelation {
                        confidence_threshold,
                    } => {
                        self.perform_adaptive_correlation(*confidence_threshold, cx)
                            .await?;
                    }
                }
        ...
        async fn perform_time_based_correlation(&mut self, cx: &Cx) -> Result<(), AsupersyncError> {
            ...
            let recent_diagnostics: Vec<_> = self
                .diagnostic_buffer
                .iter()
            ...
            for diagnostic in recent_diagnostics {
                let related_events: Vec<_> = recent_events
                    .iter()
                    .filter(|e| {
                        (e.timestamp.duration_since(diagnostic.timestamp)).as_millis() <= 100
                    })
                    .cloned()
                    .collect();
        ...
        async fn perform_task_region_correlation(
            &mut self,
            cx: &Cx,
        ) -> Result<(), AsupersyncError> {
            ...
            for diagnostic in &self.diagnostic_buffer {
                let related_events: Vec<_> = self
                    .event_buffer
                    .iter()
                    .filter(|e| {
                        e.task_id() == diagnostic.task_id || e.region_id() == diagnostic.region_id
                    })
                    .cloned()
                    .collect();
        ...
        async fn perform_adaptive_correlation(
            &mut self,
            ...
            self.perform_span_based_correlation(cx).await?;
            let span_correlations =
                self.correlation_history.len() - time_correlations - task_correlations;

            // Keep only high-confidence correlations
            self.correlation_history
                .retain(|c| c.confidence >= confidence_threshold);

            self.stats
                .correlations_successful
                .fetch_add(self.correlation_history.len() as u64, Ordering::SeqCst);
            ...
            while self.is_running.load(Ordering::SeqCst) {
                match &self.analysis_mode {
                    ...
                    AnalysisMode::PredictiveAnalysis { history_window } => {
                        self.perform_predictive_analysis(*history_window, cx)
                            .await?;
                    }
                }
        ...
        async fn perform_realtime_analysis(&mut self, cx: &Cx) -> Result<(), AsupersyncError> {
            ...
            let recent_correlations: Vec<_> = self
                .correlation_history
                .iter()
            ...
            let critical_count = recent_correlations
                .iter()
        ...
        async fn perform_batch_analysis(
            &mut self,
            batch_size: usize,
            cx: &Cx,
        ) -> Result<(), AsupersyncError> {
            if self.correlation_history.len() >= batch_size {
                ...
                let avg_confidence: f64 =
                    batch.iter().map(|c| c.confidence).sum::<f64>() / batch.len() as f64;

                eprintln!(
                    "Batch analysis: {} correlations, avg confidence: {:.2}",
                    batch.len(),
                    avg_confidence
                );
            }
        ...
        async fn perform_streaming_analysis(
            &mut self,
            window_size: Duration,
            cx: &Cx,
        ) -> Result<(), AsupersyncError> {
            let cutoff = Instant::now() - window_size;
            let window_correlations: Vec<_> = self
                .correlation_history
                .iter()
            ...
            if !window_correlations.is_empty() {
                ...
                self.metric_collector
                    .record_metric("correlation.rate", MetricValue::Gauge(correlation_rate))?;
            }
        ...
        async fn perform_predictive_analysis(
            ...
            let historical_data: Vec<_> = self
                .correlation_history
                .iter()
                .filter(|c| c.correlated_at > cutoff)
                .collect();
            ...
            if historical_data.len() > 10 {
                ...
                let recent_critical = historical_data[recent_half..]
                    .iter()
                    .filter(|c| c.diagnostic.level == DiagnosticLevel::Critical)
                    .count();
                let older_critical = historical_data[..recent_half]
                    .iter()
                    .filter(|c| c.diagnostic.level == DiagnosticLevel::Critical)
                    .count();
        ...
        async fn run_health_monitoring(&mut self, cx: &Cx) -> Result<(), AsupersyncError> {
            while self.is_running.load(Ordering::SeqCst) {
                ...
                self.metric_collector.record_metric(
                    ...
                    MetricValue::Gauge(system_health.overall_score()),
                )?;
                self.metric_collector.record_metric(
                    "diagnostics.buffer_size",
                    MetricValue::Gauge(self.diagnostic_buffer.len() as f64),
                )?;
                self.metric_collector.record_metric(
                    "events.buffer_size",
                    MetricValue::Gauge(self.event_buffer.len() as f64),
                )?;
                self.metric_collector.record_metric(
                    "correlations.history_size",
                    MetricValue::Gauge(self.correlation_history.len() as f64),
                )?;
        ...
        fn get_stats(&self) -> DiagnosticEventStats {
            DiagnosticEventStats {
                diagnostics_collected: AtomicU64::new(
                    self.stats.diagnostics_collected.load(Ordering::SeqCst),
                ),
                events_traced: AtomicU64::new(self.stats.events_traced.load(Ordering::SeqCst)),
                correlations_successful: AtomicU64::new(
                    self.stats.correlations_successful.load(Ordering::SeqCst),
                ),
                correlations_failed: AtomicU64::new(
                    self.stats.correlations_failed.load(Ordering::SeqCst),
                ),
                health_checks_performed: AtomicU64::new(
                    self.stats.health_checks_performed.load(Ordering::SeqCst),
                ),
                critical_issues_detected: AtomicU64::new(
                    self.stats.critical_issues_detected.load(Ordering::SeqCst),
                ),
    ...
    async fn test_basic_diagnostic_event_integration() -> Result<(), AsupersyncError> {
        ...
        cx.scope(|cx| async move {
            ...
            let mut system =
                DiagnosticEventSystem::new(config, correlation_strategy, analysis_mode)?;
            ...
            system
                .collect_diagnostic("test", DiagnosticLevel::Info, "Test diagnostic 1", cx)
                .await?;
            system
                .collect_diagnostic("test", DiagnosticLevel::Warning, "Test diagnostic 2", cx)
                .await?;
    ...
    async fn test_span_based_correlation() -> Result<(), AsupersyncError> {
        ...
        cx.scope(|cx| async move {
            ...
            let analysis_mode = AnalysisMode::StreamingAnalysis {
            ...
            cx.scope(|cx| async move {
                system
                    .collect_diagnostic("span_test", DiagnosticLevel::Info, "Span diagnostic", cx)
                    .await?;
    ...
    async fn test_adaptive_correlation_high_load() -> Result<(), AsupersyncError> {
        ...
        cx.scope(|cx| async move {
            ...
            let correlation_strategy = CorrelationStrategy::AdaptiveCorrelation {
