...
mod real_raptorq_systematic_e2e {
    use crate::raptorq::{
        Encoder, Decoder, SourceBlockEncoder, SourceBlockDecoder,
        Symbol, SymbolId, SourceBlock, RepairSymbol, SystematicIndex,
        EncodingConfig, DecodingConfig, ObjectTransmissionInfo,
    };
    ...
    impl RaptorQTestHarness {
        ...
        fn record_encoding_stats(&self, stats: EncodingStats) {
            ...
            self.log("raptorq_encoding", json!({
        ...
        fn record_decoding_stats(&self, stats: DecodingStats) {
            ...
            self.log("raptorq_decoding", json!({
                "source_block_id": stats.source_block_id,
        ...
        fn record_loss_simulation(&self, simulation: LossSimulation) {
            self.loss_simulations.lock().unwrap().push(simulation.clone());

            self.log("loss_simulation", json!({
        ...
        fn simulate_symbol_loss(
            ...
            match loss_pattern {
                ...
                LossPattern::Burst { burst_length } => {
                    ...
                    for (i, symbol) in symbols.iter().enumerate() {
                        if !in_burst && rng.gen::<f64>() < loss_rate / 2.0 {
                            in_burst = true;
            ...
            let simulation = LossSimulation {
                ...
                recovery_possible: remaining_symbols.len() >= self.calculate_minimum_symbols_needed(symbols.len()),
            };
        ...
        async fn test_systematic_encode_decode_cycle(
            ...
            let source_blocks = encoder.encode(data)
            ...
            for (block_id, source_block) in source_blocks.iter().enumerate() {
                ...
                let repair_symbols_needed = ((systematic_symbols.len() as f64) * loss_rate * 1.5).ceil() as usize;
                ...
                let stats = EncodingStats {
                    ...
                    encoding_rate: block_symbols.len() as f64 / source_block.source_symbols().len() as f64,
                };
            ...
            let (received_symbols, lost_indices) = self.simulate_symbol_loss(&all_symbols, loss_rate, loss_pattern);

            self.log("symbol_transmission", json!({
            ...
            for symbol in &received_symbols {
                decoder.add_symbol(symbol.clone())
            ...
            match decode_result {
                Ok(decoded_data) => {
                    ...
                    let stats = DecodingStats {
                        ...
                        symbols_lost: lost_indices.len(),
                        repair_symbols_used: received_symbols.iter()
                            .filter(|s| s.id().is_repair_symbol())
                            .count(),
                        decoding_duration_ms: decoding_time.as_millis() as u64,
                        recovery_successful,
                        recovery_efficiency: received_symbols.len() as f64 / all_symbols.len() as f64,
                    ...
                    if recovery_successful {
                        ...
                        self.log("decode_data_mismatch", json!({
                            "original_size": data.len(),
                            "decoded_size": decoded_data.len()
                        }));
                        Ok(false)
        ...
        fn validate_decoding_efficiency(&self) -> Result<(), String> {
            ...
            let successful_decodings: Vec<_> = decoding_stats.iter()
            ...
            let avg_efficiency: f64 = successful_decodings.iter()
                .map(|s| s.recovery_efficiency)
                .sum::<f64>() / successful_decodings.len() as f64;
    ...
    async fn test_raptorq_random_loss_recovery() {
        ...
        harness.log("test_start", json!({"test": "raptorq_random_loss_recovery"}));
        ...
        for (data_size, symbol_size, loss_rate) in test_cases {
            ...
            harness.log("test_case_start", json!({
                "data_size": data_size,
                "symbol_size": symbol_size,
                "loss_rate": loss_rate
            }));

            match harness.test_systematic_encode_decode_cycle(
                &test_data,
                symbol_size,
                loss_rate,
                LossPattern::Random
                ...
                Ok(recovery_successful) => {
                    ...
                    harness.log("test_case_result", json!({
                        "data_size": data_size,
                        "symbol_size": symbol_size,
                        "loss_rate": loss_rate,
                ...
                Err(e) => {
                    harness.log("test_case_error", json!({
                        "data_size": data_size,
                        "symbol_size": symbol_size,
                        "error": e
                    }));
        ...
        assert!(performance_validation.is_ok(), "Encoding performance validation failed: {:?}", performance_validation);

        let efficiency_validation = harness.validate_decoding_efficiency();
        assert!(efficiency_validation.is_ok(), "Decoding efficiency validation failed: {:?}", efficiency_validation);

        // Should recover most cases with reasonable loss rates
        let success_rate = total_successful as f64 / total_tests as f64;
        assert!(success_rate >= 0.8, "Success rate too low: {:.1}% ({}/{})",
            success_rate * 100.0, total_successful, total_tests);
    ...
    async fn test_raptorq_burst_loss_recovery() {
        ...
        for (loss_rate, burst_length) in burst_patterns {
            harness.log("burst_test_start", json!({
                "loss_rate": loss_rate,
            ...
            match harness.test_systematic_encode_decode_cycle(
                ...
                Ok(recovery_successful) => {
                    ...
                    harness.log("burst_test_result", json!({
                        "loss_rate": loss_rate,
                        "burst_length": burst_length,
                        "recovery_successful": recovery_successful
                    }));
                ...
                Err(e) => {
                    harness.log("burst_test_error", json!({
                        "loss_rate": loss_rate,
                        "burst_length": burst_length,
                        "error": e
                    }));
        ...
        assert!(successful_recoveries >= 2,
            ...
            successful_recoveries, burst_patterns.len());
    ...
    async fn test_raptorq_concurrent_encoding_decoding() {
        let harness = Arc::new(RaptorQTestHarness::new());
        harness.log("test_start", json!({"test": "raptorq_concurrent_encoding_decoding"}));
            ...
            let handle = spawn(async move {
                ...
                match harness.test_systematic_encode_decode_cycle(
                    ...
                    Ok(recovery_successful) => {
                        ...
                        harness.log("concurrent_worker_result", json!({
                            "worker_id": worker_id,
                            "data_size": data_size,
                            "recovery_successful": recovery_successful
                        }));
        ...
        let performance_validation = harness.validate_encoding_performance();
        assert!(performance_validation.is_ok(), "Concurrent encoding performance failed: {:?}", performance_validation);

        // Should maintain good success rate under concurrent load
        assert!(success_rate >= 0.75, "Concurrent success rate too low: {:.1}% ({}/{})",
            success_rate * 100.0, total_successful, total_tested);
    ...
    async fn test_raptorq_large_object_systematic() {
        let harness = Arc::new(RaptorQTestHarness::new());
        harness.log("test_start", json!({"test": "raptorq_large_object_systematic"}));
        ...
        harness.log("large_object_test_start", json!({
            "data_size": data_size,
            "symbol_size": symbol_size,
        ...
        match harness.test_systematic_encode_decode_cycle(
            ...
            Ok(recovery_successful) => {
                ...
                let throughput_mbps = (data_size as f64 * 8.0) / (test_duration.as_secs_f64() * 1_000_000.0);
                assert!(throughput_mbps > 1.0, "Throughput should be > 1 Mbps, got {:.2} Mbps", throughput_mbps);
            }
            Err(e) => {
        ...
        assert!(!encoding_stats.is_empty(), "Should have encoding stats for multiple blocks");

        let total_source_symbols: usize = encoding_stats.iter()
            .map(|s| s.source_symbols)
            .sum();

        let expected_symbols = (data_size + symbol_size - 1) / symbol_size;
        assert!(total_source_symbols >= expected_symbols * 80 / 100,
            "Should encode most expected symbols: got {}, expected ~{}",
            total_source_symbols, expected_symbols);
