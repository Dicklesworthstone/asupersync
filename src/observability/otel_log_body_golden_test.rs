...
mod tests {
    ...
    fn assert_golden_log_body(test_name: &str, body_value: &LogRecordBodyValue) {
        let golden_path = Path::new("tests/golden/log_body")
    ...
    fn log_body_string_array_golden() {
        let body = LogRecordBodyValue::StringArray(vec!["a".to_string(), "b".to_string(), "c".to_string()]);
    ...
    fn log_body_comprehensive_conformance() {
        let test_cases = vec![
            ("string_simple", LogRecordBodyValue::String("hello".to_string())),
            ...
            ("string_array_single", LogRecordBodyValue::StringArray(vec!["single".to_string()])),
            ("int_array_empty", LogRecordBodyValue::IntArray(vec![])),
            ("int_array_multi", LogRecordBodyValue::IntArray(vec![1, 2, 3])),
            ("float_array_empty", LogRecordBodyValue::FloatArray(vec![])),
            ("bool_array_mixed", LogRecordBodyValue::BoolArray(vec![true, false, true])),
        ];
        ...
        for (test_name, body_value) in test_cases {
            ...
            assert_eq!(any_value_1, any_value_2, "LogRecord body conversion must be deterministic for {}", test_name);
            ...
            assert_eq!(serialized_1, serialized_2, "AnyValue serialization must be deterministic for {}", test_name);
            ...
            assert_eq!(any_value_1, deserialized, "AnyValue round-trip failed for {}", test_name);
        }
    }
}