...
#[derive(Debug)]
struct Usage { bytes: u64, entries: u64 }
...
pub(super) fn load(configs: &[InboxConfig]) -> io::Result<Inboxes> {
    if configs.is_empty() || configs.len() > 1024 { return Err(invalid("configure 1..=1024 client inboxes")); }
    let scan_limit = configs.iter().try_fold(0_u64, |total, config| {
        total.checked_add(config.max_retained_entries)
    }).ok_or_else(|| invalid("aggregate inbox entry limit overflow"))?;
    if scan_limit > 1_000_000 { return Err(invalid("aggregate inbox scan limit exceeds one million entries")); }
    let mut result = BTreeMap::new();
    for config in configs {
        let id = selector(&config.certificate_sha256)?;
        if result.contains_key(&id) { return Err(invalid("duplicate client certificate selector")); }
...
impl Inbox {
    fn open(config: &InboxConfig) -> io::Result<Self> {
        if !config.directory.is_absolute() || !(1..=1_000_000).contains(&config.max_retained_entries) {
            return Err(invalid("inbox requires an absolute private directory and bounded entry count"));
        }
        let metadata = std::fs::symlink_metadata(&config.directory)?;
        if !metadata.is_dir() || metadata.permissions().mode() & 0o077 != 0 {
            return Err(io::Error::new(io::ErrorKind::PermissionDenied, "inbox directory must be private and not a symlink"));
        }
        ...
        if !held.is_file() || held.nlink() != 1 || held.permissions().mode() & 0o077 != 0 {
            return Err(invalid("inbox lock must be a private, single-link regular file"));
        }
        ...
        let mut usage = Usage { bytes: 0, entries: 0 };
        for entry in std::fs::read_dir(&directory)? {
            let entry = entry?;
            usage.entries = usage.entries.checked_add(1).ok_or_else(|| invalid("inbox entry count overflow"))?;
            if usage.entries > config.max_retained_entries { return Err(invalid("retained inbox entry limit exceeded")); }
            let metadata = std::fs::symlink_metadata(entry.path())?;
            if !metadata.is_file() { return Err(invalid("inbox contains a directory, symlink or special file")); }
            usage.bytes = usage.bytes.checked_add(metadata.len()).ok_or_else(|| invalid("inbox byte count overflow"))?;
            if usage.bytes > config.max_retained_bytes { return Err(invalid("retained inbox byte limit exceeded")); }
        }
        Ok(Self { directory, _lock: lock, maximum_bytes: config.max_retained_bytes,
            maximum_entries: config.max_retained_entries, usage: Mutex::new(usage) })
    }

    pub fn reserve(&self, transfer_bytes: u64) -> io::Result<()> {
        let charge = transfer_bytes.checked_mul(2).ok_or_else(|| invalid("inbox charge overflow"))?;
        let mut usage = self.usage.lock();
        let bytes = usage.bytes.checked_add(charge).filter(|bytes| *bytes <= self.maximum_bytes);
        let entries = usage.entries.checked_add(2).filter(|entries| *entries <= self.maximum_entries);
        let (Some(bytes), Some(entries)) = (bytes, entries) else {
            return Err(io::Error::new(io::ErrorKind::StorageFull, "inbox retention budget exhausted"));
        };
        usage.bytes = bytes;
