...
use super::{
    CONTROL_TICK, Options, Retention, RevocationPolicy, completion_event, deadline, key_json,
    snapshot_json,
};
use asupersync::Cx;
...
use asupersync::net::atp::sdk::native_auth::live::commit::resume::receiver_journal::file::{
    ReceiverJournalFile, ReceiverJournalObserver,
};
...
struct Tracked {
    entry: Entry,
    observer: ReceiverJournalObserver,
}
...
fn reserve_restored(catalog: &Catalog, inboxes: &storage::Inboxes, maximum: u64) -> io::Result<()> {
    for entry in catalog.entries() {
        ...
        let Some(inbox) = inboxes.get(&entry.key.client) else {
            continue;
        };
...
pub(in super::super) fn serve(
    ...
    let authority = settings::sdk(
        u32::try_from(limits.max_sessions).map_err(|_| invalid("session count overflow"))?,
        &profile,
    )?
    ...
    for client in &config.clients {
        ...
        if root.starts_with(&inbox) || inbox.starts_with(&root) {
            return Err(invalid(
                "catalog and inbox directory trees must be disjoint",
            ));
        }
    }
    let revocation_policy = if let Some(path) = revocations {
        let parent = std::fs::canonicalize(
            path.parent()
                .ok_or_else(|| invalid("policy parent required"))?,
        )?;
        if parent.starts_with(&root) {
            return Err(invalid(
                "revocation policy must be outside the dedicated catalog directory",
            ));
        }
        for client in &config.clients {
        ...
        Some(RevocationPolicy::open(path, ids, authorization)?)
    } else {
        None
    };
    CATALOG_OWNERSHIP
    ...
    emit(
        json!({"schema_version": 1, "event": "stopped", "mode": "shared_journaled", "drained": true}),
    )
}
...
async fn serve_loop(
    ...
    mut signals: Signals,
    mut revocations: Option<RevocationPolicy>,
) -> io::Result<()> {
        ...
        async move {
            ...
            };
            {
                ...
                if map.contains_key(&key) {
                    return Err(io::Error::other("duplicate catalog factory ownership"));
                }
    ...
    let mut service = authority
        .bind_resumable_service::<_>(&cx, config.bind, limits)
        .await
        .map_err(|_| io::Error::other("journaled shared listener could not bind"))?;
    if let Some(policy) = &revocations {
        policy.install(&mut service)?;
    }
    emit(
        json!({"schema_version": 1, "event": "ready", "mode": "shared_journaled",
    ...
    loop {
        ...
        for signal in signals.pending() {
            ...
            if stopping.is_none() {
                ...
                service.cancel(CancelReason::user(
                    "shared journal repeated shutdown signal",
                ));
                cancelling = true;
            }
        }
        ...
        if catalog.failed() && failure.is_none() {
            failure = Some(io::Error::other(
                "receiver catalog persistence is unconfirmed",
            ));
            service.cancel(CancelReason::user("shared journal catalog failed"));
            stopping.get_or_insert(now);
            cancelling = true;
        }
        if stopping.is_some_and(|start| now >= deadline(start, config.shutdown_grace_secs))
            && !cancelling
        {
            service.cancel(CancelReason::user("shared journal shutdown grace expired"));
            cancelling = true;
        }
        ...
        if stopping.is_none() {
            let due: Vec<_> = retention
                .iter()
                .filter_map(|(key, retained): (&ResumeSessionKey, &Retention)| {
                    if service.session_status(key) != Some(ResumeSessionStatus::Idle) {
                        return None;
                    }
                    if service.is_client_revoked(&key.client) {
                        Some((*key, Retirement::Revoked))
                    } else {
                        retained
                            .expired(now)
                            .map(|reason| (*key, expiry_reason(reason)))
                    }
                })
                .collect();
        ...
        let next = if stopping.is_some() {
            ...
            match asupersync::time::timeout(
                cx.now(),
                CONTROL_TICK,
                service.next_journaled(&cx, &scope, factory.clone()),
            )
            .await
            {
                ...
                Ok(Err(_)) => {
                    failure.get_or_insert_with(|| {
                        io::Error::other("shared journal service admission failed")
                    });
        ...
        let observation = completion
            .session
            .and_then(|key| tracked.lock().get(&key).cloned());
        ...
        if let Some(key) = completion.session {
            ...
            if let Some(reason) = reason.filter(|reason| {
                stopping.is_none()
                    || matches!(
                        reason,
                        Retirement::LocalFailure | Retirement::Initialization
                    )
            }) {
                if let Err(error) = retire(
                    &catalog,
...
fn expiry_reason(reason: &str) -> Retirement {
    if reason == "proof_recovery_expired" {
        Retirement::ProofWindow
    } else {
        Retirement::Idle
    }
}
fn retirement<W>(
    service: &ResumableService<W>,
    retention: &mut BTreeMap<ResumeSessionKey, Retention>,
    completion: &ResumeServiceCompletion,
    observed: Option<&Tracked>,
    now: u64,
    options: Options,
) -> Option<Retirement> {
    ...
    match service.session_status(&key) {
        ...
        Some(ResumeSessionStatus::Idle) => {
            ...
            if service.is_client_revoked(&key.client) {
                return Some(Retirement::Revoked);
            }
            if snapshot.failed {
                return Some(Retirement::LocalFailure);
            }
            // An original narrower ceiling is retained in its actual checkpoint.
            let ceiling = observed
                .and_then(|o| o.observer.checkpoint().ok())
                .map_or(options.attempts_per_session, |s| s.maximum_attempts());
            if snapshot.attempts >= ceiling {
                return Some(Retirement::Attempts);
            }
            if matches!(&completion.outcome, ResumeServiceOutcome::Transfer(_)) {
                let old = retention.get(&key).copied();
                retention.insert(
                    key,
                    Retention::observe(old, now, snapshot.completed.is_some(), options),
                );
            }
            retention
                .get(&key)
                .and_then(|r| r.expired(now))
                .map(expiry_reason)
        }
        ...
        None => {
            ...
            match &completion.outcome {
                ...
                ResumeServiceOutcome::Rejected(super::ResumeServiceRejection::Factory(_)) => {
                    Some(Retirement::Initialization)
                }
                ResumeServiceOutcome::Transfer(report) => match &report.outcome {
                    Err(
                        ResumeError::LocalFailure
                        | ResumeError::Transfer(LiveStreamError::Io(_) | LiveStreamError::Commit(_)),
                    ) => Some(Retirement::LocalFailure),
                    Err(ResumeError::ReceiverJournal(error)) if error.source.is_some() => {
                        Some(Retirement::LocalFailure)
                    }
                    _ => None,
                },
                ResumeServiceOutcome::Rejected(_) => None,
            }
        }
    }
}
async fn retire<W>(
    ...
    let snapshot = if service.session_status(&key) == Some(ResumeSessionStatus::Idle) {
        service
            .retire(&key)
            .map_err(|_| io::Error::other("idle journal retirement refused"))?
    } else {
        service.session_snapshot(&key).cloned()
    };
    retention.remove(&key);
    if output_open {
        emit(
            json!({"schema_version": 1, "event": "journal_session_retired", "session": key_json(key),
...
mod tests {
    ...
    fn shared_journal_cli_requires_explicit_catalog_and_storage_budgets() {
        ...
        assert!(
            JournalOptions {
                journal_snapshots: 32,
                journal_bytes: 128 * 1024 * 1024 + 1
            }
            .policy(64)
            .is_err()
        );
    ...
    fn restart_reserves_growth_for_disconnected_sessions_before_new_admission() {
        ...
        let path = state.join("catalog");
        Catalog::initialize(&path, 4, 65536).unwrap();
