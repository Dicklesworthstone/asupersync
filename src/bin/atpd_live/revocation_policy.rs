...
impl Snapshot {
    fn decode(bytes: &[u8]) -> io::Result<Self> {
        ...
        {
            return Err(invalid(
                "invalid revocation policy version, generation or count",
            ));
        }
    ...
    fn follows(&self, previous: &Self) -> io::Result<()> {
        ...
        {
            return Err(invalid(
                "revocation policy cannot roll back or remove a denial",
            ));
        }
        Ok(())
    }
...
fn read_policy(path: &Path) -> io::Result<Loaded> {
    ...
    {
        return Err(invalid(
            "revocation policy must be a bounded private single-link file",
        ));
    }
    ...
    if stamp(&file.metadata()?) != stamp(&before)
        ...
        || !parent_named.is_dir()
        || parent_named.permissions().mode() & 0o077 != 0
    ...
    Ok(Loaded {
        snapshot,
        directory: (directory_meta.dev(), directory_meta.ino()),
    })
}
...
impl RevocationPolicy {
    ...
    fn install_snapshot<W>(
        ...
        Ok(
            json!({"schema_version": 1, "event": "revocation_policy_applied",
    ...
    pub(super) async fn reload<W>(
        ...
        let candidate = asupersync::time::timeout(
            cx.now(),
            timeout,
            spawn_blocking_io(move || read_policy(&path)),
        )
        .await
    ...
    pub(super) fn fail_closed<W>(&self, service: &mut ResumableService<W>) {
        ...
        service.cancel(CancelReason::user(
            "atpd-live revocation policy unavailable",
        ));
    }
}
...
mod tests {
    ...
    fn policy_versions_are_monotonic_and_cannot_restore_authority() {
        ...
        assert!(
            Snapshot::decode(&document(9, &[&a, &b]))
                .unwrap()
                .follows(&old)
                .is_ok()
        );
    ...
    fn strict_policy_parser_bounds_input_and_rejects_ambiguous_fields() {
        ...
        for bytes in [
            ...
            document(1, &[&id, &id]),
            document(1, &["bad"]),
            b"{\"schema_version\":1,\"generation\":1,\"revoked_certificates\":[],\"extra\":true}"
                .to_vec(),
            b"{\"schema_version\":1,\"generation\":1,\"generation\":2,\"revoked_certificates\":[]}"
                .to_vec(),
    ...
    fn policy_file_is_private_complete_and_survives_an_independent_reopen() {
        ...
        assert!(!path.exists());
        let bytes = document(1, &[&"33".repeat(32)]);
        write(&path, &bytes);
        let first = read_policy(&path).unwrap();
        let second = read_policy(&path).unwrap();
    ...
    fn symlinks_hardlinks_oversized_and_torn_policy_files_are_refused() {
        ...
        let path = parent.join("valid");
        write(&path, &document(1, &[]));
        std::os::unix::fs::symlink(&path, parent.join("symlink")).unwrap();
        assert!(read_policy(&parent.join("symlink")).is_err());
        std::fs::hard_link(&path, parent.join("alias")).unwrap();
        assert!(read_policy(&path).is_err());
        write(&parent.join("torn"), b"{\"schema_version\":1,");
        assert!(read_policy(&parent.join("torn")).is_err());
