...
#[derive(Parser)]
#[command(name = "atpd-live", version, about = "Foreground mTLS live transfer with explicit committed inboxes")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn runtime<T: Send + 'static>(
    ...
    let runtime = builder.blocking_threads(1, workers.max(2)).build()
    ...
    if !runtime.shutdown_timeout(Duration::from_secs(30)) {
        ...
        return Err(io::Error::other("runtime shutdown did not confirm complete drain"));
    }
    result
}
...
async fn serve_loop(
    ...
    let scope = cx.scope();
    let mut service = receiver.bind_service(&cx, config.bind, config.max_connections as usize).await
    ...
    let factory = move |child: Cx, peer: LiveStreamPeer| {
        ...
        async move {
            ...
            let sink = LiveFileSink::create(&child, inbox.directory.clone(), filename, limit).await?;
            ...
            if publications.contains_key(&peer.address) {
                ...
                return Err(io::Error::new(io::ErrorKind::AddrInUse, "uncollected peer publication"));
            }
    ...
    loop {
        ...
        if let Some(start) = stopping_at {
            ...
            if !cancelling && elapsed >= Duration::from_secs(config.shutdown_grace_secs).as_nanos() as u64 {
                service.cancel(CancelReason::user("atpd-live shutdown grace expired"));
                cancelling = true;
            }
        }
        let completion = if stopping_at.is_some() {
            match asupersync::time::timeout(cx.now(), Duration::from_millis(50), service.drain_next()).await {
                Ok(completion) => completion,
                Err(_) => continue,
            }
        } else {
            match asupersync::time::timeout(cx.now(), Duration::from_millis(50),
                service.next_committing(&cx, &scope, factory.clone())).await
            {
        ...
        if failure.is_none() && let Err(error) = emit(event) {
...
fn report_json(report: &LiveStreamReport) -> Value {
    let (status, receipt) = match &report.outcome {
        ...
        Err(LiveStreamError::Commit(error)) => match error.as_ref() {
            LiveStreamCommitError::CommittedWithoutProof { receipt, .. } => ("committed_without_proof", Some(receipt.as_ref())),
            LiveStreamCommitError::Unconfirmed { receipt, .. } => ("commit_unconfirmed", Some(receipt.as_ref())),
        ...
        Err(LiveStreamError::Io(error)) if error.kind() == io::ErrorKind::StorageFull => ("retention_refused", None),
...
fn completion_event(completion: &LiveStreamCompletion, publication: Option<&LiveFilePublication>) -> Value {
    let (peer, transfer) = match &completion.result {
        Ok(session) => (session.peer.as_ref().map(|peer| hex(peer.certificate.as_bytes())), report_json(&session.transfer)),
...
fn send(config: SendConfig, input: PathBuf) -> io::Result<()> {
    ...
    runtime(config.workers, async move {
        ...
        let report = sender.send_reader(&cx, config.remote, File::from_std(file)).await;
        emit(json!({"schema_version": 1, "event": "send_result", "transfer": report_json(&report),
            "final_proof_direction": "received"}))?;
        report.outcome.map(|_| ()).map_err(|_| io::Error::other("send did not receive final peer Proof; no retry was attempted"))
    })
}
