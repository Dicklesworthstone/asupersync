...
impl CreateOptions {
    fn limits(self, max_data_bytes: u64) -> io::Result<ReceiverFileLimits> {
        ...
        {
            return Err(invalid("invalid journaled receiver attempt or storage budgets"));
        }
...
pub(super) fn receive(
    config: ServeConfig, journal: PathBuf, data: PathBuf, options: CreateOptions,
) -> io::Result<()> {
...
pub(super) fn resume(
    config: ServeConfig, journal: PathBuf, data: PathBuf, wait: WaitOptions,
) -> io::Result<()> {
    wait.validate()?;
    execute(config, journal, data, None, wait)
}

fn checked_paths(config: &ServeConfig, journal: &Path, data: &Path) -> io::Result<(PathBuf, PathBuf)> {
    if config.clients.len() != 1 || config.max_connections != 1 {
        return Err(invalid("journaled receiving requires exactly one client and max_connections=1"));
    }
    let normalize = |path: &Path| -> io::Result<PathBuf> {
        if !path.is_absolute() { return Err(invalid("absolute journal and data paths required")); }
        let name = path.file_name().ok_or_else(|| invalid("file name required"))?;
        let parent = path.parent().ok_or_else(|| invalid("private parent required"))?;
        let metadata = std::fs::symlink_metadata(parent)?;
        if !metadata.is_dir() || metadata.permissions().mode() & 0o077 != 0 {
            return Err(invalid("journal and data parents must be private directories, not symlinks"));
        }
    ...
    if data.parent() != Some(inbox.as_path()) || journal.parent() == Some(inbox.as_path())
        || data.file_name().is_some_and(|name| name == ".atpd-live.lock")
    {
        return Err(invalid("data must be in the selected inbox and journal outside it"));
    }
    Ok((journal, data))
}

fn execute(
    config: ServeConfig, journal: PathBuf, data: PathBuf,
    create: Option<CreateOptions>, wait: WaitOptions,
) -> io::Result<()> {
    wait.validate()?;
    ...
    {
        return Err(invalid("restore requires the original nonzero port and a bounded shutdown grace"));
    }
    ...
    let authority = settings::sdk(1, &profile)?.live_stream_receiver(
        profile, settings::identity(&config.identity)?, authorization,
    ...
    let inbox = inboxes.get(&client).ok_or_else(|| invalid("selected inbox missing"))?;
    let (journal, attempts) = match create {
        Some(options) => {
            ...
            (ReceiverJournalFile::create_new(&journal_path, &data_path,
                options.limits(config.max_transfer_bytes)?)?, options.attempts)
        }
        None => {
            let journal = ReceiverJournalFile::open_existing(&journal_path, &data_path)?;
            let saved = journal.checkpoint()?;
            if saved.client() != client || saved.phase() == ReceiverCheckpointPhase::Finalizing
                || saved.attempts() >= saved.maximum_attempts()
            {
                return Err(invalid("receiver history is unresolved, exhausted or belongs to another client"));
            }
            let length = settings::open_regular(&data_path, true)?.metadata()?.len();
            if length > config.max_transfer_bytes { return Err(invalid("retained data exceeds current limit")); }
    ...
    let outcome = runtime(config.workers, async move {
        let cx = Cx::current().ok_or_else(|| io::Error::other("missing journaled receiver context"))?;
        let scope = cx.scope();
        let mut control = Control::new(signals, config.shutdown_grace_secs);
        let entered = Arc::new(AtomicBool::new(false));
        let worker_entered = Arc::clone(&entered);
        let mut task = cx.spawn_in(&scope, move |child| {
            let future: Pin<Box<dyn Future<Output = Result<JournaledFileReceiver, ResumeError>> + Send>> =
                Box::pin(async move {
                    worker_entered.store(true, Ordering::Release);
                    if restored { journal.bind_restored(&authority, &child, config.bind, client).await }
                    else { journal.bind_new(&authority, &child, config.bind, client, attempts).await }
                });
            future
        ...
        let durable = incoming.checkpoint().ok().and_then(|saved| saved.committed_receipt());
        control.observe(&cx);
        if control.stopping() {
            return Ok(End { transfer: json!({"status": "cancelled_before_admission"}),
                checkpoint: checkpoint_json(incoming.checkpoint().ok().as_ref()), durable,
                proof_written: false, failed: false, stop: "stopped" });
        }
        emit(json!({"schema_version": 1, "event": "ready", "address": incoming.local_addr()?,
        ...
        if durable.is_some() { control.open_proof_window(cx.now().as_nanos(), wait.proof_recovery_secs); }
        drive(&cx, incoming, attempts, wait, control, durable).await
    })?;
    // Actual joins and runtime drain precede this terminal observation.
    emit(json!({"schema_version": 1, "event": "receive_result", "restored": restored,
    ...
    if outcome.durable.is_some() && !outcome.failed { Ok(()) }
    else { Err(io::Error::other("journaled receiver incomplete or unconfirmed; retained files require reconciliation")) }
}
...
impl Control {
    fn new(signals: Signals, grace_secs: u64) -> Self {
        Self { signals, stopping_at: None, grace_ns: grace_secs * 1_000_000_000,
            reason: None, proof_until: None }
    }
    ...
    fn observe(&mut self, cx: &Cx) {
        ...
        for _ in self.signals.pending() {
            if self.stopping_at.is_some() {
                self.reason.get_or_insert_with(|| CancelReason::user("journaled receiver repeated shutdown signal"));
            } else { self.stopping_at = Some(now); }
        }
        if let Some(reason) = cx.cancel_reason() { self.reason.get_or_insert(reason); }
        if self.stopping_at.is_some_and(|start| now.saturating_sub(start) >= self.grace_ns) {
            self.reason.get_or_insert_with(|| CancelReason::user("journaled receiver shutdown grace expired"));
        }
        if self.proof_until.is_some_and(|until| now >= until) {
            self.reason.get_or_insert_with(|| CancelReason::user("journaled receiver Proof window expired"));
        }
    }
    fn stopping(&self) -> bool { self.stopping_at.is_some() || self.reason.is_some() }
    async fn join<T>(&mut self, cx: &Cx, task: &mut TaskHandle<T>, entered: &AtomicBool) -> Result<T, JoinError> {
        let mut aborted = false;
        loop {
            self.observe(cx);
            if !aborted && entered.load(Ordering::Acquire) {
                if let Some(reason) = &self.reason {
                    task.abort_with_reason(reason.clone()); aborted = true;
                }
            }
            if let Ok(result) = asupersync::time::timeout(cx.now(), TICK, poll_fn(|ctx| task.poll_join(ctx))).await {
                return result;
            }
        }
    }
    async fn delay(&mut self, cx: &Cx, milliseconds: u64) {
        ...
        loop {
            ...
            if self.stopping() || remaining == 0 { return; }
...
impl End {
    fn refused(status: &'static str) -> Self {
        Self { transfer: json!({"status": status}), checkpoint: Value::Null,
            durable: None, proof_written: false, failed: true, stop: status }
    }
}

async fn drive(
    cx: &Cx, mut incoming: JournaledFileReceiver, attempts: u32, wait: WaitOptions,
    mut control: Control, durable: Option<LiveStreamReceipt>,
) -> io::Result<End> {
    let scope = cx.scope();
    let mut end = End { transfer: json!({"status": "not_attempted"}),
        checkpoint: checkpoint_json(incoming.checkpoint().ok().as_ref()), durable,
        proof_written: false, failed: false, stop: "attempts_exhausted" };
    loop {
        control.observe(cx);
        if control.stopping() { end.stop = "stopped"; return Ok(end); }
        let entered = Arc::new(AtomicBool::new(false));
        let worker_entered = Arc::clone(&entered);
        let task = cx.spawn_in(&scope, move |child| {
            let future: Pin<Box<dyn Future<Output = (JournaledFileReceiver, ResumeReport)> + Send>> =
                Box::pin(async move {
                    worker_entered.store(true, Ordering::Release);
        ...
        let mut task = match task {
            Ok(task) => task,
            Err(_) => { end.failed = true; end.stop = "spawn_failed"; end.checkpoint = Value::Null; return Ok(end); }
        };
        let (returned, report) = match control.join(cx, &mut task, &entered).await {
            Ok(result) => result,
            Err(_) => { end.failed = true; end.stop = "join_failed"; end.checkpoint = Value::Null; return Ok(end); }
        };
        incoming = returned;
        let saved = incoming.checkpoint().ok();
        if let Some(receipt) = saved.as_ref().and_then(ReceiverCheckpoint::committed_receipt) {
        ...
        emit(json!({"schema_version": 1, "event": "receiver_journal_attempt",
...
fn retryable(error: &ResumeError) -> bool {
    match error {
        ...
        ResumeError::Transfer(LiveStreamError::Timeout(_) | LiveStreamError::Tls(_)
            | LiveStreamError::Frame(_) | LiveStreamError::Protocol(_)) => true,
        ResumeError::Transfer(LiveStreamError::Io(error)) => matches!(error.kind(),
            io::ErrorKind::ConnectionReset | io::ErrorKind::ConnectionAborted | io::ErrorKind::BrokenPipe
            | io::ErrorKind::NotConnected | io::ErrorKind::TimedOut | io::ErrorKind::UnexpectedEof
...
fn checkpoint_json(saved: Option<&ReceiverCheckpoint>) -> Value {
    saved.map_or(Value::Null, |saved| json!({
...
mod tests {
    ...
    fn receiver_restore_cannot_reset_attempts_or_storage_budgets() {
        let args = ["atpd-live", "resume-receiver", "--config", "receiver.json",
            "--journal", "/private/receiver.wal", "--data", "/inbox/receiver.data"];
        assert!(matches!(Cli::try_parse_from(args).unwrap().command, Command::ResumeReceiver { .. }));
        for flag in ["--attempts", "--max-snapshots", "--max-journal-bytes"] {
            let mut invalid = args.to_vec(); invalid.extend([flag, "4"]);
            assert!(Cli::try_parse_from(invalid).is_err());
        }
        let args = ["atpd-live", "receive-journaled", "--config", "receiver.json",
            "--journal", "/private/receiver.wal", "--data", "/inbox/receiver.data",
            "--attempts", "4", "--max-snapshots", "32", "--max-journal-bytes", "65536"];
        let Command::ReceiveJournaled { options, .. } = Cli::try_parse_from(args).unwrap().command else {
            panic!("wrong command");
        };
        assert_eq!(options.limits(1024).unwrap().max_data_bytes, 1024);
        assert!(CreateOptions { attempts: 0, ..options }.limits(1024).is_err());
        assert!(CreateOptions { max_snapshots: 65_537, ..options }.limits(1024).is_err());
        assert!(CreateOptions { max_journal_bytes: 134_217_729, ..options }.limits(1024).is_err());
    }
    ...
    fn persisted_checkpoint_interruption_is_not_delivery_or_retry_permission() {
        ...
        let error = ResumeError::ReceiverJournal(Box::new(ReceiverCheckpointPersistError {
            stored: true, interruption: Some(Box::new(LiveStreamError::Timeout("receiver checkpoint"))),
            source: None,
        }));
        assert!(!retryable(&error));
        let report = ResumeReport { outcome: Err(error), prefix: None, attempts: 1,
            receipt_reused: false, retained_epoch_bytes: 8, sink_written_bytes: 3, completed: None };
