...
fn key() -> ResumeSessionKey {
    ResumeSessionKey { client: NativeClientCertificateId::from_sha256([7; 32]), nonce: [9; 32] }
}

fn receipt() -> LiveStreamReceipt {
    LiveStreamReceipt {
        prefix: LiveStreamPrefix { stream_nonce: [9; 32], epochs: 1, bytes: 4, chain: [3; 32] },
        source_sha256: [5; 32],
    }
}
...
fn connection_resident_client_and_lifetime_limits_are_independent() {
    ...
    assert_eq!((limits.max_connections, limits.max_sessions, limits.max_sessions_per_client), (2, 4, 2));
    assert_eq!((limits.max_session_keys, limits.max_attempts_per_session), (8, 3));
    for invalid in [
        Options { max_sessions: 0, ..options() },
        Options { max_sessions: 1025, ..options() },
        Options { max_sessions_per_client: 0, ..options() },
        Options { max_sessions_per_client: 5, ..options() },
        Options { max_session_keys: 3, ..options() },
        Options { max_session_keys: 65_537, ..options() },
        Options { attempts_per_session: 0, ..options() },
        Options { attempts_per_session: 1025, ..options() },
        Options { idle_retention_secs: 0, ..options() },
        Options { idle_retention_secs: 86_401, ..options() },
        Options { proof_recovery_secs: 0, ..options() },
        Options { proof_recovery_secs: u64::MAX, ..options() },
    ] { assert!(invalid.config(2).is_err(), "accepted {invalid:?}"); }
    for connections in [0, 5, u32::MAX] { assert!(options().config(connections).is_err()); }
}

#[test]
fn shared_command_requires_explicit_budgets_and_keeps_existing_commands() {
    use super::super::{Cli, Command};
    let args = ["atpd-live", "serve-resumable", "--config", "receiver.json",
        "--max-sessions", "4", "--max-sessions-per-client", "2",
        "--max-session-keys", "8", "--attempts-per-session", "3"];
    let cli = Cli::try_parse_from(args).unwrap();
    match cli.command {
        Command::ServeResumable { options, .. } => {
            assert_eq!(options.config(2).unwrap().max_sessions, 4);
            assert_eq!((options.idle_retention_secs, options.proof_recovery_secs), (300, 30));
        }
        _ => panic!("new command was not selected"),
    }
    assert!(Cli::try_parse_from(&args[..args.len() - 2]).is_err());
    assert!(matches!(Cli::try_parse_from(["atpd-live", "serve", "--config", "receiver.json"])
        .unwrap().command, Command::Serve { .. }));
    assert!(matches!(Cli::try_parse_from(["atpd-live", "send-resumable", "--config", "sender.json",
        "--input", "source.bin", "--attempts", "4"]).unwrap().command, Command::SendResumable { .. }));
}
...
fn completion_opens_one_absolute_proof_window_that_cannot_be_refreshed() {
    ...
    assert_eq!(completed, Retention::Committed { until: 5_000_000_200 });
    for now in [201, 5_000_000_199, 5_000_000_200, u64::MAX] {
        for reports_commit in [false, true] {
            assert_eq!(Retention::observe(Some(completed), now, reports_commit, options()), completed);
        }
    }
    assert_eq!(completed.expired(5_000_000_199), None);
    assert_eq!(completed.expired(5_000_000_200), Some("proof_recovery_expired"));
}

#[test]
fn incomplete_activity_renews_idle_time_but_does_not_claim_completion() {
    let initial = Retention::observe(None, 10, false, options());
    assert_eq!(initial.expired(10_000_000_010), Some("idle_retention_expired"));
    let later = Retention::observe(Some(initial), 1000, false, options());
    assert_eq!(later, Retention::Incomplete { until: 10_000_001_000 });
...
fn committed_without_proof_is_not_relabelled_as_success_or_rollback() {
    ...
    let report = ResumeReport {
        ...
        prefix: Some(receipt.prefix.clone()), attempts: 2, receipt_reused: false,
        retained_epoch_bytes: 0, sink_written_bytes: 4, completed: Some(receipt.clone()),
    };
    let event = completion_event(&ResumeServiceCompletion {
        connection: 11, address: "127.0.0.1:1234".parse().unwrap(), session: Some(key()),
    ...
    assert_eq!(event["transfer"]["completed_receipt"], receipt_json(&receipt));
...
fn refusal_projection_keeps_busy_retired_quota_and_factory_failures_distinct() {
    let cases = [
        ...
        (ResumeServiceRejection::Capacity("untrusted text must not escape"), "session_capacity_refused"),
        (ResumeServiceRejection::AttemptsExhausted, "attempts_exhausted"),
        (ResumeServiceRejection::Factory(io::Error::new(io::ErrorKind::StorageFull, "private path").into()), "retention_refused"),
        (ResumeServiceRejection::Factory(io::Error::new(io::ErrorKind::PermissionDenied, "secret sink detail").into()), "factory_failed"),
    ];
    for (rejection, status) in cases {
        let event = completion_event(&ResumeServiceCompletion {
            connection: 1, address: "127.0.0.1:1".parse().unwrap(), session: Some(key()),
...
fn failed_handshake_never_invents_an_authenticated_key_or_publication() {
    let event = completion_event(&ResumeServiceCompletion {
        connection: 0, address: "127.0.0.1:1".parse().unwrap(), session: None,
        outcome: ResumeServiceOutcome::Rejected(ResumeServiceRejection::Connection(
            ResumeError::Transfer(LiveStreamError::Tls(asupersync::tls::TlsError::Handshake("private diagnostic".to_owned()))),
        )),
    }, None);
