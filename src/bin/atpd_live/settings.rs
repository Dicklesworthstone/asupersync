...
use asupersync::net::atp::sdk::{
    AtpSdk, NativeClientCertificateId, NativeTlsIdentity, SessionConfig,
};
...
fn read_bounded(path: &Path, limit: u64, private: bool) -> io::Result<Zeroizing<Vec<u8>>> {
    ...
    if file.metadata()?.len() > limit {
        return Err(invalid("configuration input too large"));
    }
    let mut bytes = Zeroizing::new(Vec::new());
    file.take(limit + 1).read_to_end(&mut bytes)?;
    if bytes.len() as u64 > limit {
        return Err(invalid("configuration input grew beyond its limit"));
    }
    Ok(bytes)
}
...
fn certificates(path: &Path) -> io::Result<Vec<CertificateDer<'static>>> {
    ...
    for certificate in CertificateDer::pem_reader_iter(&mut reader) {
        if result.len() == 128 {
            return Err(invalid("too many certificates"));
        }
        result.push(certificate.map_err(|_| invalid("invalid certificate PEM"))?);
    }
    if result.is_empty() {
        return Err(invalid("no certificates in PEM file"));
    }
    Ok(result)
}
...
pub(super) fn identity(files: &IdentityFiles) -> io::Result<NativeTlsIdentity> {
    ...
    if chain.len() > 32 {
        return Err(invalid("identity chain too long"));
    }
    ...
    let key = keys
        .next()
        .transpose()
        .map_err(|_| invalid("invalid private key PEM"))?
        .ok_or_else(|| invalid("private key PEM is empty"))?;
    if keys.next().is_some() {
        return Err(invalid("exactly one private key is required"));
    }
...
pub(super) fn roots(path: &Path) -> io::Result<RootCertStore> {
    ...
    for certificate in certificates(path)? {
        roots
            .add(certificate)
            .map_err(|_| invalid("invalid explicit CA certificate"))?;
    }
    Ok(roots)
}
...
pub(super) fn selector(text: &str) -> io::Result<NativeClientCertificateId> {
    if text.len() != 64 || !text.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err(invalid(
            "client certificate selector must be 64 hexadecimal characters",
        ));
    }
    let mut bytes = [0; 32];
...
pub(super) fn profile(
    ...
    if version != 1
        ...
        || !(1..=MAX_LIVE_EPOCH_BYTES).contains(&epoch)
        || !(1..=86400).contains(&timeout)
        || bytes > u64::MAX / 2
    {
        return Err(invalid(
            "unsupported schema or invalid runtime/transfer limits",
        ));
    }
...
pub(super) fn sdk(connections: u32, config: &LiveStreamConfig) -> io::Result<AtpSdk> {
    if !(1..=1024).contains(&connections) {
        return Err(invalid("connection limit must be 1..=1024"));
    }
    let sdk = AtpSdk::new_in_process(SessionConfig {
        max_concurrent_transfers: connections,
        ..SessionConfig::default()
    });
    let mut policy = sdk.transfer_policy().clone();
    policy.max_transfer_size_bytes = config.max_bytes;
    policy.max_chunk_size_bytes = config
        .epoch_bytes
        .try_into()
