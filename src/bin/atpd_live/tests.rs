...
fn inbox(directory: PathBuf, bytes: u64, entries: u64) -> settings::InboxConfig {
    settings::InboxConfig {
        certificate_sha256: "ab".repeat(32), directory,
...
fn certificate_selectors_require_full_digests_and_normalize_case() {
    ...
    assert_eq!(settings::selector(&lower).unwrap(), settings::selector(&lower.to_uppercase()).unwrap());
    for value in ["".to_owned(), "0".repeat(63), "0".repeat(65), "gg".repeat(32), "é".repeat(32)] {
        assert!(settings::selector(&value).is_err());
    }
    assert_eq!(settings::hex(settings::selector(&lower).unwrap().as_bytes()), lower);
}

#[test]
fn live_configuration_refuses_unknown_fields_and_unbounded_runtime_limits() {
    let path = directory().join("settings.json");
    std::fs::write(&path, br#"{"schema_version":1,"unexpected_secret":"must-not-echo"}"#).unwrap();
    let error = settings::load::<settings::ServeConfig>(&path).err().unwrap();
    assert!(!error.to_string().contains("must-not-echo"));
    for (version, workers, epoch, bytes, timeout) in [
        (2, 1, 8, 10, 5), (1, 0, 8, 10, 5), (1, 33, 8, 10, 5),
        (1, 1, 0, 10, 5), (1, 1, 65537, 10, 5), (1, 1, 8, u64::MAX, 5), (1, 1, 8, 10, 0),
...
fn credential_open_rejects_public_keys_and_symlinks_without_changing_files() {
    ...
    assert_eq!(settings::open_regular(&file, true).unwrap_err().kind(), io::ErrorKind::PermissionDenied);
...
fn retention_reserves_aliases_and_never_refunds_an_admitted_attempt() {
    ...
    retained.reserve(5).unwrap();
    assert_eq!(retained.reserve(0).unwrap_err().kind(), io::ErrorKind::StorageFull);
    assert_eq!(retained.reserve(1).unwrap_err().kind(), io::ErrorKind::StorageFull);
}
...
fn restart_scans_all_retained_aliases_and_excludes_a_second_owner() {
    ...
    assert_eq!(storage::load(std::slice::from_ref(&config)).unwrap_err().kind(), io::ErrorKind::WouldBlock);
    assert_eq!(first[&id].reserve(3).unwrap_err().kind(), io::ErrorKind::StorageFull);
