...
use super::super::{LiveStreamPrefix, is_replay_refusal};
use super::*;
...
impl Fixture {
    fn new(committed: bool) -> Self {
        ...
        Ledger::initialize(&path, 1).unwrap();
        let ledger = Ledger::open(&path).unwrap();
        let name = format!("{}.bin", "a".repeat(32));
        ...
        let mut file = std::fs::OpenOptions::new()
            ...
            .open(&output)
            .unwrap();
        file.write_all(b"receipt").unwrap();
        ...
        if committed {
            ledger.commit_sync(key(2), receipt.clone()).unwrap();
        }
        Self {
            ledger,
            inbox,
    ...
    fn recover(&self) -> io::Result<Option<LiveStreamReceipt>> {
        self.ledger
            .recover_sync(key(2), &self.inbox, 64, &AtomicBool::new(false))
    }
}
...
fn restart_recovery_rehashes_without_appending_claims_or_receipts() {
    ...
    let identity = file_identity(&std::fs::metadata(&output).unwrap());
    drop(ledger);
    let reopened = Ledger::open(&path).unwrap();
    for _ in 0..3 {
        assert_eq!(
            reopened
                .recover_sync(key(2), &inbox, 64, &AtomicBool::new(false))
                .unwrap(),
            Some(receipt.clone())
        );
    }
    ...
    assert!(
        reopened
            .recover_sync(key(3), &inbox, 64, &AtomicBool::new(false))
            .unwrap()
            .is_none()
    );
    assert_eq!(
        reopened
            .claim_sync(key(3), format!("{}.bin", "b".repeat(32)), 64)
            .unwrap_err()
            .kind(),
...
fn changed_truncated_missing_and_symlinked_publications_are_refused() {
    for kind in ["changed", "short", "missing", "symlink", "public"] {
        ...
        match kind {
            ...
            "public" => {
                std::fs::set_permissions(&fixture.output, std::fs::Permissions::from_mode(0o644))
                    .unwrap()
            }
            _ => {
                ...
                if kind == "symlink" {
                    std::os::unix::fs::symlink(&saved, &fixture.output).unwrap();
                }
            }
        }
...
fn current_size_limit_and_poisoned_history_do_not_release_old_receipts() {
    ...
    assert!(
        fixture
            .ledger
            .recover_sync(key(2), &fixture.inbox, 6, &AtomicBool::new(false))
            .is_err()
    );
    ...
    assert!(
        fixture
            .ledger
            .recover_sync(key(3), &fixture.inbox, 64, &AtomicBool::new(false))
            .is_err()
    );
}
...
fn queued_or_abandoned_reads_keep_their_independent_admission_slot() {
    let fixture = Fixture::new(true);
    let mut permits: Vec<_> = (0..MAX_RECOVERY_READS)
        .map(|_| ReadPermit::reserve(&fixture.ledger).unwrap())
        .collect();
    assert_eq!(
        ReadPermit::reserve(&fixture.ledger).err().unwrap().kind(),
    ...
    assert_eq!(
        fixture
            .ledger
            .recover_sync(key(2), &fixture.inbox, 64, &stopped)
            .unwrap_err()
            .kind(),
        io::ErrorKind::Interrupted
    );
    assert_eq!(
        fixture.ledger.recovery_reads.load(Ordering::Acquire),
        MAX_RECOVERY_READS
    );
    drop(permits.pop());
...
fn committed_proof_recovery_requires_explicit_command_opt_in() {
    ...
    let mut opted = args.to_vec();
    opted.push("--recover-committed");
    assert!(matches!(
        Cli::try_parse_from(opted).unwrap().command,
        Command::ServeDurable {
