...
use super::{AlreadyClaimed, Entry, HEADER_BYTES, Ledger, RECORD_BYTES, corrupt, unavailable, verify_identity};
...
impl ReadPermit {
    fn reserve(ledger: &Arc<Ledger>) -> io::Result<Self> {
        ...
        }).map_err(|_| io::Error::new(io::ErrorKind::WouldBlock, "receipt recovery read capacity exhausted"))?;
        Ok(Self(Arc::clone(ledger)))
    }
}
impl Drop for ReadPermit {
    fn drop(&mut self) { self.0.recovery_reads.fetch_sub(1, Ordering::Release); }
}
...
fn check_stop(stopped: &AtomicBool) -> io::Result<()> {
    if stopped.load(Ordering::Acquire) { Err(io::Error::from(io::ErrorKind::Interrupted)) }
    else { Ok(()) }
}
...
impl Ledger {
    ...
    pub async fn recover_committed(
        self: &Arc<Self>, key: ResumeSessionKey, directory: PathBuf, maximum_bytes: u64,
        ...
        let result = spawn_blocking_io(move || {
            ...
            let _permit = permit;
            _permit.0.recover_sync(key, &directory, maximum_bytes, &stopped)
        }).await;
        drop(stop);
        result
    }
    ...
    fn recover_sync(
        &self, key: ResumeSessionKey, directory: &Path, maximum_bytes: u64, stopped: &AtomicBool,
    ) -> io::Result<Option<LiveStreamReceipt>> {
        check_stop(stopped)?;
        let Some(entry) = self.checked_entry(key)? else { return Ok(None); };
        let Some(receipt) = &entry.receipt else {
            return Err(io::Error::new(io::ErrorKind::AlreadyExists, AlreadyClaimed));
        };
        if receipt.prefix.bytes > maximum_bytes { return Err(corrupt()); }
...
fn verify_file(directory: &Path, entry: &Entry, stopped: &AtomicBool) -> io::Result<()> {
    ...
    if !private_directory(&named_parent) { return Err(corrupt()); }
    ...
    if before.len() != receipt.prefix.bytes { return Err(corrupt()); }
    ...
    while remaining != 0 {
        ...
        if count == 0 { return Err(corrupt()); }
        hash.update(&buffer[..count]);
        remaining -= count as u64;
    }
    check_stop(stopped)?;
    let actual: [u8; 32] = hash.finalize().into();
    if file.read(&mut buffer[..1])? != 0 || actual != receipt.source_sha256 { return Err(corrupt()); }
    let after = file.metadata()?;
    let named = std::fs::symlink_metadata(&path)?;
    let current_parent = std::fs::symlink_metadata(directory)?;
    if !named.is_file() || !private_directory(&current_parent)
