...
fn receipt() -> LiveStreamReceipt {
    LiveStreamReceipt {
        prefix: LiveStreamPrefix { stream_nonce: [1; 32], epochs: 1, bytes: 3, chain: [2; 32] },
        source_sha256: [3; 32],
    }
}
fn report(outcome: Result<LiveStreamReceipt, ResumeError>) -> ResumeReport {
    ResumeReport { outcome, prefix: Some(receipt().prefix), attempts: 1,
        receipt_reused: false, retained_epoch_bytes: 0, sink_written_bytes: 0, completed: None }
}

#[test]
fn checkpoint_commands_require_intent_path_and_explicit_attempt_permission() {
    let upload = ["atpd-live", "send-checkpointed", "--config", "sender.json",
        "--input", "input.bin", "--checkpoint", "/private/intent", "--attempts", "3"];
    assert!(matches!(Cli::try_parse_from(upload).unwrap().command, Command::SendCheckpointed { .. }));
    assert!(Cli::try_parse_from(&upload[..upload.len() - 2]).is_err());
    assert!(Cli::try_parse_from(["atpd-live", "send-checkpointed", "--config", "sender.json",
        "--input", "input.bin", "--attempts", "1"]).is_err());
    let recovery = ["atpd-live", "recover-proof", "--config", "sender.json",
        "--checkpoint", "/private/intent", "--attempts", "3"];
    assert!(matches!(Cli::try_parse_from(recovery).unwrap().command, Command::RecoverProof { .. }));
    assert!(Cli::try_parse_from(recovery.into_iter().chain(["--input", "must-not-be-read"])).is_err());
}

#[test]
fn legacy_commands_do_not_silently_enable_checkpointing() {
    assert!(matches!(Cli::try_parse_from(["atpd-live", "send-resumable", "--config", "s.json",
        "--input", "i.bin", "--attempts", "1"]).unwrap().command, Command::SendResumable { .. }));
    assert!(matches!(Cli::try_parse_from(["atpd-live", "send", "--config", "s.json",
        "--input", "i.bin"]).unwrap().command, Command::Send { .. }));
}

#[test]
fn attempt_and_delay_bounds_are_finite_and_independent() {
    for (attempts, retry_delay_ms) in [(1, 1), (1024, 60_000)] {
        assert!(Options { attempts, retry_delay_ms }.validate().is_ok());
    }
    for (attempts, retry_delay_ms) in [(0, 1), (1025, 1), (1, 0), (1, 60_001), (u32::MAX, u64::MAX)] {
        assert!(Options { attempts, retry_delay_ms }.validate().is_err());
    }
}
...
fn successful_storage_after_timeout_is_not_a_delivery_receipt_or_retry_permission() {
    let failed = ResumeError::Checkpoint(Box::new(FinalProofPersistError {
        stored: true, interruption: Some(Box::new(LiveStreamError::Timeout("sender final checkpoint"))),
        source: None,
    }));
...
fn simultaneous_storage_error_and_cancellation_remain_separate_without_private_text() {
    let error = ResumeError::Checkpoint(Box::new(FinalProofPersistError {
        stored: false,
        interruption: Some(Box::new(LiveStreamError::Cancelled(Some(CancelReason::user("private reason"))))),
...
fn only_transport_failures_are_retryable_and_only_proof_is_success() {
    ...
    for error in [ResumeError::LocalFailure, ResumeError::AttemptsExhausted,
        ResumeError::PeerIdentity, ResumeError::Continuity("peer changed state"),
        ResumeError::Transfer(LiveStreamError::Tls(TlsError::Handshake("private certificate".into())))]
    { assert!(!retryable(&error)); }
    let exact = receipt();
