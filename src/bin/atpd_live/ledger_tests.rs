...
fn key(client: u8, nonce: u8) -> ResumeSessionKey {
    ResumeSessionKey {
        client: NativeClientCertificateId::from_sha256([client; 32]),
        nonce: [nonce; 32],
    }
}
fn name(byte: u8) -> String {
    format!("{}.bin", hex(&[byte; 16]))
}
...
fn ledger_recovers_claims_and_actual_receipts_without_recreating_old_keys() {
    let path = fixture();
    Ledger::initialize(&path, 2).unwrap();
    let ledger = Ledger::open(&path).unwrap();
    let first = key(1, 7);
    let other_client = key(2, 7);
    ...
    for old in [first, other_client] {
        assert!(is_replay_refusal(
            &reopened.claim_sync(old, name(3), 10).unwrap_err()
        ));
    }
    assert_eq!(
        reopened
            .claim_sync(key(1, 8), name(4), 10)
            .unwrap_err()
            .kind(),
...
fn ledger_initialization_never_overwrites_and_open_never_creates() {
    let path = fixture();
    assert_eq!(
        Ledger::open(&path).unwrap_err().kind(),
        io::ErrorKind::NotFound
    );
    assert!(!path.exists());
    for limit in [0, MAX_KEYS + 1] {
        assert!(Ledger::initialize(&path, limit).is_err());
    }
    assert!(!path.exists());
...
fn ledger_torn_tails_and_corruption_are_rejected_without_repair() {
    ...
    let ledger = Ledger::open(&path).unwrap();
    ledger.claim_sync(key(1, 1), name(1), 10).unwrap();
    drop(ledger);
    let bytes = std::fs::read(&path).unwrap();
    for length in [0, 1, HEADER_BYTES - 1, HEADER_BYTES + 1, bytes.len() - 1] {
        let bad = fixture();
        write_new(&bad, &bytes[..length]);
...
fn ledger_rejects_semantically_invalid_records_even_with_valid_checksums() {
    ...
    for case in 0..4 {
        ...
        for seq in 0..2 {
            ...
            if kind == 2 {
                item.receipt = Some(receipt(item.key));
            }
            if seq == 1 {
                match case {
...
fn ledger_commit_validates_nonce_size_and_idempotent_receipt() {
    ...
    let mut wrong = receipt(key);
    wrong.prefix.stream_nonce = [9; 32];
    assert!(ledger.commit_sync(key, wrong).is_err());
    let mut wrong = receipt(key);
    wrong.prefix.bytes = 4;
    assert!(ledger.commit_sync(key, wrong).is_err());
