...
impl Options {
    fn validate(self) -> io::Result<()> {
        ...
        {
            return Err(invalid("checkpoint commands require 1..=1024 attempts and 1..=60000 ms retry delay"));
        }
        Ok(())
    }
}
...
impl Control {
    ...
    async fn join<T>(
        ...
        loop {
            ...
            if let Ok(result) = asupersync::time::timeout(
                cx.now(), CONTROL_TICK, poll_fn(|ctx| task.poll_join(ctx)),
            ).await {
                return result;
            }
        }
    ...
    async fn delay(&mut self, cx: &Cx, milliseconds: u64) {
        ...
        loop {
            ...
            asupersync::time::sleep(cx.now(), CONTROL_TICK.min(Duration::from_nanos(remaining))).await;
        }
    }
}
...
fn authority(config: &SendConfig) -> io::Result<LiveStreamSender> {
    let profile = settings::profile(
        config.schema_version, config.workers, config.epoch_bytes,
    ...
    settings::sdk(1, &profile)?.live_stream_sender(
        profile, name, settings::roots(&config.server_ca)?, settings::identity(&config.identity)?,
    ).map_err(|_| invalid("invalid authenticated checkpoint sender configuration"))
}

pub(super) fn send(
    config: SendConfig, input: PathBuf, checkpoint: PathBuf, options: Options,
) -> io::Result<()> {
    options.validate()?;
    ...
    execute(config, options, sender, Start::Upload(source), journal, signals)
}

pub(super) fn recover(
    config: SendConfig, checkpoint: PathBuf, options: Options,
) -> io::Result<()> {
    options.validate()?;
    let sender = authority(&config)?;
    let signals = Signals::new([SIGINT, SIGTERM])?;
    ...
    execute(config, options, sender, Start::Recover(Box::new(saved)), journal, signals)
}

fn execute(
    config: SendConfig, options: Options, sender: LiveStreamSender,
    start: Start, journal: FinalProofFile, signals: Signals,
) -> io::Result<()> {
    let source_free = matches!(&start, Start::Recover(_));
    let result = runtime(config.workers, async move {
        let cx = Cx::current().ok_or_else(|| io::Error::other("missing checkpoint sender context"))?;
        ...
        drive(&cx, options, Work { transfer, journal }, signals, source_free).await
    })?;
    ...
    emit(json!({"schema_version": 1, "event": "send_result", "source_free": source_free,
    ...
    else { Err(io::Error::other("checkpoint sender did not receive exact final peer Proof; reconcile saved intent")) }
}
...
async fn drive(
    cx: &Cx, options: Options, mut work: Work, signals: Signals, source_free: bool,
    ...
    loop {
        ...
        if control.reason.is_some() {
            return Ok(ResultRecord { transfer: last, persisted: Some(work.journal.is_persisted()),
                received: false, stopping: true });
        }
        let entered = Arc::new(AtomicBool::new(false));
        let worker_entered = Arc::clone(&entered);
        let task = cx.spawn_in(&scope, move |child| {
            let future: Pin<Box<dyn Future<Output = (Work, ResumeReport)> + Send>> = Box::pin(async move {
        ...
        let mut task = match task {
            ...
            Err(_) => return Ok(ResultRecord {
                ...
                persisted: None, received: false, stopping: control.reason.is_some(),
            }),
        };
        ...
        emit(json!({"schema_version": 1, "event": "checkpoint_attempt", "source_free": source_free,
        ...
        if received || !retryable || report.attempts >= options.attempts || control.reason.is_some() {
            return Ok(ResultRecord { transfer: last, persisted: Some(work.journal.is_persisted()),
                received, stopping: control.reason.is_some() });
        }
...
fn retryable(error: &ResumeError) -> bool {
    match error {
        ...
        | ResumeError::Transfer(LiveStreamError::Tls(TlsError::Io(error))) => matches!(
            error.kind(), io::ErrorKind::ConnectionRefused | io::ErrorKind::ConnectionReset
