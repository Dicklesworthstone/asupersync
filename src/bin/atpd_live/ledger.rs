...
impl fmt::Debug for Ledger {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.debug_struct("SessionLedger")
            .field("keys", &state.entries.len())
            .field("maximum_keys", &self.maximum_keys)
...
pub(super) fn is_replay_refusal(error: &io::Error) -> bool {
    error
        .get_ref()
        .is_some_and(|source| source.is::<AlreadyClaimed>())
}
...
fn corrupt() -> io::Error {
    io::Error::new(io::ErrorKind::InvalidData, "invalid or torn session ledger")
}
fn unavailable() -> io::Error {
    io::Error::other("session ledger persistence is unconfirmed; restart and reconcile")
}
...
fn parent(path: &Path) -> io::Result<(PathBuf, File)> {
    if !path.is_absolute() {
        return Err(invalid("session ledger requires an absolute path"));
    }
    let name = path
        .file_name()
        .ok_or_else(|| invalid("session ledger needs a filename"))?;
    let parent = path
        .parent()
        .ok_or_else(|| invalid("session ledger needs a parent"))?;
...
impl Ledger {
    ...
    pub fn initialize(path: &Path, maximum_keys: u32) -> io::Result<()> {
        if !(1..=MAX_KEYS).contains(&maximum_keys) {
            return Err(invalid("ledger key limit must be 1..=65536"));
        }
    ...
    pub fn open(path: &Path) -> io::Result<Arc<Self>> {
        ...
        let mut entries = BTreeMap::new();
        {
            ...
            for sequence in 0..records {
                ...
                if record[..8] != sequence.to_be_bytes()
                    || record[9..16] != [0; 7]
        ...
        Ok(Arc::new(Self {
            ...
            directory,
            maximum_keys,
            recovery_reads: std::sync::atomic::AtomicUsize::new(0),
    ...
    fn claim_sync(
        ...
        if !valid_filename(filename.as_bytes()) {
            return Err(invalid("invalid ledger publication filename"));
        }
        ...
        if state.entries.len() >= self.maximum_keys as usize {
    ...
    fn append(&self, state: &mut State, kind: u8, entry: Entry) -> io::Result<()> {
        if state.records >= u64::from(self.maximum_keys) * 2 {
            return Err(corrupt());
        }
...
fn validate_receipt(entry: &Entry, receipt: &LiveStreamReceipt) -> io::Result<()> {
    ...
    if prefix.stream_nonce != entry.key.nonce
        || prefix.bytes > entry.maximum_bytes
...
fn decode(record: &[u8; RECORD_BYTES]) -> io::Result<(u8, Entry)> {
    if !valid_filename(&record[80..116]) {
        return Err(corrupt());
    }
    let key = ResumeSessionKey {
        client: NativeClientCertificateId::from_sha256(
            record[16..48].try_into().expect("fixed client"),
        ),
    ...
    Ok((
        ...
        Entry {
            key,
            filename: String::from_utf8(record[80..116].to_vec()).map_err(|_| corrupt())?,
            maximum_bytes: count,
            receipt,
        },
    ))
...
fn apply(
    entries: &mut BTreeMap<ResumeSessionKey, Entry>,
    maximum: u32,
    kind: u8,
    entry: Entry,
) -> io::Result<()> {
    if kind == 1 {
        if entries.contains_key(&entry.key) || entries.len() >= maximum as usize {
            return Err(corrupt());
        }
        entries.insert(entry.key, entry);
    } else {
        let earlier = entries.get_mut(&entry.key).ok_or_else(corrupt)?;
        if earlier.receipt.is_some() || earlier.filename != entry.filename {
            return Err(corrupt());
        }
...
pub(super) fn inspect(path: &Path) -> io::Result<()> {
    ...
    super::emit(
        serde_json::json!({"schema_version": 1, "event": "session_ledger",
        "keys": entries.len(), "maximum_keys": ledger.maximum_keys(), "continuation_restored": false}),
    )?;
    for entry in entries {
        super::emit(
            serde_json::json!({"schema_version": 1, "event": "session_record",
