...
use crate::distributed::snapshot::{
    RegionSnapshot, TaskSnapshot, TaskState, BudgetSnapshot, SnapshotError
};
...
impl DistributedTestHarness {
    ...
    fn test_consistent_hash_rebalancing(&self) {
        ...
        self.logger.log_event("ring_initialized", serde_json::json!({
        ...
        self.logger.log_event("initial_distribution", serde_json::json!(initial_distribution));
        ...
        self.logger.log_event("scale_up_distribution", serde_json::json!(scale_up_distribution));

        // Validate rebalancing behavior
        let movement_percentage = self.calculate_key_movement(&initial_distribution, &scale_up_distribution);

        self.logger.log_assertion("reasonable_rebalancing", movement_percentage < 0.6, serde_json::json!({
            "movement_percentage": movement_percentage,
            "threshold": 0.6
        }));

        assert!(movement_percentage < 0.6,
            "Key movement should be reasonable during scale up: {:.2}% > 60%", movement_percentage * 100.0);

        // Phase 3: Remove nodes (scale down)
        self.logger.log_phase("scale_down");
        let removed_count = ring.remove_node("node_2");

        self.logger.log_event("node_removed", serde_json::json!({
            "node": "node_2",
            "vnodes_removed": removed_count
        }));

        assert_eq!(removed_count, vnodes_per_node, "Should remove all virtual nodes for node_2");

        let scale_down_distribution = self.measure_key_distribution(&ring, &test_keys);
        self.logger.log_event("scale_down_distribution", serde_json::json!(scale_down_distribution));
        ...
        assert_eq!(removed, vnodes_per_node, "Failed node should be removed completely");
        ...
        self.logger.log_event("failure_recovery_complete", serde_json::json!({
    ...
    fn test_snapshot_serialization_roundtrip(&self) {
        ...
        // Add child regions
        for i in 0..5 {
            snapshot.children.push(RegionId::new(ArenaIndex::new(100 + i), 1));
        }
        ...
        self.logger.log_event("snapshot_created", serde_json::json!({
...

        {
            ...
            file.write_all(&serialized).expect("Failed to write snapshot data");
            file.sync_all().expect("Failed to sync file to disk");
        }

        let file_size = std::fs::metadata(&snapshot_path).expect("Failed to get file metadata").len();

        self.logger.log_event("file_written", serde_json::json!({
...

        {
            ...
            file.read_to_end(&mut read_data).expect("Failed to read snapshot data");
        }

        self.logger.log_assertion("file_roundtrip", read_data == serialized, serde_json::json!({
            "original_size": serialized.len(),
            "read_size": read_data.len()
        }));

        assert_eq!(read_data, serialized, "File roundtrip should preserve data exactly");

        // Phase 4: Deserialize from bytes
        self.logger.log_phase("deserialization");
        let deserialized = RegionSnapshot::from_bytes(&read_data)
    ...
    fn test_concurrent_snapshot_operations(&self) {
        ...
        let snapshots: Vec<RegionSnapshot> = (0..5).map(|i| {
        ...
        self.logger.log_event("concurrent_snapshots_created", serde_json::json!({
            "count": snapshots.len(),
        ...
        let file_paths: Vec<PathBuf> = snapshots.iter().enumerate().map(|(i, snapshot)| {
            let serialized = snapshot.to_bytes();
            let path = self.temp_dir.path().join(format!("concurrent_{}.bin", i));

            let mut file = File::create(&path).expect("Failed to create concurrent snapshot file");
            file.write_all(&serialized).expect("Failed to write concurrent snapshot");
        ...
        let restored_snapshots: Vec<RegionSnapshot> = file_paths.iter().map(|path| {
            let mut data = Vec::new();
            let mut file = File::open(path).expect("Failed to open concurrent snapshot");
            file.read_to_end(&mut data).expect("Failed to read concurrent snapshot");

            RegionSnapshot::from_bytes(&data).expect("Failed to deserialize concurrent snapshot")
        }).collect();
        ...
        for (i, (original, restored)) in snapshots.iter().zip(restored_snapshots.iter()).enumerate() {
            ...
            self.logger.log_event("concurrent_validation_success", serde_json::json!({
                "snapshot_index": i,
        ...
        self.logger.log_assertion("concurrent_integrity", true, serde_json::json!({
    ...
    fn test_snapshot_error_handling(&self) {
        ...
        for (test_name, invalid_data) in error_test_cases {
            ...
            self.logger.log_assertion("expected_error", result.is_err(), serde_json::json!({
                "test_case": test_name,
                "data_length": invalid_data.len(),
                "error_occurred": result.is_err()
            }));

            assert!(result.is_err(), "Invalid data should cause deserialization error: {}", test_name);

            if let Err(error) = result {
                self.logger.log_event("error_details", serde_json::json!({
                    "test_case": test_name,
    ...
    fn calculate_key_movement(&self, before: &HashMap<String, usize>, after: &HashMap<String, usize>) -> f64 {
    ...
    fn validate_distribution_quality(&self, distribution: &HashMap<String, usize>) {
        ...
        self.logger.log_assertion("distribution_balance", max_deviation < 0.3, serde_json::json!({
        ...
        assert!(max_deviation < 0.3,
            "Distribution should be reasonably balanced: max deviation {:.2}% > 30%", max_deviation * 100.0);
    }

    fn validate_snapshot_roundtrip(&self, original: &RegionSnapshot, restored: &RegionSnapshot) {
        // Validate basic fields
        assert_eq!(original.region_id, restored.region_id, "Region ID should match");
        assert_eq!(original.state, restored.state, "Region state should match");
        assert_eq!(original.timestamp, restored.timestamp, "Timestamp should match");
        assert_eq!(original.sequence, restored.sequence, "Sequence should match");
        assert_eq!(original.origin_id, restored.origin_id, "Origin ID should match");
        assert_eq!(original.epoch, restored.epoch, "Epoch should match");

        // Validate tasks
        assert_eq!(original.tasks.len(), restored.tasks.len(), "Task count should match");
        for (orig_task, rest_task) in original.tasks.iter().zip(restored.tasks.iter()) {
            ...
            assert_eq!(orig_task.priority, rest_task.priority, "Task priority should match");
        }

        // Validate children
        assert_eq!(original.children, restored.children, "Children should match");

        // Validate other fields
        assert_eq!(original.finalizer_count, restored.finalizer_count, "Finalizer count should match");
        assert_eq!(original.cancel_reason, restored.cancel_reason, "Cancel reason should match");
        assert_eq!(original.parent, restored.parent, "Parent should match");
        assert_eq!(original.metadata, restored.metadata, "Metadata should match");

        // Validate budget
        let orig_budget = &original.budget;
        let rest_budget = &restored.budget;
        assert_eq!(orig_budget.deadline_nanos, rest_budget.deadline_nanos, "Budget deadline should match");
        assert_eq!(orig_budget.polls_remaining, rest_budget.polls_remaining, "Budget polls should match");
        assert_eq!(orig_budget.cost_remaining, rest_budget.cost_remaining, "Budget cost should match");
    }
}
...
fn test_distributed_coordination_full_cycle_e2e() {
    ...
    let snapshots: Vec<RegionSnapshot> = nodes.iter().enumerate().map(|(i, &node)| {
    ...
    harness.logger.log_event("combined_initial_state", serde_json::json!({
    ...
    let serialized_snapshots: Vec<Vec<u8>> = snapshots.iter()
        .map(|s| s.to_bytes())
        .collect();

    // Write snapshots to files (simulate network transfer/storage)
    let snapshot_files: Vec<PathBuf> = serialized_snapshots.iter().enumerate()
        .map(|(i, data)| {
            ...
            file.write_all(data).expect("Failed to write migration data");
    ...
    harness.logger.log_event("rebalancing_complete", serde_json::json!({
    ...
    let recovered_snapshots: Vec<RegionSnapshot> = snapshot_files.iter().map(|path| {
        let mut data = Vec::new();
        let mut file = File::open(path).expect("Failed to open migration file");
        file.read_to_end(&mut data).expect("Failed to read migration data");
    ...
    harness.logger.log_assertion("full_cycle_success", true, serde_json::json!({
