...
mod integration_tests {
    use crate::channel::oneshot::{self, Sender, Receiver, RecvError};
    ...
    impl TestInitializer {
        fn new(id: String, once_cell: Arc<OnceCell<TestData>>, oneshot_sender: Sender<TestData>,
               initialization_delay: Duration, stats: Arc<Mutex<OneshotOnceCellStats>>) -> Self {
            Self {
                id,
                once_cell,
        ...
        async fn initialize_via_oneshot(&mut self, cx: &Cx, data: TestData) -> Result<(), AsupersyncError> {
    ...
    impl OneshotOnceCellTestHarness {
        ...
        fn create_once_cell(&mut self, cell_id: &str) -> Arc<OnceCell<TestData>> {
            ...
            self.once_cells.insert(cell_id.to_string(), once_cell.clone());
        ...
        fn create_oneshot_channel(&mut self, channel_id: &str) -> (Sender<TestData>, Receiver<TestData>) {
            let (sender, receiver) = oneshot::channel();
            self.oneshot_channels.insert(channel_id.to_string(), (sender, receiver));
        ...
        fn create_initializer(&mut self, init_id: &str, once_cell: Arc<OnceCell<TestData>>,
                            oneshot_sender: Sender<TestData>, delay: Duration) -> Arc<TestInitializer> {
            ...
            self.initializers.insert(init_id.to_string(), initializer.clone());
            initializer
        }

        async fn wait_for_initialization(&self, cx: &Cx, once_cell: &OnceCell<TestData>,
                                       oneshot_receiver: Receiver<TestData>) -> Result<TestData, AsupersyncError> {
            ...
            match oneshot_result {
                Ok(data) => {
                    ...
                    match once_cell.get() {
                        ...
                        Some(_) => Err(AsupersyncError::InvalidState("OnceCell data mismatch".into())),
                        None => Err(AsupersyncError::InvalidState("OnceCell not initialized after oneshot".into())),
                    }
                }
        ...
        async fn spawn_concurrent_readers(&self, cx: &Cx, once_cell: Arc<OnceCell<TestData>>,
                                        num_readers: usize) -> Result<Vec<TestData>, AsupersyncError> {
            let readers_count = Arc::new(AtomicU64::new(0));
            let mut tasks = Vec::new();

            {
                let mut stats = self.stats.lock().unwrap();
                stats.concurrent_readers_peak = stats.concurrent_readers_peak.max(num_readers as u64);
            }
    ...
    async fn test_basic_oneshot_once_cell_initialization() -> Result<(), AsupersyncError> {
        ...
        runtime.region(Budget::default(), |cx| async move {
            ...
            let init_task = {
                let mut init_clone = Arc::try_unwrap(initializer).unwrap_or_else(|arc| (*arc).clone());
                cx.spawn(async move {
            ...
            let result = harness.wait_for_initialization(cx, &once_cell, receiver).await?;

            // Verify initialization completed
            assert_eq!(result.id, 1);
            assert_eq!(result.payload, "basic test data");
            assert_eq!(result.initialization_source, InitializationSource::OneshotCompletion);
            ...
            println!("Basic initialization completed in {:?}", stats.total_wait_time);
            Ok(())
        }).await
    }
    ...
    async fn test_multiple_readers_waiting_on_once_cell() -> Result<(), AsupersyncError> {
        ...
        runtime.region(Budget::default(), |cx| async move {
            ...
            let readers_task = {
                let once_cell_clone = once_cell.clone();
                cx.spawn(async move {
                    harness.spawn_concurrent_readers(cx, once_cell_clone, num_readers).await
                })
            };
            ...
            let init_task = {
                ...
                cx.spawn(async move {
                    init_clone.initialize_via_oneshot(cx, data_clone).await
                })
            };
    ...
    async fn test_race_condition_handling() -> Result<(), AsupersyncError> {
        ...
        runtime.region(Budget::default(), |cx| async move {
            ...
            let init1_task = {
                let mut init1_clone = Arc::try_unwrap(initializer1).unwrap_or_else(|arc| (*arc).clone());
                let data1_clone = data1.clone();
                cx.spawn(async move {
                    init1_clone.initialize_via_oneshot(cx, data1_clone).await
                })
            };

            let init2_task = {
                let mut init2_clone = Arc::try_unwrap(initializer2).unwrap_or_else(|arc| (*arc).clone());
                let data2_clone = data2.clone();
                cx.spawn(async move {
                    init2_clone.initialize_via_oneshot(cx, data2_clone).await
                })
            };
            ...
            assert!(final_value.id == 3 || final_value.id == 4, "Should have data from one of the racers");
            ...
            let successful_receives = [recv1_result.is_ok(), recv2_result.is_ok()].iter().filter(|&&x| x).count();
    ...
    async fn test_oneshot_dropped_before_completion() -> Result<(), AsupersyncError> {
        ...
        runtime.region(Budget::default(), |cx| async move {
            ...
            assert!(once_cell.get().is_none(), "OnceCell should remain uninitialized");
    ...
    async fn test_immediate_vs_delayed_initialization() -> Result<(), AsupersyncError> {
        ...
        runtime.region(Budget::default(), |cx| async move {
            ...
            immediate_cell.set(immediate_data.clone()).expect("Should initialize immediately");

            // Create oneshot for consistency, but it should not be needed
            let (immediate_sender, immediate_receiver) = harness.create_oneshot_channel("immediate-channel");
            drop(immediate_sender); // Drop since we don't need it

            // This should return immediately
            let immediate_result = harness.wait_for_initialization(cx, &immediate_cell, immediate_receiver).await;
            ...
            let (delayed_sender, delayed_receiver) = harness.create_oneshot_channel("delayed-channel");
            ...
            let init_task = {
                let mut init_clone = Arc::try_unwrap(delayed_initializer).unwrap_or_else(|arc| (*arc).clone());
            ...
            let wait_start = Instant::now();
            let delayed_result = harness.wait_for_initialization(cx, &delayed_cell, delayed_receiver).await?;
            let wait_duration = wait_start.elapsed();

            assert_eq!(delayed_result, delayed_data);
            assert!(wait_duration >= Duration::from_millis(90), "Should have waited for delay");
            ...
            println!("Immediate vs delayed: immediate={}, wait_time={:?}",
    ...
    async fn test_stress_many_once_cells_and_oneshots() -> Result<(), AsupersyncError> {
        ...
        runtime.region(Budget::default(), |cx| async move {
            ...
            for i in 0..num_cells {
                ...
                let (sender, receiver) = harness.create_oneshot_channel(&format!("stress-channel-{}", i));
                ...
                let task = {
                    ...
                    cx.spawn(async move {
                        ...
                        let init_result = init_clone.initialize_via_oneshot(cx, data_clone.clone()).await;
            ...
            assert!(successful_count >= num_cells * 80 / 100, "At least 80% should succeed");
            assert_eq!(stats.once_cells_created, num_cells as u64);
            assert_eq!(stats.oneshot_channels_created, num_cells as u64);

            println!("Stress test: {}/{} successful in {:?}",
                     successful_count, num_cells, total_duration);
            println!("Stats: successes={}, failures={}, races={}",
                     stats.successful_initializations, stats.failed_initializations, stats.race_conditions_handled);

            // Performance assertion
            assert!(total_duration < Duration::from_secs(3), "Stress test should complete within reasonable time");
    ...
    async fn test_concurrent_initialization_coordination() -> Result<(), AsupersyncError> {
        ...
        runtime.region(Budget::default(), |cx| async move {
            ...
            let (coord_sender, coord_receiver) = harness.create_oneshot_channel("coordination-channel");
            ...
            for i in 0..num_dependents {
                ...
                let (dep_sender, dep_receiver) = harness.create_oneshot_channel(&format!("dependent-channel-{}", i));
            ...
            coordination_cell.set(coord_data.clone()).expect("Should initialize coordination cell");
            coord_sender.send(coord_data.clone()).expect("Should send coordination signal");
            ...
            let stats = harness.get_stats();
            println!("Coordination test: {} dependents successfully coordinated", successful_dependents);
            println!("Total initializations: {}", stats.successful_initializations);
