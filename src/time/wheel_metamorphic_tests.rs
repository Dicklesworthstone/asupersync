...
fn arb_wheel_config() -> impl Strategy<Value = TestWheelConfig> {
    (1u64..25, 25u64..168, any::<bool>(), 1u64..50, 1usize..10)
        .prop_map(|(max_wheel_hrs, max_timer_hrs, coalesce, window_ms, group_size)| {
...
fn arb_test_timer() -> impl Strategy<Value = TestTimer> {
    (any::<u32>(), 1u64..10000, any::<u8>())
        .prop_map(|(id, offset_ms, priority)| TestTimer {
            id,
...
fn arb_wheel_operation() -> impl Strategy<Value = WheelOperation> {
    prop_oneof![
        ...
        (1u64..1000).prop_map(|advance| WheelOperation::AdvanceTime { advance_ms: advance }),
...
fn apply_operation(
    ...
    timer_handle_map: &mut HashMap<u32, TimerId>,
) {
    match operation {
        WheelOperation::InsertTimer { timer } => {
            ...
            if let Ok(handle) = wheel.insert(deadline, waker) {
        ...
        WheelOperation::AdvanceTime { advance_ms } => {
            ...
            let ready_wakers = wheel.advance_to(new_time);
...
fn mr_time_monotonicity() {
    proptest!(|(config: TestWheelConfig, operations: Vec<WheelOperation>)| {
        let wheel_config = config.to_wheel_config();
        let mut wheel = TimerWheel::with_config(Time::ZERO, wheel_config);
...
fn mr_batch_size_scaling() {
    proptest!(|(base_timer_count: usize, advance_ms: u64)| {
        ...
        let wheel_config = config.to_wheel_config();
        let mut wheel1 = TimerWheel::with_config(Time::ZERO, wheel_config.clone());
        let mut wheel2 = TimerWheel::with_config(Time::ZERO, wheel_config);
        ...
        for i in 0..base_timer_count {
            ...
            let _ = wheel1.insert(deadline, dummy_waker());
        }
        ...
        for i in 0..(base_timer_count * 2) {
            ...
            let _ = wheel2.insert(deadline, dummy_waker());
        }

        // Advance both wheels to fire all timers
        let target_time = Time::from_millis(advance_ms);
        let ready1 = wheel1.advance_to(target_time);
        let ready2 = wheel2.advance_to(target_time);
...
fn mr_cancellation_idempotence() {
    proptest!(|(timer_offset_ms: u64)| {
        ...
        let handle1 = wheel1.insert(deadline, dummy_waker()).unwrap();
        let handle2 = wheel2.insert(deadline, dummy_waker()).unwrap();
        ...
        let ready1 = wheel1.advance_to(deadline + Duration::from_millis(100));
        let ready2 = wheel2.advance_to(deadline + Duration::from_millis(100));
...
fn mr_insertion_order_independence() {
    proptest!(|(timer_count: usize, deadline_offset_ms: u64)| {
        ...
        for i in 0..timer_count {
            let _ = wheel1.insert(shared_deadline, dummy_waker());
        }

        // Insert timers in reverse order (..., 2, 1, 0)
        for i in (0..timer_count).rev() {
            let _ = wheel2.insert(shared_deadline, dummy_waker());
        }
...
fn mr_coalescing_window_grouping() {
    proptest!(|(timer_count: usize, spread_ms: u64)| {
        ...
        let mut wheel1 = TimerWheel::with_config_and_coalescing(Time::ZERO, wheel_config1, coalesce_config1);

        let wheel_config2 = config_coalesce.to_wheel_config();
        let coalesce_config2 = config_coalesce.to_coalescing_config();
        let mut wheel2 = TimerWheel::with_config_and_coalescing(Time::ZERO, wheel_config2, coalesce_config2);
...
fn mr_insert_cancel_round_trip() {
    proptest!(|(timer_offset_ms: u64)| {
        ...
        let final_ready = wheel.ready_count();
