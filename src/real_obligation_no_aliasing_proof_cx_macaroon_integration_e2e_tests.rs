...
mod integration_tests {
    use crate::cx::{
        Cx,
        macaroon::{AttenuationError, Caveat, Macaroon, MacaroonBuilder},
    };
    use crate::error::AsupersyncError;
    use crate::obligation::no_aliasing_proof::{
        AliasingViolation, CapabilityRef, NoAliasingProof, ProofContext,
    };
    ...
    impl NoAliasingMacaroonTestHarness {
        ...
        fn create_root_macaroon(
            &mut self,
            cx: &Cx,
            identifier: &str,
        ) -> Result<Macaroon, AsupersyncError> {
            ...
            self.macaroon_registry
                .insert(identifier.to_string(), macaroon.clone());
            {
        ...
        fn attenuate_macaroon(
            ...
            base_id: &str,
            caveats: Vec<Caveat>,
        ) -> Result<(String, Macaroon), AsupersyncError> {
            let base_macaroon = self
                .macaroon_registry
                .get(base_id)
            ...
            self.macaroon_registry
                .insert(attenuated_id.clone(), attenuated.clone());
            self.attenuation_chains
                .insert(attenuated_id.clone(), caveats);
        ...
        fn create_no_aliasing_proof(
            &mut self,
            cx: &Cx,
            task_id: TaskId,
            capability_refs: Vec<CapabilityRef>,
        ) -> Result<NoAliasingProof, AsupersyncError> {
        ...
        fn validate_capability_pass(
            ...
            let macaroon = self.macaroon_registry.get(macaroon_id).ok_or_else(|| {
                AsupersyncError::InvalidState("Macaroon not found for validation".into())
            })?;
            ...
            let no_aliasing_preserved = match (from_context, to_context) {
                (Some(from_ctx), Some(to_ctx)) => {
                    ...
                    let combined_refs: HashSet<_> = from_ctx
                        .capability_refs()
                        .iter()
                    ...
                    let has_potential_aliasing = combined_refs.len()
                        < (from_ctx.capability_refs().len() + to_ctx.capability_refs().len());
                    ...
                    if has_potential_aliasing {
                        ...
                        let maintains_separation =
                            self.validate_attenuation_separation(macaroon_id)?;
        ...
        fn validate_attenuation_separation(
            &self,
            macaroon_id: &str,
        ) -> Result<bool, AsupersyncError> {
    ...
    impl Caveat for TestCaveat {
        fn verify(
            &self,
            _context: &crate::cx::macaroon::VerificationContext,
        ) -> Result<bool, AttenuationError> {
    ...
    async fn test_basic_macaroon_attenuation_preserves_no_aliasing() -> Result<(), AsupersyncError>
    {
        ...
        runtime
            .region(Budget::default(), |cx| async move {
                ...
                let caveats = vec![Box::new(TestCaveat {
                    scope_restriction: "limited-scope".to_string(),
                    time_bound: Some(Duration::from_secs(60)),
                    resource_limit: Some(1024),
                }) as Box<dyn Caveat>];

                let (attenuated_id, _attenuated_macaroon) =
                    harness.attenuate_macaroon(cx, "test-root", caveats)?;
                ...
                let preserved =
                    harness.validate_capability_pass(cx, task_id, target_task, &attenuated_id)?;
                assert!(
                    preserved,
                    "Macaroon attenuation should preserve no-aliasing invariant"
                );
    ...
    #[tokio::test]
    async fn test_cross_scope_capability_passing_maintains_separation()
    -> Result<(), AsupersyncError> {
        ...
        runtime
            .region(Budget::default(), |cx| async move {
                ...
                let scope_a_caveats = vec![Box::new(TestCaveat {
                    scope_restriction: "database-only".to_string(),
                    time_bound: Some(Duration::from_secs(30)),
                    resource_limit: Some(512),
                }) as Box<dyn Caveat>];
                let scope_b_caveats = vec![Box::new(TestCaveat {
                    scope_restriction: "network-only".to_string(),
                    time_bound: Some(Duration::from_secs(45)),
                    resource_limit: Some(256),
                }) as Box<dyn Caveat>];

                let (attenuated_a_id, _) =
                    harness.attenuate_macaroon(cx, "scope-a", scope_a_caveats)?;
                let (attenuated_b_id, _) =
                    harness.attenuate_macaroon(cx, "scope-b", scope_b_caveats)?;

                // Cross-scope capability passing should preserve separation
                let a_to_b_preserved =
                    harness.validate_capability_pass(cx, task_a, task_b, &attenuated_a_id)?;
                let b_to_a_preserved =
                    harness.validate_capability_pass(cx, task_b, task_a, &attenuated_b_id)?;

                assert!(
                    a_to_b_preserved,
                    "Cross-scope capability pass A->B should preserve no-aliasing"
                );
                assert!(
                    b_to_a_preserved,
                    "Cross-scope capability pass B->A should preserve no-aliasing"
                );
        ...
        runtime
            .region(Budget::default(), |cx| async move {
                ...
                for (level, caveats) in attenuation_levels.into_iter().enumerate() {
                    ...
                    let cap_refs = vec![CapabilityRef::new(
                        &format!("resource-level-{}", level),
                        "access",
                    )];
                ...
                let preserved =
                    harness.validate_capability_pass(cx, source_task, target_task, &current_id)?;
                assert!(
                    preserved,
                    "Complex attenuation chain should preserve no-aliasing invariants"
                );
    ...
    async fn test_aliasing_violation_detection() -> Result<(), AsupersyncError> {
        ...
        runtime
            .region(Budget::default(), |cx| async move {
                ...
                let preserved =
                    harness.validate_capability_pass(cx, task_a, task_b, "aliasing-test")?;
                assert!(
                    !preserved,
                    "Should detect aliasing violation with overlapping resource access"
                );

                let stats = harness.get_stats();
                assert!(
                    stats.aliasing_violations > 0,
                    "Should have recorded aliasing violations"
                );
        ...
        runtime
            .region(Budget::default(), |cx| async move {
                ...
                for i in 0..10 {
                    ...
                    let source_refs = vec![CapabilityRef::new(
                        &format!("validation-source-{}", i),
                        "read",
                    )];
                    let target_refs = vec![CapabilityRef::new(
                        &format!("validation-target-{}", i),
                        "write",
                    )];
                    ...
                    if harness.validate_capability_pass(
                        cx,
                        source_task,
                        target_task,
                        &macaroon_id,
                ...
                assert_eq!(
                    stats.attenuations_performed,
                    (num_macaroons * num_attenuations_per_macaroon) as u64
                );
                assert!(
                    preserved_count >= 8,
                    "Most capability passes should preserve no-aliasing under stress"
                );
                assert!(
        ...
        runtime
            .region(Budget::default(), |cx| async move {
                ...
                for i in 0..task_chain.len().saturating_sub(1) {
                    ...
                    let preserved = harness.validate_capability_pass(
                        cx,
                        *source_task,
                        *target_task,
                        macaroon_id,
                ...
                let circular_preserved =
                    harness.validate_capability_pass(cx, last_task, first_task, last_macaroon)?;

                assert!(
                    all_preserved,
                    "Nested delegation chain should preserve no-aliasing invariants"
                );
                assert!(
                    circular_preserved,
                    "Circular delegation with distinct capabilities should be valid"
                );
