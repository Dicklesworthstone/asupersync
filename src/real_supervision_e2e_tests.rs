...
mod real_supervision_e2e {
    use crate::supervision::{Supervisor, SupervisionStrategy, ChildSpec, RestartPolicy};
    use crate::actor::{GenServer, Actor, ActorRef, ActorSystem, Mailbox};
    use crate::runtime::{Runtime, spawn};
    use crate::cx::Cx;
    use crate::time::{sleep, Duration, timeout};
    ...
    impl SupervisionTestHarness {
        ...
        fn get_message_count(&self, actor_name: &str) -> usize {
            self.message_counts.lock().unwrap()
                .get(actor_name).copied().unwrap_or(0)
        }
    }
    ...
    impl Actor for TestWorkerActor {
        ...
        async fn handle_message(&mut self, message: Self::Message, cx: &Cx) -> Option<Self::Reply> {
            ...
            self.harness.log("actor_message", json!({
                "actor": self.name,
            ...
            match message {
                WorkerMessage::DoWork(work) => {
                    ...
                    if let Some(ref panic_trigger) = self.panic_on_message {
                        if work.contains(panic_trigger) {
                            ...
                            self.harness.log("actor_panic", json!({
                                "actor": self.name,
                ...
                WorkerMessage::SetPanicTrigger(trigger) => {
                    ...
                    self.harness.log("panic_trigger_set", json!({
                        "actor": self.name,
                ...
                WorkerMessage::Shutdown => {
                    self.harness.log("actor_shutdown", json!({
                        "actor": self.name
                    }));
    ...
    async fn test_supervision_restart_on_panic() {
        ...
        let worker_ref = supervisor_ref.add_child(child_spec).await
            .expect("Failed to add child to supervisor");

        // Set panic trigger
        worker_ref.send(WorkerMessage::SetPanicTrigger("crash".to_string())).await
            .expect("Failed to set panic trigger");

        // Send normal messages (should work)
        for i in 0..3 {
            let reply = worker_ref.send(WorkerMessage::DoWork(format!("work_{}", i))).await
                .expect("Failed to send work message");
            harness.log("work_reply", json!({
                "work_id": i,
                "reply": format!("{:?}", reply)
            }));
        }
        ...
        // Trigger panic
        let panic_result = worker_ref.send(WorkerMessage::DoWork("crash_work".to_string())).await;
        harness.log("panic_result", json!({
        ...
        let post_restart_reply = timeout(Duration::from_secs(2),
            worker_ref.send(WorkerMessage::DoWork("post_restart_work".to_string()))
        ).await;

        harness.log("post_restart_reply", json!({
            "success": post_restart_reply.is_ok(),
            "reply": format!("{:?}", post_restart_reply)
        }));

        // Get final stats
        let stats_reply = worker_ref.send(WorkerMessage::GetStats).await
            .expect("Failed to get stats");

        harness.log("final_stats", json!({
            "stats": format!("{:?}", stats_reply),
            "total_messages": harness.get_message_count("test_worker")
        }));

        // Verify restart worked
        assert!(post_restart_reply.is_ok(), "Worker should be available after restart");
        ...
        assert!(message_count >= 5, "Should have processed at least 5 messages, got {}", message_count);
    ...
    async fn test_supervision_one_for_all_restart() {
        ...
        let worker1_ref = supervisor_ref.add_child(child1_spec).await.expect("Failed to add worker1");
        let worker2_ref = supervisor_ref.add_child(child2_spec).await.expect("Failed to add worker2");
        let worker3_ref = supervisor_ref.add_child(child3_spec).await.expect("Failed to add worker3");

        // Set panic trigger on worker2 only
        worker2_ref.send(WorkerMessage::SetPanicTrigger("boom".to_string())).await
            .expect("Failed to set panic trigger on worker2");

        // Send initial work to all workers
        worker1_ref.send(WorkerMessage::DoWork("task_1".to_string())).await.expect("Worker1 failed");
        worker2_ref.send(WorkerMessage::DoWork("task_2".to_string())).await.expect("Worker2 failed");
        worker3_ref.send(WorkerMessage::DoWork("task_3".to_string())).await.expect("Worker3 failed");

        // Trigger panic in worker2 (should restart all workers due to OneForAll)
        let panic_result = worker2_ref.send(WorkerMessage::DoWork("boom_task".to_string())).await;
        harness.log("one_for_all_panic", json!({
        ...
        let worker1_post = timeout(Duration::from_secs(2),
            worker1_ref.send(WorkerMessage::DoWork("post_restart_1".to_string()))).await;
        let worker2_post = timeout(Duration::from_secs(2),
            worker2_ref.send(WorkerMessage::DoWork("post_restart_2".to_string()))).await;
        let worker3_post = timeout(Duration::from_secs(2),
            worker3_ref.send(WorkerMessage::DoWork("post_restart_3".to_string()))).await;

        harness.log("one_for_all_recovery", json!({
        ...
        assert!(worker1_post.is_ok(), "Worker1 should be available after OneForAll restart");
        assert!(worker2_post.is_ok(), "Worker2 should be available after OneForAll restart");
        assert!(worker3_post.is_ok(), "Worker3 should be available after OneForAll restart");
    ...
    async fn test_supervision_message_replay_after_restart() {
        ...
        // Set panic trigger
        worker_ref.send(WorkerMessage::SetPanicTrigger("replay_crash".to_string())).await
            .expect("Failed to set panic trigger");
        ...
        for i in 3..6 {
            let reply = worker_ref.send(WorkerMessage::DoWork(format!("post_restart_{}", i))).await;
            harness.log("post_restart_message", json!({
                "message_id": i,
                "success": reply.is_ok(),
                "reply": format!("{:?}", reply)
            }));
        ...
        harness.log("replay_final_stats", json!({
            "stats": format!("{:?}", stats_reply),
        ...
        assert!(total_messages >= 8, "Should have replayed buffered messages, got {}", total_messages);

        // Verify worker is responsive
        let final_reply = worker_ref.send(WorkerMessage::DoWork("final_test".to_string())).await;
        assert!(final_reply.is_ok(), "Worker should be responsive after message replay");
    }
    ...
    async fn test_supervision_escalation_chain() {
        ...
        let child_supervisor_spec = ChildSpec::new("child_supervisor", child_supervisor, RestartPolicy::Permanent);
        let child_ref = root_ref.add_child(child_supervisor_spec).await
        ...
        let worker_ref = child_ref.add_child(worker_spec).await
            .expect("Failed to add escalation worker");

        // Set panic trigger
        worker_ref.send(WorkerMessage::SetPanicTrigger("escalate".to_string())).await
            .expect("Failed to set panic trigger");

        // Cause multiple failures to trigger escalation
        for i in 0..4 {
            let panic_result = worker_ref.send(WorkerMessage::DoWork(format!("escalate_work_{}", i))).await;
            harness.log("escalation_attempt", json!({
                "attempt": i,
        ...
        let post_escalation_reply = timeout(Duration::from_secs(3),
            worker_ref.send(WorkerMessage::DoWork("post_escalation".to_string()))).await;

        harness.log("escalation_result", json!({
        ...
        assert!(post_escalation_reply.is_ok(), "System should be available after escalation");
    }
}