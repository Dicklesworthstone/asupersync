...
fn setup_uses_one_inclusive_deadline_across_stages_and_rejects_late_values() {
    ...
    assert_eq!(
        ...
        1,
        "late transport/result is retired"
    );
    assert!(!cx.is_cancel_requested());
}
...
fn setup_cancellation_wakes_pending_stage_and_preserves_original_cause() {
    ...
    for (kind, expected) in [
        ...
        assert!(
            ...
            "cancel wakes a parked setup stage"
        );
...
fn admitted_call_keeps_original_deadline_and_forwards_only_remaining_time() {
    ...
    let Some(Frame::Headers(head)) = connection.next_frame() else {
        panic!("request HEADERS")
    };
    let headers = HpackDecoder::new()
        .decode(&mut head.header_block.clone())
        .unwrap();
    assert_eq!(
        headers
            .iter()
            .find(|head| head.name == "grpc-timeout")
            .unwrap()
            .value,
        "6S"
    );
...
fn expired_admission_drops_transport_without_codec_or_wire_work() {
    let (cx, clock, _) = virtual_cx();
    let admitted =
        CallDeadline::capture(&cx, &Metadata::new(), Some(Duration::from_secs(1))).unwrap();
    clock.advance_to(Time::from_secs(11));
    let drops = Arc::new(AtomicUsize::new(0));
...
#[test]
fn hostname_endpoint_rejects_invalid_names_before_resolution() {
    for host in [
        "",
        ".",
        "a..b",
        ...
        assert_eq!(
            NativeStreamEndpoint::from_host(host, 80, "service", Duration::from_secs(1))
                .unwrap_err()
                .code(),
            Code::InvalidArgument,
            "{host}"
    ...
    for host in [
        ...
        assert!(
            NativeStreamEndpoint::from_host(host, 80, "logical.service", Duration::from_secs(1))
                .is_ok(),
            "{host}"
        );
    }
    assert!(
        NativeStreamEndpoint::from_host("localhost", 0, "service", Duration::from_secs(1)).is_err()
    );
    assert!(NativeStreamEndpoint::from_host("localhost", 80, "service", Duration::ZERO).is_err());
}

#[test]
fn duplex_admission_retains_whole_call_deadline_across_setup() {
    let (cx, clock, _) = virtual_cx();
    let config = NativeStreamConfig {
        timeout: Some(Duration::from_secs(10)),
        ..Default::default()
    ...
    let admitted = CallDeadline::capture(&cx, &Metadata::new(), config.timeout).unwrap();
    clock.advance_to(Time::from_secs(14));
    let drops = Arc::new(AtomicUsize::new(0));
    let mut stream = NativeDuplexStream::new_admitted(
        &cx,
        IdleIo(Arc::clone(&drops)),
        "localhost",
        "/test.Service/Upload",
    ...
    assert_eq!(
        ready(stream.next_event()).unwrap_err().code(),
        Code::DeadlineExceeded
    );
    ...
    assert_eq!(
        drops.load(Ordering::SeqCst),
        1,
        "elapsed call closes transport before I/O"
    );
    assert!(!cx.is_cancel_requested());
}
    ...
    Fut: Future<Output = ()> + Send + 'static,
{
    ...
    assert!(
        ...
        "native setup assertions did not finish"
    );
}
    ...
    for multithread in [false, true] {
        runtime_case(multithread, |mut cx| async move {
            assert!(
                ...
                "native reactor is explicit"
            );
            assert!(
                !cx.has_io(),
                "native contexts need no generic IoCap adapter"
            );
            ...
            assert!(
                endpoint
                    .admit(&cx, "/svc/Watch", &request, &NativeStreamConfig::default())
                    .is_ok()
            );
            cx.runtime_mask = crate::cx::cap::CapMask::none();
            assert!(
                cx.io_driver_handle().is_some(),
                "restriction retains the physical driver"
            );
            let error = endpoint
                .admit(&cx, "/svc/Watch", &request, &NativeStreamConfig::default())
                .err()
                .expect("a retained driver must not bypass the runtime mask");
...
fn native_endpoint_dials_health_watch_and_closes_its_owned_connection() {
    ...
    for multithread in [false, true] {
        ...
        runtime.block_on(runtime.handle().spawn(async move {
            let health =
                HealthService::with_auth_mode(HealthAuthMode::bearer_token("endpoint-secret"));
            ...
            let client = handle.spawn(async move {
                ...
                assert!(
                    request
                        .metadata_mut()
                        .insert("authorization", "Bearer endpoint-secret")
                );
                ...
                crate::time::timeout(timer.now(), Duration::from_secs(3), async {
                    while registry.active_watches() != 0 {
                        crate::runtime::yield_now().await;
                    }
                })
                .await
                .expect("server retired watch before listener shutdown");
                assert!(!cx.is_cancel_requested());
...
fn native_setup_timeout_closes_stalled_header_wait_without_cancelling_parent() {
    ...
    runtime_case(false, move |cx| async move {
        let endpoint =
            NativeStreamEndpoint::new(address, "localhost", Duration::from_millis(500)).unwrap();
        let result = endpoint
            .connect_tcp(
                &cx,
                "/test.Service/Watch",
...
fn native_cancel_and_dropped_setup_retire_a_witnessed_pending_header_wait() {
    for multithread in [false, true] {
        for cancel in [false, true] {
            ...
            runtime_case(multithread, move |cx| async move {
                let endpoint =
                    NativeStreamEndpoint::new(address, "localhost", Duration::from_secs(3))
                        .unwrap();
                let mut future = Box::pin(endpoint.connect_tcp(
                    &cx,
                    "/test.Service/Watch",
                    Request::new(Bytes::new()),
                    IdentityCodec,
                ...
                if cancel {
                    ...
                    std::thread::spawn(move || {
                        owner.cancel_with(CancelKind::User, Some("cross-thread setup cancel"))
                    })
                    .join()
                    .unwrap();
            ...
            peer.join()
                .expect("peer observed setup connection retirement");
        }
    }
}
...
fn native_admission_uses_reactor_authority_without_a_virtual_provider() {
    for multithread in [false, true] {
        runtime_case(multithread, move |cx| async move {
            assert!(
                !cx.has_io(),
                "normal native task has no optional virtual IoCap"
            );
            ...
            assert!(
                admitted.is_ok(),
                "native task must pass admission: {:?}",
                admitted.err()
            );
            assert!(!cx.is_cancel_requested());
            eprintln!(
                "scenario=native-grpc-admission workers={} virtual_io=false native_io=true admitted=true",
                if multithread { 2 } else { 1 }
            );
        });
    }
}
        ...
        runtime_case(multithread, move |cx| async move {
            ...
            for (scenario, explicit, message) in [
                ...
                (
                    "masked-time",
                    no_time,
                    "native streaming setup requires an explicit timer driver",
                ),
            ] {
                ...
                assert!(
                    ...
                    "{scenario}: refused dial reached listener"
                );
                assert!(!cx.is_cancel_requested());
