...
const CERT: &[u8] = include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/tls/server.crt"));
const KEY: &[u8] = include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/tls/server.key"));
...
fn server(socket: StdTcpStream, h2: bool) -> rustls::StreamOwned<rustls::ServerConnection, StdTcpStream> {
    let provider = Arc::new(rustls::crypto::ring::default_provider());
    let mut config = rustls::ServerConfig::builder_with_provider(provider)
        .with_safe_default_protocol_versions().unwrap().with_no_client_auth()
        .with_single_cert(vec![CertificateDer::from_pem_slice(CERT).unwrap()],
            PrivateKeyDer::from_pem_slice(KEY).unwrap()).unwrap();
    if h2 { config.alpn_protocols = vec![b"h2".to_vec()]; }
...
fn rpc_request() -> Request<Bytes> {
    ...
    assert!(request.metadata_mut().insert("authorization", "Bearer fixture-token"));
    request
}
...
fn response_head(io: &mut impl Write) {
    headers(io, &[(":status", "200"), ("content-type", "application/grpc"),
        ("x-setup", "accepted")], false);
}

fn response_body(io: &mut impl Write) {
    // Two independent golden envelopes, not the production encoder.
    frame(io, 0, 0, 1, &[0, 0, 0, 0, 3, b'o', b'n', b'e', 0, 0, 0, 0, 0]);
...
fn request(io: &mut (impl Read + Write)) -> Vec<Header> {
    ...
    for _ in 0..64 {
        ...
        let length = (usize::from(bytes[0]) << 16) | (usize::from(bytes[1]) << 8) | usize::from(bytes[2]);
        assert!(length <= 16 * 1024);
...
fn assert_request(fields: &[Header]) {
    for (name, value) in [(":scheme", "https"), (":authority", "logical.service"),
        (":path", PATH), ("authorization", "Bearer fixture-token")]
    {
        assert_eq!(fields.iter().find(|field| field.name == name).map(|field| field.value.as_str()), Some(value));
    }
}
...
fn native_tls_endpoint_authenticates_streams_and_retains_metadata() {
    for multithread in [false, true] {
        ...
        let done = native(multithread, move |cx| async move {
            ...
            let mut stream = endpoint.connect_tls(&cx, "localhost", &connector, PATH,
                rpc_request(), IdentityCodec, config()).await.unwrap();
...
fn wrong_identity_untrusted_certificate_and_absent_h2_send_no_grpc_bytes() {
    for (name, trusted, h2) in [("wrong.invalid", true, true), ("localhost", false, true), ("localhost", true, false)] {
        let (listener, address) = listener();
        let peer = std::thread::spawn(move || {
            ...
            match socket.read(&mut application) {
                Ok(0) => {}
                Err(error) => assert!(matches!(error.kind(), io::ErrorKind::InvalidData
                    | io::ErrorKind::UnexpectedEof | io::ErrorKind::ConnectionReset
                    | io::ErrorKind::ConnectionAborted | io::ErrorKind::BrokenPipe), "{error:?}"),
        ...
        let done = native(false, move |cx| async move {
            ...
            let result = endpoint.connect_tls(&cx, name, &connector, PATH,
                rpc_request(), IdentityCodec, config()).await;
...
fn tls_setup_rejects_invalid_inputs_without_dialing_or_borrowing_ambient_authority() {
    ...
    let done = native(false, move |cx| async move {
        ...
        let connector = connector(true);
        let wrong_scheme = endpoint.connect_tls(&cx, "localhost", &connector, PATH,
            rpc_request(), IdentityCodec, NativeStreamConfig::default()).await;
        assert_eq!(wrong_scheme.unwrap_err().code(), Code::InvalidArgument);
        let bad_name = endpoint.connect_tls(&cx, "not a domain", &connector, PATH,
            rpc_request(), IdentityCodec, config()).await;
        ...
        let bad_metadata = endpoint.connect_tls(&cx, "localhost", &connector, PATH,
            forged, IdentityCodec, config()).await;
        assert_eq!(bad_metadata.unwrap_err().code(), Code::InvalidArgument);
        let detached = Cx::for_testing();
        let no_authority = endpoint.connect_tls(&detached, "localhost", &connector, PATH,
            rpc_request(), IdentityCodec, config()).await;
...
enum Interrupt { Cancel, Drop, Deadline }
    ...
    for multithread in [false, true] {
        for mode in [Interrupt::Cancel, Interrupt::Drop, Interrupt::Deadline] {
            ...
            let peer = std::thread::spawn(move || {
                ...
                observed.publish();
                let cx = parks.recv_timeout(WATCHDOG).expect("public setup returned Pending after ClientHello");
                ...
                loop {
                    let read = socket.read(&mut bytes).expect("TLS setup should close its acquired socket");
                    if read == 0 { break; }
                    remaining = remaining.checked_sub(read).expect("unexpected retained TLS output");
                }
            });
            let done = native(multithread, move |cx| async move {
                let endpoint = endpoint(address, Duration::from_secs(2));
                let connector = connector(true);
                let mut setup = Box::pin(endpoint.connect_tls(&cx, "localhost", &connector,
                    PATH, rpc_request(), IdentityCodec, config()));
                    ...
                    match setup.as_mut().poll(task) {
                        ...
                        Poll::Pending => {
                            if witness.received.load(Ordering::Acquire) && !notified {
                                ...
                                if matches!(mode, Interrupt::Drop) { return Poll::Ready(None); }
                            }
                            Poll::Pending
                        }
                    }
                ...
                match mode {
                    ...
                    Interrupt::Deadline => assert_eq!(result.unwrap().unwrap_err().code(), Code::DeadlineExceeded),
                }
                if !matches!(mode, Interrupt::Cancel) { assert!(!cx.is_cancel_requested()); }
            });
