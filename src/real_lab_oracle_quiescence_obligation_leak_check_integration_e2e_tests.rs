...
mod tests {
    ...
    use crate::{
        ...
        error::{Error, Result},
        lab::{
            DeterministicScheduler, LabRuntime, VirtualTime,
            oracle::{
                ObservationWindow, OracleEvent, OracleMetrics,
                quiescence::{
                    QuiescenceConfig, QuiescenceOracle, QuiescenceState, QuiescenceTransition,
                    StateObserver, TransitionEvent,
                },
            },
        },
        obligation::{
            ObligationEvent, ObligationHandle, ObligationId, ObligationKind, ObligationRegistry,
            ObligationState,
            leak_check::{
                LeakAssertion, LeakCheckConfig, LeakChecker, LeakCheckpoint, LeakDetectionResult,
                LeakSeverity, ObligationTracker,
            },
        },
    ...
    impl QuiescenceLeakTestFramework {
                    ...
                    match observer.wait_for_transition().await {
                        Ok(transition) => {
                            ...
                            let event = IntegrationEvent {
                                ...
                                metadata: [("transition".to_string(), format!("{:?}", transition))]
                                    .into(),
                            };
            ...
            let tasks: Vec<_> = (0..worker_count)
                .map(|worker_id| {
                    spawn(cx, Budget::unlimited(), async move {
                        ...
                        let interval =
                            Duration::from_nanos(1_000_000_000 / obligations_per_worker as u64);

                        for obligation_seq in 0..obligations_per_worker * 30 {
                            // 30 seconds worth
                            let obligation_id =
                                ObligationId::new_for_test(worker_id as u64, obligation_seq as u64);
                            ...
                            let current_phase =
                                framework_ref.load_generator.lock().await.load_phase;
        ...
        async fn create_tracked_obligation(
            ...
            // Track in load generator
            {
                ...
                generator
                    .active_obligations
                    .insert(obligation_id, lifecycle);
            ...
            Self {
                ...
                leak_scenarios: vec![
                    ...
                    LeakScenario {
                        ...
                        trigger_condition: LeakTriggerCondition::QuiescenceState(
                            QuiescenceState::Achieved,
                        ),
                        leak_count: 2,
    ...
    async fn test_oracle_leak_detection_integration() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                // Wait for quiescence
                let quiescence_achieved = framework
                    .wait_for_quiescence(Duration::from_secs(2))
                    .await
                    .unwrap();
                assert!(
                    quiescence_achieved,
                    "Should achieve quiescence after obligations complete"
                );

                // Verify oracle recorded appropriate events
                let leak_detections = framework
                    .get_integration_event_count(IntegrationEventType::LeakDetected)
                    .await;
                assert!(leak_detections > 0, "Should have detected leaks");

                let state_transitions = framework
                    .get_integration_event_count(IntegrationEventType::StateTransition)
                    .await;
                assert!(
                    state_transitions > 0,
                    "Should have oracle state transitions"
                );
    ...
    async fn test_production_load_leak_detection() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                assert!(
                    oracle_accurate,
                    "Oracle should maintain accuracy under production load"
                );
                ...
                assert!(
                    leak_detections > 0,
                    "Should detect leaks under production load"
                );

                // Verify load metrics
                let metrics = framework.get_load_metrics().await;
                assert!(
                    metrics.total_obligations_created > 1000,
                    "Should create substantial obligations"
                );
                assert!(
                    metrics.total_obligations_leaked > 0,
                    "Should have some intentional leaks"
                );
                ...
                assert!(
                    quiescence_achieved,
                    "Should achieve quiescence after production load"
                );
    ...
    async fn test_concurrent_leak_assertion_accuracy() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                let expected_leaks = framework
                    .get_integration_event_count(IntegrationEventType::ObligationLeaked)
                    .await;
                let detected_leaks = leak_results.len();

                assert!(detected_leaks > 0, "Should detect some leaks");
                assert!(
                    detected_leaks >= expected_leaks / 2,
                    "Should detect at least half of intentional leaks"
                );

                // Oracle should observe the leak detection activity
                let oracle_events = framework
                    .get_integration_event_count(IntegrationEventType::StateTransition)
                    .await;
    ...
    async fn test_quiescence_timing_with_leak_assertions() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                let clean_quiescence = framework
                    .wait_for_quiescence(Duration::from_secs(2))
                    .await
                    .unwrap();
                assert!(
                    clean_quiescence,
                    "Should achieve quiescence with clean obligations"
                );
                ...
                assert!(
                    !matches!(current_state, QuiescenceState::Achieved),
                ...
                assert_eq!(
                    leak_results.len(),
                    10,
                    "Should detect all 10 intentional leaks"
                );
                ...
                assert!(
                    state_transitions >= 2,
                    "Should have transitions: achieved -> broken"
                );
    ...
    async fn test_leak_severity_classification() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                let high_severity = leak_results
                    .iter()
                    .filter(|r| matches!(r.severity, LeakSeverity::High))
                    .count();
                let medium_severity = leak_results
                    .iter()
                ...
                assert!(
                    high_severity > 0,
                    "Should classify some leaks as high severity"
                );
                assert!(
                    medium_severity > 0,
                    "Should classify some leaks as medium severity"
                );
    ...
    async fn test_oracle_recovery_after_leak_resolution() {
        ...
        runtime
            .region(Budget::unlimited(), |cx| async move {
                ...
                for i in 0..5 {
                    framework
                        .complete_obligation(ObligationId::new_for_test(1, i))
                        .await
                        .unwrap();
                }
                ...
                let recovery_achieved = framework
                    .wait_for_quiescence(Duration::from_secs(2))
                    .await
                    .unwrap();
                assert!(
                    recovery_achieved,
                    "Oracle should achieve quiescence after leak resolution"
                );

                // Verify no more leaks detected
                let final_leaks = framework.trigger_leak_assertions().await.unwrap();
                assert!(
                    final_leaks.is_empty(),
                    "Should detect no leaks after resolution"
                );
                ...
                assert!(
                    state_transitions >= 2,
                    "Should have transitions for leak introduction and resolution"
                );
