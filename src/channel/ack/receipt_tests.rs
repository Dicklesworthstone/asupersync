...
fn take<T>(receipt: &mut Receipt<T>) -> Settlement<T> {
    receipt
        .try_take()
        .unwrap()
        .expect("actual terminal publication")
}
...
fn receipt_is_not_an_enqueue_receive_or_nack_acknowledgement() {
    ...
    assert_eq!(
        (result.sequence, result.attempts, result.deliveries),
        (receipt.sequence(), 2, 2)
    );
...
fn rejection_returns_the_same_mutated_allocation_and_is_not_successful_drain() {
    ...
    **delivery = 73;
    ...
    assert_eq!(
        futures_lite::future::block_on(tx.wait_drained(&cx)),
        Err(QueueError::Rejected)
    );
}
...
fn receiver_abandonment_returns_queued_and_outstanding_payloads_separately() {
    ...
    let mut second = tx
        .try_send_tracked(&cx, String::from("still queued"))
        .unwrap();
    ...
    assert!(
        matches!(queued.outcome, SettlementOutcome::Abandoned(value) if value == "still queued")
    );
...
fn cancelled_and_dropped_borrowing_waits_preserve_the_terminal_receipt() {
    ...
    }
    {
        ...
        assert!(matches!(
            ...
            Poll::Ready(Err(ReceiptError::Cancelled))
        ));
    }
...
impl Wake for Reenter {
    fn wake(self: Arc<Self>) {
        assert!(
            self.shared.state.try_lock().is_some(),
            "receipt callback ran under queue lock"
        );
...
impl Drop for Bomb {
    fn drop(&mut self) {
        assert!(!self.explode, "untracked payload drop sentinel");
    }
}
...
fn one_untracked_destructor_panic_cannot_suppress_later_abandonment_receipts() {
    ...
    .unwrap_or_else(|_| panic!("send bomb"));
    let mut first = tx
        .try_send_tracked(
            &cx,
            Bomb {
        ...
        .unwrap_or_else(|_| panic!("send one"));
    let mut second = tx
        .try_send_tracked(
            &cx,
            Bomb {
        ...
        .unwrap_or_else(|_| panic!("send two"));
    ...
    for (receipt, expected) in [(&mut first, 1), (&mut second, 2)] {
        assert!(
            matches!(take(receipt).outcome, SettlementOutcome::Abandoned(value) if value.value == expected)
        );
    }
    assert_eq!(tx.stats().unfinished(), 0);
    assert!(tx.stats().abandoned);
...
fn independent_receipts_follow_identity_not_acknowledgement_order() {
    let cx = Cx::for_testing();
    let (tx, rx) = channel(3);
    let mut receipts = (0..3)
        .map(|value| tx.try_send_tracked(&cx, value).unwrap())
        .collect::<Vec<_>>();
    ...
    for (index, receipt) in receipts.iter_mut().enumerate() {
        ...
        if index == 1 {
            assert!(matches!(result.outcome, SettlementOutcome::Rejected(1)));
        } else {
            assert!(matches!(result.outcome, SettlementOutcome::Acknowledged));
        }
    }
...
fn send_only_nonclone_payloads_and_debug_do_not_require_or_reveal_payload_traits() {
    ...
    let (tx, rx) = channel(1);
    let mut receipt = tx
        .try_send_tracked(&cx, Job(Cell::new(99)))
