...
fn take<T>(receipt: &mut Receipt<T>) -> Settlement<T> {
    receipt
        .try_take()
        .unwrap()
        .expect("terminal receipt must be published")
}
    ...
    Fut: Future<Output = ()> + Send + 'static,
{
    ...
    join.try_join()
        .unwrap()
        .expect("retry lifecycle completes within step bound");
    ...
    let (tasks, wakes) = lab
        .state
        .cancel_request(root, &CancelReason::user("receipt test complete"), None)
        .into_parts();
...
fn limit_includes_first_delivery_and_preserves_all_mutations_on_exhaustion() {
    for limit in [1, 2, 3, 8] {
        ...
        let mut receipt = tx
            .try_send_tracked_with_policy(&cx, value, limited(limit))
            .unwrap();
        ...
        assert_eq!(
            futures_lite::future::block_on(tx.close_and_drain(&cx)),
...
fn acknowledgement_on_the_last_allowed_delivery_is_still_success() {
    for limit in 1..=4 {
        ...
        let (tx, rx) = channel(1);
        let mut receipt = tx
            .try_send_tracked_with_policy(&cx, 41, limited(limit))
            .unwrap();
        for _ in 1..limit {
...
fn checked_admission_refusals_do_not_spend_the_worker_delivery_allowance() {
    run_case(|cx| async move {
        ...
        let (tx, rx) = channel(1);
        let mut receipt = tx
            .try_send_tracked_with_policy(&Cx::for_testing(), 17, limited(1))
            .unwrap();
        for _ in 0..16 {
            assert!(matches!(
        ...
        assert!(matches!(
            result.outcome,
            SettlementOutcome::RetryExhausted(17)
        ));
...
fn cancelled_receiver_never_spends_the_first_delivery() {
    ...
    let (tx, rx) = channel(1);
    let mut receipt = tx
        .try_send_tracked_with_policy(&live, 23, limited(1))
        .unwrap();
    cancelled.cancel_fast(CancelKind::User);
    assert!(matches!(
        rx.try_recv_with_ack(&cancelled),
    ...
    assert!(matches!(
        take(&mut receipt).outcome,
        SettlementOutcome::Acknowledged
    ));
}
...
fn poisoned_item_returns_to_tail_then_leaves_without_suppressing_healthy_work() {
    ...
    let (tx, rx) = channel(2);
    let mut poison = tx
        .try_send_tracked_with_policy(&cx, "bad", limited(2))
        .unwrap();
    ...
    assert!(matches!(
        take(&mut healthy).outcome,
        SettlementOutcome::Acknowledged
    ));
    rx.try_recv_with_ack(&cx).unwrap().nack();
    assert!(matches!(
        take(&mut poison).outcome,
        SettlementOutcome::RetryExhausted("bad")
    ));
    tx.try_send(&cx, "next").unwrap();
    assert_eq!(rx.try_recv_with_ack(&cx).unwrap().ack(), "next");
    assert!(
        tx.stats().rejected,
        "later successes cannot erase rejected work"
    );
}
...
fn receiver_abandonment_wins_over_the_retry_limit_but_explicit_ack_is_not_revoked() {
    for acknowledge in [false, true] {
        ...
        let (tx, rx) = channel(1);
        let mut receipt = tx
            .try_send_tracked_with_policy(&cx, 31, limited(1))
            .unwrap();
        let item = rx.try_recv_with_ack(&cx).unwrap();
        drop(rx);
        if acknowledge {
            assert_eq!(item.ack(), 31);
        } else {
            drop(item);
        }
        let result = take(&mut receipt);
...
fn maximum_delivery_limit_never_wraps_into_a_fresh_allowance() {
    ...
    let (tx, rx) = channel(1);
    let mut receipt = tx
        .try_send_tracked_with_policy(&cx, 47, limited(u64::MAX))
        .unwrap();
    {
    ...
    assert!(matches!(
        result.outcome,
        SettlementOutcome::RetryExhausted(47)
    ));
}
...
fn sealed_preissued_permit_keeps_its_policy_and_eof_waits_for_exhaustion() {
    ...
    assert!(matches!(
        take(&mut receipt).outcome,
        SettlementOutcome::RetryExhausted(53)
    ));
}
...
fn exhausted_delivery_wakes_a_producer_blocked_by_unacknowledged_capacity() {
    ...
    let (tx, rx) = channel(1);
    let mut receipt = tx
        .try_send_tracked_with_policy(&cx, 59, limited(1))
        .unwrap();
    let item = rx.try_recv_with_ack(&cx).unwrap();
    ...
    assert!(matches!(
        take(&mut receipt).outcome,
        SettlementOutcome::RetryExhausted(59)
    ));
...
fn checked_handoff_preserves_retry_allowance_and_receipt_after_source_task_exit() {
    run_case(|cx| async move {
        let (tx, rx) = channel(1);
        let mut receipt = tx
            .try_send_tracked_with_policy(&cx, 67_u8, limited(1))
            .unwrap();
        ...
        let mut destination = cx
            .spawn(move |destination| async move {
                ...
                assert_eq!(
                    item.obligation.as_ref().unwrap().holder(),
                    destination.task_id()
                );
        ...
        let result = receipt.wait(&cx).await.unwrap();
        assert_eq!(
            (result.sequence, result.deliveries),
            (receipt.sequence(), 1)
        );
        assert!(matches!(
            result.outcome,
            SettlementOutcome::RetryExhausted(67)
        ));
        assert_eq!(tx.close_and_drain(&cx).await, Err(QueueError::Rejected));
    });
}
