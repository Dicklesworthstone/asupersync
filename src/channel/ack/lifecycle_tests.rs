    ...
    Fut: Future<Output = ()> + Send + 'static,
{
    ...
    lab.run_until_idle();
    join.try_join().unwrap().expect("queue lifecycle completes within step bound");
    ...
    let (tasks, wakes) = lab.state.cancel_request(root, &CancelReason::user("queue test complete"), None).into_parts();
...
fn checked_send_and_ack_refusals_leave_physical_ownership_intact() {
    run_case(0, |cx| async move {
        ...
        assert!(matches!(error.error, QueueError::Admission(ObligationAdmissionError::LimitReached { limit: 0, .. })));
        ...
        assert!(matches!(rx.try_recv_with_ack(&cx), Err(QueueError::Admission(ObligationAdmissionError::LimitReached { limit: 0, .. }))));
        let stats = tx.stats();
...
fn ack_and_send_credits_share_the_actual_runtime_region_quota() {
    run_case(1, |cx| async move {
        ...
        assert_eq!(permit.obligation.as_ref().unwrap().kind(), ObligationKind::SendPermit);
        tx.try_send(&Cx::for_testing(), 13).unwrap();
        assert!(matches!(rx.try_recv_with_ack(&cx), Err(QueueError::Admission(ObligationAdmissionError::LimitReached { limit: 1, .. }))));
        drop(permit);
...
fn handoff_moves_liability_before_the_original_holder_exits_at_full_quota() {
    run_case(1, |cx| async move {
        ...
        let mut destination = cx.spawn(move |destination| async move {
            ...
            assert_eq!(delivery.obligation.as_ref().unwrap().holder(), destination.task_id());
            assert_eq!(delivery.obligation.as_ref().unwrap().kind(), ObligationKind::Ack);
...
fn untracked_transfer_never_fabricates_destination_tracking() {
    ...
    let failure = rx.try_recv_with_ack(&cx).unwrap().try_transfer(&cx).unwrap_err();
    assert_eq!(failure.error, ObligationTransferError::SourceNotChecked);
...
fn last_receiver_abandonment_is_not_reported_as_successful_drain() {
    for in_flight in [false, true] {
        ...
        let held = if in_flight { Some(rx.try_recv_with_ack(&cx).unwrap()) } else { None };
        drop(rx);
        let mut drain = Box::pin(tx.close_and_drain(&cx));
        let mut context = Context::from_waker(Waker::noop());
        if in_flight { assert!(drain.as_mut().poll(&mut context).is_pending()); }
        drop(held);
        assert!(matches!(drain.as_mut().poll(&mut context), Poll::Ready(Err(QueueError::Abandoned))));
...
fn cancelled_drain_keeps_admission_closed_and_all_unfinished_work() {
    ...
    assert!(matches!(drain.as_mut().poll(&mut context), Poll::Ready(Err(QueueError::Cancelled))));
    drop(drain);
