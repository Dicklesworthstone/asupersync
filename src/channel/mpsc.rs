...
pub mod backpressure_metamorphic {
    ...
    ///
    /// Property: When concurrent producers perform reserve/send operations with some
    /// reserves being cancelled mid-flight, the messages that do complete must still
    /// preserve FIFO ordering per producer and globally across all producers.
    ///
    /// This test specifically focuses on the interleaving patterns between:
    /// - reserve() calls that complete normally
    /// - reserve() calls that get cancelled before send()
    /// - send() operations on completed reserves
    /// - cancellation timing relative to the reserve/send lifecycle
    ///
    /// Critical invariants:
    /// - Cancelled reserves don't leak capacity
    /// - Completed sends maintain per-producer FIFO order
    /// - Global FIFO order preserved across all producers
    /// - No spurious wakeups or lost wakeups due to cancellation
    #[test]
    fn mr7_fifo_preservation_under_concurrent_cancel_reserve_send_interleaving() {
        ...
        runner
            .run(&backpressure_config_strategy(), |config| {
                if config.sender_count == 0 || config.messages_per_sender == 0 {
                    return Ok(());
                }
                ...
                crate::lab::runtime::test(config.seed, |lab| {
                    ...
                    let (test_task, _) = lab.state.create_task(root, Budget::INFINITE, async move {
                        let cx = crate::cx::Cx::for_testing();
                        let _test_res: Result<(), proptest::test_runner::TestCaseError> = async {
                            // Test-specific configuration for interleaving patterns
                            let capacity = std::cmp::max(1, config.capacity);
                            let sender_count = std::cmp::min(config.sender_count, 4); // Keep manageable
                            let msgs_per_sender = std::cmp::min(config.messages_per_sender, 8);

                            let (sender, mut receiver) = channel::<u32>(capacity);

                            // Tracking structures for verification
                            let received_messages = Arc::new(parking_lot::Mutex::new(Vec::new()));
                            let completed_sends = Arc::new(parking_lot::Mutex::new(Vec::new()));
                            let cancelled_reserves = Arc::new(AtomicUsize::new(0));
                            let total_reserve_attempts = Arc::new(AtomicUsize::new(0));

                            // Receiver thread that collects all successfully sent messages
                            let recv_ref = Arc::clone(&received_messages);
                            let recv_cx = cx.clone();
                            let recv_handle = std::thread::spawn(move || {
                                futures_lite::future::block_on(async move {
                            ...
                            // Producer threads with interleaved cancellation patterns
                            let mut producer_handles = Vec::new();
                            let start_barrier = Arc::new(std::sync::Barrier::new(sender_count + 1));

                            for producer_id in 0..sender_count {
                                let sender_clone = sender.clone();
                                let completed_ref = Arc::clone(&completed_sends);
                                let cancelled_ref = Arc::clone(&cancelled_reserves);
                                let total_attempts_ref = Arc::clone(&total_reserve_attempts);
                                let barrier = Arc::clone(&start_barrier);
                                let prod_cx = cx.clone();
                                let cancel_prob = config.cancel_probability;

                                let handle = std::thread::spawn(move || {
                                    barrier.wait(); // Synchronize start for maximum contention

                                    futures_lite::future::block_on(async move {
                                        let mut rng_state = (config.seed + producer_id as u64)
                                            .wrapping_mul(1103515245).wrapping_add(12345);

                                        for msg_ordinal in 0..msgs_per_sender {
                                            total_attempts_ref.fetch_add(1, Ordering::SeqCst);
                                            let message_value = (producer_id as u32 * 1000) + msg_ordinal as u32;

                                            // Decide whether to cancel this reserve based on pseudo-random
                                            rng_state = rng_state.wrapping_mul(1103515245).wrapping_add(12345);
                                            let should_cancel = config.inject_cancellation &&
                                                (rng_state as f64 / u64::MAX as f64) < cancel_prob;

                                            if should_cancel {
                                                // Pattern 1: Start reserve, then cancel before send
                                                match sender_clone.try_reserve() {
                                                    Ok(permit) => {
                                                        // Cancel by dropping the permit without calling send()
                                                        drop(permit);
                                                        cancelled_ref.fetch_add(1, Ordering::SeqCst);
                                                    }
                                                    Err(_) => {
                                                        // Reserve failed (channel full), try blocking reserve then cancel
                                                        if let Ok(permit) = sender_clone.reserve(&prod_cx).await {
                                                            drop(permit);
                                                            cancelled_ref.fetch_add(1, Ordering::SeqCst);
                                                        }
                                                    }
                                                ...
                                                // Pattern 2: Complete normal reserve/send cycle
                                                if let Ok(permit) = sender_clone.reserve(&prod_cx).await {
                                                    permit.send(message_value);
                                                    completed_ref.lock().push((producer_id, message_value));
                                                }
                                            }

                                            // Add small randomized delay to increase interleaving opportunities
                                            if (rng_state >> 16) % 4 == 0 {
                                                std::thread::yield_now();
                                            }
                                        }
                            ...
                            drop(sender); // Close channel to wake receiver
                            recv_handle.join().unwrap();

                            // Verification phase
                            let received = received_messages.lock().clone();
                            let completed = completed_sends.lock().clone();
                            let cancelled_count = cancelled_reserves.load(Ordering::SeqCst);
                            let total_attempts = total_reserve_attempts.load(Ordering::SeqCst);

                            // Invariant 1: All received messages must correspond to completed sends
                            assert_eq!(
                                received.len(),
                                completed.len(),
                                "Mismatch between received messages ({}) and completed sends ({})",
                                received.len(), completed.len()
                            );

                            // Invariant 2: Per-producer FIFO order must be preserved
                            let mut producer_sequences: std::collections::HashMap<usize, VecDeque<u32>> = std::collections::HashMap::new();
                            for (producer_id, value) in &completed {
                                producer_sequences.entry(*producer_id).or_default().push_back(*value);
                            }

                            for &received_value in &received {
                                let producer_id = (received_value / 1000) as usize;
                                if let Some(expected_queue) = producer_sequences.get_mut(&producer_id) {
                                    let expected_value = expected_queue.pop_front()
                                        .unwrap_or_else(|| panic!(
                                            "Received unexpected message {} from producer {}",
                                            received_value, producer_id
                                        ));
                                    assert_eq!(
                                        received_value, expected_value,
                                        "FIFO violation for producer {}: expected {}, got {}",
                                        producer_id, expected_value, received_value
                                    );
                                }
                            }

                            // Invariant 3: No messages should be left in producer sequences
                            for (producer_id, remaining) in producer_sequences {
                                assert!(
                                    remaining.is_empty(),
                                    "Producer {} has unreceived messages: {:?}",
                                    producer_id, remaining
                                );
                            }

                            // Invariant 4: Capacity conservation check
                            // (cancelled_count + completed.len()) should equal total_attempts
                            assert_eq!(
                                cancelled_count + completed.len(),
                                total_attempts,
                                "Capacity conservation: {} cancelled + {} completed != {} total attempts",
                                cancelled_count, completed.len(), total_attempts
                            );

                            // Invariant 5: Global ordering consistency
                            // The received messages should maintain the same relative order as they
                            // appear in the completed sends when grouped by producer
                            let mut completed_by_order: Vec<_> = completed.clone();
                            completed_by_order.sort_by_key(|(producer_id, value)| (*producer_id, *value));

                            let mut received_by_producer: std::collections::HashMap<usize, Vec<u32>> = std::collections::HashMap::new();
                            for &value in &received {
                                let producer_id = (value / 1000) as usize;
                                received_by_producer.entry(producer_id).or_default().push(value);
                            }

                            for (producer_id, values) in received_by_producer {
                                let expected: Vec<_> = completed_by_order.iter()
                                    .filter(|(pid, _)| *pid == producer_id)
                                    .map(|(_, val)| *val)
                                    .collect();
                                assert_eq!(
                                    values, expected,
                                    "Producer {} global ordering violation: received {:?}, expected {:?}",
                                    producer_id, values, expected
                                );
                            }
            ...
            .expect("FIFO preservation under concurrent cancel+reserve/send interleaving failed");
    }
}
