...
use crate::channel::broadcast::{channel, RecvError, TryRecvError, SendError};
use crate::cx::Cx;
use crate::lab::{LabRuntime, LabConfig};
use crate::types::{Budget, RegionId, TaskId};
use crate::util::{DetRng, ArenaIndex};
use std::sync::Arc;
...
/// Simulate sending multiple messages to a broadcast channel.
async fn send_messages<T: Clone>(
    cx: &Cx,
    ...
    for msg in messages {
        let permit = match sender.reserve(cx) {
            Ok(p) => p,
            Err(SendError::Closed(())) => return Err(SendError::Closed(msg.clone())),
        };
