    ...
    for &a in &test_elements {
        for &b in &test_elements {
            for &c in &test_elements {
                let gf_a = Gf256::from(a);
                let gf_b = Gf256::from(b);
                let gf_c = Gf256::from(c);

                // Addition is commutative: a + b = b + a
                assert_eq!(gf_a + gf_b, gf_b + gf_a,
                    "Addition not commutative for {} + {}", a, b);

                // Addition is associative: (a + b) + c = a + (b + c)
                assert_eq!((gf_a + gf_b) + gf_c, gf_a + (gf_b + gf_c),
                    "Addition not associative for ({} + {}) + {} vs {} + ({} + {})",
                    a, b, c, a, b, c);

                // Multiplication is commutative: a * b = b * a
                assert_eq!(gf_a.mul_field(gf_b), gf_b.mul_field(gf_a),
                    "Multiplication not commutative for {} * {}", a, b);
...
fn test_identity_elements() {
    let zero = Gf256::from(0);
    let one = Gf256::from(1);

    for i in 0..=255u8 {
        let gf_i = Gf256::from(i);
        ...
        assert_eq!(gf_i.mul_field(one), gf_i, "Multiplicative identity fails for {}", i);
        assert_eq!(one.mul_field(gf_i), gf_i, "Multiplicative identity fails for 1 * {}", i);

        // Additive inverse: a + a = 0 (in GF(2^n), every element is its own additive inverse)
        assert_eq!(gf_i + gf_i, zero, "Additive inverse fails for {} + {}", i, i);
    }
}
...
fn test_multiplicative_inverses() {
    ...
    for i in 1..=255u8 { // Skip 0 as it has no multiplicative inverse
        let gf_i = Gf256::from(i);
        let gf_i_inv = gf_i.inv();

        // a * inv(a) = 1
        assert_eq!(gf_i.mul_field(gf_i_inv), one,
            "Multiplicative inverse fails: {} * inv({}) != 1", i, i);
        assert_eq!(gf_i_inv.mul_field(gf_i), one,
            "Multiplicative inverse fails: inv({}) * {} != 1", i, i);
    }
}
...
fn test_division() {
    ...
    for &a in &test_elements {
        for &b in &test_elements {
            ...
            assert_eq!(div_result, mul_inv_result,
                "Division inconsistent: {} / {} != {} * inv({})", a, b, a, b);

            // Division identity: (a / b) * b = a
            assert_eq!(div_result.mul_field(gf_b), gf_a,
                "Division identity fails: ({} / {}) * {} != {}", a, b, b, a);
        }
    }
}
...
fn test_exponentiation() {
    ...
    for &base in &test_bases {
        let gf_base = Gf256::from(base);

        for &exp in &test_exps {
            let pow_result = gf_base.pow(exp);

            // Manual calculation for verification
            let mut manual_result = Gf256::from(1);
            for _ in 0..exp {
                manual_result = manual_result.mul_field(gf_base);
            }

            assert_eq!(pow_result, manual_result,
                "Exponentiation mismatch: {}^{}", base, exp);
        }

        // Special cases
        assert_eq!(gf_base.pow(0), Gf256::from(1), "{}^0 should be 1", base);
...
fn test_field_completeness() {
    ...
    for i in 1..=255u8 {
        let gf_i = Gf256::from(i);
        let inv_val = gf_i.inv().value();

        // Every non-zero element should have a unique inverse
        assert!(inv_val != 0, "Element {} has zero inverse", i);
        assert!(!inverses_seen.contains(&inv_val),
            "Duplicate inverse {} for element {}", inv_val, i);
...
fn test_slice_operations_correctness() {
    ...
    for i in 0..test_data_a.len() {
        let expected = Gf256::from(test_data_a[i]) + Gf256::from(test_data_b[i]);
        assert_eq!(slice_result[i], expected.value(),
            "Slice addition mismatch at index {}: {} + {} = {} (expected {})",
            i, test_data_a[i], test_data_b[i], slice_result[i], expected.value());
    }
    ...
    for i in 0..test_data_a.len() {
        let expected = Gf256::from(test_data_a[i]).mul_field(multiplier);
        assert_eq!(mul_result[i], expected.value(),
            "Slice multiplication mismatch at index {}: {} * {} = {} (expected {})",
            i, test_data_a[i], multiplier.value(), mul_result[i], expected.value());
    }
}
...
fn test_dual_slice_operations() {
    ...
    let multiplier = Gf256::from(73);

    // Test gf256_add_slices2
    gf256_add_slices2(&mut test_data_a, &test_data_b, &mut test_data_c, &test_data_d);

    for i in 0..test_data_a.len() {
        let expected_a = Gf256::from(test_data_a_orig[i]) + Gf256::from(test_data_b[i]);
        let expected_c = Gf256::from(test_data_c_orig[i]) + Gf256::from(test_data_d[i]);

        assert_eq!(test_data_a[i], expected_a.value(),
            "Dual slice addition mismatch for slice A at index {}", i);
        assert_eq!(test_data_c[i], expected_c.value(),
            "Dual slice addition mismatch for slice C at index {}", i);
    }
    ...
    for i in 0..test_data_a.len() {
        let expected_a = Gf256::from(test_data_a_orig[i]).mul_field(multiplier);
        let expected_c = Gf256::from(test_data_c_orig[i]).mul_field(multiplier);

        assert_eq!(test_data_a[i], expected_a.value(),
            "Dual slice multiplication mismatch for slice A at index {}", i);
        assert_eq!(test_data_c[i], expected_c.value(),
            "Dual slice multiplication mismatch for slice C at index {}", i);
    }
}
...
fn test_kernel_dispatch_determinism() {
    ...
    for &(len_a, len_b) in &test_sizes {
        ...
        assert_eq!(decision1, decision2,
            "Dual mul kernel decision should be deterministic for sizes ({}, {})", len_a, len_b);

        let addmul_decision1 = dual_addmul_kernel_decision(len_a, len_b);
        let addmul_decision2 = dual_addmul_kernel_decision(len_a, len_b);
        assert_eq!(addmul_decision1, addmul_decision2,
            "Dual addmul kernel decision should be deterministic for sizes ({}, {})", len_a, len_b);
    }
}
...
fn test_profile_pack_consistency() {
    ...
    assert!(!manifest.available_packs.is_empty(), "Should have at least one profile pack");
    assert!(manifest.available_packs.contains(&policy.active_profile_pack),
        "Active profile pack should be in available packs list");

    // Verify dual kernel thresholds are reasonable
    match policy.dual_policy {
        DualKernelPolicy::Auto => {
            assert!(policy.mul_threshold > 0, "Mul threshold should be positive");
            assert!(policy.addmul_threshold > 0, "Addmul threshold should be positive");
        }
        DualKernelPolicy::AlwaysSingle => {
            // Thresholds don't matter in this mode
        }
        DualKernelPolicy::AlwaysDual => {
            // Should use dual kernels regardless of size
        }
        DualKernelPolicy::Manual { .. } => {
            // Manual thresholds should be reasonable
            assert!(policy.mul_threshold > 0, "Manual mul threshold should be positive");
            assert!(policy.addmul_threshold > 0, "Manual addmul threshold should be positive");
        }
    }
}
...
fn test_rfc6330_compliance() {
    ...
    for i in 0..=255u8 {
        all_results.insert(Gf256::from(i).value());
    }
    assert_eq!(all_results.len(), 256, "Field should have exactly 256 elements");

    // Test that multiplication by 2 (α) works correctly
    // In GF(256), α is a primitive element
    let alpha = Gf256::from(2);
    let alpha_squared = alpha.mul_field(alpha);
    assert_eq!(alpha_squared.value(), 4, "α² should equal 4 in GF(256)");

    // Test that α^255 = 1 (since α is primitive in GF(2^8))
    let alpha_255 = alpha.pow(255);
    assert_eq!(alpha_255.value(), 1, "α^255 should equal 1 in GF(256)");
    ...
    for &(a, b, expected) in &test_cases {
        let result = Gf256::from(a).mul_field(Gf256::from(b));
        assert_eq!(result.value(), expected,
            "RFC test case failed: {} * {} = {} (expected {})", a, b, result.value(), expected);
    }
}
...
fn test_random_operations_stress() {
    ...
    for _ in 0..1000 {
        ...
        assert!(sum.value() <= 255, "Sum result out of range");
        if let Some(prod) = product {
            assert!(prod.value() <= 255, "Product result out of range");
        }

        // Test division when possible
        if b != 0 {
            let division = gf_a.div_field(gf_b);
            assert!(division.value() <= 255, "Division result out of range");

            // Verify division correctness: (a / b) * b = a
            assert_eq!(division.mul_field(gf_b), gf_a,
                "Division verification failed for {} / {}", a, b);
        }
    }
}