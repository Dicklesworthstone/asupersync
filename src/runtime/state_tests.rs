...
fn task_completion_observer_contains_panicking_payload_destructor() {
    ...
    let metrics =
        PanickingCompletionMetrics::panic_with_panicking_drop_payload(Arc::clone(&payload_drops));
    let state = Arc::new(ContendedMutex::new(
        "runtime_state",
        RuntimeState::new_with_metrics(metrics.clone()),
    ));
...
fn external_completion_orders_delayed_cancel_receipt_before_terminal_retirement() {
    fn run_case(panic_after_cancel: bool) {
        ...
        assert_eq!(
            state.external_handle_cancel_request_violation(task_id, region_id, spawned_at, true,),
            None
        );
...
fn task_completed_clears_cancel_waker_under_concurrent_install() {
    ...
    for trial in 0..32 {
        ...
        let cx_inner = state
            ...
            .clone();
        {
            let cancel_waker = std::sync::Arc::new(crate::types::task_context::CancelWaker::new(
...
fn region_cancel_deferred_queue_publishes_all_tasks_before_panic_isolated_wakers() {
    ...
    let wakers: Vec<_> = audits
        .iter()
        .map(|audit| reentrant_cancellation_waker(&state, &expected_tasks, &published, audit, true))
        .collect();
...
fn sibling_cancel_defers_wakers_and_excludes_policy_child() {
    ...
    for (inner, audit) in [(&first_inner, &audits[0]), (&second_inner, &audits[1])] {
        let waker = reentrant_cancellation_waker(&state, &expected_tasks, &published, audit, false);
        let target = Arc::new(crate::types::task_context::CancelWaker::new(waker.clone()));
...
fn timer_driver_timestamps_async_finalizer_deadline_and_history() {
    ...
    let expected_deadline = Time::from_nanos(123).saturating_add_nanos(FINALIZER_TIME_BUDGET_NANOS);
...
fn cancel_request_propagates_to_descendants() {
    ...
    let is_cancel_requested = matches!(&root_task_record.state, TaskState::CancelRequested { .. });
    assert!(
        is_cancel_requested,
        "expected CancelRequested, got {:?}",
    ...
    let is_cancel_requested = matches!(&child_task_record.state, TaskState::CancelRequested { .. });
    assert!(
        is_cancel_requested,
        "expected CancelRequested, got {:?}",
...
fn quiescence_observation_matrix_has_no_early_true_reports() {
    ...
    let effects = cancel_requested.cancel_request(cancel_root, &CancelReason::user("stop"), None);
...
fn redundant_region_close_requests_quiesce_once_without_double_finalize() {
    ...
    let fourth_effects = state.cancel_request(root, &CancelReason::user("finalizer close"), None);
    ...
    crate::test_complete!("redundant_region_close_requests_quiesce_once_without_double_finalize");
}
...
fn cancel_drain_finalize_nested_regions() {
    ...
    let root_closing = matches!(
        root_state,
        crate::record::region::RegionState::Closing | crate::record::region::RegionState::Draining
    );
    crate::assert_with_log!(
        root_closing,
...
fn leak_response_silent_marks_leaked_no_log() {
    ...
    let (mut state, _region, task, obl) = setup_leakable_obligation(ObligationLeakResponse::Silent);
...
fn leak_response_log_marks_leaked() {
    ...
    let (mut state, _region, task, obl) = setup_leakable_obligation(ObligationLeakResponse::Log);
...
fn leak_response_panic_panics() {
    ...
    let (mut state, _region, task, _obl) = setup_leakable_obligation(ObligationLeakResponse::Panic);
...
fn read_biased_region_snapshot_tracks_draining_runtime_transitions() {
    ...
    let (_first_tasks, first_wakes) = first_effects.into_parts();
    let draining_snapshot = crate::obligation::lyapunov::StateSnapshot::from_runtime_state(&state);
    crate::assert_with_log!(
    ...
    let (_second_tasks, second_wakes) = second_effects.into_parts();
    state.advance_region_state(child);
    let closed_snapshot = crate::obligation::lyapunov::StateSnapshot::from_runtime_state(&state);
    crate::assert_with_log!(
...
fn maybe_write_read_biased_region_snapshot_report(path: &str, report: &Value) {
    ...
    if let Some(parent) = report_path.parent() {
        fs::create_dir_all(parent).expect("create read-biased region snapshot report directory");
    }
    fs::write(
        report_path,
        serde_json::to_string_pretty(report).expect("serialize read-biased region snapshot report"),
    )
...
fn run_read_biased_region_snapshot_scenario(scenario: &ReadBiasedRegionSnapshotScenario) -> Value {
    match scenario.workload_class.as_str() {
        "read-heavy" => {
            let baseline_state =
                prepare_read_biased_region_snapshot_state(scenario.fixture.draining_regions, false);
            ...
            let read_biased_state =
                prepare_read_biased_region_snapshot_state(scenario.fixture.draining_regions, true);
            let _ =
                crate::obligation::lyapunov::StateSnapshot::from_runtime_state(&read_biased_state);
...
fn create_task_with_authorization_check() {
    ...
    let denied = state.create_task_with_auth(&test_cx, region, Budget::INFINITE, async { 7_u32 });
    crate::assert_with_log!(
