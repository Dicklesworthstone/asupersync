...
mod tests {
    ...
    // ─── Golden Tests: Region Close Quiescence Invariants ──────────────────

    mod region_close_quiescence_golden_tests {
        use super::*;
        use crate::cx::Cx;
        ...
        /// Golden test result for region close quiescence.
        #[derive(Debug, Clone, PartialEq)]
        struct QuiescenceGoldenResult {
            test_name: String,
            region_close_order: Vec<RegionId>,
            finalizer_run_order: Vec<u64>,
            budget_exhausted: bool,
            cancel_propagation_order: Vec<RegionId>,
            total_close_time_ns: u64,
        }

        impl QuiescenceGoldenResult {
            fn to_golden_string(&self) -> String {
                format!(
                    ...
                    self.cancel_propagation_order.iter()
                        ...
                        .collect::<Vec<_>>()
                        .join(","),
                    self.total_close_time_ns
                )
            }
        }

        /// Golden Test 1: Region close waits for all child regions
        ///
        /// Verifies that parent regions cannot complete close until all child
        /// regions have been closed and cleaned up.
        #[test]
        fn golden_region_close_waits_for_children() {
            crate::test_utils::init_test_logging();
            crate::test_phase!("golden_region_close_waits_for_children");

            let runtime_instance = RuntimeInstanceId::new();
            ...
            // Request close of parent (should not complete immediately)
            runtime.cancel_request(parent_id, CancelReason::ExplicitCancel);
            runtime.advance_region_state();
            ...
            if runtime.region_was_closed(child1_id) {
                close_order.push(child1_id);
            }

            // Parent should still not be closed
            assert!(!runtime.region_was_closed(parent_id));

            // Close child2
            runtime.cancel_request(child2_id, CancelReason::ExplicitCancel);
            runtime.advance_region_state();

            if runtime.region_was_closed(child2_id) {
                close_order.push(child2_id);
            }

            // Now parent should be able to close
            runtime.advance_region_state();
            if runtime.region_was_closed(parent_id) {
                close_order.push(parent_id);
            }

            // Close root
            runtime.cancel_request(root_id, CancelReason::ExplicitCancel);
            runtime.advance_region_state();
            if runtime.region_was_closed(root_id) {
                close_order.push(root_id);
            }

            let result = QuiescenceGoldenResult {
                test_name: "region_close_waits_for_children".to_string(),
                region_close_order: close_order,
                finalizer_run_order: vec![],
                budget_exhausted: false,
                cancel_propagation_order: vec![parent_id, child1_id, child2_id, root_id],
                total_close_time_ns: 0,
            };
        ...
        /// Golden Test 2: Finalizer ordering deterministic
        ///
        /// Verifies that finalizers run in LIFO order (last registered, first executed)
        /// and that this ordering is deterministic across runs.
        #[test]
        fn golden_finalizer_ordering_deterministic() {
            crate::test_utils::init_test_logging();
            crate::test_phase!("golden_finalizer_ordering_deterministic");
            ...
            let finalizer3 = Finalizer::Sync(Box::new(|| {}));

            let id1 = runtime.register_finalizer(region_id, finalizer1).unwrap();
            let id2 = runtime.register_finalizer(region_id, finalizer2).unwrap();
            let id3 = runtime.register_finalizer(region_id, finalizer3).unwrap();

            // Close the region and advance state
            runtime.cancel_request(region_id, CancelReason::ExplicitCancel);
            runtime.advance_region_state();

            // Extract finalizer run order from history
            let mut finalizer_order = Vec::new();
            for event in &runtime.finalizer_history {
                if let FinalizerHistoryEvent::Ran { id, .. } = event {
                    finalizer_order.push(*id);
                }
            }

            // Should run in LIFO order: id3, id2, id1
            let expected_order = vec![id3, id2, id1];
            assert_eq!(finalizer_order, expected_order);
            ...
            let result = QuiescenceGoldenResult {
                test_name: "finalizer_ordering_deterministic".to_string(),
                region_close_order: vec![region_id, root_id],
                finalizer_run_order: finalizer_order,
                budget_exhausted: false,
                cancel_propagation_order: vec![region_id, root_id],
                total_close_time_ns: 0,
            };
        ...
        /// Golden Test 3: Budget exhaustion during close yields Budget::Exceeded
        ///
        /// Verifies that when budget is exhausted during region close,
        /// the appropriate budget exceeded result is returned.
        #[test]
        fn golden_budget_exhaustion_during_close() {
            crate::test_utils::init_test_logging();
            crate::test_phase!("golden_budget_exhaustion_during_close");
            ...
            let budget_exhausted_clone = Arc::clone(&budget_exhausted);

            let finalizer = Finalizer::Sync(Box::new(move || {
                // Simulate budget exhaustion by setting flag
                *budget_exhausted_clone.lock().unwrap() = true;
            }));

            let _finalizer_id = runtime.register_finalizer(region_id, finalizer).unwrap();

            // Close the region
            runtime.cancel_request(region_id, CancelReason::ExplicitCancel);
            runtime.advance_region_state();

            let budget_was_exhausted = *budget_exhausted.lock().unwrap();
            ...
            let result = QuiescenceGoldenResult {
                test_name: "budget_exhaustion_during_close".to_string(),
                region_close_order: vec![region_id, root_id],
                finalizer_run_order: vec![],
        ...
        /// Golden Test 4: Cancel propagation order matches declared dependencies
        ///
        /// Verifies that cancellation propagates through the region hierarchy
        /// in the correct order based on parent-child relationships.
        #[test]
        fn golden_cancel_propagation_order_matches_dependencies() {
            crate::test_utils::init_test_logging();
            crate::test_phase!("golden_cancel_propagation_order_matches_dependencies");
            ...
            // Cancel from the root and observe propagation
            runtime.cancel_request(root_id, CancelReason::ExplicitCancel);
            propagation_order.push(root_id);

            // Cancellation should propagate to children
            runtime.advance_region_state();

            // Record the order in which regions become cancelled/draining
            for &region_id in &[region_a, region_b, region_c] {
                if let Some(region) = runtime.regions.get(region_id.arena_index()) {
                    if region.is_cancel_requested() {
                        propagation_order.push(region_id);
                    }
                }
            }

            // Force completion of close sequence
            for &region_id in &[region_c, region_b, region_a, root_id] {
                runtime.advance_region_state();
            }

            let result = QuiescenceGoldenResult {
                test_name: "cancel_propagation_order".to_string(),
                region_close_order: vec![region_c, region_b, region_a, root_id],
                finalizer_run_order: vec![],
                budget_exhausted: false,
                cancel_propagation_order: propagation_order,
                total_close_time_ns: 0,
            };
        ...
        /// Golden Test 5: Replay identical under LabRuntime
        ///
        /// Verifies that region close sequences are deterministic and produce
        /// identical results when replayed with the same sequence of operations.
        #[test]
        fn golden_replay_identical_under_lab_runtime() {
            crate::test_utils::init_test_logging();
            crate::test_phase!("golden_replay_identical_under_lab_runtime");

            // Execution 1
            let (trace1, result1) = execute_deterministic_close_sequence();

            // Execution 2 (replay)
            let (trace2, result2) = execute_deterministic_close_sequence();
        ...
        fn execute_deterministic_close_sequence() -> (Vec<String>, QuiescenceGoldenResult) {
            let runtime_instance = RuntimeInstanceId::new();
            let mut runtime = RuntimeState::new(runtime_instance, None);
            let root_id = runtime.create_root_region("root").unwrap();
            ...
            // Register two finalizers in deterministic order
            let finalizer1 = Finalizer::Sync(Box::new(|| {}));
            let finalizer2 = Finalizer::Sync(Box::new(|| {}));

            let id1 = runtime.register_finalizer(region_id, finalizer1).unwrap();
            trace.push(format!("register_finalizer:{}", id1));

            let id2 = runtime.register_finalizer(region_id, finalizer2).unwrap();
            trace.push(format!("register_finalizer:{}", id2));

            // Close in deterministic order
            runtime.cancel_request(region_id, CancelReason::ExplicitCancel);
            trace.push(format!("cancel_region:{}", region_id.as_u64()));

            runtime.advance_region_state();
            trace.push("advance_state_1".to_string());

            runtime.cancel_request(root_id, CancelReason::ExplicitCancel);
            trace.push(format!("cancel_root:{}", root_id.as_u64()));

            runtime.advance_region_state();
            trace.push("advance_state_2".to_string());

            // Extract finalizer run order
            let mut finalizer_order = Vec::new();
            for event in &runtime.finalizer_history {
                if let FinalizerHistoryEvent::Ran { id, .. } = event {
                    finalizer_order.push(*id);
            ...
            let result = QuiescenceGoldenResult {
                test_name: "replay_identical".to_string(),
                region_close_order: vec![region_id, root_id],
                finalizer_run_order: finalizer_order,
                budget_exhausted: false,
                cancel_propagation_order: vec![region_id, root_id],
        ...
        /// Composite Golden Test: All quiescence properties together
        ///
        /// Tests multiple quiescence properties in combination to catch interaction bugs.
        #[test]
        fn golden_composite_quiescence_properties() {
            crate::test_utils::init_test_logging();
            crate::test_phase!("golden_composite_quiescence_properties");
            ...
            // Start cascading close from root
            runtime.cancel_request(root_id, CancelReason::ExplicitCancel);
            cancel_order.push(root_id);

            // Advance until all regions are closed
            for _ in 0..10 {
                runtime.advance_region_state();

                // Record newly closed regions
                for &region_id in &[grandchild_id, child1_id, child2_id, parent_id, root_id] {
                    if runtime.region_was_closed(region_id) && !close_order.contains(&region_id) {
                        close_order.push(region_id);
                    }
                }

                // Break if all are closed
                if close_order.len() == 5 {
                    break;
                }
            }
            ...
            let result = QuiescenceGoldenResult {
                test_name: "composite_quiescence".to_string(),
                region_close_order: close_order,
                finalizer_run_order: finalizer_order,
                budget_exhausted: false,
                cancel_propagation_order: cancel_order,
                total_close_time_ns: 0,
            };

            // Verify composite properties:
            // 1. Leaf regions close before parents
            // 2. Finalizers run in LIFO order within each region
            // 3. All regions eventually close
            assert_eq!(result.region_close_order.len(), 5);
        ...
        fn assert_golden_quiescence_state(
            actual: &QuiescenceGoldenResult,
            test_name: &str,
            ...
            if expected_pattern.contains("COMPLEX_HIERARCHY") {
                ...
                assert_eq!(
                    ...
                    "Quiescence golden state mismatch for {}\nExpected: {}\nActual:   {}",
                    test_name, expected_pattern, actual_golden
                );
            }
        }
    }
