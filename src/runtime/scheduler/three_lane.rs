...
mod tests {
    ...
    fn mr_preemption_resumption_invariance() {
        ...
        proptest!(|(task_count in 2..=8_usize, preemption_delay_ms in 1..=50_u64)| {
            let clock = VirtualClock::new();
            let runtime = LabRuntime::with_clock(clock.clone());

            // Scenario A: No preemption - all tasks run to completion
            let result_no_preempt = {
                let mut scheduler = ThreeLaneScheduler::new(
                ...
                // Add tasks to ready lane
                let task_ids: Vec<_> = (0..task_count).map(|i| TaskId::new(i as u64)).collect();
                for &task_id in &task_ids {
                    scheduler.schedule_task(task_id, TaskPriority::Normal);
                }

                // Execute all tasks without preemption
                let mut executed_order = Vec::new();
                while let Some(task_id) = scheduler.next_task() {
                    executed_order.push(task_id);
                }

                executed_order
            };

            // Scenario B: With preemption - inject high priority task mid-execution
            let result_with_preempt = {
                let mut scheduler = ThreeLaneScheduler::new(
                    1,
                    &runtime.state(),
                    None,
                ...
                // Execute tasks, injecting preemption after first task
                while let Some(task_id) = scheduler.next_task() {
                    executed_order.push(task_id);

                    // Inject high-priority preemption after first task
                    if !preempted && executed_order.len() == 1 {
                        let preempt_task = TaskId::new(1000);
                        scheduler.schedule_task(preempt_task, TaskPriority::Urgent);
                        preempted = true;
                    }
                }
            ...
            // MR: The number of normal-priority tasks executed should be the same
            // (high-priority tasks may interleave but don't affect normal task completion)
            let normal_tasks_no_preempt = result_no_preempt.len();
            let normal_tasks_with_preempt = result_with_preempt.iter()
                .filter(|&&id| id.raw() < 1000)
                .count();

            prop_assert_eq!(normal_tasks_no_preempt, normal_tasks_with_preempt,
                "Preemption should not affect normal task completion count");
        });
    }
    ...
    fn mr_lane_priority_fairness_bound() {
        ...
        proptest!(|(ready_tasks in 1..=5_usize, cancel_tasks in 1..=10_usize)| {
            let clock = VirtualClock::new();
            let runtime = LabRuntime::with_clock(clock.clone());
            ...
            // Set fairness limit
            let fairness_limit = DEFAULT_CANCEL_STREAK_LIMIT;

            // Add ready tasks first
            let ready_task_ids: Vec<_> = (0..ready_tasks)
                .map(|i| TaskId::new(i as u64))
                .collect();
            for &task_id in &ready_task_ids {
                scheduler.schedule_task(task_id, TaskPriority::Normal);
            }

            // Add cancel tasks (higher priority)
            let cancel_task_ids: Vec<_> = (1000..1000 + cancel_tasks)
                .map(|i| TaskId::new(i as u64))
                .collect();
            for &task_id in &cancel_task_ids {
                scheduler.schedule_task(task_id, TaskPriority::Urgent);
            }

            // Execute and track consecutive cancel dispatches
            let mut executed_order = Vec::new();
            let mut consecutive_cancel_count = 0;
            ...
            while let Some(task_id) = scheduler.next_task() {
                ...
                if task_id.raw() >= 1000 {
                    // This is a cancel task
                    consecutive_cancel_count += 1;
                    max_consecutive_cancel = max_consecutive_cancel.max(consecutive_cancel_count);
                } else {
                    // This is a ready task - reset counter
                    consecutive_cancel_count = 0;
                }
            }

            // MR: Maximum consecutive cancel dispatches should respect fairness bound
            prop_assert!(max_consecutive_cancel <= fairness_limit + 1,  // +1 for fallback path
                "Consecutive cancel dispatches ({}) exceeded fairness limit ({})",
                max_consecutive_cancel, fairness_limit);

            // MR: All ready tasks should eventually execute (no starvation)
            let ready_executed = executed_order.iter()
                .filter(|&&id| id.raw() < 1000)
                .count();
            prop_assert_eq!(ready_executed, ready_tasks,
                "All ready tasks should eventually execute despite higher priority tasks");
        });
    }
    ...
    fn mr_cancel_chain_preservation() {
        ...
        proptest!(|(task_count in 2..=6_usize)| {
            let clock = VirtualClock::new();
            let runtime = LabRuntime::with_clock(clock.clone());

            // Create tasks with cancellation dependencies
            let task_ids: Vec<_> = (0..task_count)
                .map(|i| TaskId::new(i as u64))
                .collect();

            // Scenario A: Cancel without preemption
            let cancel_result_direct = {
                let mut scheduler = ThreeLaneScheduler::new(
                    1,
                    &runtime.state(),
                    None,
                ...
                // Cancel first task directly
                scheduler.schedule_task(task_ids[0], TaskPriority::Urgent);
            ...
            // Scenario B: Cancel during preemption
            let cancel_result_preempted = {
                let mut scheduler = ThreeLaneScheduler::new(
                    1,
                    &runtime.state(),
                    None,
                ...
                // Execute one task, then preempt and cancel
                let mut result = Vec::new();
                if let Some(first_task) = scheduler.next_task() {
                    result.push(first_task);

                    // Now inject preemption and cancel
                    let preempt_task = TaskId::new(2000);
                    scheduler.schedule_task(preempt_task, TaskPriority::Urgent);
                    scheduler.schedule_task(task_ids[0], TaskPriority::Urgent);
                }
            ...
            // MR: Both scenarios should handle the same tasks, just in different order
            let mut tasks_direct: Vec<_> = cancel_result_direct.iter()
                .filter(|&&id| id.raw() < 2000)
                .copied()
                .collect();
            let mut tasks_preempted: Vec<_> = cancel_result_preempted.iter()
                .filter(|&&id| id.raw() < 2000)
                .copied()
                .collect();

            tasks_direct.sort();
            tasks_preempted.sort();

            prop_assert_eq!(tasks_direct, tasks_preempted,
                "Cancel chain preservation: same tasks should execute regardless of preemption timing");
        });
    }
    ...
    fn mr_preemption_atomicity() {
        ...
        let task_ids = vec![TaskId::new(1), TaskId::new(2), TaskId::new(3)];

        // Scenario A: Execute tasks without any preemption
        let no_preempt_result = {
            let mut scheduler = ThreeLaneScheduler::new(1, &runtime.state(), None, None);

            // Add all tasks to ready lane
            for &task_id in &task_ids {
                scheduler.schedule_task(task_id, TaskPriority::Normal);
            }
        ...
        // Scenario B: Execute with preemption that gets immediately resolved
        let preempt_resume_result = {
            let mut scheduler = ThreeLaneScheduler::new(1, &runtime.state(), None, None);
            ...
            let mut executed = Vec::new();
            if let Some(first_task) = scheduler.next_task() {
                executed.push(first_task);

                // Inject preemption that immediately completes (empty work)
                let preempt_task = TaskId::new(5000);
                scheduler.schedule_task(preempt_task, TaskPriority::Urgent);

                // Execute the preemption (which does nothing)
                if let Some(preempt) = scheduler.next_task() {
                    if preempt == preempt_task {
                        // Preemption handled, continue with remaining tasks
                    }
                }
            }

            // Execute remaining normal tasks
            while let Some(task_id) = scheduler.next_task() {
                if task_id.raw() < 5000 {
                    // Only count normal tasks
                    executed.push(task_id);
                }
            }
            executed
        ...
        // MR: For deterministic scheduling, both should produce same task sequence
        assert_eq!(
            no_preempt_result, preempt_resume_result,
            "Preemption atomicity: preempt-then-resume should be equivalent to no preemption for deterministic tasks"
        );
    }
}
