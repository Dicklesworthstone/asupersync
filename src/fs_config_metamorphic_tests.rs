...
mod fs_config_tests {
    use crate::config::{EncodingConfig, ResourceConfig, RaptorQConfig, TimeoutConfig, TransportConfig};
    ...
    use crate::runtime::env_config::{
        ...
        ENV_THREAD_NAME_PREFIX, ENV_STEAL_BATCH_SIZE
    };
...
impl MockVfs {
    ...
    pub fn create_file(&mut self, path: &Path, content: &[u8]) -> io::Result<()> {
        ...
        self.files.insert(canonical_path, MockVfsFileData {
...
impl Arbitrary for MockRuntimeConfig {
    ...
    fn arbitrary_with(_: ()) -> Self::Strategy {
        ...
        .prop_map(|(worker_threads, task_queue_depth, thread_stack_size, thread_name_prefix, steal_batch_size, enable_parking)| {
            MockRuntimeConfig {
                worker_threads,
                task_queue_depth,
                thread_stack_size,
...
mod no_fs_config_fallback {
    ...
    fn fs_config_feature_disabled() {
        ...
        assert!(true, "Filesystem/config features disabled - metamorphic tests skipped");
    }
}