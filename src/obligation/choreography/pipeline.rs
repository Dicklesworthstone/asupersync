...
fn render_saga_module(
    ...
    render_saga_handler_body(
        &projection_to_local_type(projection),
        &mut code,
        1,
        protocol,
        participant,
    ...
    writeln!(code).ok();
    writeln!(code, "    // Emit evidence for saga execution").ok();
    writeln!(code, "    let plan = {fn_name}_saga_plan();").ok();
    writeln!(
        code,
...
/// Reconstruct the local type from a ProjectionOutput's session type string.
///
/// Since we have access to the original protocol, we can re-project. But since
/// we already have the projection output, we use the handler skeleton as the
/// authoritative rendering and enhance it with Cx calls.
///
/// For the saga module, we re-render from the local type directly.
fn projection_to_local_type(_projection: &ProjectionOutput) -> LocalType {
    // The projection output already contains the rendered handler; we
    // generate a fresh rendering with Cx integration from the same
    // compilation output. The local type was already computed during
    // compile() — we regenerate it here for the saga-aware rendering.
    //
    // In practice, the pipeline calls compile() which produces both the
    // ProjectionOutput and retains access to the local type. However,
    // since ProjectionOutput doesn't carry the LocalType (it's rendered
    // to a string), we re-project from the protocol in generate().
    //
    // This function is a placeholder; the actual local type is threaded
    // through render_saga_module_from_local() below.
    LocalType::End
}
