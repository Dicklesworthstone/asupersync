...
mod bytes_io_time_tests {
    ...
    use crate::bytes::buf::{Buf, BufMut, Chain};
    use crate::io::{AsyncRead, AsyncWrite, ReadBuf};
    use crate::test_utils::init_test_logging;
    use crate::time::{Time, Interval, Sleep};
    use crate::types::Time as TimeType;
...
pub fn arbitrary_text_with_lines() -> BoxedStrategy<String> {
    prop::collection::vec(
        ...
        0..=20
    ).prop_map(|parts| parts.join("")).boxed()
}
...
mod no_bytes_io_time_fallback {
    ...
    fn bytes_io_time_feature_disabled() {
        ...
        assert!(true, "bytes/io/time test-internals feature disabled - metamorphic tests skipped");
    }
}