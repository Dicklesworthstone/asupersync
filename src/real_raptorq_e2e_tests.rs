...
mod raptorq_e2e_tests {
    ...
    use crate::raptorq::{RaptorQReceiverBuilder, RaptorQSender, RaptorQSenderBuilder, ReceiveOutcome, SendOutcome};
    ...
    impl RaptorQE2ELogger {
        ...
        pub fn log_encode_start(&self, object_id: &ObjectId, data_size: usize) {
            ...
            details.insert("encode_start_time".to_string(),
        ...
        pub fn log_encode_complete(&self, outcome: &SendOutcome) {
            let mut details = HashMap::new();
            details.insert("encode_complete_time".to_string(),
                serde_json::Value::Number(serde_json::Number::from(
                    self.start_time.elapsed().as_millis() as u64
                )));
        ...
        pub fn log_decode_start(&self, object_id: &ObjectId, symbols_available: usize) {
            let mut details = HashMap::new();
            details.insert("decode_start_time".to_string(),
                serde_json::Value::Number(serde_json::Number::from(
                    self.start_time.elapsed().as_millis() as u64
                )));
        ...
        pub fn log_decode_complete(&self, outcome: &ReceiveOutcome, object_id: &ObjectId) {
            let mut details = HashMap::new();
            details.insert("decode_complete_time".to_string(),
                serde_json::Value::Number(serde_json::Number::from(
                    self.start_time.elapsed().as_millis() as u64
                )));
            details.insert("authenticated".to_string(),
        ...
        pub fn log_symbol_loss(&self, object_id: &ObjectId, symbols_lost: usize, total_symbols: usize) {
            let mut details = HashMap::new();
            details.insert("loss_rate".to_string(),
                serde_json::Value::Number(serde_json::Number::from_f64(
                    symbols_lost as f64 / total_symbols as f64
    ...
    impl RealRaptorQCodec {
        ...
        fn validate_test_environment() -> Result<(), String> {
            if std::env::var("NODE_ENV").unwrap_or_default() == "production" {
                return Err("Cannot run real RaptorQ E2E tests in production environment".to_string());
            }
        ...
        pub fn create_test_fixtures(&self) -> Result<Vec<(PathBuf, Vec<u8>)>, std::io::Error> {
            ...
            // Medium file fixture
            let medium_data = b"This is a medium-sized test file for RaptorQ encoding/decoding. ".repeat(100);
        ...
        pub async fn encode_decode_cycle(
            ...
            self.stats.source_symbols_generated.fetch_add(send_outcome.source_symbols as u64, Ordering::Relaxed);
            self.stats.repair_symbols_generated.fetch_add(send_outcome.repair_symbols as u64, Ordering::Relaxed);
            self.stats.symbols_transmitted.fetch_add(send_outcome.symbols_sent as u64, Ordering::Relaxed);
            self.stats.bytes_encoded.fetch_add(data.len() as u64, Ordering::Relaxed);
            ...
            let stream = if simulate_loss && loss_rate > 0.0 {
                ...
                self.stats.symbols_lost.fetch_add(symbols_to_lose as u64, Ordering::Relaxed);
            ...
            self.stats.symbols_received.fetch_add(symbols_available as u64, Ordering::Relaxed);
            ...
            match receiver.receive_object(cx, object_id).await {
                Ok(receive_outcome) => {
                    ...
                    self.stats.bytes_decoded.fetch_add(receive_outcome.data.len() as u64, Ordering::Relaxed);
    ...
    async fn test_real_raptorq_basic_encode_decode() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let receive_outcome = codec.encode_decode_cycle(
            &cx,
            object_id,
            test_data,
            false, // No symbol loss
            0.0,
            &logger,
        ).await?;

        // Verify decoded data matches original
        assert_eq!(&receive_outcome.data, test_data, "Decoded data should match original");
        assert!(receive_outcome.symbols_received > 0, "Should have received symbols");

        // Verify statistics
        let stats = codec.stats();
        assert_eq!(stats.objects_encoded.load(Ordering::Relaxed), 1, "Should have encoded one object");
        assert_eq!(stats.objects_decoded.load(Ordering::Relaxed), 1, "Should have decoded one object");
        assert_eq!(stats.decode_successes.load(Ordering::Relaxed), 1, "Should have one successful decode");
        assert_eq!(stats.bytes_encoded.load(Ordering::Relaxed), test_data.len() as u64, "Should track encoded bytes");
        assert_eq!(stats.bytes_decoded.load(Ordering::Relaxed), test_data.len() as u64, "Should track decoded bytes");

        eprintln!("RaptorQ Basic E2E structured log:\n{}", logger.export_json());
        Ok(())
    }
    ...
    async fn test_real_raptorq_with_symbol_loss() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(&receive_outcome.data, test_data, "Decoded data should match original despite symbol loss");
        assert!(receive_outcome.symbols_received > 0, "Should have received symbols");

        // Verify statistics show symbol loss
        let stats = codec.stats();
        assert!(stats.symbols_lost.load(Ordering::Relaxed) > 0, "Should have lost some symbols");
        assert_eq!(stats.decode_successes.load(Ordering::Relaxed), 1, "Should successfully decode despite losses");

        eprintln!("RaptorQ Symbol Loss E2E structured log:\n{}", logger.export_json());
        Ok(())
    }
    ...
    async fn test_real_raptorq_file_fixtures() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (fixture_path, fixture_data) in &fixtures {
            ...
            assert_eq!(&receive_outcome.data, fixture_data,
                "Decoded data should match fixture file: {}",
                fixture_path.display());

            eprintln!("Successfully encoded/decoded fixture: {} ({} bytes)",
                fixture_path.display(), fixture_data.len());
        }

        // Verify statistics for all fixtures
        let stats = codec.stats();
        assert_eq!(stats.objects_encoded.load(Ordering::Relaxed), fixtures.len() as u64,
            "Should have encoded all fixture files");
        assert_eq!(stats.objects_decoded.load(Ordering::Relaxed), fixtures.len() as u64,
            "Should have decoded all fixture files");

        eprintln!("RaptorQ File Fixtures E2E structured log:\n{}", logger.export_json());
        Ok(())
    }
    ...
    async fn test_real_raptorq_large_data_multi_block() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(&receive_outcome.data, &large_data,
            "Large multi-block data should decode correctly");
        assert!(receive_outcome.symbols_received > 10, "Should have received many symbols");

        // Verify statistics for large data
        let stats = codec.stats();
        assert!(stats.source_symbols_generated.load(Ordering::Relaxed) > 10,
            "Should generate multiple source symbols for large data");
        assert!(stats.repair_symbols_generated.load(Ordering::Relaxed) > 0,
            "Should generate repair symbols");

        eprintln!("RaptorQ Large Data E2E structured log:\n{}", logger.export_json());
        Ok(())
    }
}
