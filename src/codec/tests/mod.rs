...
mod regression_tests {
    ...
    fn bytes_codec_round_trip_identity() {
        ...
        for original in test_cases {
            ...
            // Round-trip test
            codec.encode(bytes_input.clone(), &mut encode_buf).expect("encode failed");
            let decoded = codec.decode(&mut encode_buf).expect("decode failed")
    ...
    fn lines_codec_newline_variants() {
        ...
        for (expected, input) in test_cases {
            ...
            let decoded = codec.decode(&mut src).expect("decode failed")
                .expect("incomplete frame");
    ...
    fn capacity_growth_bounded() {
        ...
        for size in [100, 1000, 10000] {
            ...
            codec.encode(large_input, &mut buffer).expect("encode failed");

            let cap_after = buffer.capacity();

            // Capacity invariants from fuzzing
            assert!(cap_after >= cap_before, "Capacity decreased!");
            assert!(cap_after <= buffer.len() * 4,
                "Excessive capacity growth: cap={}, len={}", cap_after, buffer.len());
        }
    }
    ...
    fn lines_codec_error_recovery() {
        ...
        let decoded = codec.decode(&mut src).expect("recovery failed")
            .expect("incomplete frame");
