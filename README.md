...
cross-thread abort against a task parked on a multi-worker runtime, using
...
cross another `Pending` without being truncated. Ordinary `Cx::spawn*`
preserves typed results after cancellation acknowledgement while retaining
v0.4.3 task-level cancellation for cancellation-blind late values;
cancellation-dominant combinators and blocking wrappers are tested separately.
Structural source audits and aggregate test counts cannot substitute for this
native behavioral lane. Its canonical remote-required command is recorded as
`native-parked-task-cancellation` in the proof-lane manifest. The runtime
additionally gives spawn adapters a private terminal-result sender with no
...
validates declared lengths at HEADERS, before admitting a request region. Its
actual request task creates a bounded `StreamingRawBody` with that task's
`Cx`, so handlers can consume DATA before peer FIN. Queue pressure pauses
the affected request stream; consumer progress wakes the connection driver
without requiring another peer packet. Each request reserves its queue capacity
plus one bounded pending frame against the connection aggregate, and retains
that charge through request-region closure. The existing
`router.max_buffered_body_bytes` remains a total body-size ceiling, tightened
by the resolved Router policy.

Framed input reaches EOF only after actual protocol FIN, including bodies with
Content-Length zero or already-consumed declared lengths. Length mismatches
...
`Http1Listener::bind_upgradeable`, and keep the callback scoped to the
listener-owned connection task. The listener flushes exactly one validated
`101` before transferring the socket, preserves request-codec read-ahead
(including a first frame coalesced with the handshake), and cancels the session
during graceful drain. A bare
...
`src/raptorq/` implements RFC 6330 systematic RaptorQ codes, a fountain code where any K-of-N encoded symbols suffice to recover the original K source symbols. Its production consumer today is the ATP file-transfer data plane. The distributed snapshot-distribution model in `src/distributed/` encodes region state into symbols, assigns replicas via consistent hashing, and supports quorum recovery. On native targets with `tls`, `RemoteSymbolTransport` implements `DistributorTransport` over authenticated mTLS sessions and bounded symbol storage. Cross-process scenarios live in `tests/distribution_hedge_process.rs` and `tests/symbol_durable_process.rs`; their presence does not establish current test success, arbitrary Rust-future migration, or general production-WAN reliability.
