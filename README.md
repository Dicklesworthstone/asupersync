...
| QUIC + HTTP/3 (default static-only QPACK; opt-in dynamic QPACK field-section and instruction-stream state machine) | `quinn`, `h3`, `h3-quinn` | `src/net/quic_core/`, `src/net/quic_native/`, `src/http/h3_native.rs` (native core feature surfaces exposed via `quic`/`http3`; historical wrapper sources in `src/net/quic/` and `src/http/h3/` remain parked outside the core feature graph; support matrix: `artifacts/http3_qpack_support_matrix_v1.json`. **SECURITY (`asupersync-7pwwwe`, fixed fail-closed): the native `quic_native` TLS layer performs no in-handshake X.509 verification (no `Certificate`/`CertificateVerify`/chain/hostname/trust handling). The client path now FAILS CLOSED — `on_handshake_confirmed` returns `ServerCertificateUnverified` unless the application verified the server identity and called `record_verified_server_identity()` (no insecure skip-verify default), so the prior silent-accept MITM exposure is removed. It still must not be used against untrusted peers without application-supplied certificate verification until full in-handshake X.509 (chain + hostname + signature) lands.**) | Feature-gated | Active transport/QPACK surfaces; **in-handshake X.509 verification NOT implemented; client fails closed (no silent-accept)** | Mixed | Medium |
