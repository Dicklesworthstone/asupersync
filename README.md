...
| Web framework (router/extractors/middleware/request-region; `Sse` finite bounded batch; `StreamingSse` pull API has request-region E2E proof, transport integration in progress) | `axum`, `warp`, `tower-http` | `src/web/`, `src/service/`, `src/server/` | In progress | Active | Mixed | Medium |
