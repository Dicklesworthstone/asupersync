...
const MAX_SYMBOL_SIZE_CORRUPTION: usize = 256;
...
fn normalize_corruption_config(config: &mut CorruptionConfig) {
    ...
    config.symbol_size = config.symbol_size.clamp(1, MAX_SYMBOL_SIZE_CORRUPTION as u8);
    ...
    for corruption in &mut config.corruptions {
        match corruption {
            SymbolCorruption::BitFlip { symbol_idx, bit_positions } => {
                *symbol_idx = *symbol_idx % config.k;
                bit_positions.truncate(MAX_BIT_FLIPS);
                for pos in bit_positions {
                    *pos = *pos % ((config.symbol_size as u16) * 8);
                }
            },
            SymbolCorruption::InvalidEsi { symbol_idx, .. } => {
                *symbol_idx = *symbol_idx % config.k;
            },
            SymbolCorruption::Truncate { symbol_idx, new_length } => {
                *symbol_idx = *symbol_idx % config.k;
                *new_length = *new_length % config.symbol_size;
            },
            SymbolCorruption::Extend { symbol_idx, extra_bytes } => {
                *symbol_idx = *symbol_idx % config.k;
                extra_bytes.truncate(64); // Limit size
            },
            SymbolCorruption::CorruptCoefficients { symbol_idx, bad_coefficients } => {
                *symbol_idx = *symbol_idx % config.k;
            ...
            SymbolCorruption::RandomGarbage { symbol_idx, garbage } => {
                *symbol_idx = *symbol_idx % config.k;
...
fn generate_source_data(k: usize, symbol_size: usize, pattern: &SourcePattern, seed: u64) -> Vec<Vec<u8>> {
    match pattern {
        ...
        SourcePattern::Alternating => {
            (0..k).map(|i| {
                (0..symbol_size).map(|j| {
                    if (i + j) % 2 == 0 { 0xAA } else { 0x55 }
                }).collect()
            }).collect()
        },
...
fn apply_corruptions(symbols: &mut Vec<ReceivedSymbol>, corruptions: &[SymbolCorruption]) {
    for corruption in corruptions {
        match corruption {
            ...
            SymbolCorruption::InvalidEsi { symbol_idx, new_esi } => {
                if let Some(symbol) = symbols.get_mut(*symbol_idx as usize) {
                    symbol.esi = *new_esi;
                }
            },
            ...
            SymbolCorruption::CorruptCoefficients { symbol_idx, bad_coefficients } => {
                if let Some(symbol) = symbols.get_mut(*symbol_idx as usize) {
                    symbol.coefficients = bad_coefficients.iter()
                        .map(|&b| Gf256::new(b))
                        .collect();
                }
            },
...
fn execute_corruption_test(config: &CorruptionConfig) -> CorruptionResult {
    ...
    let encoder_result = std::panic::catch_unwind(|| {
    ...
    let decode_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        decoder.decode_with_proof(&received_symbols, object_id, 0)
    }));

    match decode_result {
        Ok(Ok(_decode_success)) => {
...
fuzz_target!(|mut config: CorruptionConfig| {
    normalize_corruption_config(&mut config);

    // Skip degenerate cases
    if config.k == 0 || config.symbol_size == 0 || config.corruptions.is_empty() {
        return;
    }
    ...
    assert!(!result.panic_occurred,
        "RaptorQ codec panicked on corrupted symbols: {:?}", result.error_info);
    ...
    // Additional invariants can be added here based on specific corruption types
    // For now, the main goal is crash-free operation
});
...
mod tests {
    ...
    fn test_multiple_corruptions() {
        ...
        let result = execute_corruption_test(&config);
        assert!(!result.panic_occurred, "Should not panic on multiple corruptions");
    }
}