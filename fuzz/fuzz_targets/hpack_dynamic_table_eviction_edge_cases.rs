...
use asupersync::http::h2::hpack::{Header, DynamicTable, DEFAULT_MAX_TABLE_SIZE};
...
fn header_name_arbitrary(u: &mut Unstructured) -> arbitrary::Result<String> {
    ...
    Ok(match choice % 10 {
        ...
        6..=9 => {
            ...
            (0..len).map(|_| char::from(u.arbitrary::<u8>()? % 128)).collect()
        },
        _ => unreachable!(),
    })
}
...
fn header_value_arbitrary(u: &mut Unstructured) -> arbitrary::Result<String> {
    ...
    Ok(match choice % 10 {
        ...
        4 => "\x80\x81\x82".to_string(),      // High bytes
        5 => " \t\r\n".to_string(),           // Whitespace
        6..=9 => {
            // Random string with controlled length
            let len: u8 = u.arbitrary()?;
            let len = (len % 128) as usize; // Limit to reasonable size
            (0..len).map(|_| char::from(u.arbitrary::<u8>()? % 256)).collect()
        },
        _ => unreachable!(),
    })
}
        ...
        match operation {
            EvictionOperation::Insert { name, value } => {
                ...
                assert!(table.size() <= max_size,
                    ...
                    table.size(), max_size, operations_executed);
            ...
            EvictionOperation::BurstInsertTiny { count, name_prefix, value_prefix } => {
                for i in 0..count {
...
fn assert_table_invariants(table: &DynamicTable) {
    ...
    assert!(table.max_size() <= 1024 * 1024,
        "Max size {} exceeds global maximum", table.max_size());
