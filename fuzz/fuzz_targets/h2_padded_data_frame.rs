...
fuzz_target!(|input: PaddedDataInput| {
    ...
    match input.test_variant % 8 {
        0 => {
            ...
            let frame = PaddedDataFrame::new(input.stream_id, input.frame_flags, pad_length, payload);
            ...
            if frame.payload.len() > 0 && pad_length as usize >= frame.payload.len() {
                assert!(conn.has_protocol_errors(), "Pad length >= payload size should cause PROTOCOL_ERROR");
            }
        }
        1 => {
            ...
            assert!(!conn.has_protocol_errors(), "Pad length = 0 should not cause errors");
            assert_eq!(frame.padding.len(), 0, "Zero pad length should mean no padding bytes");
        }
        2 => {
            ...
            if payload.len() == 1 {
                ...
                assert!(!accepted, "Pad length 255 with 1 byte payload should be rejected");
        ...
        3 => {
            ...
            let frame = PaddedDataFrame::new(input.stream_id, input.frame_flags, excessive_pad, payload);
            conn.process_padded_data_frame(&frame);

            if frame.payload.len() > 0 && excessive_pad as usize >= frame.payload.len() {
                assert!(conn.has_protocol_errors(), "Excessive padding should cause PROTOCOL_ERROR");
            }
        }
        4 => {
            // Test case 5: Empty payload with padding
            let empty_payload = Vec::new();
            let frame = PaddedDataFrame::new(input.stream_id, input.frame_flags, input.pad_length, empty_payload);
            let accepted = conn.process_padded_data_frame(&frame);
        ...
        5 => {
            ...
            let mut frame = PaddedDataFrame::new(input.stream_id, flags_with_end_stream, input.pad_length % 32, payload);
            ...
            if frame.payload.len() > 0 && frame.pad_length as usize >= frame.payload.len() {
                assert!(!accepted, "Pad length >= payload should be rejected even with END_STREAM");
            }
        }
        6 => {
            ...
            if payload.len() > 1 {
                ...
                let frame = PaddedDataFrame::new(input.stream_id, input.frame_flags, max_valid_pad, payload);
                let accepted = conn.process_padded_data_frame(&frame);

                assert!(accepted, "Maximum valid padding should be accepted");
                assert!(!conn.has_protocol_errors(), "Maximum valid padding should not error");
            }
        }
        7 => {
            ...
            for pad_len in [0, 1, 5, 10, input.pad_length % 50] {
                ...
                let frame = PaddedDataFrame::new(input.stream_id + pad_len as u32, input.frame_flags, pad_len, test_payload);
                conn.process_padded_data_frame(&frame);
            }
            ...
            if conn.total_data_bytes > 0 {
                ...
                assert!(efficiency > 0.0 && efficiency <= 1.0, "Padding efficiency should be between 0 and 1");
            }
        }
        _ => unreachable!(),
    }
...
mod tests {
    ...
    fn test_excessive_padding() {
        ...
        assert!(matches!(result.unwrap_err(), PaddedFrameError::PadLengthExceedsPayload { .. }));
    }
