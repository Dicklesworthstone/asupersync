...
impl MockH2Connection {
    ...
    fn process_window_update(&mut self, frame: &WindowUpdateFrame) -> Result<(), String> {
        ...
        let increment = i32::try_from(frame.increment)
            .map_err(|_| "window increment too large".to_string())?;
...
fn generate_edge_case_frames() -> Vec<WindowUpdateFrame> {
    vec![
        ...
        WindowUpdateFrame { stream_id: 0, increment: 0 },      // Connection-level zero
        WindowUpdateFrame { stream_id: 1, increment: 0 },      // Stream-level zero
        WindowUpdateFrame { stream_id: 3, increment: 0 },      // Another stream-level zero

        // Valid delta cases (should succeed)
        WindowUpdateFrame { stream_id: 0, increment: 1 },      // Minimum valid increment
        WindowUpdateFrame { stream_id: 1, increment: 1 },      // Stream minimum valid
        WindowUpdateFrame { stream_id: 0, increment: 32768 },  // Mid-range
        WindowUpdateFrame { stream_id: 3, increment: 65535 },  // Large valid

        // Edge cases for overflow testing
        WindowUpdateFrame { stream_id: 0, increment: i32::MAX as u32 },           // Maximum i32
        WindowUpdateFrame { stream_id: 1, increment: (i32::MAX as u32) + 1 },     // Just over i32::MAX
        WindowUpdateFrame { stream_id: 3, increment: u32::MAX },                  // Maximum u32

        // Multiple stream testing
        WindowUpdateFrame { stream_id: 5, increment: 1024 },
        WindowUpdateFrame { stream_id: 7, increment: 2048 },
        WindowUpdateFrame { stream_id: 9, increment: 4096 },
    ]
}
...
fuzz_target!(|scenario: WindowUpdateScenario| {
    ...
    for frame in &test_frames {
        ...
        match result {
            Ok(()) => {
                ...
                if test_frame.increment == 0 {
                    panic!("WINDOW_UPDATE with zero delta incorrectly accepted: stream_id={}", test_frame.stream_id);
                }
            ...
            Err(err) => {
                if test_frame.increment == 0 {
                    ...
                    if !err.contains("WINDOW_UPDATE with zero increment") {
                        panic!("Wrong error message for zero delta: expected 'WINDOW_UPDATE with zero increment', got '{}'", err);
                    }
                } else if test_frame.increment > i32::MAX as u32 {
                    overflow_errors += 1;

                    // Verify correct error message for overflow
                    if !err.contains("window increment too large") && !err.contains("flow control window overflow") {
                        // Either "too large" (at parse) or "overflow" (at application) is valid
                    }
                } else {
                    // Unexpected error for valid increment
                    panic!("Unexpected error for valid WINDOW_UPDATE increment {}: {}", test_frame.increment, err);
                }
            }
        }
    }
    ...
    if expected_zero_failures > 0 && zero_delta_errors != expected_zero_failures {
        panic!("Expected {} zero-delta failures, got {}. Some zero deltas were incorrectly accepted.",
...
fn test_specific_rfc_violations(connection: &mut MockH2Connection) {
    ...
    assert!(result.is_err(), "Connection-level WINDOW_UPDATE with zero increment should fail");
    assert!(result.unwrap_err().contains("WINDOW_UPDATE with zero increment"));
    ...
    assert!(result.is_err(), "Stream-level WINDOW_UPDATE with zero increment should fail");
    assert!(result.unwrap_err().contains("WINDOW_UPDATE with zero increment"));
    ...
    assert!(result.is_ok(), "WINDOW_UPDATE with increment=1 should succeed: {:?}", result);
    ...
    assert!(result.is_ok(), "WINDOW_UPDATE with large valid increment should succeed: {:?}", result);
    ...
    assert!(result.is_err(), "WINDOW_UPDATE with increment > i32::MAX should fail");
    ...
    assert!(result.is_ok(), "WINDOW_UPDATE with increment=i32::MAX should succeed: {:?}", result);
}
...
mod tests {
    ...
    fn test_mixed_valid_invalid() {
        let scenario = WindowUpdateScenario {
            frames: vec![
                WindowUpdateFrame { stream_id: 0, increment: 1024 },    // Valid
                WindowUpdateFrame { stream_id: 1, increment: 0 },       // Invalid (zero)
                WindowUpdateFrame { stream_id: 2, increment: 2048 },    // Valid
                WindowUpdateFrame { stream_id: 0, increment: 0 },       // Invalid (zero)
                WindowUpdateFrame { stream_id: 3, increment: u32::MAX },// Invalid (too large)
                WindowUpdateFrame { stream_id: 4, increment: 512 },     // Valid
            ],
            include_edge_cases: false,
            max_frames: 10,
        };
